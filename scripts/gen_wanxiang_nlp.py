...
def oss_upload(local: Path, oss_path: str) -> bool:
    cmd = [OSSUTIL, "cp", "-f", "--meta", "Cache-Control:public, max-age=300, must-revalidate", "--meta", "Cache-Control:public, max-age=300, must-revalidate", str(local), f"oss://{OSS_BUCKET}/{oss_path}"]
