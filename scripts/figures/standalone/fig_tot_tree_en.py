#!/usr/bin/env python3
"""Tree-of-Thought Decision Tree (EN) — 24 Game exploration."""
...
# Level 0: Root
root = draw_box(ax, 2.0, 5.0, 2.8, 0.7, PURPLE, "{4, 9, 10, 13}", fontsize=12, bold=True)
ax.text(2.0, 5.55, "Root", ha="center", va="bottom", fontsize=10, color="#6b7280", fontweight="bold")

# Level 1: Three branches
# Branch A (top, kept)
draw_arrow(ax, 3.4, 5.0, 5.3, 4.6)
bA = draw_box(ax, 6.5, 4.6, 2.0, 0.6, BLUE, "{4, 4, 10}", fontsize=11)
draw_note(ax, 4.0, 4.95, "13 - 9 = 4", fontsize=9.5, color="#374151")
draw_note(ax, 7.65, 4.6, "v=8  (kept)", fontsize=9.5, color=BLUE)

# Branch B (middle, explored)
draw_arrow(ax, 3.4, 4.85, 5.3, 3.4)
bB = draw_box(ax, 6.5, 3.4, 2.0, 0.6, AMBER, "{6, 9, 13}", fontsize=11)
draw_note(ax, 4.0, 3.85, "10 - 4 = 6", fontsize=9.5, color="#374151")
draw_note(ax, 7.65, 3.4, "v=5  (explored)", fontsize=9.5, color=AMBER)

# Branch C (bottom, pruned)
draw_arrow(ax, 3.4, 4.7, 5.3, 2.2)
bC = draw_box(ax, 6.5, 2.2, 2.4, 0.6, RED, "{10, 13, 13}", fontsize=11)
draw_note(ax, 4.0, 2.65, "9 + 4 = 13", fontsize=9.5, color="#374151")
draw_note(ax, 7.85, 2.2, "v=3  (pruned)", fontsize=9.5, color=RED)
# strikethrough effect for pruned
ax.plot([5.35, 7.65], [2.2, 2.2], color=RED, lw=1.5, alpha=0.35, zorder=5)

# Level 2: Branch A continues
draw_arrow(ax, 7.5, 4.35, 8.8, 4.6)
bA2 = draw_box(ax, 9.7, 4.6, 1.5, 0.6, BLUE, "{4, 6}", fontsize=11)
...
sol = draw_box(ax, 11.7, 3.6, 1.3, 0.65, GREEN, "24", fontsize=14, bold=True)
draw_note(ax, 10.5, 4.0, "6 * 4", fontsize=9.5, color="#374151")
draw_note(ax, 11.7, 3.1, "SOLVED", fontsize=10, color=GREEN)

# Title
ax.set_title("Tree of Thought — Exploring the 24 Game",
             fontsize=14, fontweight="bold", color="#1f2937", pad=12)

plt.savefig("/Users/kchen/Desktop/Project/skilltest/fig_tot_tree_en.png",
            dpi=160, bbox_inches="tight", facecolor=BG, pad_inches=0.1)
plt.close()
print("Saved fig_tot_tree_en.png")
