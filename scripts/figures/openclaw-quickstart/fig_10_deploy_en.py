...
ax.text(50, 97, "Production Deploy Stack",
        ha="center", va="top", fontsize=14, fontweight="bold", color=TITLE_COLOR)

cx = 50  # center x

# --- Layer 1: Internet ---
make_box(ax, cx, 85, 18, 7, "#6b7280", "Internet", fontsize=12)

# Arrow
arrow_down(ax, cx, 81.5, 76)

# --- Layer 2: nginx ---
make_box(ax, cx, 72, 30, 7, BLUE, "nginx (reverse proxy + TLS)", fontsize=11)

# Arrow
arrow_down(ax, cx, 68.5, 63)

# --- Layer 3: pm2 ---
make_box(ax, cx, 59, 28, 7, AMBER, "pm2 (process supervisor)", fontsize=11)

# Arrow
arrow_down(ax, cx, 55.5, 50)

# --- Layer 4: OpenClaw Gateway (large, with sub-components) ---
gw_y, gw_h = 38, 18
...
ax.text(cx, gw_y + 7, "OpenClaw Gateway", ha="center", va="center",
        fontsize=13, fontweight="bold", color=PURPLE)

# Sub-components inside gateway
sub_w, sub_h = 12, 6
sub_y = gw_y - 3
make_box(ax, cx - 14, sub_y, sub_w, sub_h, BLUE, "Channels", fontsize=10)
make_box(ax, cx, sub_y, sub_w, sub_h, GREEN, "Agent\nLoop", fontsize=10)
make_box(ax, cx + 14, sub_y, sub_w, sub_h, PURPLE, "Memory", fontsize=10)

# Arrow down from gateway
arrow_down(ax, cx, gw_y - gw_h/2, 20)

# --- Layer 5: LLM Provider ---
make_box(ax, cx, 16, 28, 7, "#6b7280", "LLM Provider (DashScope)", fontsize=11)

# --- Side annotation: acme.sh ---
acme_x = 85
acme_y = 72
...
# Dashed line from acme.sh to nginx
ax.annotate("", xy=(cx + 15, 72), xytext=(acme_x - 6, acme_y),
            arrowprops=dict(arrowstyle="-|>", color="#059669", lw=1.5,
                           linestyle="dashed", mutation_scale=14))
ax.text(76, 76, "auto-renew\ncerts", fontsize=9.5, color="#059669",
        ha="center", va="bottom", style="italic")

plt.savefig("fig_10_deploy_en.png", dpi=160, bbox_inches="tight",
            facecolor=BG, pad_inches=0.1)
plt.close()
print("saved fig_10_deploy_en.png")
