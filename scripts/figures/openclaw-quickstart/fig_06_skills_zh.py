...
ax.text(48, 80, "每日简报技能", ha="center", va="center",
        fontsize=10.5, color="#9ca3af", style="italic")

# Cron trigger
draw_box(ax, 10, 50, 11, 10, GREEN, "定时\n(7:00)", fontsize=10.5, bold=True)

# Path A: today-calendar -> gcalcli
draw_box(ax, 35, 65, 14, 9, BLUE, "今日日程", fontsize=10, bold=True)
draw_small_box(ax, 52, 65, 10, 7, "gcalcli", fontsize=9)

# Path B: summarize-headlines -> Playwright MCP -> HN
draw_box(ax, 35, 35, 16, 9, BLUE, "摘要\n头条新闻", fontsize=9.5, bold=True)
draw_box(ax, 55, 35, 14, 9, AMBER, "Playwright\nMCP", fontsize=9.5)
draw_small_box(ax, 70, 35, 7, 7, "HN", fontsize=9)

# Compose message
draw_box(ax, 82, 50, 12, 10, GREEN, "组合\n消息", fontsize=10, bold=True)
...
plt.savefig("/Users/kchen/Desktop/Project/skilltest/fig_06_skills_zh.png",
            dpi=160, bbox_inches="tight", facecolor=BG, pad_inches=0.1)
plt.close()
print("Saved fig_06_skills_zh.png")
