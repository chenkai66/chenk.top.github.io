#!/usr/bin/env python3
"""Storage Media Selection Guide (EN)."""
...
# Root: > 4 TB single drive?
draw_box(ax, 2.2, 4.6, 3.4, 0.65, PURPLE, "> 4 TB single drive?", fontsize=12, bold=True)

# Yes -> HDD
draw_arrow(ax, 3.9, 4.45, 5.8, 4.6, label="Yes", label_offset=(0, 0.12))
draw_box(ax, 7.5, 4.6, 2.6, 0.55, RED, "HDD", fontsize=12, bold=True)
ax.text(9.0, 4.6, "cost dominates", ha="left", va="center",
        fontsize=9.5, color="#6b7280", style="italic", zorder=4)

# No -> SSD
draw_arrow(ax, 2.2, 4.28, 2.2, 3.55, label="No", label_offset=(-0.4, 0.0))
draw_box(ax, 2.2, 3.1, 2.0, 0.65, BLUE, "SSD", fontsize=12, bold=True)

# Old motherboard -> SATA SSD
draw_arrow(ax, 3.2, 3.3, 5.8, 3.5, label="Old board, no M.2", label_offset=(0, 0.12))
draw_box(ax, 7.5, 3.5, 2.6, 0.55, AMBER, "SATA SSD", fontsize=11, bold=True)

# Boot/games -> NVMe Gen 3
draw_arrow(ax, 3.2, 3.0, 5.8, 2.4, label="Boot / games", label_offset=(0, 0.12))
draw_box(ax, 7.5, 2.4, 2.6, 0.55, GREEN, "NVMe Gen 3", fontsize=11, bold=True)
ax.text(9.0, 2.4, "sweet spot", ha="left", va="center",
        fontsize=9.5, color=GREEN, fontweight="bold", zorder=4)

# Data centre / AI -> NVMe Gen 5
draw_arrow(ax, 3.2, 2.85, 5.8, 1.3, label="Data-centre / AI", label_offset=(0.2, 0.12))
draw_box(ax, 7.5, 1.3, 2.6, 0.55, PURPLE, "NVMe Gen 5", fontsize=11, bold=True)
ax.text(9.0, 1.3, "max bandwidth", ha="left", va="center",
        fontsize=9.5, color="#6b7280", style="italic", zorder=4)

ax.set_title("Storage Media Selection Guide",
             fontsize=14, fontweight="bold", color="#1f2937", pad=12)

plt.savefig("/Users/kchen/Desktop/Project/skilltest/fig_ssd_decision_en.png",
            dpi=160, bbox_inches="tight", facecolor=BG, pad_inches=0.1)
plt.close()
print("Saved fig_ssd_decision_en.png")
