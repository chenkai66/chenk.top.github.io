...
# Root
draw_box(ax, 2.2, 4.6, 3.0, 0.65, PURPLE, "需要冗余吗？", fontsize=12, bold=True)

# No -> RAID 0
draw_arrow(ax, 3.7, 4.45, 5.8, 4.6, label="否", label_offset=(0, 0.12))
draw_box(ax, 7.5, 4.6, 2.6, 0.55, RED, "RAID 0", fontsize=11, bold=True)
ax.text(9.0, 4.6, "最快，零保护", ha="left", va="center",
        fontsize=9.5, color="#6b7280", style="italic", zorder=4)

# Yes -> How many disks?
draw_arrow(ax, 2.2, 4.28, 2.2, 3.55, label="是", label_offset=(-0.4, 0.0))
draw_box(ax, 2.2, 3.1, 3.0, 0.65, BLUE, "多少块盘？", fontsize=11, bold=True)

# 2 -> RAID 1
draw_arrow(ax, 3.7, 3.3, 5.8, 3.5, label="2 块", label_offset=(0, 0.12))
draw_box(ax, 7.5, 3.5, 2.6, 0.55, GREEN, "RAID 1", fontsize=11, bold=True)
ax.text(9.0, 3.5, "镜像", ha="left", va="center",
        fontsize=9.5, color="#6b7280", style="italic", zorder=4)

# 3-5 SSD -> RAID 5
draw_arrow(ax, 3.7, 3.0, 5.8, 2.4, label="3-5 块 SSD", label_offset=(0, 0.12))
draw_box(ax, 7.5, 2.4, 2.6, 0.55, AMBER, "RAID 5", fontsize=11, bold=True)
ax.text(9.0, 2.4, "单校验", ha="left", va="center",
        fontsize=9.5, color="#6b7280", style="italic", zorder=4)

# 6+ HDD -> RAID 6
draw_arrow(ax, 3.7, 2.85, 5.8, 1.3, label="6+ 块 HDD", label_offset=(0.2, 0.12))
draw_box(ax, 7.5, 1.3, 2.6, 0.55, PURPLE, "RAID 6", fontsize=11, bold=True)
ax.text(9.0, 1.3, "双校验", ha="left", va="center",
        fontsize=9.5, color="#6b7280", style="italic", zorder=4)

ax.set_title("RAID 级别决策树",
             fontsize=14, fontweight="bold", color="#1f2937", pad=12)

plt.savefig("/Users/kchen/Desktop/Project/skilltest/fig_raid_decision_zh.png",
            dpi=160, bbox_inches="tight", facecolor=BG, pad_inches=0.1)
plt.close()
print("Saved fig_raid_decision_zh.png")
