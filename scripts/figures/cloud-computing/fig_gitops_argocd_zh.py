#!/usr/bin/env python3
"""基于 ArgoCD/Flux 的 GitOps — 水平流 + 向下分支 (ZH)."""
import matplotlib
...
ax.set_ylim(0, 5)
ax.axis("off")
ax.set_title("基于 ArgoCD / Flux 的 GitOps", fontsize=14, fontweight="bold",
             pad=12, color="#1e293b")

top_boxes = [
    (0.5,  3.6, 2.8, 0.8, PURPLE, "开发者提交\n代码"),
    (4.2,  3.6, 2.8, 0.8, BLUE,   "CI 构建\n容器镜像"),
    (7.9,  3.6, 3.2, 0.8, AMBER,  "CI 更新配置仓库\n中的 Manifest"),
]
bot_boxes = [
    (7.9,  1.6, 3.2, 0.8, RED,    "ArgoCD / Flux\n监听仓库变更"),
    (7.9,  0.2, 3.2, 0.8, GREEN,  "集群自动收敛\n到期望状态"),
]
...
fig.savefig("fig_gitops_argocd_zh.png", dpi=160, bbox_inches="tight",
            facecolor=BG, pad_inches=0.1)
plt.close()
print("saved fig_gitops_argocd_zh.png")
