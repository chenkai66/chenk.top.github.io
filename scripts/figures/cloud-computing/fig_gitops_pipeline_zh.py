#!/usr/bin/env python3
"""GitOps Deployment Pipeline (ZH) — GitOps 部署流水线。"""
...
draw_box(ax, 5.5, 3.8, 3.8, 0.8, BLUE, "CI：测试、构建、扫描、\n签名、推镜像", fontsize=10.5, bold=True)

# Row 2: Update manifest
draw_arrow(ax, 5.5, 3.35, 5.5, 2.85)
draw_box(ax, 5.5, 2.3, 3.6, 0.7, AMBER, "更新 Manifest 仓库\n（image tag bump）", fontsize=10.5, bold=True)

# Row 3: ArgoCD
draw_arrow(ax, 5.5, 1.9, 5.5, 1.45)
draw_box(ax, 5.5, 0.9, 3.4, 0.7, GREEN, "ArgoCD 检测差异\n并同步集群", fontsize=10.5, bold=True)

# Cluster
draw_arrow(ax, 7.25, 0.9, 8.8, 0.9)
draw_box(ax, 10.0, 0.9, 2.2, 0.7, GREEN, "K8s 集群", fontsize=11, bold=True)

ax.text(10.0, 0.35, "线上负载已更新", ha="center", va="top",
