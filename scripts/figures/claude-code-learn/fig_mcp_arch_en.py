#!/usr/bin/env python3
"""MCP Server Communication Flow (EN)."""
...
draw_box(ax, 6.0, 3.5, 2.8, 0.9, BLUE, "@playwright/mcp\nprocess", fontsize=11)
draw_box(ax, 9.8, 3.5, 2.2, 0.9, AMBER, "Real Browser", fontsize=11, bold=True)
draw_box(ax, 2.2, 1.2, 2.4, 0.9, GREEN, "Tool Results", fontsize=11, bold=True)
draw_box(ax, 9.8, 1.2, 2.4, 0.9, RED, "Page Snapshots", fontsize=10.5)

# Arrows
# Claude Code -> MCP process (stdio)
draw_arrow(ax, 3.55, 3.5, 4.55, 3.5, label="stdio", label_offset=(0, 0.22))
# MCP process -> Browser
draw_arrow(ax, 7.45, 3.5, 8.65, 3.5, label="commands", label_offset=(0, 0.22))
# Browser -> Page Snapshots (down)
draw_arrow(ax, 9.8, 3.0, 9.8, 1.7)
# Page Snapshots -> Tool Results
draw_arrow(ax, 8.55, 1.2, 3.45, 1.2, label="returns to agent", label_offset=(0, 0.22))
# Claude Code -> Tool Results (down)
draw_arrow(ax, 2.2, 3.0, 2.2, 1.7)

ax.set_title("MCP Server Communication Flow",
             fontsize=14, fontweight="bold", color="#1f2937", pad=12)

plt.savefig("/Users/kchen/Desktop/Project/skilltest/fig_mcp_arch_en.png",
            dpi=160, bbox_inches="tight", facecolor=BG, pad_inches=0.1)
plt.close()
print("Saved fig_mcp_arch_en.png")
