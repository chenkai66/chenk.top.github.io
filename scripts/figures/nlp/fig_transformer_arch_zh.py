...
# === ENCODER PATH (top row) ===
enc_y = 3.6

draw_box(ax, 1.2, enc_y, 2.0, 0.6, PURPLE, "源序列", fontsize=10.5, bold=True)
draw_arrow(ax, 2.2, enc_y, 3.0, enc_y)
draw_box(ax, 4.0, enc_y, 2.2, 0.6, BLUE, "嵌入 + PE", fontsize=10.5)
draw_arrow(ax, 5.1, enc_y, 5.8, enc_y)
draw_box(ax, 7.2, enc_y, 2.4, 0.6, AMBER, "编码器 x N", fontsize=11, bold=True)
draw_arrow(ax, 8.4, enc_y, 9.3, enc_y)
draw_box(ax, 10.5, enc_y, 2.2, 0.6, GREEN, "编码器输出", fontsize=10.5, bold=True)

# === K, V connection ===
dec_y = 1.4
draw_arrow(ax, 10.5, enc_y - 0.3, 10.5, dec_y + 0.7, label="K, V", label_offset=(0.4, 0.0))

# === DECODER PATH (bottom row) ===
draw_box(ax, 1.2, dec_y, 2.0, 0.6, PURPLE, "目标序列", fontsize=10.5, bold=True)
draw_arrow(ax, 2.2, dec_y, 3.0, dec_y)
draw_box(ax, 4.0, dec_y, 2.2, 0.6, BLUE, "嵌入 + PE", fontsize=10.5)
draw_arrow(ax, 5.1, dec_y, 5.8, dec_y)
draw_box(ax, 7.2, dec_y, 2.4, 0.6, RED, "解码器 x N", fontsize=11, bold=True)
draw_arrow(ax, 8.4, dec_y, 9.3, dec_y)
draw_box(ax, 10.2, dec_y, 1.4, 0.6, BLUE, "线性层", fontsize=10.5)
draw_arrow(ax, 10.9, dec_y, 11.3, dec_y)
draw_box(ax, 12.1, dec_y, 1.5, 0.6, GREEN, "Softmax", fontsize=10.5)
draw_arrow(ax, 12.85, dec_y, 13.2, dec_y)
ax.text(13.5, dec_y, "输出", ha="center", va="center", fontsize=10.5,
        color="#374151", fontweight="bold", zorder=4)

# Decoder annotation
ax.text(7.2, dec_y - 0.55, "掩码自注意力 + 交叉注意力",
        ha="center", va="center", fontsize=9.5, color="#6b7280",
        style="italic", zorder=4)

# Dashed highlight for cross-attention
