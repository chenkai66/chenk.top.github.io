...
A prompt that works on 100 examples in a notebook can fail on 10 % of inputs in production for reasons that have nothing to do with cleverness. This chapter is about prompting as engineering: where chain-of-thought actually helps (and where it doesn't), how prompt caching changes the cost equation, how to compose few-shot, ToT, and self-consistency without paying for every trick at once, and how to defend against the jailbreaks and injections that production traffic will produce within a week of launch.
