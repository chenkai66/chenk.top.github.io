---
title: "Computer Fundamentals (6): Deep Dive and System Integration"
...
Hardware engineers have spent fifty years building impressive machines, but most software still treats them like 1990s PCs. Bridging this gap is the most high-leverage skill in performance engineering.
...
The left side of the figure shows the familiar stack: application, framework, compiler, OS, driver, ISA, microarchitecture, and silicon. The right side shows what happens when you allow information to flow up and down the stack, rather than treating it as a sealed pipe.
...
## Appendix — Three Things to Try This Week
...
# Then run a simple H2D bandwidth test
python -c "
import torch, time
...
PCIe Gen4 x16 is theoretically 32 GB/s, in practice you'll see 24-28. If you get half of that, you're plugged into a Gen3 slot or a x8 lane. The chipset diagram in your motherboard manual will tell you which slot is which — most boards have exactly one CPU-direct x16 slot.

## Appendix — Cache Lines and ML Workloads

The cache line is 64 bytes on every modern CPU. In ML pipelines this single number explains a lot:

- A `float32` tensor: 16 floats per cache line. Iterating row-major over `(B, C, H, W)` data hits the line that's already loaded; iterating column-major across the batch dimension misses every time.
- The same model can train **2-3x faster** with `channels_last` memory format on convolutional networks, because the convolution kernel reads channels contiguously.
- `torch.compile` and XLA fuse operations partly to keep tensors in cache between ops. The win isn't theoretical FLOPs — it's avoiding round-trips to DRAM at 80 ns each.
- A 256 KB tile of activations fits in L2. A 24 MB layer of weights does not. This is why FlashAttention tiles the attention matrix into L2-sized blocks: the algorithm hasn't changed, the memory traffic has.

If you take one thing from the deep-dive: **the math operations are usually free; the memory movement is the cost.** This is true on CPU, more true on GPU, and almost the entire reason custom AI silicon exists.
