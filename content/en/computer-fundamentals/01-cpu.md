---
title: "Computer Fundamentals (1): CPU and the Computing Core"
...
---
![Computer Fundamentals (1): CPU and the Computing Core — Chapter overview](https://blog-pic-ck.oss-cn-beijing.aliyuncs.com/posts/en/computer-fundamentals/01-cpu/illustration_1.png)
...
## Appendix A — Modern Pipeline Reality

The five-stage RISC pipeline (fetch -> decode -> execute -> memory -> writeback) you see in textbooks is a 1990s artifact. A 2025 Zen 5 or Lion Cove core is wider, deeper, and stranger:

- **Decode width 4-8.** Up to eight x86 instructions decoded per cycle. The bottleneck is the instruction fetch front-end, not the back-end.
- **Micro-op cache.** Decoded uops are cached so hot loops bypass the decoder entirely. A loop that fits in the uop cache can sustain ~6 IPC; spilling out drops you to ~2 IPC for the same code.
- **~600 in-flight instructions.** The reorder buffer (ROB) is huge — Zen 5 is around 448 entries, Lion Cove around 576. The CPU is constantly running the next ~150 ns of your code speculatively.
- **Two integer rename queues x 4-6 ALUs.** What you call "one core" is internally a small distributed scheduler.
- **Memory ordering buffers of 200+ loads and 100+ stores.** Stores to L1 are not committed until the store retires, but loads can speculatively bypass them through store-to-load forwarding.

The textbook pipeline still tells you the right story for *why* a branch misprediction hurts. It does not tell you that the actual penalty on a modern core is ~16-20 cycles only because every other structure (rename, schedulers, ROB) has to be flushed and refilled. The 5-stage diagram makes it look cheap; the real diagram does not.

## Appendix B — Profile This on Your Machine

Three commands every engineer should know. Run them on a workload you actually care about, not on a synthetic benchmark.
...
The numbers to read: **IPC** (`instructions / cycles`) — if it's below 1.0 the CPU is starved; above 2.0 means the front-end and back-end are both healthy. **Branch miss rate** above 2 % means a hot branch is unpredictable. **Cache miss rate** above 5 % usually points to a working set larger than L2.

**macOS — Instruments -> Time Profiler / Counters template.** The Counters template exposes Apple Silicon performance counters: `INST_ALL`, `CYCLES`, `L1D_CACHE_MISS_LD`, `BRANCH_MISPRED_NONSPEC`. Same metrics, different names.

**Anywhere — `taskset` + `time`.** Pin a single-threaded benchmark to one core to eliminate scheduler noise:

```bash
taskset -c 3 perf stat -r 5 ./bench
```

The `-r 5` runs five times and reports mean +/- stddev. If your stddev is more than 5 %, something else is using the CPU.

## Appendix C — Where the GPU is Different

A GPU SM (streaming multiprocessor) is not a CPU core scaled down. It is a fundamentally different machine:

- **No branch prediction.** A GPU executes branches by *masking* — both sides run, the wrong-side results are thrown away. A divergent `if` inside a warp halves throughput. A CPU's branch predictor would be useless because the GPU has no per-thread program counter at this granularity.
- **No out-of-order execution.** Latency is hidden by **switching to another warp**, not by reordering the current one. So an SM holds 32-64 warps in flight; the CPU holds one.
- **Coalesced memory access.** 32 threads in a warp issue 32 loads, but the hardware fuses them into one 128-byte transaction *if and only if* the addresses are contiguous and aligned. A scattered access pattern that a CPU shrugs off (the cache hides it) can cost a GPU 32x the bandwidth.
- **Register file is huge.** An H100 SM has 256 KB of registers — bigger than its L1. The CPU register file is a couple of KB. Different optimization advice applies.

The practical takeaway: code that benchmarks well on a CPU profiler can benchmark badly on a GPU profiler for completely different reasons. Don't port tuning intuitions one-for-one.
