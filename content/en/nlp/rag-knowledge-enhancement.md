...
<!-- wanx-hero -->
![NLP (10): RAG and Knowledge Enhancement Systems — visual](https://blog-pic-ck.oss-cn-beijing.aliyuncs.com/posts/en/nlp/rag-knowledge-enhancement/illustration_1.jpg)
...
<!-- wanx-mid -->
![NLP (10): RAG and Knowledge Enhancement Systems — visual](https://blog-pic-ck.oss-cn-beijing.aliyuncs.com/posts/en/nlp/rag-knowledge-enhancement/illustration_2.jpg)
...
| Stage | p50 latency | What dominates |
|-------|-------------|----------------|
| Query embedding | 15--30 ms | One forward pass through a small encoder (bge-small, 110M params) |
| Vector search (top-50, HNSW, 10M docs) | 8--20 ms | Memory bandwidth, not compute |
| BM25 retrieval | 5--15 ms | Inverted index lookup, IO-bound |
| RRF fusion | <1 ms | List merge |
| Reranker (top-50 → top-5, cross-encoder) | 80--200 ms | 50 cross-encoder forward passes, batchable |
| LLM generation (4K context, 200 token answer) | 1500--4000 ms | Decoder autoregression, the wall |

The reranker is where teams forget to optimise. A bge-reranker-base on a single A10 GPU does 50 pairs in ~150 ms when batched correctly; the same code, called serially, takes 2 seconds. Always batch.

The LLM call dominates everything else by an order of magnitude, which means: **make the answer shorter before you make retrieval faster**. Cap `max_tokens` at 300 unless you have a reason. Use a smaller model for the final synthesis step (Llama-3-8B or Qwen-2.5-7B can handle most factual synthesis if the retrieved context is good).
...
**Failure 1: the answer is right but cited the wrong chunk.** The LLM hallucinates plausible sources. Fix: force the model to quote the chunk verbatim before generating the answer ("Quote the exact sentence supporting your answer, then explain"). Cuts hallucination of citations by ~70%.

**Failure 2: retrieval returns the FAQ entry instead of the actual doc.** FAQ pages are short, dense, and embed extremely close to user questions — they outrank longer, more detailed source docs. Fix: index FAQ separately and only fall back to it when no source doc scores above a threshold. Or downweight FAQ chunks at fusion time.

**Failure 3: the user asks "compare X and Y" and you retrieve only X-related chunks.** The dense embedding of the query is dominated by whichever entity has more name-mentions. Fix: query decomposition — use a small LLM to split "compare X and Y" into two retrievals, then merge.

**Failure 4: multilingual queries return monolingual results.** A Chinese question retrieves only Chinese docs even when the English doc is more relevant. Fix: use a multilingual embedder (bge-m3, multilingual-e5-large) and translate the query into the corpus's dominant language before BM25.

**Failure 5: the reranker downgrades the right answer.** Cross-encoder rerankers are trained on MS MARCO, which has a specific question style. Out-of-domain questions (very long, conversational, code-snippet-bearing) score poorly. Fix: fine-tune the reranker on 500--2000 in-domain query-document pairs. Cheap, ~2 hours on a single GPU, and almost always worth it.
