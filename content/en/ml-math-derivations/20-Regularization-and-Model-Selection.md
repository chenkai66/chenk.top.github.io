...
---
![ML Math Derivations (20): Regularization and Model Selection — visual](https://blog-pic-ck.oss-cn-beijing.aliyuncs.com/posts/en/ml-math-derivations/20-Regularization-and-Model-Selection/illustration_1.jpg)
...
![ML Math Derivations (20): Regularization and Model Selection — visual](https://blog-pic-ck.oss-cn-beijing.aliyuncs.com/posts/en/ml-math-derivations/20-Regularization-and-Model-Selection/illustration_2.jpg)
...
# Ridge with built-in alpha sweep, leave-one-out CV (closed form, very fast)
ridge = make_pipeline(
    StandardScaler(),
    RidgeCV(alphas=np.logspace(-3, 3, 50), cv=None),  # cv=None => efficient LOO
)

# Lasso with k-fold CV; n_jobs=-1 for parallel folds
lasso = make_pipeline(
    StandardScaler(),
    LassoCV(alphas=None, cv=5, n_jobs=-1, max_iter=10000),
)

# Elastic net: l1_ratio in [0,1] interpolates ridge (0) and lasso (1)
enet = make_pipeline(
    StandardScaler(),
...
`max_iter=10000` is not a magic number — it is what I have learned to set after watching `ConvergenceWarning` flood my logs on high-dimensional poorly conditioned data. If you still see warnings, your features are collinear; either drop the redundant ones or switch to ridge.
