...
## Code: empirical PAC and bias-variance for linear regression
...
|---------|------------|-------------------|
| PAC sample complexity (realizable, finite) | $m \geq \tfrac{1}{\varepsilon}(\ln \lvert \mathcal{H}\rvert + \ln \tfrac{1}{\delta})$ | Eliminating bad hypotheses is exponentially easy. |
| PAC sample complexity (agnostic, finite) | $m \geq \tfrac{2}{\varepsilon^2}(\ln \lvert \mathcal{H}\rvert + \ln \tfrac{2}{\delta})$ | Estimating expectations costs an extra $1/\varepsilon$. |
| VC sample complexity | $m = O\!\bigl(\tfrac{d + \ln(1/\delta)}{\varepsilon^2}\bigr)$ | $\mathrm{VCdim}$ replaces $\ln \lvert \mathcal{H}\rvert$ for infinite classes. |
