...
- **Pure ID recommendation with rich behaviour.** When you have years of click logs, hundreds of impressions per item, and short item titles like SKU codes, a vanilla SR-GNN on IDs alone is competitive. The LLM has nothing to encode except `"Product 8472"`, and the alignment training becomes a parameter-burning ceremony.
- **Heavy session length (>50 items).** GNN message passing on long sessions over-smooths even before the LLM features arrive. The paper's experiments are on sessions averaging under 10 items; at session length 100 the head-vs-tail gap collapses and a Transformer-based ranker (BERT4Rec) tends to win.
- **Latency budgets under 10 ms total.** With message passing alone consuming 5-10 ms, LLMGR has no headroom for cross-region embedding lookup or feature joining. Stick with a Two-Tower model whose serving path is a single dot product.

The sweet spot is what the original paper targets: short anonymous sessions on a catalogue with descriptive text and a long tail. Outside that envelope, LLMGR's win shrinks fast.
