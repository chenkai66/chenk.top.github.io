...
**Per-request prefix-cache memory.** With $L = 24$, $d = 1024$, $L_{\text{prefix}} = 10$, fp16, the prefix K/V tensors are $2 \cdot 24 \cdot 1024 \cdot 10 \cdot 2 = 983{,}040$ bytes per request — about 960 KB. With 100 concurrent requests on a single GPU that is 96 MB of VRAM dedicated purely to prefixes, on top of normal KV-cache. A 1 % overhead on an A10G's 24 GB, but with $L_{\text{prefix}} = 200$ the same calculation yields 19 GB. The lesson: long prefixes turn into a real VRAM bill very quickly under load.

**Prefix load latency.** Reading a 1 MB prefix from local SSD costs sub-millisecond, from S3 it is 30-80 ms. If your serving stack switches tasks per request and loads from object storage, that 30-80 ms dominates total latency for short generations. The fix is a prefix LRU cache pinned in host memory — with 30 prefixes at 1 MB each, the entire working set is 30 MB and fits in any reasonable container.

**Decode throughput hit.** Adding 10 prefix tokens to the KV-cache costs roughly 10/N extra attention flops per generated token, where N is the cache length at that step. For a 200-token prompt with 100-token generation, the average extra cost is about 6-8 %. For a 4 K-token chat history the same prefix is closer to 0.3 %. Long-context serving makes Prefix-Tuning effectively free; tight conversational loops feel the hit.
