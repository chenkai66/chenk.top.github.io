...
A standard GCN layer is$$H^{(l+1)} = \sigma\!\left(\tilde D^{-1/2}\tilde A\tilde D^{-1/2}\,H^{(l)}\,W^{(l)}\right)$$where $\tilde A = A + I$ adds self-loops and $\tilde D$ is its degree matrix. Run this $L$ times on each graph and you get paper embeddings $h^p$ and repo embeddings $h^r$. They are useful for in-graph tasks (citation prediction on the paper side, co-star prediction on the repo side), but they live in two unrelated $d$-dimensional spaces - dot products across towers are meaningless.

The "constrained" in constrained GCN is what fixes that. For every bridged pair $(i, i)$ we require$$h^{p}_{i}\!\cdot h^{r}_{i} \;\geq\; 1 - \delta, \qquad \delta \approx 10^{-3}.$$With both embeddings $\ell_2$-normalized, the dot product is the cosine similarity, and the constraint says: bridged paper-repo pairs should be near-identical in direction. Since the two towers share no parameters, the only way to satisfy this is for the GCNs and encoders to learn aligned axes.
...
WARP (Weighted Approximate-Rank Pairwise) is a margin-based ranking loss with a rank-aware weight. For a positive pair $(p, r^+)$ and a sampled negative $r^-$,$$\ell(p, r^+, r^-) = L\!\left(\mathrm{rank}(p, r^+)\right) \cdot \big[\gamma - h^p\!\cdot h^{r^+} + h^p\!\cdot h^{r^-}\big]_+$$where $[\cdot]_+ = \max(0, \cdot)$ is the hinge and $L(k) = \sum_{j=1}^{k} 1/j$ is a non-decreasing weight that grows when the positive is ranked far down. The rank is estimated by sampling negatives until one violates the margin and counting how many were tried.
...
## Cross-tower retrieval at serving time

The training story stops at "we have aligned embeddings". The serving story is what makes paper2repo deployable. Three details that the paper sidesteps but a real system has to solve:

**Index choice.** With 7,571 repos a brute-force dense matrix-vector product on the GPU is in the sub-millisecond regime — `numpy.dot` on a CPU is fine too. Once you scale to GitHub's actual ~300 M repositories the choice matters: HNSW (`hnswlib`) at $M=32, ef\_construction=200$ gives recall ~0.99 at 5 ms / query / 10 M items on a single machine. For paper2repo-style cosine similarity over $\ell_2$-normalized vectors, FAISS `IndexFlatIP` is an honest baseline and `IndexHNSWFlat` is the production target.

**Refresh cadence.** Citations and stars accrue continuously. A static GCN trained quarterly is fine for paper embeddings (citations move slowly); repo embeddings need rebuilding more often because star edges shift fast and tag-overlap thresholds drift with the corpus's TF-IDF distribution. A practical split: rebuild repo index nightly, paper index weekly, cross-tower alignment fine-tune monthly. The alignment is the slow knob — it depends on bridged pairs, which only grow when authors add `code available at` links to new papers.

**Cold-start recovery loop.** A new repo with no edges has only its TF-IDF / CNN feature vector to lean on, which by construction is roughly what a keyword search would give you. Two operational fixes work well: first, treat the TF-IDF cosine over README + tags as a fallback ranker for the bottom 20 % of degree-distribution repos; second, run a periodic "self-bridging" pass that promotes top-1 paper-repo predictions with confidence above a threshold into the bridged set, expanding alignment supervision over time. The second fix is the kind of thing a paper would call "semi-supervised" but in deployment is just "cron job that grows training data".

## Comparing paper2repo to dual-encoder text retrievers

The 2020 design predates dense passage retrieval, ColBERT, and CLIP-style cross-modal alignment. It is worth asking what would change today.

A pure dual-encoder (e.g., a sentence-transformer fine-tuned on bridged paper-repo pairs with InfoNCE) would skip the GCNs entirely and lean on the text encoders alone. On bridged repos with rich READMEs this approximate, and the architecture is much simpler. Where dual-encoders consistently lose is on stub repos: a one-line README has almost no text signal, but the *graph* signal — co-stars, tag-overlap — is exactly what carries the embedding into a useful neighborhood. The gap on cold-start head repos is small; the gap on long-tail repos with weak text is where the GCN earns its complexity.
