...
![Recommendation Systems (12): Large Language Models and Recommendation — Chapter overview](https://blog-pic-ck.oss-cn-beijing.aliyuncs.com/posts/en/recommendation-systems/12-llm-recommendation/illustration_1.png)
...
- Embeddings, ranking, and the retrieval/ranking split ([Part 1](/en/recommendation-systems/01-fundamentals/), [Part 4](/en/recommendation-systems/04-ctr-prediction/))
...
![Recommendation Systems (12): Large Language Models and Recommendation — Chapter summary](https://blog-pic-ck.oss-cn-beijing.aliyuncs.com/posts/en/recommendation-systems/12-llm-recommendation/illustration_2.png)
...
## Inference at 10K QPS: where the LLM-as-ranker dream dies

The literature on LLM4Rec is happy to ignore the latency reality. A typical production ranking surface needs:

- 10K--100K QPS at peak
- p99 latency < 200 ms end-to-end (recall + rank + business rules)
- 50--500 candidates ranked per request

Here's what those numbers do to LLM-based rankers.

**Direct LLM ranking is infeasible.** A single GPT-4o-mini call ranking 50 items is ~800 ms p50, ~2.5 sec p99. At 10K QPS that's 25,000 concurrent calls, $0.0007 each → $7/sec → $600K/day. Off the table.

**Distilled small LLM ranking is borderline.** A 3B-parameter distilled ranker with 4-bit quantisation does 50-item ranking in ~80 ms p50 on an A10. At 10K QPS you need ~150 GPUs (assuming 100 QPS/GPU with batching). $1.5K/day on cloud, $50K/month on reserved instances. Plausible for a top-funnel surface, painful for the long tail.

**LLM as feature provider is the actual production answer.** Run the LLM offline, dump its outputs (item summaries, semantic tags, embedding deltas) into a feature store, and have the online ranker (a tiny DCN or DIN model) consume those features in 5--10 ms. This is what every production "LLM4Rec" system actually does — including the ones whose blog posts make it sound like the LLM is in the request path.

Concrete decomposition for a real surface:

| Component | Latency budget | Implementation |
|-----------|---------------|----------------|
| Recall | 30 ms | ANN over 10M items, return top 1000 |
| Pre-rank | 20 ms | Tiny MLP on 1000 → top 200 |
| Rank | 60 ms | DIN with LLM-derived item features → top 50 |
| Re-rank / business | 30 ms | Diversity, freshness, business rules |
| Network + serialization | 60 ms | Slack |
| **Total** | **200 ms** | |

The LLM's contribution is the "LLM-derived item features" — precomputed, fetched from a feature store at <2 ms.

## Why production teams replaced prompt-based ranking with LLM-augmented features

Two-year arc I've watched at multiple companies.
