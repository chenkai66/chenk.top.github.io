...
---
![Linux (2): File Permissions: rwx, chmod, chown, and Beyond — Chapter overview](https://blog-pic-ck.oss-cn-beijing.aliyuncs.com/posts/en/linux/file-permissions/illustration_1.png)
...
Linux is a multi-user system, so every inode (file, directory, symlink, device, etc.) has three identity attributes:
...
Use it when you need a **known final state**, such as in scripted deployments or for new files where the previous state doesn't matter.
...
Use symbolic notation when you want to **tweak one aspect** without changing the rest.
