...
| Conditional | $f(x|y) = f(x,y)/f_Y(y)$ | Distribution of $X$ knowing $Y=y$ |
| Independence | $f(x,y) = f_X(x) f_Y(y)$ | Joint factors into marginals |
| Jacobian | $f_Y(y) = f_X(g^{-1}(y)) |dg^{-1}/dy|$ | Transform densities correctly |
...
| MVN conditional | $\boldsymbol{\mu}_{1|2} = \boldsymbol{\mu}_1 + \boldsymbol{\Sigma}_{12}\boldsymbol{\Sigma}_{22}^{-1}(\mathbf{x}_2 - \boldsymbol{\mu}_2)$ | Linear regression |
