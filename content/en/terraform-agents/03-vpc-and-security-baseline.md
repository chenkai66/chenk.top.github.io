...
![Terraform for AI Agents (3): A Reusable VPC and Security Baseline — visual](https://blog-pic-ck.oss-cn-beijing.aliyuncs.com/posts/en/terraform-agents/03-vpc-and-security-baseline/illustration_1.jpg)
...
![Terraform for AI Agents (3): A Reusable VPC and Security Baseline — visual](https://blog-pic-ck.oss-cn-beijing.aliyuncs.com/posts/en/terraform-agents/03-vpc-and-security-baseline/illustration_2.jpg)
...
Networks drift. Someone opens a port in the console at 11pm to debug a thing. Someone adds a SNAT rule to test a workaround. The route table gets a temporary entry that nobody removes. Six months later, the prod VPC and the HCL diverge silently — until the next `terraform apply` either reverts the manual change (breaking whoever's depending on it) or, worse, recreates a resource because the diff confuses the provider's update logic.
...
A GitHub Actions workflow that runs `terraform plan -lock=false -detailed-exitcode` on every workspace at 3am, and posts to DingTalk if the exit code is `2` (which means "plan would make changes"):
...
When you suspect drift on a single resource, `terraform refresh` is the surgical tool. It re-reads the resource from the API and updates state, but doesn't apply HCL changes:
...
This shows you what the world looks like right now without changing it. Once you see the diff, you decide: revert (regular `apply`) or codify (edit HCL to match).
...
## Module versioning: when the VPC module evolves
...
The wrong way to handle this is "edit the module in place and `terraform apply` everywhere". One Friday afternoon you'll change the public subnet from `/28` to `/27`, the existing subnets will need recreation, and you'll cascade-destroy your NAT and EIPs across three environments.

The right way is **versioned modules** and an explicit upgrade path:

```hcl
# Pin to a specific module version, like a library
module "vpc" {
...
For a small team I use a single repo for modules with git tags as versions. For a larger org, the Terraform Registry private module support (or the Aliyun-equivalent) gives you a published artefact with a UI. Either way the principle is the same: **modules are libraries, not snippets**. Treat them with the same release discipline you'd give a Python package.

The practical ergonomics:
...
terraform plan
# review carefully, especially the destroys
terraform apply
```

If `dev` survives a week, repeat in `staging`. Then `prod`. The whole rollout is in PRs, each one is small, each one is reversible by reverting the commit.

> **Real-world tip:** When a breaking module change requires resource recreation (e.g. our `/28` → `/27` subnet expansion), use a `moved` block + a manual data migration. The `moved` block tells Terraform "this old subnet's identity is now this new one"; the data migration copies any state across. For VPC subnets specifically, the easier path is to add new subnets *alongside* and migrate workloads zone-by-zone — never destroy a production subnet that has live ECS in it.

## Cost arithmetic for the network baseline

The article says "¥150-300/month". That number is real but worth breaking apart so you can size for your traffic.

The fixed costs (you pay these even with zero traffic):
...
For a medium-traffic prod workspace (1 TB egress, 100k KMS calls — heavy LLM streaming): ¥147 + ¥800 + ¥500 ≈ **¥1450/mo**.

The lever is egress. If your agents are streaming long completions from public LLM endpoints, you want PrivateLink or VPC peering to hosted models when available, because PrivateLink traffic is ~¥0.1/GB instead of ¥0.8/GB. For DashScope, the PrivateLink endpoint is `com.aliyun.dashscope` — wire it in your VPC and your egress bill drops by ~80%.
