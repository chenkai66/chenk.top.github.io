...
`/clear` 和 `/init` 这类内置斜杠命令，只是冰山一角。整个系统的设计初衷，正是让你**亲手编写属于自己的命令**——它们就存放在你的代码仓库里。
...
整段提示词会立即执行，你将直接获得一份结构清晰的安全审计报告，无需手动回忆和逐条输入那些命令。
...
经过两年的实践沉淀，我提炼出一套高复用率的命令模板，以下为完整的 prompt 内容：
