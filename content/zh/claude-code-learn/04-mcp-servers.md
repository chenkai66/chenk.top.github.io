...
---
如果在 Claude Code 里只学一种扩展机制，那必须是 MCP。这本质上是“自动补全”与“平台级能力”的分水岭。
...
MCP 是 **Model Context Protocol** —— 一个轻量级的开放协议，让 Claude Code 能调用外部服务器的工具和资源。这里的 "Server" 可以是任意支持 MCP 协议的进程——无论通过 stdio 还是 HTTP 通信。对 Claude 来说，MCP 工具和内置工具没区别：由模型决定调用时机，你来确认或自动授权，结果以纯文本形式返回。
...
Playwright 是标准的“入门首选”，因为效果立竿见影。注意，要在 Shell 里跑，*别*在 Claude Code 里面装：
...
MCP 服务器是独立进程。Claude Code 通过 stdio（远程则是 HTTP）跟它通信。在初始化握手阶段，Claude 会询问“你能提供哪些工具？”，服务器则返回一组工具 Schema。之后模型就能像调用内置工具一样调用它们。
...
建议对任何可能修改系统状态的 MCP 工具，都不要选择“Always allow”。仅执行读操作的工具（如搜索、获取、查询）风险较低，可酌情放宽权限。但凡涉及写文件、发消息、调有副作用的 API——每次确认，或者把权限 scope 缩窄。
...
设计原则很明确：每个服务器专注单一职责，打包为独立的 npm 或 PyPI 包，并暴露 3–10 个工具接口。
