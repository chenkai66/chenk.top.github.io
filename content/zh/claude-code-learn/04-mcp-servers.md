...
添加后重启 Claude Code（或开启新会话），你应该能在工具列表中看到 Playwright 工具。试试这个简单指令：



Claude 会先调用 `browser_navigate`，再用 `browser_snapshot` 读取页面内容，最后汇总结果。首次调用每个工具时，你会看到权限提示。
...
Filesystem MCP 让 Claude 能安全访问项目根目录之外的文件。默认情况下，Claude Code 只能读写当前工作目录内的文件，而 Filesystem MCP 在严格限制下扩展了这一能力。
...
该服务器强制执行白名单策略。即使模型尝试读取 `/etc/passwd`，服务器也会返回错误——这是设计上的沙盒机制。

### 分步指南：GitHub MCP

GitHub MCP 让 Claude 直接访问 GitHub API——包括 Issues、PR、仓库等。相比直接调用 `gh` CLI，它返回的是结构化数据，对模型更友好。
...
GitHub MCP 提供了仓库、Issues、PR、分支、文件和搜索等工具，是目前工具最丰富的 MCP 服务器之一，包含 30 多个工具。

### 分步指南：Postgres MCP

Postgres MCP 让 Claude 连接到 PostgreSQL 数据库，专为只读探索设计——分析表结构、运行查询、解释 schema。
...
- 别用具备写权限的账号连生产库，建议用副本。
- 密码若写在连接字符串中，会明文存入配置文件。改用环境变量：



### 分步指南：Slack MCP

Slack MCP 让 Claude 能搜索你的 Slack 工作区、读取频道消息，甚至发消息。
...
这个工具很强大，但我始终开启“每次调用确认”。毕竟，能向全公司 Slack 发消息的工具，值得谨慎对待。
...
- **环境变量**用 `${VAR}` 语法引用，Claude Code 启动时解析。切勿在配置文件中硬编码密钥。
- **`args` 数组**直接传给命令，不经过 shell 解析。
