...
---
如果说 MCP 是 Claude 向外扩展能力的机制，那么 Hooks 就是你向内施加约束的手段。它的作用是强制执行你关心的规则，而非依赖模型自觉遵守。
...
Hook 本质上就是一个 shell 命令。Claude Code 会在若干明确定义的时机触发 Hook。你最常用到的两个时刻：

- **`PreToolUse`** — 在工具调用前运行。退出码为 0 表示允许执行；非零则中止该操作。
- **`PostToolUse`** — 在工具返回后运行。此时退出码仅作信息提示，不会影响流程；你可用它来格式化文件、运行 linter 或记录日志。

还有其他的（`UserPromptSubmit`, `Stop`, `Notification`, `SubagentStop`）。但在日常工作中，这两个 Hook 已覆盖绝大多数使用场景。
...
Hook 命令通过 stdin 接收工具调用的输入（JSON 格式），并从环境变量中读取当前对话上下文。最简单的 hook 就是读取 stdin，做出判断，然后返回合适的退出码。
...
`PreToolUse` 匹配器 `Bash`。阻断时退出码为 1；stderr 上的消息会回传给模型，让它知道原因并调整行为。这正是‘代理静默失败’与‘代理主动请求许可’的关键区别。
...
`PostToolUse` 匹配器 `Write|Edit`。每次 Claude 触碰文件，在代理进入下一轮对话前都会先格式化。你无需再反复提醒代理格式化代码——代码风格由 Hook 自动保障。
...
这个功能我正好用过三次。三次都是事后复盘，我需要知道“在那 40 分钟的会话里 Claude 到底做了什么”。这点磁盘开销完全值得。
...
这听起来可能有些激进——实际上也的确如此。重点在于你再也无法*意外*提交坏代码了。模型需主动绕过 Hook 才可能出错，而它默认不会这样做。
...
Claude Code 会在一个不可预知的工作目录中执行 Hook。始终使用绝对路径，或者使用 `$CLAUDE_PROJECT_DIR` 环境变量：
