...
![Claude Code 的 7 个 Hook 事件按触发时机分类。](https://blog-pic-ck.oss-cn-beijing.aliyuncs.com/posts/zh/claude-code-learn/05-hooks/fig2_taxonomy.png)
*Claude Code 的 7 个 Hook 事件按触发时机分类。*

## Hook 生命周期

下图展示了一次完整对话轮次中，每个 Hook 触发的时机。
...
让我详细说明 Claude 调用工具且已配置钩子（hook）时，**实际发生的完整流程**。理解这一生命周期，是编写正确、可靠钩子的关键。

---

### `PreToolUse` — 工具调用前的守门人

当 Claude 决定调用某个工具时，执行以下顺序：
...
5. 若**任一钩子以非零退出码（non-zero exit code）退出，则整个工具调用被阻断**；该钩子写入 `stderr` 的内容将作为错误消息返回给模型；
...
5. 若钩子向 **stdout 写入内容，则该输出将完全替代原始工具输出，供模型后续消费**——这是实现过滤、脱敏或标注的核心机制；
...
当 Claude 完成响应、即将交还控制权给用户时触发。典型用途包括：
...
当 Claude Code 发出桌面通知（如长耗时任务结束）时触发。可用于：
...
当主 agent 启动的**子 agent（用于并行任务）完成时**触发。适用于：
...
- 它在调用格式化工具前，先通过 `command -v` 检查其是否存在；若目标工具未安装，钩子会静默降级（gracefully degrade），不报错也不中断流程。  
- 使用 `2>/dev/null` 抑制格式化器输出的警告信息——避免这些无关日志污染模型上下文（model’s context）。  
- 对 Python 文件同时运行 `ruff format`（代码风格）和 `ruff check --fix`（静态检查自动修复），实现双重保障。

注册方式如下（置于 `.claude/config.json` 的 `hooks` 字段中）：
...
此版本在提交前自动识别项目所用测试框架，并执行对应命令。原生支持 npm、Make、pytest 和 Cargo：
...
覆盖更全面的密钥模式：GitHub tokens（`ghp_`, `gho_`, `ghs_`, `ghr_`）、Slack tokens（`xoxb-`, `xoxp-`, `xoxs-`）、AWS Access Key ID（`AKIA...`）、私钥块（`-----BEGIN ... PRIVATE KEY-----`）、含密码的连接字符串、JWT tokens，以及配置文件中常见的 `password=` 字段。
...
任务结束时触发系统通知（macOS 使用 `osascript`，Linux 使用 `notify-send`），并可选推送 Slack：
...
注册于 `Notification` 事件，适用于长时间运行任务（如批量编辑、大型重构）完成后提醒用户。
...
效果包括：  
- 危险命令拦截（如 `rm -rf /`）  
- 受保护文件写入防护（如 `.env`、`package-lock.json`）  
- 写入后自动格式化（如 Prettier）  
- 敏感信息脱敏（如密码、token）  
- 工具调用完成通知  

⚠️ **重要提醒：钩子不是安全边界**  
钩子与 Claude Code 运行于同一进程上下文。具备足够创造力的模型**可能绕过钩子**。它们是「护栏」（guardrails），而非「防火墙」（firewalls）。
