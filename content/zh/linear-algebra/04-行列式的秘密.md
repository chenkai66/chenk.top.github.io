...
---
![线性代数（四）：行列式的秘密 — visual](https://blog-pic-ck.oss-cn-beijing.aliyuncs.com/posts/zh/linear-algebra/04-行列式的秘密/illustration_1.jpg)
...
记住公式，代入数字，算出结果，然后继续往下学。这种教法完全忽略了行列式的本质。
...
这些变换形状各不相同，但行列式抓住了它们共同的一个数字：面积变化了多少。
## 行列式的符号：方向
...
这也提供了一个快速判断线性相关的方法：算行列式就行。
## 三维行列式：体积缩放因子

![线性代数（四）：行列式的秘密 — visual](https://blog-pic-ck.oss-cn-beijing.aliyuncs.com/posts/zh/linear-algebra/04-行列式的秘密/illustration_2.jpg)
