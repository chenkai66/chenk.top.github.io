...
> 张量是标量、向量和矩阵的自然延伸，可以推广到任意维度。矩阵相关的知识可以直接套用到张量上，或在失效时揭示一些有趣的规律。

我将从大家熟悉的概念开始，逐步引入张量的语言（如纤维、切片、展开），并讲解两种核心分解方法——**CP 分解**和**Tucker 分解**——看看它们如何压缩神经网络，并为推荐系统注入上下文感知能力。
...
每升一阶，就多一根轴，用于按索引取值。这条阶梯不会在 3 阶停止。
