...
---
如果你用过 PyTorch 或 TensorFlow，"张量"这个词你一定见过无数次。PyTorch 把所有数组都叫 `torch.Tensor`，TensorFlow 更是直接把张量写进了名字。但张量到底是什么？为什么这些框架要用一个听起来像物理术语的词来描述看起来像是多维数组的东西？

本章的核心答案很简单：

> 张量是标量、向量和矩阵的自然延伸，可以推广到任意维度。矩阵相关的知识，要么能直接套用到张量上，要么会在失效时揭示一些有趣的规律。

我会从大家熟悉的概念开始，逐步引入张量的语言（比如纤维、切片、展开），然后讲解两种核心分解方法 —— **CP 分解** 和 **Tucker 分解** —— 看看它们如何压缩神经网络，又如何为推荐系统注入上下文感知能力。
...
每升一阶，就多一根轴，用来按索引取值。这条阶梯不会在 3 阶停下。
