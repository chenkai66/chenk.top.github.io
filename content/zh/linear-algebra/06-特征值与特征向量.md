...
$$
那么 $\vec{v}$ 就是 $A$ 的**特征向量**，$\lambda$ 是对应的**特征值**。
...
将 $A\vec{v} = \lambda\vec{v}$ 重新整理，得到
$$
(A - \lambda I)\vec{v} = \vec{0}.
$$
为了让 $\vec{v}$ 是非零向量，矩阵 $A - \lambda I$ 必须是奇异的，也就是说
$$
\det(A - \lambda I) = 0.
$$
这就是**特征方程**。它是关于 $\lambda$ 的 $n$ 次多项式，根据代数基本定理，在复数域 $\mathbb{C}$ 上恰好有 $n$ 个根（含重数）。这些根就是特征值。
...
假设矩阵 $A$ 有 $n$ 个线性无关的特征向量 $\vec{v}_1, \ldots, \vec{v}_n$，对应的特征值是 $\lambda_1, \ldots, \lambda_n$。把特征向量按列拼成矩阵 $P$，把特征值放到对角矩阵 $D$ 的对角线上，那么
$$
A = P D P^{-1}.
$$
...
$$
它的特征方程是 $\lambda^2 + 1 = 0$，解得 $\lambda = \pm i$。这个矩阵没有实特征向量，几何上看非常明显： 90° 旋转会把实平面上**所有**方向都转走。
...
在处理大规模特征值问题时，通常不需要计算**所有**特征值，只需要找到最大的那个。最简单的算法可以用一行公式表示：
$$
...
三个不同起点，三种颜色，最后都落在同一条绿线上。为什么这方法有效？把 $\vec{v}_0$ 展开成特征基的线性组合 $\sum_i c_i \vec{v}_i$，那么
$$
A^k\vec{v}_0 = \sum_i c_i \lambda_i^k \vec{v}_i.
$$
经过 $k$ 次迭代后，$|\lambda_i|$ 最大的那一项会以 $(|\lambda_1|/|\lambda_2|)^k$ 的比例压倒其他项。归一化去掉绝对长度，剩下的就是主特征向量。收敛速度是几何级数，取决于**谱隙** $|\lambda_2|/|\lambda_1|$——谱隙越大，收敛越快。
...
先构造一个链接矩阵 $H$。如果网页 $j$ 链接到网页 $i$，那么 $H_{ij} = 1/L_j$，其中 $L_j$ 是网页 $j$ 的出链总数。 PageRank 向量 $\vec{r}$ 满足以下公式：
$$
\vec{r} = H\vec{r},
$$
这其实就是 $H\vec{r} = 1\cdot\vec{r}$，也就是特征值为 1 的特征向量。

为了保证算法收敛，同时处理悬挂页面（没有出链的页面）， Google 引入了一个阻尼因子 $d = 0.85$，公式如下：
$$
...
$$
矩阵 $\bigl(\begin{smallmatrix}1&1\\1&0\end{smallmatrix}\bigr)$ 的特征值是：
$$
\phi = \tfrac{1+\sqrt 5}{2}\approx 1.618 \quad\text{（黄金比例）},\qquad \hat\phi = \tfrac{1-\sqrt 5}{2}\approx -0.618.
$$
对矩阵进行对角化后，取第一个分量，就能得到 **Binet 公式**：
$$
...
任何一个实对称矩阵都可以拆成秩一矩阵的叠加：
$$
A = \lambda_1\,\vec{q}_1\vec{q}_1^T + \lambda_2\,\vec{q}_2\vec{q}_2^T + \cdots + \lambda_n\,\vec{q}_n\vec{q}_n^T.
$$
如果只保留绝对值最大的前 $k$ 个 $\lambda_i$，就能得到 $A$ 在 Frobenius 范数下的**最优秩-$k$ 近似**。这正是低秩去噪和压缩的核心原理。
