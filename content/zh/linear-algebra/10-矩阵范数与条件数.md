...
| 范数 | 公式 | 计算方式 |
|---|---|---|
| $\|A\|_1$（列和） | $\max_j \sum_i \vert a_{ij}\vert $ | 对每列取绝对值之和，再取最大值 |
| $\|A\|_2$（谱范数） | $\sigma_{\max}(A)$ | 最大奇异值 |
| $\|A\|_\infty$（行和） | $\max_i \sum_j \vert a_{ij}\vert $ | 对每行取绝对值之和，再取最大值 |
