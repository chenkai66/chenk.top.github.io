...
---
![线性代数（十二）：稀疏矩阵与压缩感知 -- 少即是多的数学奇迹 — visual](https://blog-pic-ck.oss-cn-beijing.aliyuncs.com/posts/zh/linear-algebra/12-稀疏矩阵与压缩感知/illustration_1.jpg)
## 「少即是多」的奇迹

一张 2400 万像素的原始照片，大小大约是 70 MB。用 JPEG 压缩后，只有几百 KB——压缩比高达 100 倍——但你看不出区别。传统 MRI 扫描需要 30 分钟；现代压缩感知 MRI 只需 5 分钟，图像质量完全一样。

这两个奇迹的背后，靠的是同一个核心：**稀疏性**。大多数自然信号，只要用合适的基表示，真正有意义的系数就那么几个，剩下的几乎全是零。

**压缩感知**把这一特性变成了算法。因为信号本身稀疏，测量次数可以远低于经典采样定理的要求，却仍然能精确恢复信号。本章会讲清楚背后的原理。
...
很多信号在标准基下并不稀疏，但换一个基底就会变得稀疏。选择一个**字典**$D \in \mathbb{R}^{n \times p}$，它的列称为**原子**，然后寻找：$\vec{x} \;=\; D\vec{\alpha}, \qquad \vec{\alpha}\;\text{稀疏}.$常见字典如下：
...
当$p > n$时，字典是**过完备**的：信号可以有多种表示方式，而我需要找到其中最稀疏的那个。
...
CSR 在约 67% 密度（8 / (8+4)）时与稠密存储持平。密度低于 10% —— 大多数实际稀疏矩阵都处于这个区间 —— 内存和计算节省能达到数量级。
...
在字典$D$中找$\vec{x}$的最稀疏表示：$\min_{\vec{\alpha}}\;\|\vec{\alpha}\|_0 \quad \text{s.t.}\quad D\vec{\alpha} = \vec{x}.$ 这是个**NP 难**问题。最直接的方法是枚举所有$\binom{p}{k}$种可能的支撑组合，结果就是组合爆炸。目前没有已知的多项式时间算法能解决一般情况。

### $L_1$ 作为凸松弛

用$\|\cdot\|_1 = \sum |\alpha_i|$代替$\|\cdot\|_0$：$\min_{\vec{\alpha}}\;\|\vec{\alpha}\|_1 \quad \text{s.t.}\quad D\vec{\alpha} = \vec{x}.$ 这是个**线性规划**问题，能在多项式时间内求解。神奇的是，在对$D$施加一些温和假设后，$L_1$ 的解与$L_0$ 的解完全一致。凸松弛在这里是**精确**的。
...
当特征高度相关时，纯$L_1$倾向于从一组相关特征中选一个，其余全丢弃。结合两种惩罚项：$\min_{\vec{\beta}}\; \tfrac{1}{2}\|X\vec{\beta} - \vec{y}\|^2 + \lambda_1\|\vec{\beta}\|_1 + \lambda_2\|\vec{\beta}\|_2^2$既保留了$L_1$的稀疏性，又借$L_2$获得了稳定性。这就是**Elastic Net**，在高维回归中更稳妥的默认选择。
...
**LASSO**——Least Absolute Shrinkage and Selection Operator（Tibshirani, 1996）：$\min_{\vec{\beta}}\;\tfrac{1}{2}\|X\vec{\beta} - \vec{y}\|^2 \;+\; \lambda\|\vec{\beta}\|_1.$  
...
![LASSO 系数路径，正则强度从左到右减弱；带 * 的是真正非零的特征](https://blog-pic-ck.oss-cn-beijing.aliyuncs.com/posts/zh/linear-algebra/12-稀疏矩阵�%与压缩感知/fig5_lasso_path.png)

真正活跃的特征（粗线）很早就进入模型，系数也迅速增大；伪特征要么始终不出现，要么很晚才进来，系数也很小。我通常用交叉验证选$\lambda$，并采用"1-SE 法则"：在最小误差$\lambda$附近一个标准误差范围内，挑最大的$\lambda$，这样更倾向于稀疏解。
## 压缩感知

![线性代数（十二）：稀疏矩阵与压缩感知 —— 少即是多的数学奇迹](https://blog-pic-ck.oss-cn-beijing.aliyuncs.com/posts/zh/linear-algebra/12-稀疏矩阵与压缩感知/illustration_2.jpg)
