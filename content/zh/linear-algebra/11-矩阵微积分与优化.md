...
---
![线性代数（十一）：矩阵微积分与优化 -- 从梯度到反向传播 — visual](https://blog-pic-ck.oss-cn-beijing.aliyuncs.com/posts/zh/linear-algebra/11-矩阵微积分与优化/illustration_1.jpg)

## 从淋浴龙头到神经网络

每天早上，我都在训练一个微型神经网络。水太冷了，我就拧一下水龙头旋钮——这是一个**参数**。一秒钟后，我感受到新的水温——这是**误差信号**。接着再拧一次。三四次调整后，水温就刚刚好了。
...
假设我开了一家奶茶店，利润 $f$ 取决于价格 $x_1$ 和广告预算 $x_2$。我想知道：*如果稍微调整一下价格或者广告预算，利润会怎么变化？*
...
梯度不仅仅是偏导数的简单堆叠，它的形式由以下三个几何意义决定。

**1. 方向。** 梯度指向函数值增长最快的方向。在所有可能的单位方向中，沿着 $\nabla f$ 的方向走，函数值增长最快；反方向则是下降最快的方向——这就是梯度下降算法的核心思想。
...
- $\theta = 0°$（顺着梯度方向走）：函数值增加最快，变化率为$\|\nabla f\|$。
- $\theta = 90°$（沿着等高线走）：函数值不变。
- $\theta = 180°$（逆着梯度方向走）：函数值减少最快，变化率为$-\|\nabla f\|$。

最后一行正是**梯度下降**的理论基础：如果想让$f$下降得最快，就沿着$-\nabla f$的方向前进。
## Jacobian：向量输入，向量输出

如果输出也是向量，单靠一个梯度就不够了。我需要一个偏导数矩阵，每个输入和输出的组合都对应一个偏导数。
...
它的行列式是$\det(J) = r$——这正是极坐标积分中把$dx\,dy$转换为$r\,dr\,d\theta$时多出来的那个$r$。微积分课上这个$r$看起来像是凭空出现的，但在这里，我亲手把它推导出来了。
...
如果二阶偏导连续，Hessian 就是对称矩阵（Schwarz/Clairaut 定理）。这个对称性是后续讨论的基础，它让我可以直接分析$H$的特征值。
...
在神经网络中，参数通常是矩阵（比如权重矩阵）。我需要计算的是“标量损失对权重矩阵的导数”。
...
![线性代数（十一）：矩阵微积分与优化 -- 从梯度到反向传播 — visual](https://blog-pic-ck.oss-cn-beijing.aliyuncs.com/posts/zh/linear-algebra/11-矩阵微积分与优化/illustration_2.jpg)
