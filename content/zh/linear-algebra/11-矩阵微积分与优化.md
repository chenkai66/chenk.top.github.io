...
- 一元微积分（导数、链式法则）
- 向量与点积（[第 1 章](/zh/linear-algebra/01-向量的本质/)）
- 矩阵乘法（[第 3 章](/zh/linear-algebra/03-矩阵作为线性变换/)）
- 对称矩阵与正定性（[第 8 章](/zh/linear-algebra/08-对称矩阵与二次型/)）
...
$\lambda I$ 将 $X^TX$ 的所有特征值都增加了 $\lambda$，这不仅保证了矩阵可逆，还降低了条件数。这是[第 10 章](/zh/linear-algebra/10-矩阵范数与条件数/)关于条件数直觉的直接应用。
...
- Goodfellow, I., Bengio, Y. & Courville, A. (2016). *Deep Learning*, [第 6 章](/zh/linear-algebra/06-特征值与特征向量/)——反向传播详解。
