...
**前置知识**：[第 1 章](/zh/linear-algebra/01-向量的本质/)（向量、加法、数乘）和[第 2 章](/zh/linear-algebra/02-线性组合与向量空间/)（线性无关、基、张成空间）。
...
判断是否可逆的标准很简单——我们会在[第 4 章](/zh/linear-algebra/04-行列式的秘密/)详细讨论——就是：
...
其中 $n$ 是输入空间的维数。简单说：**矩阵每压扁一维（核多一维），输出就少一维（像少一维）**。对我们的 $S$ 来说，$1 + 1 = 2$。[第 5 章](/zh/linear-algebra/05-线性方程组与列空间/)讨论线性方程组时还会再见到这个公式。
...
**[第 4 章](/zh/linear-algebra/04-行列式的秘密/)：行列式的秘密。** 行列式是一个数字，用来表示变换对体积的缩放比例。我会解释它在复合变换时为什么是相乘关系，符号变化为什么会反映方向反转，以及 $\det = 0$ 为什么正好对应我们刚刚讨论的奇异情况。
