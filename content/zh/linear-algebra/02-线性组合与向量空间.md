...
- [第 1 章](/zh/linear-algebra/01-向量的本质/)：向量、加法、标量乘法，以及 $\mathbb{R}^2$ 和 $\mathbb{R}^3$ 的几何图像。
...
这正是容斥原理在向量空间中的体现。[第 5 章](/zh/linear-algebra/05-线性方程组与列空间/)讨论线性方程组解的结构时，我们将再次遇见此公式。
...
- **[第 3 章](/zh/linear-algebra/03-矩阵作为线性变换/)**：矩阵的列空间即其列向量的**张成空间**；
- **[第 4 章](/zh/linear-algebra/04-行列式的秘密/)**：**行列式**用单个数值检验线性无关性；
- **[第 5 章](/zh/linear-algebra/05-线性方程组与列空间/)**：方程 $A\vec{x}=\vec{0}$ 的解集是**子空间**（零空间）；
- **[第 6 章](/zh/linear-algebra/06-特征值与特征向量/)**：**特征向量**是能对角化矩阵的特殊基；
- **[第 9 章](/zh/linear-algebra/09-奇异值分解SVD/)**：**SVD** 提供一对“最优”的标准正交基。
...
正确工具是 SVD（[第 9 章](/zh/linear-algebra/09-奇异值分解SVD/)）。奇异值 $\sigma_1 \ge \sigma_2 \ge \cdots$ 连续度量列的“独立程度”。比值 $\sigma_1/\sigma_n$ 即条件数；在双精度下，若该比值超过 $10^8$，此“基”便不可用。**数值秩**定义为大于阈值（通常为 $\max(m,n)\cdot \varepsilon \cdot \sigma_1$）的奇异值个数。
...
**[第 3 章](/zh/linear-algebra/03-矩阵作为线性变换/)：矩阵即线性变换**。矩阵并非静态数字表，而是**变换的执行者**。我们将看到：
