...
---
随便找个资深机器学习工程师问一句：“你每天实际用得最多的数学是什么？”答案几乎肯定是**线性代数**。微积分用于推导公式，概率用于建模，但在实际运行 ML 系统时，大部分时间都花在矩阵向量乘法、分解和投影上。PyTorch 的 `Linear`、scikit-learn 的 `PCA`、Spark MLlib 的 `ALS`，还有 Transformer 的注意力头，其实都是同一个线性代数基本操作换了个马甲。
...
>
> **前置知识：** 特征分解（[第 6 章](/zh/linear-algebra/06-特征值与特征向量/)）、SVD（[第 9 章](/zh/linear-algebra/09-奇异值分解SVD/)）、矩阵范数（[第 10 章](/zh/linear-algebra/10-矩阵范数与条件数/)）、矩阵微积分基础（[第 11 章](/zh/linear-algebra/11-矩阵微积分与优化/)）。
...
Word2Vec、GloVe 和现代 LLM 的 embedding 层将 token 映射为稠密向量，使语义相近的词在空间中靠得更近，简单的**向量算术**就能捕捉语义关系。
...
- **去噪**：在金融领域，收益率矩阵的前几个主成分对应市场和板块因子，尾部基本是噪声（这就是[第 14 章](/zh/linear-algebra/14-随机矩阵理论/)提到的尖峰协方差模型）。
