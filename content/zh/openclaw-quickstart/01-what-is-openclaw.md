...
---
总有人问我："OpenClaw 不就是套了个 LLM 的壳吗？" 短回答是：不是。这事儿值得专门写篇文说清楚，然后再进 QuickStart。
...
动手之前，我先打个预防针，说说我默认你已经具备的条件：

**技术底子。** 你得习惯用终端，会改配置文件，看得懂 TypeScript 或 JavaScript 的报错堆栈。这一系列不需要你写代码，但东西跑不起来的时候，你得能盯着错误日志分清是网络超时还是语法写错了。

**Node.js 环境。** OpenClaw 跑在 Node 18 或更高版本上。如果你还没装 Node，或者版本太老，下一篇会手把手教安装。现在你只需要知道，`node --version` 至少得显示 `v18.0.0`。
...
**聊天平台账号。** 这个系列用 Telegram，因为 setup 最简单——找 BotFather 创建个 bot，复制 token，完事。你现在还不需要有 Telegram 账号，但到第四篇的时候最好有一个。如果你更想用钉钉、微信或 Discord，概念是通用的，只是我不会手把手带你走它们的注册流程。

**为什么这些很重要。** OpenClaw 不是填个表就能用的 SaaS。它是你自己跑的基础设施。如果想到要 SSH 连 VPS 或者编辑 `.env` 文件你就头大，那 hosted 方案比如 ChatGPT Plus 或 Claude Pro 可能更适合你。这没高低之分——那些都是 excellent products。但如果你来这里是因为想把 agent 跑在自己的基础设施上，读本地文件，且不向外部打电话汇报，那你来对地方了。
...
如果你用过 Claude Code 或 Cursor，这个 agent 循环你会觉得很熟悉。区别在于那些产品 owns the surface——你在它们的 UI 里跟它们对话。OpenClaw 把这个反转了。surface 是你已经在用的 messenger。agent 是来找你的。
...
**渠道适配器。** 每个聊天平台都有自己的适配器模块。适配器知道怎么认证、维持连接、解析 incoming events，以及序列化 outgoing messages。有些平台用 webhook（Slack, Discord），有些用长轮询（Telegram），有些用持久 WebSocket 流（钉钉）。适配器把这些全抽象了。从 agent core 的视角看，它只是在收发结构化的 JSON 数据块。这种分离为什么加一个新渠道通常只要一两天而不是几周——你实现一个接口，剩下的自动就好使。

**技能系统。** 技能是 agent 的词汇表。一个技能是一个 Markdown 文件，描述一个任务，可选的辅助脚本，还有一个 manifest 声明什么时候该把这个技能提供给 agent。运行时，只有 manifest 被加载到内存。当 agent 把任务匹配到技能时，完整的技能 body 才会被懒加载进来。这 keeps the system prompt small，让你能积累几十个技能而不爆 context window。第五篇文章我会展示怎么写自定义技能。
...
OpenClaw 是我在 2026 年找到的住在这个格子里最 polished 的方案。
...
**LangChain agents。** LangChain 给你原语（chains, agents, retrievers, memory），但你还是得自己搭服务器，处理聊天平台认证，自己建编排循环。如果你是库派的人，LangChain 是对的 tool。OpenClaw 是个 batteries-included 的 runtime——服务器、循环、渠道适配器、技能系统全都现成。你写配置，不写基础设施。
...
**实话实说。** OpenClaw 放弃了 GUI 和托管 hosting。你得改 YAML，看日志，改配置时得重启进程。作为回报，你拿到数据的完全控制权，能在自己的硬件上跑，而且能灵活接入任何模型或渠道，不用等 SaaS 厂商加支持。
...
用了内置适配器，你能避开这些坑。钉钉用持久 WebSocket 连接，带自定义心跳协议，要求你 20 秒内 ACK 每条消息，否则重发。Discord 全局速率限制是所有端点每秒 50 请求，再加上每路由限制。Telegram 最简单——REST webhook 或长轮询——但你也得处理 parse modes（Markdown, HTML, MarkdownV2）和 inline keyboards 的 quirks。企业微信（WorkBuddy）需要企业认证、回调签名验证和注册 IP 白名单。适配器把这些全处理了。

**2. 技能不只是 system prompts。** 一个 Skill 是一个 Markdown 文件加可选的辅助脚本加一个 manifest。Agent 启动时只加载 manifest——技能的 body 只有在技能匹配当前任务时才会懒加载进来。这听起来不性感，但这决定了你是每次对话烧掉 4k context window 的 system prompt，还是拥有一个 40k 的库而每次对话几乎不花钱。
...
**3. 记忆跨会话存活。** 这里有一个类型化的记忆存储（用户画像、项目状态、参考资料、反馈）加上一个可索引的 `MEMORY.md`。当同一个对话在三天后于不同的聊天中恢复时，agent 不会冷启动。

记忆存储有五个部分：`user_identity`（姓名、角色、偏好）、`project_context`（活跃项目及其状态）、`references`（API 文档、配置示例）、`feedback`（用户告诉 agent 别再做的事）、`learning_profile`（每个主题的深度偏好）。每个部分都是一个 Markdown 文件。Agent 在会话开始时读取它们，当用户说了改变上下文的内容时写回它们。
