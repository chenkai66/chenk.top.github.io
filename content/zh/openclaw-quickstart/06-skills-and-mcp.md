...
1. 每个工作日早上 7 点自动运行
2. 通过 Playwright MCP Server 抓取 Hacker News 的头条新闻
3. 通过封装 `gcalcli` 的 Skill 读取当天的日历安排
4. 将上述内容汇总成一段文字，并推送到我的 Telegram
...
- `when summarize-headlines should run` —— 循环定义，对模型毫无意义
...
1. **固定命令字面量**。在 Skill 正文中硬编码具体命令，不要写成“运行任意所需 shell 命令”。模型一旦有发挥空间，就可能构造出危险指令。
2. **在配置中声明 `trusted_commands`**。将该 Skill 实际使用的命令显式加入可信列表，避免每次调用都弹出确认提示：
   ```json
   "tools": {
     "exec": {
   ...
   ```
3. **校验输出内容**。如果 Skill 会解析工具输出并将其传回模型，需考虑异常输出的影响——例如，恶意构造的日历事件标题理论上可能注入指令。
...
1. 将 `morning-briefing` 的正文载入提示词；
2. 发现 `skills_required: [today-calendar, summarize-headlines]`；
3. 将这两个子 Skill 的正文一并载入提示词，与父 Skill 并列；
4. 此时模型同时拥有全部三个 SOP，可以按序执行各步骤。

Agent 循环照常运行：模型规划工具调用 → 网关执行 → 结果返回。关键区别在于：由于三个 Skill 正文都在提示词中，模型对**每个工具的用途和上下文**理解更充分。
...
1. **显式编号所有步骤**（如上例所示）；
2. **在 Skill 正文末尾添加核对清单**：“发送前请确认：日历内容已包含？新闻摘要已包含？日期是否正确？”
...
1. **无确认提示**：如果 Skill 使用 `exec` 且所执行命令未列入 `trusted_commands`，Cron 任务会因等待永不出现的用户确认而挂起。请务必把所有 Cron 触发的命令加入可信命令列表。
2. **无后续交互**：如果模型响应中包含提问（例如：“是否包含加密货币新闻？”），无人可作答；消息发布至 channel 后即结束会话。因此，Cron 触发的 Skill 必须自包含——所有决策应由 SOP 明确定义，不可依赖用户实时交互。
