...
---
如果整个 OpenClaw 项目中你只打算修改一个文件，那一定是它。

`~/.openclaw/openclaw.json` 管着模型、工具、渠道、记忆、定时任务和技能加载。初始化向导会生成默认配置，本文将聚焦于你实际需要手动调整的关键部分。
...
常见误区是将最昂贵的模型设为默认，误以为这样更‘稳妥’。请避免这种做法。规划循环每轮都要跑。在这儿填 `qwen-plus`，只在那些真正需要推理的特定技能里覆盖成 `qwen3-max`。
...
OpenAI 模型主要在两种情况下有用：(1) 你就是需要 GPT-5.4，或者 (2) 你的技能是基于 OpenAI 原型的，不想重写。否则建议优先避开：Claude 在推理任务上表现更优，而 Qwen 在批量处理任务中成本更低。
...
如果你只是在单台笔记本上个人使用 OpenClaw，我不推荐 Coding Plan —— DashScope 的免费额度够了。但若需运行定时任务或多渠道接入，则强烈推荐启用该方案。
