...
两个字段最关键：名字（用户看到的）和默认模型。默认模型用于 Agent 的规划循环，除非工具或技能显式覆盖。

常见误区是将最昂贵的模型设为默认模型，误以为这样更‘稳妥’。建议避免这种做法。规划循环每轮都要跑。在这儿填 `qwen-plus`，只在那些真正需要推理的特定技能里覆盖成 `qwen3-max`。
...
OpenAI 模型主要在两种情况下有用：(1) 你需要 GPT-5.4，或者 (2) 你的技能基于 OpenAI 原型且不想重写。其他情况下建议优先选用 Claude 或 Qwen，前者在推理任务中表现更优，后者在批量处理任务中成本更低。
...
这让你可以在便宜的 token 上运行规划循环，仅在需要的地方切换到昂贵的推理模型。典型的模式如下：
...
阿里云百炼有一个 "Coding Plan" 订阅：每月 200 元可获得八个模型——Claude Sonnet 4.5、Qwen3-Max、Qwen3-Coder-Plus、DeepSeek V3.1、GLM-4.6 等——工作时间用量不限。对于一个每天循环几百次的 Agent 来说，这个价格非常划算。
...
如果你只是在单台笔记本上个人使用 OpenClaw，不推荐 Coding Plan，因为 DashScope 的免费额度已经足够。但如果需要运行定时任务或多渠道接入，建议启用该方案。
...
还有 24 个其他工具，大多数默认值都合理。我改过的几个包括：
...
`auto_write` 让 Agent 自己决定什么时候提交内容到记忆。禁用该选项后，需显式调用记忆指令（例如‘记住我更喜欢 Python 而不是 Node.js’）；启用后， Agent 可自主推断并存档上下文。我们默认启用，容忍少量误记。

## `security` —— Agent 不能碰什么

安全配置是可选的，但如果你的 Agent 无人值守运行（如 cron 任务）或暴露给其他用户（如共享 Telegram 机器人），强烈建议启用。
...
`allowed_user_ids` 字段精神上是非可选的 —— 没有它，你的机器人会回答任何找到它的人。切勿在未配置该字段的情况下上线部署。
...
该功能使 OpenClaw 从交互式聊天机器人转变为后台自动化工作代理。cron 条目触发一个技能，技能产生输出，输出发送到默认渠道（或技能指定的渠道）。晨间简报案例就是基于此构建的。
...
有些配置项可以直接用环境变量覆盖，特别是在运行 Docker 时，你肯定不想将 API Key 明文写入配置文件并提交到仓库：
...
敏感信息一律通过环境变量传递，其他配置留在文件里即可，因为文件可以进行版本控制，便于审计。
...
校验失败时，错误信息会带上行号和字段路径。改好后再验证一遍，确认无误后再重启：
...
若修改配置后出现“tool not registered”或“provider not found”等错误提示，表明需要重启网关服务。

## 如果今天我来改你的配置

如果你是用向导初始化的，并且之后一直没动过配置文件：
