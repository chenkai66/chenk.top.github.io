...
---
如果你只打算修改 OpenClaw 项目中的一个文件，那一定是这个。
...
OpenAI 模型主要在两种情况下有用：(1) 你就是需要 GPT-5.4，或者 (2) 你的技能是基于 OpenAI 原型的，不想重写。其他情况下建议优先选用 Claude 或 Qwen：前者在推理任务中表现更优，后者在批量处理任务中成本更低。
...
这让你可以在便宜的 token 上运行规划循环，仅在需要的地方切换到昂贵的推理模型。一个典型模式：
...
如果你只是在单台笔记本上个人使用 OpenClaw，我不推荐 Coding Plan —— DashScope 的免费额度够了。但若需运行定时任务或多渠道接入，建议启用该方案。
...
安全配置是可选的，但如果你的 Agent 无人值守运行（如 cron 任务）或暴露给其他用户（如共享 Telegram 机器人），强烈推荐。
...
默认是空的。本系列下一篇将接入 Telegram 渠道，其基础配置结构如下：
...
该功能使 OpenClaw 从交互式聊天机器人转变为后台自动化工作代理。 cron 条目触发一个技能；技能产生输出；输出发送到默认渠道（或技能指定的渠道）。晨间简报案例就是基于这个建的。
