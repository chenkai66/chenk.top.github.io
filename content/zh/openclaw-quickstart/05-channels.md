...
---
OpenClaw 的初衷是让 Agent 主动找你。目前它还只停留在 TUI 界面上。是时候接个通道了。
...
哪怕你不打算在生产环境用 Telegram，我也总劝大家从这里入手。它的配置最干净，没有其他依赖组件，能让你完整验证 Agent 的端到端流程。
...
**5. 群聊 — 如何开启群模式。** 默认情况下，你的 bot 只在私聊工作。要把它加进群，告诉 BotFather 允许群访问：发送 `/setjoingroups`，选中你的 bot，选 "Enable"。在群里，bot 只能看到提及它的消息（`@your_bot_name what's the weather`），除非你在 BotFather 里用 `/setprivacy` 关闭 Privacy Mode。

## 钉钉 — 十五分钟

钉钉更复杂，因为它需要一个真正注册的“机器人应用”并带有权限。简单说：
...
值得理解这个架构，因为这就是为什么你能在 NAT 后面的笔记本上跑它。
...
腾讯的检测手段很 sophisticated。他们不只是看客户端版本字符串里的标记。封禁模式包括：
...
封禁不总是即时的。腾讯有时会让账号跑几天，收集行为数据，然后批量封禁。一旦封禁，通常是永久的。
...
想要微信，就用 WorkBuddy。预留一个晚上的时间做配置。
