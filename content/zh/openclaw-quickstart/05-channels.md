...
---
OpenClaw 的设计初衷是让 Agent 主动联系你。目前它仅支持 TUI（文本界面）。现在，是时候接入一个聊天通道了。
...
即使你不打算在生产环境中使用 Telegram，我也建议优先从它开始。它的配置最简洁，无需额外依赖，能让你完整验证 Agent 的端到端流程。
...
封禁并非总是即时触发。腾讯可能先让账号运行数天，持续收集行为数据，再进行批量封禁。一旦封禁，通常是永久的。
...
如需接入微信，请使用 WorkBuddy，并预留一晚时间完成配置。
