...
---
第五章快速对比了 Telegram、DingTalk 和 WeChat，而本章则聚焦国内企业场景——所有渠道落地前均需通过公司 IT 部门审批；渠道众多，官方文档分散在数十个 README 中，网上的各类‘对比表’也大多过时。
...
流式模式（long-poll）是钉钉快速上线的唯一原因——无需公网 IP 和域名验证，注册机器人后获取两个凭证即可。
...
**频率限制每秒 20 条消息。** 按机器人算，不是按用户。群聊活跃时很容易触顶，插件会自动排队重试，但延迟会飙升。建议把重度用户拆分到不同的机器人实例。

**组织管理员审批流。** 你创建了应用，加了 scope，在开发者控制台测试都正常，但一发布机器人就哑火。原因是企业应用发消息给开发者部门以外的用户需要组织管理员审批。查 钉钉管理后台 → 工作台 → 应用管理 → 待审批。
...
90% 的情况下，如果你的团队需要一个机器人，智能机器人就是你的首选。
...
**自建应用注册。** 需要已验证域名（要 ICP 备案）、回调用的公网 IP 和互通 license（约 2000 RMB/年起）。拿到 license 后，外部微信用户能把你的机器人加为联系人，但不能跟外部用户群聊，不支持流式，消息格式也受限。

**互通 license 成本。** 基础版（100 外部联系人）：约 2000 RMB/年；中级版（1000 联系人）：约 10000 RMB/年；企业版：50000 RMB/年起谈。如果测试，申请 3 个月试用（最多 50 联系人）。
...
WorkBuddy 是腾讯官方的 QClaw 桥接，运行在你的桌面上，不需要 IP 和应用审核，直接连接你真实的个人微信和 QQ，是目前唯一能实现‘当晚即在微信中启用 Agent’且无需降级的方案。

局限在于，作为桌面应用，宿主机休眠会导致进程终止，这在个人助理场景中可接受，但在生产环境中不适用。
...
你选择了钉钉，但现在需要企微触达外部用户。如何切换且不丢失历史记录：
