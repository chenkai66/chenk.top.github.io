...
---
第一次部署 OpenClaw 时，我一直主动发消息；两天后才意识到——它当时只是一个聊天机器人，远非真正的 Agent，跃迁的标志是它开始主动向我发消息。

这篇文章将介绍如何实现这两种主动触发机制。
...
在生产环境中，两者并用：Cron 负责每日 7:00 的晨间简报和 17:00 的 shutdown 总结，而 Heartbeat 每 45 分钟巡检一次，专门检查遗漏的 PR。
...
上面的例子只是最小集。生产环境中，你需要分类、优先级以及周末模式。
...
优先级机制是核心设计：缺少它，每项检查都可能触发干扰通知；有了它，低优先级事项会静默累积，并在每日总结中由 Cron 统一呈现。周末模式确保基础设施监控持续运行，同时将可延至周一处理的低优先级事项静默过滤。

## Cron —— 闹钟

有两种用法：CLI 方式更持久，即使 Gateway 重启也不受影响。
...
Cron 任务会持久化到磁盘，重启 Gateway 后任务仍然存在。
...
只有存在新内容时才发送，“reply empty”这条指令意味着不会产生“一切正常”的垃圾消息。
...
结果是每 45 分钟，你所在的每个渠道都会显示“Patrolled, no anomalies.”，包括凌晨 3 点，甚至那些根本没要求这个功能的群聊。
...
窗口不要设得太宽——如果中午才收到晨间简报就失去了意义。

**故障 4：任务耗时过长。** 复杂的 skill 链跑了 8 分钟，命中超时，你只得到部分输出或什么都没有。用 `openclaw cron logs --name "competitor-watch" --last 1 --full` 诊断（找 `timeout` 或 `context_cancelled`）。修复：`openclaw cron edit --name "competitor-watch" --timeout "5m"`。如果确实需要更多时间，简化 prompt 并将重活卸载到后台 skill。
...
由此，三个独立定时任务构成了一套具备问责机制的连贯工作流。如果晨间简报说“Review PR #412”，而到了下午 2 点还没有处理，Heartbeat 会提醒我，EOD Cron 随后记录是否跟进到位。
## 定时任务的 Token 开销

定时任务需要消耗 Token，我们来计算一下。
...
**Cron：** 开销更大，因为它们必定会产生输出。晨间简报：3,500 tokens。仓库监控（每小时触发，共 8 次）：12,000 tokens。竞品监控：10,000 tokens。每日总结：4,000 tokens。总计：约 29,500 tokens，每天 0.09 美元。

**Combined：** 加起来 $0.15-0.20/天，用 qwen-plus 一个月大概 $4.50-6.00。如果换成 gpt-4o 或者 claude-sonnet，价格会翻 5-8 倍。

**Recommendation：** Heartbeat 提示词越短越好——系统提示词和 HEARTBEAT.md 每次巡逻都得加载，每多写一行，每天就要为此多付 17 次费用。重活交给 Cron（复杂技能、长总结），因为它们只跑一次。如果发现 Heartbeat 比 Cron 还贵，说明任务分配不合理。

## 结语

OpenClaw 的核心在于让 Agent 主动找你。Heartbeat 和 Cron 是实现这一点的两种方式。严格按时执行的任务交给 Cron，仅在异常发生时才需响应的任务交给 Heartbeat。不要混淆它们。