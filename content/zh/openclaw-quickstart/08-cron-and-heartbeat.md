...
---
第一次部署 OpenClaw 时，我一直主动向它发送消息。两天后才意识到：它当时只是一个聊天机器人，还远未成为一个真正的 Agent。它真正成为 Agent 的标志，在于开始主动向我发送消息。
...
> Heartbeat 像一名巡逻警察，Cron 则像厨房里的定时器。

在生产环境中，我两者都会使用：Cron 负责每天早上 7 点的晨间简报和下午 5 点的 shutdown 总结。Heartbeat 则每 45 分钟巡逻一次，检查是否有我遗漏的 PR。
...
优先级机制是关键设计。缺乏该机制时，每一项检查都可能触发一次干扰性通知。有了它，低优先级事项会静默累积，最终由 Cron 任务汇总至每日总结中。周末模式确保基础设施监控持续运行，同时将可延至周一处理的低优先级事项静默过滤。
...
天气、当天的日历、最活跃的三個 GitHub 事项。选择 6:47 而非 7:00 是刻意为之：确保你清晨拿起手机时，消息已就绪。
...
这把三个独立的定时任务变成了一个有问责制的连贯日常工作流。如果我的晨间简报说"Review PR #412"，而到了下午 2 点我还没动它，Heartbeat 就会 提醒我EOD Cron 随后记录我是否跟进到位。
## 定时任务的 Token 开销
...
OpenClaw 的核心就在于让 Agent 主动找你。Heartbeat 和 Cron 就是实现这点的两扇门。需严格按时执行的任务，交由 Cron；仅在异常发生时才需响应的任务，交由 Heartbeat。别把它们搞混了。