...
---
DQN 证明深度强化学习可以搞定 Atari，但它有个明显的天花板：**只适用于离散动作空间**。如果让它去控制一个有七个连续关节角度的机械臂，立马就歇菜了——每次选择动作时，你还得先解一个内部优化问题。

**策略梯度方法**换了一条路子。它不学价值函数，也不靠价值函数“推”出策略，而是**直接优化策略本身**。就这么一个改动，直接让强化学习迈入了连续动作、随机策略的大门，还能应对那些最优解本身就需要随机性的问题（比如石头剪刀布）。
![强化学习（三）：Policy Gradient与Actor-Critic方法 — visual](https://blog-pic-ck.oss-cn-beijing.aliyuncs.com/posts/zh/reinforcement-learning/03-policy-gradient-and-actor-critic/illustration_1.jpg)
...
### 2.2 带 Baseline 的 REINFORCE 在 CartPole 上的应用
...
![强化学习（三）：Policy Gradient与Actor-Critic方法 — visual](https://blog-pic-ck.oss-cn-beijing.aliyuncs.com/posts/zh/reinforcement-learning/03-policy-gradient-and-actor-critic/illustration_2.jpg)
...
这里只依赖一步随机性，而不是整个轨迹的尾部。方差大幅降低，代价是不完美的 critic 引入了一些偏差，但这笔交易非常划算。
...
同样的轨迹，同样的期望梯度。右图展示了实际收益：方差缩小了一个数量级。这正是 A2C/PPO 的训练曲线比 REINFORCE 平滑得多的原因。
...
DDPG 继承了 DQN 的高估偏差，而且非常脆弱。**TD3**（Fujimoto 等，2018）用三个独立且有效的改进解决了这个问题：
