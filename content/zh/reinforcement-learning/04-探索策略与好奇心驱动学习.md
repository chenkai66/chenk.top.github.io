...
---
把一个全新的智能体放进《蒙特祖玛的复仇》。想拿到第一分，它得完成一系列动作：向右走、跳过骷髅、爬上绳子、跳到平台上，最后抓起钥匙——**整整一百个动作都不能出错**。在拿到钥匙之前，环境给出的奖励始终是 0。

用教科书式的 DQN，设 $\varepsilon = 0.1$，随机探索凑齐这一百个动作的概率大约是 $0.1^{100} \approx 10^{-100}$。毫不意外，普通 DQN 在这个游戏上永远拿 **0 分**——不是“分数低”，而是字面意义上的零分，每一轮训练都如此。

这就是 **稀疏奖励问题**。它揭示了一个残酷的事实：再精妙的 Bellman 备份，碰不到非零奖励也是白费。深度强化学习算法的效果，几乎完全取决于它的探索策略。接下来，我会从最简单的随机探索讲起，逐步介绍 **好奇心驱动学习** 的方法——让智能体自己生成奖励，主动发现新事物。
![强化学习（四）：探索策略与好奇心驱动学习 — visual](https://blog-pic-ck.oss-cn-beijing.aliyuncs.com/posts/zh/reinforcement-learning/04-exploration-and-curiosity-driven-learning/illustration_1.jpg)
...
3. **这些曲线完全没看状态。** 探索只和训练步数有关。这是它的核心弱点，也是后续方法要解决的问题。
...
![不同温度下的 Boltzmann 动作分布与策略熵](https://blog-pic-ck.oss-cn-beijing.aliyuncs.com/posts/zh/reinforcement-learning/04-探索策略�%与好奇心驱动学习/fig2_boltzmann_softmax.png)
...
但 Boltzmann 和 ε-greedy 有一个共同的致命缺陷：它根据 **当前的 Q 估计值** 分配概率，完全不考虑智能体在哪些状态区域已经探索过多少次。两个从未见过的状态，依然会得到相同的 softmax 分布。
...
第一项负责利用，第二项负责探索——拉得越少的臂，不确定性奖励越大。

![5 臂赌博机上 UCB1 的得分构成与拉臂统计](https://blog-pic-ck.oss-cn-beijing.aliyuncs.com/posts/zh/reinforcement-learning/04-探索策略�%与好奇心驱动学习/fig3_ucb_bandit.png)

观察上面这张图。$t = 50$ 时，橙色的 UCB bonus 占主导，所有臂看起来都值得一试，拉臂数被均匀分散。$t = 1000$ 时，最优臂 3 已经被拉了几百次，bonus 显著下降，但其他臂的 bonus 仍然较高，算法逐渐稳定在"主要拉 3，偶尔抽查其他"的模式。这才是合理的探索——它**由数据驱动**，而不是依赖一个固定的 schedule。
...
![3 个 Bernoulli 臂经过 10、50、300 次拉臂后的 Beta 后验](https://blog-pic-ck.oss-cn-beijing.aliyuncs.com/posts/zh/reinforcement-learning/04-探索策略�%与好奇心驱动学习/fig4_thompson_sampling.png)
...
现代探索方法的核心思路转变是：**不再把探索当成训练步数的函数，而是当成智能体自身经验的函数。** 婴儿就是这样——他们玩的总是新玩具，不是旧的。
## 2. 好奇心的核心设计：内在奖励

![强化学习（四）：探索策略与好奇心驱动学习 — visual](https://blog-pic-ck.oss-cn-beijing.aliyuncs.com/posts/zh/reinforcement-learning/04-exploration-and-curiosity-driven-learning/illustration_2.jpg)
...
![Montezuma's Revenge 上不同方法的得分曲线与"首次拿到奖励"耗时](https://blog-pic-ck.oss-cn-beijing.aliyuncs.com/posts/zh/reinforcement-learning/04-探索策略�%与好奇心驱动学习/fig6_sparse_reward_montezuma.png)
