...
---
把一个全新的智能体放进《蒙特祖玛的复仇》。想拿到第一分，它必须精确完成一系列动作：向右走、跳过骷髅、爬上绳子、跳到平台上，最后抓起钥匙——**连续一百个动作中任意一个出错都会失败**。在拿到钥匙之前，环境给出的奖励始终是 0。

用教科书式的 DQN，设 $\varepsilon = 0.1$，随机探索凑齐这一百个动作的概率大约是 $0.1^{100} \approx 10^{-100}$。毫不意外，普通 DQN 在该游戏上始终得分为 **0**——并非得分极低，而是严格意义上的零分，每一轮 episode 均无法获得任何正向奖励。

这就是 **稀疏奖励问题**。它揭示了一个根本局限：若长期无法观测到非零奖励，再精妙的 Bellman 备份也无法有效更新价值函数。深度强化学习算法的效果，几乎完全取决于它的探索策略。接下来，我会从最简单的随机探索讲起，逐步介绍 **好奇心驱动学习** 的方法——让智能体自己生成奖励，主动发现新事物。
...
但 Boltzmann 和 ε-greedy 共享一个根本缺陷：其动作选择仅依赖于当前 Q 值估计，完全不感知各状态区域的历史访问频次。面对两个均未访问过的状态，策略将输出完全相同的 softmax 动作分布。
