...
![ICM 内在好奇模块:特征编码器 + 逆动力学 + 前向动力学,以预测误差作为内在奖励。](https://blog-pic-ck.oss-cn-beijing.aliyuncs.com/posts/en/reinforcement-learning/04-探索策略与好奇心驱动学习/fig04_icm.png)
...
表格型 MDP 让探索问题看起来早就解决了。维护一个计数器 $N(s,a)$，加上 UCB 风格的奖励 $r^+(s,a) = \beta / \sqrt{N(s,a)}$，regret bound 直接照搬多臂老虎机教材。但“表格型”有个硬伤：智能体必须能认出同一个状态。一旦 $s$ 变成 $210 \times 160 \times 3$ 的 Atari 画面，这套逻辑直接失效。

我试过把原始观测直接塞进 dict 当 key。跑完 5 万步，dict 里就躺着 5 万条记录。像素数组根本不可能逐比特完全相同。奖励直接退化成常数 $\beta / 1$。智能体跑起来跟原版 $\varepsilon$-greedy 没两样，只是多绕了点弯路。

Tang 等人 (2017) 给出的解法是 **SimHash**。用固定的随机矩阵把状态投影成 $k$ 个 bit，直接统计生成的 bit-string。视觉相近的状态会掉进同一个 bucket，噪声自然被抹平。$k=64$ 时 bucket 空间高达 $2^{64}$。听起来很大，实际占用的 bucket 数量却很克制。投影操作本身就把画面相似的状态捆在了一起。
...
把这模块接到任意 DQN 智能体前面，把 `bonus(obs)` 叠到环境奖励上。在 Montezuma's Revenge 上，Tang 等人报告 5000 万帧内分数从 **0** 飙到 **2,500** 左右。算不上 SOTA，但三十行代码能跑通，已经证明了可行性。

实际跑起来容易踩两个坑。一是忘记归一化，奖励直接反客为主。$\beta = 0.1$ 放在每 1000 步才给一次真实奖励的游戏里，curiosity 直接接管 policy gradient。二是投影矩阵吃白化数据。$[0, 255]$ 的原始像素会让所有 hash 在高幅值维度上疯狂碰撞。投影前记得除以 255，或者上游直接接个训练好的 encoder。

$k$ 的取值没想象中那么敏感。$k < 32$ 时 bucket 太粗，Montezuma 里不同的房间会撞车。$k > 256$ 时每个状态都独一无二，计数根本攒不起来。中间区间都能跑，简单环境可以稍微往小了调。
...
计数法能跑，但手法偏糙。Pathak 等人 (2017) 往前推了一步：跟 policy 并行训练两个小网络，拿预测误差当奖励。这套组件叫 **Intrinsic Curiosity Module (ICM)**，拆成三块。

特征 encoder $\phi: \mathcal{S} \to \mathbb{R}^d$ 负责把观测压成 embedding。**inverse model** $g(\phi(s_t), \phi(s_{t+1})) \to \hat a_t$ 负责反推连接前后状态的动作。**forward model** $f(\phi(s_t), a_t) \to \hat\phi_{t+1}$ 拿当前 embedding 和动作去猜下一个 embedding。

内在奖励直接取 forward model 的残差：
$$
r^i_t = \eta \, \bigl\| \hat\phi_{t+1} - \phi(s_{t+1}) \bigr\|^2.
$$
这套设计最妙的地方是抗 noisy-TV 干扰。$\phi$ 只吃 inverse model 的 loss。只有当某个特征维度能帮上动作预测时，梯度才会流进 encoder。随机像素噪声天生跟动作预测无关，直接被 $\phi$ 过滤掉。forward model 不用去猜噪声，智能体也不会对着雪花屏发呆。
...
接进 PPO 的流程很固定。每个 rollout step 跑一遍 `r_int, fwd_loss, inv_loss = icm(s, s_next, a)`。算 advantage 之前把 `r_int` 叠到环境奖励上。把 `fwd_loss + inv_loss` 当辅助 loss 喂给 policy 的 optimizer。论文默认 $\eta = 0.5$，forward 和 inverse loss 权重按 0.2/0.8 切分。

在 VizDoom 的 MyWayHome（稀疏奖励导航）上，纯外在奖励 PPO 跑完全部预算还是 0 分。ICM-PPO 大概 500 万帧就能通关。同一套模块扔给 Mario，零外在奖励也能跑完 1-1 关卡。往前走会不断给 forward model 制造预测误差，智能体自然就学会了往前冲。

调参时注意一点。内在奖励应该随训练逐步衰减，因为 forward model 在变准。如果曲线一直走平，要么 forward model 容量太小，要么 encoder $\phi$ 坍缩成了常数。inverse 和 forward loss 同时逼近零就是典型信号。补救办法很简单：给 $\phi$ 加一点 reconstruction loss 防坍缩，或者把 inverse model 拓宽，逼它真正用上特征。
...
ICM 能跑，但在随机环境里训 forward dynamics model 极其脆弱。Pathak 自己的后续工作也承认，inverse model 的 trick 没法完全抹平动作相关的随机性。Burda 等人 (2018) 直接砍掉整套 dynamics 结构。核心思路很直白：没必要去猜*下一个*状态。只需要一个靶子，状态访问越多，靶子误差越小。

随机初始化一个网络 $f$，永久冻结。再训一个网络 $\hat f$ 去拟合它在观测状态上的输出。奖励直接取平方残差：
$$
r^i(s) = \bigl\| \hat f(s) - f(s) \bigr\|^2.
$$
常访问的 $s$，predictor 已经收敛，残差很小。没见过的 $s$，predictor 没吃过梯度，输出贴近随机初始值，离靶子十万八千里。

相比 ICM 的核心优势是 $f$ *固定不动*。预测和靶子漂移之间不存在 Bellman 式的死循环。靶子纯粹是 $s$ 的确定性函数。环境转移再随机也无所谓，$f$ 根本不看转移过程，只认状态。

```python
class RND(nn.Module):
    """Random Network Distillation (Burda 2018) 精简版"""
...
论文里有两个超参绝对不能动。一是用滑动标准差归一化内在奖励。不做这一步，奖励尺度会在训练过程中漂移几个数量级，要么直接淹没外在奖励，要么彻底消失。二是 policy 必须上**两个 value head**，一个管 $r^{\text{ext}}$，一个管 $r^{\text{int}}$，折扣因子拆开设置（$\gamma_{\text{int}} = 0.99$，$\gamma_{\text{ext}} = 0.999$）。我试过的所有游戏，混成单 head 全部掉点。

在 Montezuma's Revenge 上，RND 是第一个击穿人类专家 7,385 分基准的方法。均分 8,152，偶尔能破 1.1 万。代码比 ICM 简洁，训练也更快，毕竟省掉了 inverse model 的反向传播。
...
好奇心奖励分两派思路。ICM 和 RND 拿*预测误差*衡量新颖性。另一派扎根于 Bayesian active learning，用*信息增益*说话：看到这次状态转移，能多大程度更新我对环境的认知？

VIME (Houthooft et al., 2016) 把这套逻辑落了地。维护 dynamics model 参数的 Bayesian posterior $p(\theta \mid h_t)$，专门奖励那些能推动 posterior 变化的转移：
$$
r^i_t = D_{\mathrm{KL}}\bigl( p(\theta \mid h_t, a_t, s_{t+1}) \,\|\, p(\theta \mid h_t) \bigr).
$$
模型早就吃透的转移，KL 散度逼近零，没有信息增益。出乎意料的转移会拉动 posterior，直接发奖励。这本质上就是 active learning 里“挑信息量最大的点查询”的 RL 版本。

原版实现依赖 Bayes-by-Backprop，调起来极其折磨。Pathak 等人 (2019) 证明 **ensemble disagreement** 效果一样好，成本低一个数量级。做法很直接：用不同的 bootstrap 样本训 $K$ 个 dynamics model，直接拿预测方差当奖励。

```python
class EnsembleDisagreement(nn.Module):
    """基于 ensemble 方差的信息增益奖励 (Pathak 2019)"""
    ...
    def bonus(self, s, a):
        ...
        return preds.var(dim=0).mean(dim=-1)  # 计算 ensemble 间的方差
...
每个 ensemble 成员吃 replay buffer 的不同随机子集。智能体没怎么探过的区域，模型间的分歧自然拉大。这套奖励有干净的 Bayesian 解释，近似 posterior 方差。跟 RND 不同，它的尺度跟着 *dynamics* model 的不确定性走，而不是 feature predictor。遇到新颖性藏在转移结构里、而非边缘状态分布的任务，这点差异很关键。

这套思路跟 active learning 的关联不止停留在表面。VIME 和 ensemble disagreement 底层逻辑一致：行动的目标是最大化关于 $\theta$ 的期望信息增益。这正是标准的 Bayesian experimental design 目标。RL 的特殊点在于，智能体得输出完整 policy，不能单次挑 query。奖励必须走 Bellman backup 传播，不能贪心硬吃。落地时直接把 $r^i$ 当普通奖励扔进去，让 value function 沿轨迹做累积就行。
...
好奇心奖励不是白送的。我上线过一些智能体，内在奖励刷出新高，实际任务表现却一塌糊涂。三种翻车模式反复出现，值得单独拎出来命名。

**Noisy TV。** 原版 ICM 论文专门提过这个场景：环境里摆一台播雪花屏的电视。朴素的 forward model 永远猜不准下一帧，误差直接锁死在最大值。智能体学会杵在屏幕前发呆。ICM 的 inverse model trick 能过滤动作无关的噪声。RND 能扛住是因为随机 CNN 会把所有噪声帧压到相近的特征点，predictor 很快就能拟合均值。但两者都防不住动作*相关*的随机性。

**Bonus reward farming。** 更隐蔽，也更搞心态。智能体偶然发现某类状态的残差略高，比如跳跃中途光照差了一个字节的画面。它学会原地反复横跳。仪表盘上内在奖励曲线一片大好，外在奖励直接归零。这种问题只能靠肉眼回放轨迹才能抓出来。

**Drowning the task signal。** $\eta$ 设得太高，policy gradient 直接被内在回报接管，外在奖励沦为舍入误差。补救办法要么在训练过程中 anneal $\eta$，要么把两路奖励都归一化到单位方差，按*比例*调权重，别盯绝对值。

写个轻量监控脚本，三种情况一把抓：

```python
def curiosity_health_check(r_int_buf, r_ext_buf, window=10_000):
    """记录 r_int / r_ext 比例，内在奖励反客为主时报警"""
    r_int_mean = sum(r_int_buf[-window:]) / window
    r_ext_mean = sum(r_ext_buf[-window:]) / window + 1e-8
    ratio = r_int_mean / r_ext_mean
    if ratio > 5.0:
        print(f"[WARN] r_int/r_ext = {ratio:.1f} — 内在奖励正在接管策略")
    if r_ext_mean < 1e-6 and len(r_ext_buf) > 5 * window:
        print("[WARN] 外在奖励连续 5 个窗口静默 — 检查环境接线")
    return ratio
```

我最终敲定的实操配方很固定：永远用滑动标准差归一化 $r^i$。每 1 万步打一次上面的比例日志。初始值设 $\eta = 0.01$（RND）或 $\eta = 0.5$（ICM，作用于*归一化后*的内在奖励）。如果智能体吃掉 20% 训练预算后外在进度依然卡死，先把 $\eta$ 砍半，再去动其他配置。Curiosity 是一套需要精细调参的体系，从来不是免费午餐。
