...
---
GPT-3（2020年6月）和ChatGPT（2022年11月）共享了大部分权重。基础模型能写出流畅的散文、补全代码、续写任意给定模式，但直接向其提问时，却常答非所问、无端拒绝、编造虚假引用，甚至输出有害内容。这两年半没有用于扩大Transformer模型的规模，而是聚焦于一个更根本的问题——如何让模型真正有用，最终发现这是一个强化学习问题。
...
预训练模型学会了互联网文本的**分布**，包括我不想要的部分；而RLHF让模型学会我想要的东西。2022年从OpenAI流传出来的这套方法已成为行业标准，分为三个清晰的阶段：监督阶段、偏好阶段和强化阶段。
