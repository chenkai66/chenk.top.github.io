...
第三步给你一个**信号**——保持平衡时是小小的奖励，摔倒时是狠狠的惩罚。经过多次尝试，大脑逐渐形成一套**策略**：从“我现在感知到什么”到“我接下来该做什么”的映射。这套策略不会写出来，而是通过这个循环本身刻进你的反射里。

强化学习就是用数学形式描述这个循环。图中的“你”叫**智能体**（agent），自行车和路面是**环境**（environment），倾斜和转向是**动作**（action），倾斜角度和速度是**状态**（state），不摔倒的感觉是**奖励**（reward）。文章后面的内容，不过是围绕这张图做更细致的记录而已。
