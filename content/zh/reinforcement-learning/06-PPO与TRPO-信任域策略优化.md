...
3. **不可逆性**：监督学习走错一步只是损失变差；RL 走错一步会直接丢掉数据分布，事后修复需要的样本量可能是出错代价的十倍。
...
教训很明确：**参数空间里固定步长的更新，可能在策略行为上引发任意大的变化**。任何"安全"保证都必须建立在*分布空间*中，而 KL 散度 $D_{KL}(\pi_{\text{old}} \| \pi_\theta)$ 是最自然的度量。
