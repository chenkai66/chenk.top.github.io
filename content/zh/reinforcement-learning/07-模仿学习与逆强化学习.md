...
即使有最大熵正则化，$\hat r$ 仍只能恢复到**整形不变性**（shaping invariance）的程度：添加势函数 $\Phi(s') - \Phi(s)$ 不改变最优策略，但会改变奖励值。因此，恢复的奖励仅提供状态间的**相对排序**，而非绝对尺度。对抗式逆强化学习（AIRL，见下文）显式地解耦了整形成分。
...
- **上一篇**: [第 6 部分 — 高级策略优化](/zh/reinforcement-learning/06-ppo与trpo-信任域策略优化/)
- **下一篇**: [第 8 部分 — AlphaGo 与蒙特卡洛树搜索](/zh/reinforcement-learning/08-alphago与蒙特卡洛树搜索/)
- [查看强化学习系列全部 12 篇](/zh/tags/reinforcement-learning/)
