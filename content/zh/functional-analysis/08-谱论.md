...
具体反例：$L^2[0,1]$ 上 $M f(x) = x f(x)$。自伴（$\langle Mf, g \rangle = \int x f \bar g = \langle f, Mg\rangle$），有界（$\|M\| = 1$），谱 $[0, 1]$，但**没有任何特征向量**。求 $x f(x) = \lambda f(x)$ 即 $(x - \lambda) f(x) = 0$ 几乎处处，逼出 $f = 0$ 在 $L^2$ 中。所以"特征向量正交基"对 $M$ 不存在。
