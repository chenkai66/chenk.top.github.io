...
**Cauchy-Schwarz.** 对于所有 $x, y \in \mathcal{H}$，$|\langle x, y \rangle| \leq \|x\| \|y\|$，当且仅当 $x, y$ 线性相关时取等号。
...
**Riesz 表示定理。** 设 $\varphi: \mathcal{H} \to \mathbb{C}$ 是 Hilbert 空间上的连续线性泛函。则存在唯一的 $y \in \mathcal{H}$ 使得对于所有 $x \in \mathcal{H}$ 有 $\varphi(x) = \langle x, y \rangle$。此外，$\|\varphi\| = \|y\|$。
