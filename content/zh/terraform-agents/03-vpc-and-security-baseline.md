...
---
今天要写的是我 Agent 项目里被复制次数最多的 Terraform 代码：一个 `vpc-baseline` 模块。它给后续所有组件（ECS、RDS、OpenSearch、ACK）提供了一个合理的落脚点。总共大概 200 行 HCL，值得亲手敲一遍，以后反复引用。
...
为什么要三个可用区？因为阿里云随时可能在某个周日进行可用区级维护，单可用区部署意味着整个窗口期你的 Agent 都得下线。VPC 内跨区流量免费；三个可用区唯一的成本是子网划分的运维复杂度，这部分 Terraform 会帮你 absorb 掉。
...
强制要求三个可用区确实有点 opinionated，但符合架构图。如果你需要两区或四区，直接 fork 这个模块——别把它做成条件判断。带条件的模块会变得难以阅读，而难以阅读的模块半年后就得从头重写。
...
- `substr(each.value, -1, 1)` 提取可用区 ID 的最后一个字符（`l`/`m`/`n`），这样资源名在控制台里排序好看。
...
Enhanced NAT 是现代类型——Tablestore、PrivateLink 和大多数新服务都要求用它。老式的 "Standard" NAT 迟早会被 deprecated，别在新项目上用。按流量计费（PayByTraffic）适合 Agent 负载，因为出站带宽是突发性的（LLM streaming）而不是steady的。

SNAT 条目才是让私网子网实例能通互联网的关键。少了它们，`private-a` 里的 Agent 解析不了 `dashscope.aliyuncs.com`——第一次遇到这问题你会花一个小时调试。这坑我踩过，血泪教训。

## 安全组，分层设计

在阿里云上做安全组的正确方式是**每层一个 SG**，规则引用 SG ID 而不是 CIDR：
...
> **实战建议：** 阿里云的默认行为是*拒绝*所有入站，*允许*所有出站。这个默认值是对的——别加什么“拒绝所有出站”规则，那只会把 SDK 调用搞挂。除非你有特定的合规要求，否则限制出站流量没必要；对 Agent 系统来说，出站全开是常态。
...
`pending_window_in_days = 7` 意味着删除的密钥有 7 天窗口期可以恢复。别缩短这个时间——误删密钥是能终结职业生涯的错误，这个恢复窗口救过我不止一次。
## 模块的输出
