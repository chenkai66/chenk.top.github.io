...
---
Agent 具备非确定性、多步骤执行特性，又频繁调用高成本 API——三者叠加意味着：若上线首日未完成可观测性埋点，故障发生后将极难定位与调试。本文用 Terraform 打通日志、链路追踪、指标三条管线，全部汇聚至统一仪表盘；配套六个可直接用于排查真实故障的 SLS 查询，以及四个已在生产环境成功拦截故障的钉钉告警。

完成配置后，你将拥有一个高可用钉钉告警群：账单异常激增、延迟持续飙升、错误率显著偏离基线，或 Agent 出现死循环迹象时，系统即时通知；再搭配 SLO 预算看板，把模糊的运维直觉变成可度量的数据。
...
这个仪表盘就是“哪个 agent 在烧我的预算？”这个问题的答案——该问题每月都会被提出，提问者通常是不熟悉 Agent 架构的相关方（如财务或业务团队）。
## 六个真正派得上用场的 SLS 查询
