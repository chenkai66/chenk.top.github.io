...
---
Agent 具备非确定性、多步骤执行特性，并且频繁调用高成本 API——这三者叠加意味着，若上线首日未完成可观测性埋点，故障发生后将极难定位与调试。本文用 Terraform 打通日志、链路追踪和指标三条管线，全部汇聚至统一仪表盘，并配套六个可直接用于排查真实故障的 SLS 查询及四个已在生产环境成功拦截故障的钉钉告警。

完成配置后，你将拥有一个高可用钉钉告警群。当账单异常激增、延迟持续飙升、错误率显著偏离基线或 Agent 出现死循环迹象时，系统会即时通知。再搭配 SLO 预算看板，可以将模糊的运维直觉变成可度量的数据。
...
不要只接日志或只接指标，日志、链路追踪和指标三者缺一不可：

- 日志回答"agent 做了什么？”
- 链路追踪回答“时间花哪儿了？”
- 指标回答“这事儿是不是比平时发生得更频繁？”

这里最省钱的错误做法是只靠 `print()` 输出到 stdout 就上线。最昂贵的错误做法是三者都上了，但直到第一次出事故才打开仪表盘——这时候你才发现因为 SDK 没传递上下文，`agent` 字段有时候是 `null`。务必在第一天就打通全部管线，并坚持每周验证，确保其持续可用。

## Step 1: SLS project 和 logstores

所有可观测性相关的工作都始于一个 SLS 项目。每个环境一个是对的，每个 agent 一个就太细了。
...
Logtail 配置，包括采集哪些文件和如何解析，是一个 Terraform 资源：
...
现在，任何 tagged 机器上匹配 `/var/log/agents/*.log` 的文件都会以 JSON 格式流入 SLS，所有字段都可查询。 Agent 代码只需做 `logger.info(json.dumps({...}))`，剩下的自动完成。关键在于对 schema 保持纪律——尽早确定 `ts`, `agent`, `session_id`, `step`, `phase`, `tokens`, `latency_ms`, `cost_cny`, `status`, `error_message`，写好文档，然后拒绝那些 emit 临时字段的 PR。每个未文档化的字段，都可能在凌晨三点阻碍你编写出有效的诊断查询。

## Step 3: 通过 OpenTelemetry → ARMS 做链路追踪

对于 traces， ARMS APM 兼容 OpenTelemetry。 Terraform 配置很简单——开通一个 ARMS 实例和环境：
...
好处在于：在 ARMS 里你能看到“这次 agent 运行花了 12 秒；其中 9 秒是第三次调用 qwen-max 的 LLM 请求”。这种可见性真正会改变你构建 agent 的方式。我在真实生产会话中首次观察到 span 树后，立即重构了 planner：对较短的用户消息跳过 retrieval 环节，当周 p95 延迟因此下降约 30%。
...
对于自定义应用指标，如“research-agent 消耗的 tokens”，将其作为带结构化字段的 SLS 日志条目发出，然后通过 SLS 查询告警。 SLS-as-metrics 是阿里云主推的模式；虽然 CloudMonitor 自定义指标也能用，但从 Terraform 接入更笨重，还会迫使你将告警拆分到两个控制台。选择一个即可。我选择 SLS 处理所有事情，除了主机 CPU、内存和磁盘。
...
SLS 支持在这些数据上执行 SQL 查询——例如‘每个 agent 的每日成本’：
...
仪表盘适合回答长期趋势问题，故障排查则依赖即席查询。经过两年 Agent 生产环境运维实践，我在个人知识库中沉淀了约 12 条高频使用的 SLS 查询语句。本文精选其中 6 条，因其具备跨技术栈复用能力，适用于多种 Agent 实现。
...
凌晨三点收到告警时的你，定会感激此刻配置完善的自己。其余五条查询同理：仅需一个晚上的 Terraform 编写工作，即可在首次生产告警触发时即收回投入。
...
当天 LLM 花费超过 800 块就触发。阈值按你实际预算调。节流（Throttling）至关重要——否则单个问题持续一小时就会触发 12 次告警，导致团队迅速将告警渠道静音。
...
这个告警回本了两次。 Stop condition 存在缺陷的 Agent，一夜之间可能消耗上万元 Token；该告警可在 2 分钟内捕获异常，为你争取 Kill 掉问题实例的时间。第一次在我生产环境触发时，大概省了 6400 块——一个 malformed JSON tool response 让 planner 永远回到第一步。
## 报警疲劳：我保持频道高信噪比的几条原则
...
这样就把“某一分钟出错就呼人”变成了“连续五分钟出错才呼人”。由单次 LLM 调用延迟引发的分钟级抖动不会触发告警，只有持续性问题才会。
...
有了 Runbook 链接，“为啥呼我”就变成了“我知道该干嘛”。 MTTR 大概能砍掉一半。应像管理代码一样维护 Runbook：报警规则变更时同步 Review，每季度清理过期内容。
