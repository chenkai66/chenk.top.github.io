...
---
Agent 具备非确定性、多步骤执行特性，并且频繁调用高成本 API——这三者叠加意味着，若上线首日未完成可观测性埋点，故障发生后将极难定位与调试。本文用 Terraform 打通日志、链路追踪和指标三条管线，全部汇聚至统一仪表盘，并配套六个可直接用于排查真实故障的 SLS 查询和四个已在生产环境成功拦截故障的钉钉告警。
