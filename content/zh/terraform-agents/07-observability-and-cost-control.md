...
---
Agent 具有非确定性、多步骤执行特性，且频繁调用高成本 API。这三者叠加导致：若上线首日未完成可观测性埋点，故障发生后将极难定位和调试。这篇文章通过 Terraform 打通三条管线——日志、链路追踪、指标——汇聚到一个统一仪表盘，配套六个能直接用于排查真实故障的 SLS 查询，并配置四个已在生产环境成功拦截故障的钉钉告警。

完成这套配置后，你将获得一个高可用的钉钉告警群：当出现账单异常激增、延迟持续飙升、错误率显著偏离基线，或 Agent 出现死循环迹象时，系统会及时通知你；同时配套 SLO 预算看板，将模糊的运维直觉转化为可度量的运维数据。
...
仪表盘适合回答长期问题，处理故障得靠即席查询。经过两年 Agent 生产环境运维实践，我在个人知识库中沉淀了约 12 条高频使用的 SLS 查询语句。本文精选其中 6 条，因其具备跨技术栈复用能力，适用于多种 Agent 实现。
