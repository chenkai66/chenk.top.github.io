...
---
Agent 具备非确定性、多步骤执行特性，并且频繁调用高成本 API。这三者叠加意味着，若上线首日未完成可观测性埋点，故障发生后将极难定位与调试。本文用 Terraform 打通日志、链路追踪和指标三条管线，全部汇聚至统一仪表盘，并配套六个可直接用于排查真实故障的 SLS 查询和四个在生产环境中成功拦截故障的钉钉告警。

完成配置后，你将拥有一个高可用钉钉告警群。当账单异常激增、延迟持续飙升、错误率显著偏离基线或 Agent 出现死循环迹象时，系统会即时通知。搭配 SLO 预算看板，可以将模糊的运维直觉变成可度量的数据。
...
好处在于，在 ARMS 里你能看到“这次 agent 运行花了 12 秒；其中 9 秒是第三次调用 qwen-max 的 LLM 请求”。这种可见性真正会改变你构建 agent 的方式。在真实生产会话中首次观察到 span 树后，我立即重构了 planner：对较短的用户消息跳过 retrieval 环节，当周 p95 延迟因此下降约 30%。
