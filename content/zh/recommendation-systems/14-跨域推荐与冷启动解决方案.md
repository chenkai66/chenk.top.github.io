...
---
Netflix 进入新国家时，接手的是几百万完全没有历史记录的用户和一个没有任何本地评分的内容库。亚马逊每次开拓新品类时也会遇到同样的问题。纯协同过滤（CF）——推荐系统在热启动阶段的主力方法——在这种场景下完全无用武之地。要让推荐系统在这种冷启动环境中发挥作用，需要依靠一整套技术：首次请求时使用启发式 Bootstrap 方法快速生成初始推荐，积累少量交互后切换到元学习提升模型适应能力，如果存在相关领域则利用跨域迁移引入外部知识，最后在模型有一定置信度后用 Bandit 策略持续探索未知空间。这篇文章会详细拆解这套技术栈，并且每一层都直接对应到其来源的经典论文。

![推荐系统（十四）—— 跨域推荐与冷启动解决方案 — visual](https://blog-pic-ck.oss-cn-beijing.aliyuncs.com/posts/zh/recommendation-systems/14-跨域推荐与冷启动解决方案/illustration_1.jpg)
## 你将学到什么
...
一个新用户注册了。我可能知道他的设备类型、IP 地址归属地、引流广告渠道，甚至从注册表单里获取了一个年龄段信息。但显式交互行为为 **零**。他在评分矩阵中的那一行完全是空白的。行业数据显示，首会话推荐质量差的用户，30 天流失率大约是普通用户的 3 倍。解决这个问题的关键在于 **稀疏信号推断 + 主动探索**。

### 物品冷启动

商家上架了一个新 SKU，片方发布了一部新电影，创作者上传了一段新视频。这些新物品在矩阵中新增了一整列空白。协同过滤（CF）无法检索到这些物品，因为没有任何用户与它们发生过交互。如果不干预，这些物品永远得不到曝光，无法积累评分，最终沉入长尾 —— 这就是典型的“强者愈强”问题。解决这一问题的核心是 **内容特征**：标题、封面图、预训练编码器生成的嵌入向量，以及通过相似的“温”物品进行冷启分数传递。

### 系统冷启动

一个新平台上线，或者现有平台扩展到一个全新业务领域。此时，用户-物品矩阵几乎完全空白，行和列同时稀疏。解决这一问题的关键在于 **迁移**：从相关且数据丰富的源域借用嵌入向量、映射关系，甚至是排序模型。
...
EMCDR 的短板在于，单一映射函数无法捕捉不同用户跨域迁移的差异。比如，恐怖片爱好者和纪录片爱好者映射到图书域的方式可能完全不同。[Zhu 等人，WSDM 2022](https://arxiv.org/abs/2110.11154) 提出了 **PTUPCDR (Personalized Transfer of User Preferences for Cross-Domain Recommendation)**：不再使用单一的 $\phi$，而是通过元网络根据用户在源域的行为，为每个用户生成个性化的 $\phi_i$。

$$\phi_i = h_\theta\bigl(\{\mathbf{v}_j^S : j \in \mathcal{H}_i^S\}\bigr), \qquad \hat{\mathbf{u}}_i^T = f_{\phi_i}(\mathbf{u}_i^S)$$

简单来说：读取用户在源域的历史行为，将其压缩为一组桥接权重，再用这组个性化桥接参数映射用户的源域嵌入。在 Amazon 跨品类标准基准测试中，PTUPCDR 的 MAE 比 EMCDR 降低了 5–10%。
...
跨域推荐不是万能的。如果源域和目标域关联性较弱（例如新闻阅读 vs. 生鲜购物），桥接可能会**拖累**目标域的表现。判断方法很简单：跑一个仅使用目标域数据的基线模型做 A/B 测试，观察冷启动用户的指标。如果跨域模型在相同训练成本下无法超越基线，说明两个域的关联性太弱了。
## 元学习冷启动
...
![推荐系统（十四）—— 跨域推荐与冷启动解决方案 — visual](https://blog-pic-ck.oss-cn-beijing.aliyuncs.com/posts/zh/recommendation-systems/14-跨域推荐与冷启动解决方案/illustration_2.jpg)
...
![左：四个臂的估计奖励柱状图，每根柱子上方加了一段表示不确定性奖励的须；少拉的臂须更长。右：累积 regret 对比，UCB 与 Thompson 是对数级，ε-greedy 线性增长但更慢，pure greedy 卡在次优臂上线性涨，random 涨得最快](https://blog-pic-ck.oss-cn-beijing.aliyuncs.com/posts/zh/recommendation-systems/14-跨域推荐�%与冷启动解决方案/fig5_ucb_exploration.png)
