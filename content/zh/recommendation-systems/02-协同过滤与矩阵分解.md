...
这个简单的想法——让群体行为说话——撑起了 Amazon、YouTube、Spotify 以及你每天刷到的推荐流。这篇文章将详细讲解背后的算法演变：从 1990 年代的邻域方法，到拿下 Netflix Prize 的矩阵分解模型。
...
这就够了。 CF 不需要类型标签、演员列表或用户画像，只需记录“谁对什么有过交互”。基于这一假设，衍生出两种方法：
