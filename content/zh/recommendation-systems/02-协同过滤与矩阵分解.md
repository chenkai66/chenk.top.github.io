...
---
刚看完《肖申克的救赎》，想找一部类似感觉的电影。用类型筛选，结果会塞满一堆糟糕的监狱题材烂片。协同过滤换了个思路：它根本不关心电影本身，而是问了一个简单的问题——**和你看过相同内容的人，还喜欢什么？**

这个简单的想法——让群体行为说话——撑起了 Amazon、YouTube、Spotify 和今天你刷到的每一条推荐流。这篇文章要讲清楚背后的算法演变：从 1990 年代的邻域方法，到拿下 Netflix Prize 的矩阵分解模型。

> **你将学到：**
> - 邻域协同过滤的两条路线（User-CF 和 Item-CF）以及各自的适用场景
> - 如何用余弦、皮尔逊、调整余弦衡量“相似口味”
> - 为什么在稀疏数据上矩阵分解比邻域方法更有效
> - SGD 和 ALS：同一个模型，训练方式完全不同
...
这就够了。CF 不需要类型标签、演员列表，也不需要用户画像，只需要记录“谁对什么有过交互”。从这个假设出发，自然衍生出两种方法：

- **User-CF**：找到和你历史行为相似的用户，推荐他们喜欢的东西。
- **Item-CF**：找到和你已经喜欢的物品相似的其他物品，推荐给你。
...
**User-CF 的逻辑**：Alice 和 Bob 在《肖申克》和《阿甘》上的评分完全一致，说明他们的品味接近。Bob 给《当幸福》打了 4 分，于是我把《当幸福》推荐给 Alice。

**Item-CF 的逻辑**：《肖申克》和《阿甘》在所有用户中的评分分布几乎相同，说明这两部电影相似。如果你喜欢其中一部，大概率也会喜欢另一部。

### 优势与局限

CF 的优势在于**简单**（不需要特征工程）和**惊喜感**（能挖掘出非显而易见的关联，比如经典的“啤酒与尿布”案例）。它还能跨品类推荐——比如把播客推荐给一个爱看小说的用户。

但它的局限也很明显：

- **冷启动**：新用户或新物品没有历史数据。
- **稀疏性**：真实场景中的评分矩阵 99% 以上都是空的。
