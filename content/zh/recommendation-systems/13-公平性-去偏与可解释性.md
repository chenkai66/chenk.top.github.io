...
---
用户打开 Spotify，听到的还是那重复的五十首歌。打开 Amazon，首页推荐的商品总是之前浏览过的。打开 YouTube，每条推荐都指向一个不记得自己点过的“兔子洞”。这些问题都有名字、有原因、也有解决办法。本文就来聊聊这三点。

![推荐系统（十三）—— 公平性、去偏与可解释性 — visual](https://blog-pic-ck.oss-cn-beijing.aliyuncs.com/posts/zh/recommendation-systems/13-公平性-去偏与可解释性/illustration_1.jpg)
## 你将学到什么

- **七种偏差**如何系统性扭曲用户看到的内容：每种偏差的来源、测量方法和影响
...
- 一个实用的**权衡框架**，让我能在准确率和公平性的 Pareto 边界上选择最佳工作点
## 前置知识

- 熟悉基于嵌入的推荐系统（[第 4 篇](/zh/recommendation-systems/04-ctr预估与点击率建模/) 和 [第 5 篇](/zh/recommendation-systems/05-embedding表示学习)）
- 了解基本的因果推断术语会有帮助，但不是必须的——我会从头讲起
- 能看懂 PyTorch 风格的伪代码
## 第一部分 —— 七种偏差

推荐系统中的偏差不是一个单一问题，而是至少七个问题的叠加。以下分类来自 Chen 等人（2023 年，《Bias and Debias in Recommender System》）的综述——这是了解完整文献地图的最佳起点。
...
少数物品占据了绝大部分用户交互，推荐系统进一步放大了这种集中效应。右图说明了一切：即使目录分布均匀，前 20 个物品仍占据了超过 60% 的推荐位。
...
取对数削弱了头部热门物品的影响，避免指标被极少数超级热门物品主导。要按每个 slate 单独追踪，而不是只看全局——全局均值会掩盖用户层面的集中现象。
...
用户更倾向于检视靠前位置的内容，而忽略靠后位置。经典的*检视假设*（Richardson、Joachims 等）将点击分解为两个独立事件：
$$P(\text{click} \mid u, i, k) = P(\text{examine} \mid k) \cdot P(\text{relevant} \mid u, i)$$
如果直接用日志点击数据训练模型，你会把"排在第一位"和"真正相关"混为一谈。位置偏差是工业界 learning-to-rank 中研究最透彻的偏差，修复方案——**逆倾向得分（IPS）**——也是这个季度就能落地的技术。
...
用户不会随机打分。他们要么喜欢某个物品，要么对它失望；不温不火的中间地带很少进入日志。这是典型的*非随机缺失*（MNAR）。Marlin 和 Zemel（2009）证明，在 MovieLens 类数据集上忽略它会让 RMSE 虚高 10–30%。解决方法与位置偏差类似：显式建模缺失机制，然后加权或填补。

### 4. 曝光偏差 —— 看不到就点不到

系统只能学习它展示过的物品。新物品、长尾物品、小众创作者的作品获得的曝光更少，导致交互更少、预测相关性更低、曝光进一步减少。这就是让推荐系统随时间衰退的闭环反馈。
...
一旦模型认为它懂你，每条推荐都会强化这个信念。多样性消失，惊喜不再，用户接触到的思想范围逐渐缩小。这是监管机构最担心的偏差，因为它作用于群体层面，而非个体。
...
修任何东西之前，先把仪表盘搭好。下面这个类是我每次上线新推荐系统前必跑的最小集合。
...
Gini 系数借自经济学，用来衡量收入不平等。在这里，它衡量的是*曝光不平等*。Gini > 0.8 意味着少数物品独占 slate，其余物品无人问津。
## 第二部分 —— 推荐系统的因果推断

![推荐系统（十三）—— 公平性、去偏与可解释性 — visual](https://blog-pic-ck.oss-cn-beijing.aliyuncs.com/posts/zh/recommendation-systems/13-公平性-去偏与可解释性/illustration_2.jpg)
