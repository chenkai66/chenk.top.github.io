...
少数物品占据了大部分用户交互，推荐系统进一步放大了这种集中效应。右图直观呈现了这一现象：即使物品目录本身分布均匀，前 20 个物品仍占据了超过 60% 的推荐曝光。
...
Gini 系数借自经济学，用来衡量收入不平等。在这里，它衡量的是*曝光不平等*。Gini 系数大于 0.8 表明曝光高度集中：少数物品占据绝大部分 slate 曝光，其余物品几乎无曝光。
## 第二部分 —— 推荐系统的因果推断
