...
---
用户打开 Spotify，反复听到的仍是那五十首歌；打开 Amazon，首页推荐的商品总离不开此前浏览过的；打开 YouTube，每条推荐都把人引向一个自己都不记得点进过的“兔子洞”。这些问题各有其名、成因与解法。本文就来聊聊这三点。
...
少数物品占据了绝大部分用户交互，推荐系统进一步放大了这种集中效应。右图直观呈现了这一现象：即便物品目录本身分布均匀，前 20 个物品仍占据了超 60% 的推荐曝光。
...
取对数可缓解头部热门物品的支配效应，防止指标被极少数超级热门物品扭曲。需按每个 slate 单独统计，而非仅依赖全局均值——后者会掩盖用户粒度的集中现象。
...
用户更倾向于检视靠前位置的内容，而忽略靠后位置。经典的*检视假设*（Richardson、Joachims 等）将点击行为解耦为两个独立过程：
$$P(\text{click} \mid u, i, k) = P(\text{examine} \mid k) \cdot P(\text{relevant} \mid u, i)$$
若直接使用日志中的点击数据训练模型，模型易将“排在首位”误判为“内容相关”。位置偏差是工业界 learning-to-rank 中研究最透彻的偏差，修复方案——**逆倾向得分（IPS）**——也是这个季度就能落地的技术。
...
用户不会随机打分。他们要么喜欢某个物品，要么对它失望；不温不火的中间地带很少进入日志。这属于典型的*非随机缺失*（MNAR）情形。Marlin 和 Zemel（2009）证明，在 MovieLens 类数据集上忽略它会让 RMSE 虚高 10–30%。解决方法与位置偏差类似：显式建模缺失机制，然后加权或填补。

### 4. 曝光偏差 —— 看不到就点不到

系统只能学习它展示过的物品。新物品、长尾物品及小众创作者的作品曝光不足，进而导致交互稀疏、相关性预测不准，最终曝光进一步萎缩——形成加速系统退化的闭环反馈。
...
Gini 系数借自经济学，用来衡量收入不平等。在这里，它衡量的是*曝光不平等*。Gini 系数 > 0.8 表明曝光高度集中：少数物品占据绝大部分 slate 曝光，其余物品几无曝光。
## 第二部分 —— 推荐系统的因果推断
