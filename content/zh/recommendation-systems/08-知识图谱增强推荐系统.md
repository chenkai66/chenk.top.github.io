...
---
在流媒体平台搜索《蝙蝠侠：黑暗骑士》时，系统做的不只是记录你观看了这部电影。它还知道克里斯蒂安·贝尔饰演了蝙蝠侠，克里斯托弗·诺兰是导演，这部电影属于蝙蝠侠三部曲，并且和许多烧脑动作片有相似的电影基因。这种丰富的语义网络就是 **知识图谱（KG）**——一个由实体（电影、演员、导演、类型）通过带类型的关系（`acted_in`、`directed_by`、`part_of`）连接而成的结构化网络。

为什么这对推荐系统很重要？因为纯协同过滤有一个致命缺陷：它只能推荐已经有交互历史的物品。一部刚上线、播放量为零的新电影，在协同过滤眼中是完全“隐形”的。但如果这部电影的导演是你喜欢的诺兰，知识图谱在第一天就能发现这条关联。知识图谱把推荐从简单的模式匹配提升到了 **语义推理** 的层面。

---

![推荐系统（八）—— 知识图谱增强推荐系统 — visual](https://blog-pic-ck.oss-cn-beijing.aliyuncs.com/posts/zh/recommendation-systems/08-知识图谱增强推荐系统/illustration_1.jpg)
## 你将学到什么

- 知识图谱的定义，以及如何用三元组编码现实世界中的事实  
- 知识图谱嵌入方法（TransE、TransR、DistMult）如何用向量表示实体和关系  
...
![推荐系统（八）—— 知识图谱增强推荐系统 — visual](https://blog-pic-ck.oss-cn-beijing.aliyuncs.com/posts/zh/recommendation-systems/08-知识图谱增强推荐系统/illustration_2.jpg)
