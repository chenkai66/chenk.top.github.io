...
这些体验背后没有魔法，只有一种默默运行在几乎所有消费产品中的机器学习系统——**推荐系统**。

为什么各大平台愿意在这里投入大量资源？几个被反复引用的数据可以说明问题：麦肯锡报告显示，Amazon **35% 的销售额** 来自推荐引擎；YouTube 工程师在 RecSys 2016 论文中提到，约 **70% 的观看时长** 由推荐驱动；Netflix 产品负责人公开表示，平台上约 **75% 的播放量** 来自推荐，而不是用户主动搜索。

这是 16 篇系列文章的第一篇，目标是帮你建立一个完整且真实的推荐系统认知模型。读完之后，再看任何一篇推荐论文或工业架构，你都不会感到陌生。
...
- YouTube 每分钟新增 **500 小时** 视频
- Spotify 收录 **超过 1 亿首** 歌曲
- Amazon 商品数量 **以亿计**

没有过滤的情况下，用户花在搜索上的时间会远超消费内容的时间。选择越多，决策越差，满意度反而下降——这就是“选择悖论”。推荐系统的任务，就是为每个用户打造一副个性化滤镜，把真正重要的那一小部分内容筛选出来。
