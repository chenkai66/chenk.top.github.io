...
---
刷淘宝时，你看到的每个商品图片背后，模型其实同时在解决好几个问题：你会不会点击、点击后会不会加购物车、加购后会不会付款、付款后会不会退货、买了之后会不会给好评？这些问题对应不同的**任务**，数据分布、稀疏程度和业务意义各不相同，但它们之间又紧密相关——愿意点击的人更可能下单，下单的人更可能复购，而高 CTR 的吸睛缩略图反而可能降低用户观看时长。
...
这篇文章是我的思维框架和可运行代码，带你了解工业界常用的四种架构：**Shared-Bottom、ESMM、MMoE、PLE**。我会解释为什么简单方案会失败（如负迁移、梯度冲突、样本选择偏差），以及不确定性加权、GradNorm 和 Pareto 折衷如何解决这些问题。
## 你将学到
