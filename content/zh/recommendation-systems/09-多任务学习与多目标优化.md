...
---
刷淘宝时，你看到的每个商品图片背后，模型其实同时在解决好几个问题：你会不会点击？点击后会不会加购物车？加购后会不会付款？付款后会不会退货？买了之后会不会给好评？这些问题对应不同的**任务**，数据分布、稀疏程度和业务意义都不一样。但它们之间又紧密相关——愿意点击的人更可能下单，下单的人更可能复购，但 CTR 高的吸睛缩略图也可能拉低观看时长。

工业界的解决方案是**多任务学习（Multi-Task Learning，MTL）**。与其为每个目标单独训练一个模型再拼接分数，不如用一个神经网络输出多个目标，让共享的主干网络同时学习服务所有任务的表示。难点不在于画架构图，而在于如何让这些任务头在共享参数上**协作**而不是**内耗**。
...
想象一个餐厅推荐系统。如果只优化**点击率**，模型很快学会推荐那些夸张的美食图片和吸引眼球的标题——结果转化率崩了。如果只优化**预订量**，推出来的全是稳妥的连锁店，用户根本感受不到惊喜。如果只优化**评分**，推荐的全是昂贵的高档餐厅，但没人真的会去。真正合理的目标应该是一组指标：
