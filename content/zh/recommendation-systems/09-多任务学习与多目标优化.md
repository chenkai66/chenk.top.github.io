...
---
刷淘宝时，你看到的每个商品图片背后，模型其实同时在解决好几个问题：你会不会点击？点击后会不会加购物车？加购后会不会付款？付款后会不会退货？买了之后会不会给好评？这些问题对应不同的**任务**，数据分布、稀疏程度和业务意义都不一样。但它们之间又紧密相关——愿意点击的人更可能下单，下单的人更可能复购，但高 CTR 的吸睛缩略图反而可能降低用户观看时长。

工业界的解决方案是**多任务学习（Multi-Task Learning，MTL）**。与其为每个目标单独训练一个模型再拼接分数，不如用一个神经网络输出多个目标，让共享的主干网络同时学习服务所有任务的表示。难点不在于设计架构图，而在于如何让各任务头在共享参数上协同优化，而非相互干扰。

这篇文章是我的思维框架和可运行代码，带你过一遍工业界真正会用到的四种架构：**Shared-Bottom、ESMM、MMoE、PLE**。我会讲清楚为什么简单方案会崩（负迁移、梯度冲突、样本选择偏差），以及不确定性加权、GradNorm 和 Pareto 折衷是如何把这些坑填平的。
## 你将学到

- 排序为什么天生是多目标问题，忽略这一点会带来什么问题
- CVR 预估中的**样本选择偏差**以及 ESMM 如何用链式法则解决它
- 四种架构：Shared-Bottom、ESMM、MMoE、PLE，分别在什么情况下表现最佳
- 损失平衡方法：Uncertainty Weighting、GradNorm、Pareto 前沿
...
想象一个餐厅推荐系统。如果只优化**点击率**，模型很快会偏向推荐夸张的美食图片和吸睛标题，导致转化率大幅下降。如果只优化**预订量**，推荐结果集中于稳妥的连锁店，缺乏多样性与惊喜感。如果只优化**评分**，推荐的全是昂贵的高档餐厅，但没人真的会去。真正合理的目标应该是一组指标：
...
这些指标**相关但不相同**。MTL 模型的任务是利用它们的相关性（CTR 信号帮助 CVR Head 理解用户意图），同时避免某个目标完全压倒其他目标。
...
我在"被点击的曝光"子集上训练，但在线上却要预测整个曝光集合。这就是典型的**样本选择偏差**：训练分布和线上分布不一致。

ESMM 的解决方法基于概率链式法则：

$$P(\text{购买} \mid \text{曝光}) = P(\text{点击} \mid \text{曝光}) \cdot P(\text{购买} \mid \text{点击})$$
用人话说就是："看到商品后买它的概率，等于点它的概率乘以点了之后买的概率。" 第一项（CTR）和乘积（CTCVR）都可以在**完整曝光空间**上观测。我直接监督这两个目标，CVR 就能作为副产品自然学出来——再也不用依赖有偏差的切片数据。
