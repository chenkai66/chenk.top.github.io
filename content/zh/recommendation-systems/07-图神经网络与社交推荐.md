...
| 聚合器 | 工作原理 |
|---|---|
| **Mean** | 邻居嵌入取平均：$\tfrac{1}{\mid\mathcal{N}\mid}\sum_u \mathbf{h}_u$ |
...
| 模型 | 前向传播 | 内存 |
|---|---|---|
| GCN / LightGCN | $O(L\cdot\midE\mid\cdot d)$ | $O(\midV\mid\cdot d + \midE\mid)$ |
| GAT | $O(L\cdot\midE\mid\cdot d\cdot H)$ | $O(\midV\mid\cdot d\cdot H + \midE\mid)$ |
| GraphSAGE（采样） | $O(L\cdot\midV\mid\cdot k\cdot d)$ | $O(\midV\mid\cdot d + k\cdot\midV\mid)$ |
