...
---
当 Netflix 决定推荐什么内容时，它不会只盯着你的观看历史。背后有一张复杂的关系网：电影之间共享演员、用户之间口味重叠、评分在目录中层层传递。“图”这个概念不是比喻——每一张交互矩阵 *就是* 一张图，把它当作图来处理，能解锁扁平的用户/物品嵌入无法表达的可能性。

**图神经网络（Graph Neural Networks，GNN）** 是让我能在图上进行推理的工具。它不再单独学习每个用户和物品的表示，而是说：*你的表示由你的邻居塑造。* 这一思路直接推动了 Pinterest 的十亿节点 PinSage、用极简设计打败复杂基线模型的 LightGCN，以及将“你看了什么”和“朋友看了什么”结合的社交推荐系统。
...
看上图，Alice 和 Bob 从未见过面，但高亮的边告诉我们他们共享了两部电影。GNN 可以沿着图传播这个信号："Bob 喜欢《银翼杀手》；Bob 和 Alice 因为共享电影在图上接近；所以 Alice 很可能也喜欢《银翼杀手》。" 矩阵分解永远问不出这个问题——它只能看到单个单元格。
...
一句话总结：**传统方法孤立地学 embedding；GNN 在上下文中学习 embedding。**
...
**电话游戏的比喻。** 想象一个电话游戏：每个人同时悄悄告诉所有朋友自己的信息。一轮后，你知道了朋友的想法；两轮后，你知道了朋友的朋友的想法。GNN 就是用可学习、可微分的函数实现了这件事。
## 图卷积网络（GCN）
