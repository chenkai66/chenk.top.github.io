...
---
当 Netflix 决定推荐什么内容时，它不会只盯着你的观看历史。背后是一张复杂的关系网：电影之间共享演员，用户之间口味相似，评分信息在物品目录中通过交互关系逐层传播。“图”这个概念不是比喻——每一张交互矩阵 *就是* 一张图，把它当作图来处理，能解锁扁平的用户/物品嵌入无法表达的可能性。
...
这篇文章从直觉出发，带你走一遍这片领域。我先从用户-物品二部图开始，搭建消息传递框架，然后逐一剖析 GCN、GAT、GraphSAGE、PinSage、LightGCN、NGCF、社交 GNN，以及让这些方法在大规模场景下依然高效的采样技巧。每个模型都配有可运行的 PyTorch 代码。
...
- 推荐数据为什么天然呈现图结构，这样做有什么好处  
- 核心 GNN 模块 —— GCN、GAT、GraphSAGE 的原理和适用场景  
- 工业级模型（PinSage、LightGCN、NGCF）如何将 GNN 落地到生产环境  
- 社交信号能提升 5–15% 的效果，以及需要注意的失效模式  
- 小批量邻居采样技术，让 GNN 从 MovieLens 扩展到 Pinterest 规模  
...
随便找一个推荐系统，仔细看看它的交互矩阵：行是用户，列是物品，每个非零值代表一次点击、观看、购买或评分。把这个矩阵重新画成 **二部图**：

- **节点** 是用户和物品。
- **边** 把用户连接到他交互过的所有物品。
- **协同信号** 藏在图的结构里：交互过相似物品的用户，通过这些共享物品被隐式地关联起来。
...
一句话总结：**传统方法孤立地学习节点嵌入；GNN 则在图结构上下文中联合学习节点嵌入。**
