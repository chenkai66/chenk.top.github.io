...
---
当 Netflix 决定推荐内容时，并不只看你的观看历史。背后是一张复杂的关系网：电影共享演员、用户口味相似、评分信息通过交互关系逐层传播。“图”这个概念不是比喻——每张交互矩阵都是一张图，将其作为图处理，可以解锁扁平的用户/物品嵌入无法表达的可能性。
...
这篇文章从直觉出发，带你了解这一领域。首先从用户-物品二部图入手，搭建消息传递框架，然后逐一剖析 GCN、GAT、GraphSAGE、PinSage、LightGCN、NGCF、社交 GNN 及其在大规模场景下的高效采样技巧。每个模型都配有可运行的 PyTorch 代码。
