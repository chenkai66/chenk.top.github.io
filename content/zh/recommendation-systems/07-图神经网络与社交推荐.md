...
| 聚合器 | 工作原理 |
|---|---|
| **Mean** | 邻居嵌入取平均：$\tfrac{1}{\vert \mathcal{N}\vert }\sum_u \mathbf{h}_u$ |
...
| 模型 | 前向传播 | 内存 |
|---|---|---|
| GCN / LightGCN | $O(L\cdot\vert E\vert \cdot d)$ | $O(\vert V\vert \cdot d + \vert E\vert )$ |
| GAT | $O(L\cdot\vert E\vert \cdot d\cdot H)$ | $O(\vert V\vert \cdot d\cdot H + \vert E\vert )$ |
| GraphSAGE（采样） | $O(L\cdot\vert V\vert \cdot k\cdot d)$ | $O(\vert V\vert \cdot d + k\cdot\vert V\vert )$ |
