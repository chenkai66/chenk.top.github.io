...
实现时，我学一个转移矩阵 $M \in \mathbb{R}^{|I| \times |I|}$，其中 $M_{ij} = P(j \mid i)$ 通过频次估计：
...
**3. 位置编码**：自注意力本身对顺序不敏感——它是置换等变的。为了注入顺序信息，我使用经典正弦位置编码：
$$
...
> **等等，未来的信息也能用？** 训练时确实可以！我会随机隐藏一些位置，让双向编码器根据其他位置的信息重建它们。推理时，则在序列末尾追加一个 `[MASK]` 标记，预测它的值。同样的主干，不同的预训练任务，学到的表示更丰富。
