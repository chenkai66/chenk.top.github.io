...
---
每次你刷信息流、点击商品推荐或者观看推荐视频时，背后都有一个 CTR（点击率）模型在决定给你展示什么。这个模型要回答的问题看似简单：

> **"这个用户此时此刻点击这个物品的概率是多少？"**

这个问题背后隐藏着机器学习领域最具经济价值的挑战之一。CTR 提升 1%，在 Google、Amazon、阿里这种规模的平台上，意味着数百万美元的额外收益；同样的模型还驱动着信息流、应用商店、新闻 App 和社交 App 等场景。CTR 预估是推荐系统**排序阶段**的核心：召回阶段生成几千个候选物品，而 CTR 模型从中选出最终展示给用户的十几个。

这篇文章带你回顾过去十年 CTR 模型的演进历程——从简单的 Logistic Regression 到基于注意力机制的复杂架构。我不会只停留在公式层面，而是针对每个模型提出三个问题：

1. **上一代模型的哪些缺陷催生了这个设计？**
2. **它的几何或概率直观解释是什么？**
3. **如何实现并将其部署到生产环境？**

读完之后，你应该能够读懂任何一篇现代 CTR 论文，凭记忆画出模型架构图，并为自己的系统选择合适的基线模型。
## 你将学到什么
...
CTR 预估是**带有极端结构的二分类问题**。给定用户、物品和上下文，目标是估计点击概率：
...
实际中，CTR 定义为点击数除以曝光数：$\text{CTR} = \text{点击数} / \text{曝光数}$。模型输出用于对候选物品进行**排序**、**过滤低质量物品**，并服务于下游业务目标（例如广告中的 eCPM = CTR x 出价，或信息流中的多目标加权分数）。

### 为什么 CTR 预估很难？

CTR 预估表面上是一个标准的分类任务，但实际上完全不同，主要有以下五个原因：

**1. 极端类别不平衡。** 展示广告的点击率通常在 0.1%-2%，电商在 1%-5%，信息流在 2%-10%。一个“全预测不点击”的模型可以达到 95% 以上的准确率，但毫无意义。因此，AUC 和 Logloss 取代了准确率作为评估指标。

**2. 高维稀疏特征。** 经过 one-hot 编码后，特征空间维度达到 $10^6$ 到 $10^9$，而每个样本只激活其中几十个维度。为每对特征存储一个权重是不可能的。

**3. 信号隐藏在特征交互中。** 单独的“年轻用户”是一个弱信号，而“年轻用户 x 动作片 x 晚上”则是强信号。如何**自动且高效地**捕捉这些交互，是建模的核心问题。

**4. 数据分布持续变化。** 新物品上线、爆款出现、工作日与周末的周期性波动都会导致分布漂移。模型每天甚至每小时需要重新训练，仅靠离线 AUC 无法反映全貌。
...
- **Embedding 是共享基础设施。** 所有深度 CTR 模型（FM、DeepFM、xDeepFM、DCN、AutoInt、FiBiNet）都使用同一张 Embedding 表，不同架构主要定义了**Embedding 如何交互**。
- **在线反馈形成闭环。** 昨天的服务日志就是今天的训练数据。模型的新鲜度往往比复杂度更重要。

带着这张地图，我沿着时间线梳理一下架构的发展历程。
## Logistic Regression：推荐系统的起点，也是 FM 诞生的原因

即使生产环境中到处都是巨型神经网络，Logistic Regression（LR）依然屹立不倒。它是推荐系统中的通用基线、校准锚点，在延迟敏感的场景中，仍然是不少请求的实际打分模型。
...
从几何上看，LR 的局限性一目了然。它只能在特征空间中学习一个超平面。任何需要“特征 A 只有在特征 B 同时激活时才有用”的模式，它都无能为力。最典型的例子是 XOR 形状的点击行为：
...
左图中，“年轻+动作片”和“老年+喜剧”会点击，但“年轻+喜剧”和“老年+动作片”不会。无论怎么调整，线性边界都无法区分这些数据——AUC 始终徘徊在 0.5 左右。右图中，加入一个交互项 $x_1 \cdot x_2$，问题迎刃而解。LR 之后的所有 CTR 模型，本质上都在回答一个问题：
...
如果用朴素的“带交互的 LR”，需要为每对特征学习一个权重 $w_{ij}$。假设特征数是 $d$，那么参数量就是 $O(d^2)$。但问题来了：大多数特征对在训练集中**从未同时出现过**，根本没法学。

FM 的解决办法是，用两个低维向量的内积代替每对特征的权重：
$$w_{ij} \approx \langle \mathbf{v}_i, \mathbf{v}_j \rangle = \sum_{f=1}^{k} v_{i,f}\, v_{j,f}.$$
> **类比：** 假设有 1000 部电影。如果为每对电影存一个权重，需要一百万个数，而绝大多数组合从未见过。换个思路：给每部电影分配一个 $k$ 维“性格向量”。两部电影的交互强度取决于它们的向量是否相似。这样只需要 $1000 \cdot k$ 个数，而且即使某对电影从未在训练集中共现，也能预测它们的交互强度——因为每个向量是从大量其他共现样本中学到的。

FM 的真正魔法在于它能泛化到未见过的特征对。这正是它在极端稀疏场景下依然有效的原因，而决策树和线性模型在这种情况下往往无能为力。
...
FFM（2016）在 FM 的基础上做了一个关键改进：**每个特征针对与其交互的每个 field，都有一套独立的嵌入向量。**

### 核心思想

在 FM 中，无论“动作片”是和“用户年龄”交互还是和“时段”交互，用的都是同一个向量。但直觉上，“年龄 vs 类别”和“时段 vs 类别”代表的是不同的关系。FFM 为每个特征分配了一组嵌入向量，每对应一个对方 field 就有一个专属向量。
...
两者都只处理二阶交互。如果想扩展到高阶交互，有两种选择：**堆叠非线性层**（深度网络），或者**显式构造高阶特征**（CIN、Cross）。DeepFM 走的是第一种路线；xDeepFM 和 DCN 走的是第二种路线。
