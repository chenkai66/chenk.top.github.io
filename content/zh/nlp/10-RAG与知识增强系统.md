...
理念一句话即可概括，工程实现才是本文的重点。一个可落地的 RAG 系统通常涉及十余个关键参数——分块大小、 Embedding 模型的选择、索引类型、$k$ 值、混合权重策略、重排序深度、 Prompt 模板设计、引用格式规范、拒答策略等——且这些参数彼此耦合。接下来逐一探讨每个参数背后的数学原理、权衡取舍及可运行的代码示例。
