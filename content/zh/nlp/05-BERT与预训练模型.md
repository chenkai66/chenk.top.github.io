...
---
2018 年 10 月， Google 推出了 BERT，一举刷新了 11 项 NLP 基准测试的记录。方法出人意料地简洁：仅需一个 Transformer 编码器，通过让模型根据双向上下文预测被随机遮盖的词来预训练，再将同一模型在下游任务上微调即可。在 BERT 出现之前，每个任务都需要从零开始训练一个专属模型；而 BERT 的出现彻底改变了这一局面，“一次预训练、多次微调”迅速成为该领域的标准范式。
...
在 BERT 出现之前，每个 NLP 任务都得从零开始。模型随机初始化后，用专门标注的数据集进行训练。这种方式不仅计算成本高，还存在明显的弊端：不同任务之间无法共享知识，而且由于标注数据量有限，模型的稳定性也难以保证。此后，预训练技术历经四个关键发展阶段，最终重塑了这一格局。
