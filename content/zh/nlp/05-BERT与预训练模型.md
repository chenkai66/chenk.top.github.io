...
---
2018 年 10 月，Google 推出了 BERT，一举刷新了 11 项 NLP 基准测试的记录。方法简单得让人有点不好意思：拿一个 Transformer 编码器，训练它根据上下文（包括左侧和右侧）预测被随机遮盖的词，然后用同一个预训练模型针对具体任务进行微调即可。在 BERT 出现之前，每个任务都需要从零开始训练一个专属模型；而 BERT 的出现彻底改变了这一局面，“一次预训练，到处微调”迅速成为整个领域的标准范式。

如果你近几年接触过情感分析 API、能够理解用户意图的搜索引擎，或者智能客服机器人，那么大概率背后的核心技术就是 BERT 或它的某个改进版本在发挥作用。


<!-- wanx-hero -->
![自然语言处理（五）：BERT与预训练模型 — 配图](https://blog-pic-ck.oss-cn-beijing.aliyuncs.com/posts/zh/nlp/bert-pretrained-models/illustration_1.jpg)
## 这一篇你会学到
...
<!-- wanx-mid -->
![自然语言处理（五）：BERT与预训练模型 — 配图](https://blog-pic-ck.oss-cn-beijing.aliyuncs.com/posts/zh/nlp/bert-pretrained-models/illustration_2.jpg)

在 BERT 出现之前，每个 NLP 任务都得从零开始。模型随机初始化后，用专门标注的数据集进行训练。这种方式不仅计算成本高，还存在明显的弊端：不同任务之间无法共享知识，而且由于标注数据量有限，模型的稳定性也难以保证。后来，领域内经历了四个关键阶段的发展，彻底改变了这一局面。

### 简短的发展历程

**Word2Vec（2013）**  
通过无标注文本学习静态词向量。这种方法的问题在于，同一个词无论出现在什么上下文中，其向量表示都是一样的。例如，“bank” 在 *river bank* 和 *bank account* 中的表示完全相同，完全没有考虑上下文对词义的影响。
...
GPT-1 表现不俗，但它只能单向建模。例如，在处理句子 "the bank is closed" 时，当模型看到 "bank" 时，尚未接触到 "closed"，因此无法利用后续信息来消除歧义。

**BERT（2018 年 10 月）**  
真正的突破来了：BERT 改变了预训练的目标，让每个词都能同时看到双向上下文信息。仅此一项改动，就让各种任务的性能得到了全面提升。
...
1. **预训练**  
在海量无标注文本（如书籍、维基百科、网页等）上，使用自监督任务训练模型。这一步虽然成本较高，但只需执行一次。
2. **微调**  
...
- **强大的基线性能**  
即使是简单的 BERT 微调，也能轻松超越过去那些复杂的定制化架构。
## BERT 的架构
