...
---
2017 年 6 月，Google Brain 和 Google Research 的八位研究者发表了一篇标题相当引人注目的论文：*Attention Is All You Need*。这篇论文提出的 **Transformer** 架构彻底抛弃了循环结构——不再使用 LSTM 或 GRU，也不再需要从左到右逐步扫描句子。相反，序列中的每个 token 都可以通过一个数学操作直接“看到”其他所有 token，这个操作就是缩放点积注意力（scaled dot-product attention）。

这一设计选择带来了深远的影响：它不仅让 GPU 的大规模并行计算能力得以充分发挥，还一举解决了困扰 RNN 数十年的长距离依赖问题。更重要的是，Transformer 成为了后续一系列大模型的基础架构，包括 BERT、GPT、T5、LLaMA、Claude 等几乎所有现代大语言模型。如果能扎实理解这篇文章的内容，那么本系列后续的部分基本上就是在这一主题上的延伸和变化。
...
举个例子，假设我们要翻译句子 *"The cat that chased the mouse that ate the cheese was very tired"* 到法语。编码器需要将“猫”“老鼠”“奶酪”“追逐”“吃”“疲惫”，以及它们之间的语法关系，统统塞进一个 512 维的向量中。而解码器只能依靠这 512 个数字来还原整句话，且无法再回头查看源句。

这种方法存在两个致命问题：

- **信息容量有限**。一个固定大小的向量无法无损地存储任意长度的序列信息。实验表明，传统的 Seq2Seq 模型在处理超过 30 个词的句子时，BLEU 分数会显著下降。
- **缺乏聚焦能力**。当解码器生成法语单词“猫”时，它应该关注源句中的“猫”，而不是“奶酪”。然而，静态上下文向量对所有内容一视同仁，无法区分哪些信息更重要。

用一个比喻来说明：想象一下，你先背诵一段文字，然后合上书本，凭记忆翻译；再对比专业译者的工作方式，他们会把原文放在手边，每翻译一个词前都会回头看一眼相关的部分。**注意力机制就是让神经网络学会像专业译者那样工作的方法。**
...
上面的热力图展示了一个英译法的小例子中，$\alpha_{tj}$ 的具体表现。每一行的概率值加起来等于 1，而亮色区域正好反映了语言学上的对齐关系（例如 Le ↔ The、chat ↔ cat、tapis ↔ mat）。特别值得注意的是，模型并没有被告知这些对齐关系，而是通过训练过程中最小化翻译损失自己学出来的。从这一刻起，注意力机制不再仅仅是一个提升性能的工具，而是成为了一个具有语言学意义且高度可解释的研究对象。
...
Vaswani 等人提出了一个简单却深刻的问题：**如果只用注意力机制呢？** 如果每个 token 都可以直接与其他所有 token 建立联系，那么我们就能获得两大显著优势：
...
除以 $\sqrt{d_k}$ 将方差拉回 1，确保 softmax 工作在有意义的区间内。这看似简单的一行代码，却对训练稳定性有着至关重要的影响。
## 5. 多头注意力
机制
