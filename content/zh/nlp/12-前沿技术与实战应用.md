...
<!-- wanx-hero -->
![自然语言处理（十二）：前沿技术与实战应用 — 配图](https://blog-pic-ck.oss-cn-beijing.aliyuncs.com/posts/en/nlp/frontiers-applications/illustration_1.png)

---

## 你将学到什么
<!-- wanx-mid -->
![自然语言处理（十二）：前沿技术与实战应用 — 配图](https://blog-pic-ck.oss-cn-beijing.aliyuncs.com/posts/en/nlp/frontiers-applications/illustration_2.png)
...
- 如果你已经读过本系列的前十一章，尤其是[第 4 章](/zh/nlp/04-注意力机制与Transformer/)（Transformer）、[第 6 章](/zh/nlp/06-GPT与生成式语言模型/)（GPT）、[第 8 章](/zh/nlp/08-模型微调与PEFT/)（PEFT）、[第 9 章](/zh/nlp/09-大语言模型架构深度解析/)（LLM 内部机制）和[第 10 章](/zh/nlp/10-RAG与知识增强系统/)（RAG），这将是一个很好的基础，这些内容会在后续讨论中频繁用到。
...
针对这些问题，缓解措施需要多管齐下：通过 **RAG** 提升事实准确性（详见[第 10 章](/zh/nlp/10-RAG与知识增强系统/)），利用 **约束解码** 和 JSON 模式确保结构上的忠实性，采用 **自一致性采样 + 多数投票** 解决算术问题，借助 **过程监督** 和推理模型处理逻辑错误，通过 **强制引用机制** 提高可验证性，最后还可以通过 **弃权训练**（学会说“我不知道”）作为兜底策略。
...
**扩展上下文最经济的方式是什么？** 如果可以进行微调，推荐使用 RoPE base scaling 加上一次小规模的 LongLoRA 微调。如果没有微调条件，那就采用分块加 RAG 的方式（详见[第 10 章](/zh/nlp/10-RAG与知识增强系统/)）。真正的百万 token 上下文很少是最佳选择——基于检索的方法不仅成本更低，调试起来也更方便，而且通常准确率更高。
