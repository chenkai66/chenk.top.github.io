...
---
经过十一章的探索，我们从原始文本一路走到了多模态基础模型。作为第十二章，也是最后一章，这里既是技术的最前沿，也是实际应用的起点。在这一阶段，研究不再仅仅是纸上谈兵的论文，而是真正落地为服务：一个能够调用工具、编写和调试代码、完成上百步推理、处理 20 万 token 的合同文件，并通过 FastAPI 接口以 p95 延迟低于 300 毫秒支持上千并发用户的大型语言模型（LLM）。

能力的提升往往伴随着新的问题。模型可能会自信地“编造”事实，被诱导时生成有害内容，泄露训练数据，或者因为部署不当而耗费巨额成本。因此，本章分为两部分。前半部分聚焦于模型能力的边界——Agent、代码生成、长上下文处理和复杂推理；后半部分则关注如何将这些前沿能力安全、可靠地投入生产环境，包括安全性、评估方法和高效部署，确保既不伤害用户体验，也不让预算失控。
...
- 如果你已经读过本系列的前十一章，那会是一个很好的基础，特别是第 4 章（Transformer）、第 6 章（GPT）、第 8 章（PEFT）、第 9 章（LLM 内部机制）和第 10 章（RAG），这些内容会在后续讨论中频繁用到。
- 需要对 Python 编程、基础的 asyncio 异步编程以及 Docker 容器的基本结构有一定的了解。
- 如果你对强化学习有一些初步的认识，那么在阅读对齐部分时会更加得心应手。如果想深入学习，可以参考 [强化学习第 12 章 —— RLHF 与 LLM 应用](/en/reinforcement-learning/12-rlhf-and-llm-applications/)。
...
开发者常犯的三个错误。首先，**让模型自己决定是否调用工具**（`tool_choice="auto"`）。如果强行调用，模型可能会生成毫无意义的参数。其次，**始终在沙箱中执行工具**——只要你的 schema 允许，模型完全可能生成类似 `delete_all_users()` 的危险调用。最后，工具描述本身就是一种 prompt：需要不断调整优化，直到模型能够稳定地选择正确的工具。
