...
---
几十年来，计算机一直把“国王”和“王后”当作完全无关的符号——它们不过是词汇表中的两个独立条目，彼此毫无关联。然而，一个简单却革命性的想法彻底改变了这一局面：如果每个词都存在于一个连续的空间中，并且语义可以用*方向*来表示呢？这个想法一经提出，便让模型能够计算出：

$$\vec{\text{king}} - \vec{\text{man}} + \vec{\text{woman}} \approx \vec{\text{queen}}$$

自然语言处理（NLP）的发展轨迹也因此发生了根本性转变，进入了“表示学习”的时代。本文将沿着这条脉络展开：从独热编码（one-hot vectors）为何失效，到 Word2Vec 如何通过浅层神经网络打开新思路；从 GloVe 如何利用全局统计信息，到 FastText 如何借助子词 n-gram 处理未见过的词；最终，我们会将词嵌入与催生它们的语言模型联系起来，揭示两者之间的深层关系。


<!-- wanx-hero -->
![自然语言处理（二）：词向量与语言模型 — 配图](https://blog-pic-ck.oss-cn-beijing.aliyuncs.com/posts/zh/nlp/word-embeddings-lm/illustration_1.png)
## 你将学到什么
<!-- wanx-mid -->
![自然语言处理（二）：词向量与语言模型 — 配图](https://blog-pic-ck.oss-cn-beijing.aliyuncs.com/posts/zh/nlp/word-embeddings-lm/illustration_2.png)
...
**前置知识**：第 1 部分（文本预处理基础）、基本线性代数知识（如点积和矩阵乘法），以及对 softmax 和随机梯度下降的基本理解
## 从稀疏到稠密：为什么词嵌入如此重要
...
本文提到的所有方法都基于语言学家 J. R. Firth 提出的一个核心观点：**“一个词的意义由它所在的上下文决定。”** 换句话说，经常出现在相似上下文中的词，往往具有相似的含义。
...
![FastText：每个词的向量是其字符 n-gram 嵌入之和，天然支持未登录词](https://blog-pic-ck.oss-cn-beijing.aliyuncs.com/posts/zh/nlp/02-词向量%与语言模型/fig7_subword_fasttext.png)
...
Word2Vec、GloVe 和 FastText 生成的都是**静态嵌入**——无论“bank”出现在“river bank”还是“bank account”中，它的向量始终保持不变。这是它们的主要局限。而下一波模型——ELMo、BERT、GPT——生成的是**上下文嵌入**，每次出现都会根据上下文生成一个独特的向量。这部分内容我们将在第 5 和第 6 部分深入探讨。
## 用 Gensim 实战训练
