...
---
同一个模型，既可能给出精准而深刻的回答，也可能自信满满地“一本正经胡说八道”。问题的关键往往不在于模型的权重，而在于你如何引导它。如果你只是简单地输入一句“分析这段文本”，得到的结果大概率是一个泛泛而谈的总结；但如果你在提示中明确角色、提供两个清晰的示例，并规定严格的输出格式，那么结果很可能就是一个结构化的 JSON，能够直接被下游解析器使用。**提示工程的核心，就是将这种从偶然到必然的差距转化为一套可重复、可操作的方法论。**

让这一切成为可能的机制叫做 In-Context Learning（上下文学习，简称 ICL）。当你在提示中加入几个示例时，模型并不会重新训练，而是通过这些示例调整其前向传播的条件分布，从而**推断出任务的具体要求**。理解 ICL 的能力和局限，是区分一个开发者是在与模型“较劲”还是在“驾驭”模型的关键。

本文是 NLP 系列的第七篇，假设你已经对 Transformer 解码器逐 token 生成的过程（第四篇）以及自回归语言模型（第六篇）有了基本了解。以下内容均基于已发表的研究成果，但需要提醒的是：提示工程领域的研究噪声较大，很多数据和结论都高度依赖具体的模型和数据集。因此，图中的柱状图仅供示意参考，不应被视为任何基准测试的绝对结果。
