...
这篇文章将带你从头梳理循环神经网络家族的发展历程：从最基础的 RNN 开始，分析其为何无法记住超过十几个 token 的信息；接着探讨 LSTM 和 GRU 如何通过引入门控机制突破这一限制；最后用 PyTorch 实现一个英法翻译模型。读完本文，你将清晰理解 RNN 如何逐步演进为注意力机制（Attention）和 Transformer。
...
正是这个瓶颈问题直接推动了**注意力机制**的诞生，也就是[第 4 部分](/zh/nlp/04-注意力机制与Transformer/)的主题：与其让解码器依赖于单一的上下文向量 $c$，不如在每一步解码时，允许它动态地回顾编码器生成的*所有*隐藏状态。
...
$$
这样一来，上下文向量就变成了编码器状态的*动态加权和*，能够根据时间步的变化灵活调整。这一突破性思想为从 RNN 到 Transformer 的演进铺平了道路，我们将在[第 4 部分](/zh/nlp/04-注意力机制与Transformer/)深入探讨。
## 常见问题
...
- **Seq2Seq 编码器-解码器架构**可以实现序列到序列的映射，但受限于单一的上下文向量 $c$——这一瓶颈正是[第 4 部分](/zh/nlp/04-注意力机制与Transformer/)引入注意力机制的主要动机。