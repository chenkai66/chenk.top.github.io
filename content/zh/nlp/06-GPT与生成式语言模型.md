...
如果说 BERT 是“理解”的王者，那么 GPT 就是“生成”的霸主。本文将带你回顾 GPT 的发展历程，深入剖析自回归解码的运行机制，让你直观感受不同解码策略的实际影响，并教你如何在自己的笔记本电脑上运行一个功能完备的聊天机器人。
...
为什么这个设计如此重要？因为它让训练和推理过程完全一致。在训练时，尽管模型能看到整个句子，但因果掩码会挡住未来的内容，迫使每个位置只能根据过去的信息预测下一个词——这正是推理时的场景。训练与推理过程完全一致，无需额外切换“生成模式”，训练时用于计算损失的前向传播等同于推理时生成下一个 token 的过程。
