...
---
当你向 ChatGPT 提问并看到一段流畅的多段落回答逐字逐句地生成时，你实际上正在见证一个看似简单却极其强大的循环过程：将“到目前为止生成的所有内容”输入 Transformer 解码器，观察其在词汇表上的概率分布，从中选择一个词追加到已生成的内容末尾，然后重复这一过程——这是自回归语言模型的核心逻辑。真正令人惊叹的是，当我们将循环背后的神经网络扩展到数千亿参数，并用近乎整个互联网的文本数据进行训练时，它展现出的强大能力。

如果说 BERT 是“理解”的王者，那么 GPT 就是“生成”的霸主。本文将带你完整回顾 GPT 的发展历程，深入剖析自回归解码的运行机制，让你直观感受不同解码策略的实际影响，并教你如何在自己的笔记本电脑上运行一个功能完备的聊天机器人。
...
- 仅使用解码器的 Transformer 如何通过“预测下一个 token”实现通用人工智能
- 因果（掩码）自注意力机制——这是 GPT 与 BERT 的关键区别
- 四种经典的解码策略：贪心搜索、束搜索、 top-k 采样和 top-p 采样，及其温度参数的作用
- 上下文学习（零样本/少样本）为何无需更新梯度就能生效
- 实验中观察到的缩放定律及模型能力涌现的奇特现象
- 如何评估生成文本的质量（BLEU、 ROUGE、困惑度），以及这些指标在哪些场景下失效
- 如何基于 GPT-2 和 HuggingFace 构建支持多轮对话的聊天机器人
...
为什么这个设计如此重要？因为它让训练和推理过程完全一致。在训练时，尽管模型能看到整个句子，但因果掩码会挡住未来的内容，迫使每个位置只能根据过去的信息预测下一个词——这正是推理时的场景。训练与推理过程完全一致，无需额外切换“生成模式”；训练时用于计算损失的前向传播，等同于推理时生成下一个 token 的过程。
