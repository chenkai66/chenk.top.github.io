...
---
当你向 ChatGPT 提问并看到一段流畅的多段落回答逐字逐句地生成时，你实际上正在见证一个看似简单却极其强大的循环过程：将“到目前为止生成的所有内容”输入 Transformer 解码器，观察其在词汇表上的概率分布，从中选择一个词追加到已生成的内容末尾，然后重复这一过程——这就是自回归语言模型的核心逻辑。真正令人惊叹的是，当我们把循环背后的神经网络扩展到数千亿参数，并用近乎整个互联网的文本数据进行训练时，它所展现出的强大能力。
