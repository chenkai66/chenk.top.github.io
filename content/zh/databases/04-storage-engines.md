...
你写的每一条 SQL 语句，最终都会转化为写入磁盘的字节流。负责完成这一转换的组件——**存储引擎（Storage Engine）**——对数据库性能的影响，远超其他绝大多数因素。两张结构完全相同、数据完全一致的表，仅因底层存储引擎不同，性能可能天差地别。深入理解这一层，才能真正明白**数据库为何如此表现**。
...
当你执行 `SELECT * FROM users WHERE id = 42` 时，数据库并不会在磁盘上“寻道”到第 42 行。它会先将**包含该行的页**加载进内存，再从中提取出该行并返回。如果该页已在缓冲池（Buffer Pool，即内存缓存）中，则完全无需磁盘 I/O。

## B-Tree 存储引擎

B-Tree 引擎（PostgreSQL、MySQL InnoDB、Oracle、SQL Server 所采用）将数据以 B+ 树结构组织在磁盘上。该树结构既是表数据本身，也是所有索引的基础。

### InnoDB：MySQL 的默认引擎

InnoDB 是一种 B-Tree 引擎，具备若干鲜明特性。

#### 聚簇索引（Clustered Index）

在 InnoDB 中，**表数据本身就是一个 B+ 树，且按主键排序组织**。这被称为 **聚簇索引（Clustered Index）**（或主索引）。
...
- **随机主键插入代价高昂**：使用 UUID 作为主键插入，会导致 B+ 树中间位置频繁发生页分裂（Page Split）。
- **二级索引体积更大**：二级索引存储的是主键值（而非物理地址）作为行指针。通过二级索引查找需两次树遍历——一次查二级索引，一次查聚簇索引。
...
随机主键将引发：
1. **页分裂（Page Splits）**：当页已满，而新行应插入中间位置时，该页必须一分为二。
2. **碎片化（Fragmentation）**：分裂后，页填充率下降。
3. **缓冲池抖动（Buffer Pool Thrashing）**：随机访问不同页，导致大量有用的缓存页被驱逐。

若业务确实需要 UUID，请考虑使用时间有序的 UUIDv7，或采用 `BIGINT AUTO_INCREMENT` 作为主键，并额外增加一个 UUID 字段。

#### 缓冲池（Buffer Pool）

InnoDB 的缓冲池是一块内存区域，用于缓存数据页和索引页。它是最重要的性能相关配置项。
...
新页首先插入旧子链表头部。若在可配置的时间窗口内再次被访问，则晋升至新子链表。此举防止了一次性的全表扫描将所有热页全部挤出缓存——纯 LRU 算法会如此。

### PostgreSQL 存储

PostgreSQL 采用不同策略。表以 **堆文件（Heap File）** 形式存储——即页的无序集合。默认情况下**没有聚簇索引**。
...
行之间没有任何特定顺序。所有索引（包括主键索引）均存储一个物理元组 ID `(page_number, offset)`，指向堆中的具体位置。
...
这意味着：
- **主键查找需一次索引遍历 + 一次堆获取**（与 InnoDB 的二级索引查找开销相同）
- **随机主键无惩罚**——堆本身就是无序的
- **VACUUM 至关重要**——删除/更新留下的死元组（Dead Tuples）必须被回收
...
日志结构合并树（Log-Structured Merge-tree, LSM-tree）引擎采取了根本不同的设计哲学。它不就地更新数据（如 B-Tree），而是将写操作批量暂存于内存，再以**顺序方式**刷入磁盘。这使得写入速度大幅提升，但读取路径更复杂。

LSM-Tree 引擎包括：RocksDB、LevelDB、Cassandra 的存储引擎、HBase、CockroachDB（基于 RocksDB 构建）以及 TiKV（TiDB 的存储层）。
...
随着时间推移，SSTable 数量不断累积。多个 SSTable 可能包含同一 key 的不同版本。合并压缩将 SSTable 合并，以实现：
- 清除过时版本（仅保留最新版）
...
误报率（False Positive Rate）可配置（通常为 1%）。1% 的误报率约需每个 key 分配 10 位（bits）。
...
**空间放大（Space Amplification）**：磁盘总占用空间 / 实际数据大小。B-Tree 因页未填满而浪费空间；LSM-Tree 因合并前多版本数据跨层存在而浪费空间。
...
为何不直接写数据文件？
- 数据文件需随机 I/O（该行可能在任意位置）
- WAL 是仅追加（Sequential I/O），速度比随机 I/O 快 10–100 倍
- 若系统在步骤 4 和 5 之间崩溃，WAL 中的信息足以重放（Replay）所有变更
...
若无检查点，崩溃恢复需重放整个 WAL 历史。检查点限定了恢复所需时间。
