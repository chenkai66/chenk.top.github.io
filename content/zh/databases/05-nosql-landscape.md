---
title: "数据库（5）：NoSQL — 文档型、键值型、列式与图数据库"
...
并非所有数据都能被整齐地塞进行与列中。社交网络中的好友关系图、属性千差万别的商品目录、实时排行榜、推荐引擎背后的关系网络——这些工作负载会将关系型数据库推向尴尬境地。NoSQL 数据库之所以存在，是因为不同的数据模型更擅长解决不同的问题。关键在于：你得知道该在何时选用哪一种。
...
当**关系本身即为核心数据**——社交网络、欺诈检测、推荐引擎、知识图谱——图数据库便是最自然的选择。
