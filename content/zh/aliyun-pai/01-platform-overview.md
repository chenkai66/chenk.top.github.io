...
---
只要你的团队在阿里云上训模型或者做服务，迟早得进 PAI 控制台。PAI 是一个平台级整合层（umbrella），真正干活的是底下的核心产品：笔记本（DSW）、分布式训练服务（DLC）、模型推理服务（EAS）、可视化编排界面（Designer）和快速部署能力。支撑一个 AI 营销平台跑满十八个月真实 LLM 负载后，我写下这篇系列文章：它是我希望团队在上线首个 EAS endpoint 前就掌握的实战指南。

第一篇先摸清路子。本文代码示例较少，重点在于建立整体认知；深度实操内容集中在第 2 至第 5 篇。这里的目的是，等到后面我提到"DLC job"或者"EAS endpoint"时，你心里已经清楚它们属于哪个筐了。
...
建议自底向上阅读：最底层是基础设施层（CPU、GPU、RDMA 网络等硬件资源），其上运行 ACK（阿里云 Kubernetes 服务）。在这之上，*灵骏*（Lingjun）给你超高密度的 AI 算力，*通用算力*给你日常的 ECS-backed GPU 资源池。平台与工具层是你每天待的地方：PyTorch / Megatron / DeepSpeed，加上 PAI 的优化玩具（TorchAcc, BladeLLM, EasyCkpt, AIMaster），还有可见的产品（DSW, DLC, EAS, Designer, FeatureStore, iTAG）。应用层是 PAI 怎么插进阿里剩下的 MaaS 世界（ModelScope, Bailian/DashScope, Model Studio）。业务层则是给行业用例看的营销幻灯片。

选择 PAI 而非裸 ECS 的主要原因在于：它预装了 CUDA/PyTorch 镜像，自动挂载 OSS Bucket，提供监控仪表盘，并支持按秒计费。
