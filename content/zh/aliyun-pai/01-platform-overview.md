...
---
只要你的团队在阿里云上训练模型或提供服务，迟早会用到 PAI 控制台。PAI 是一个平台级整合层（umbrella），真正干活的是底下的核心产品：笔记本（DSW）、分布式训练服务（DLC）、模型推理服务（EAS）、可视化编排界面（Designer）和快速部署能力。在支撑一个 AI 营销平台运行了十八个月的真实 LLM 负载后，我写下了这篇系列文章，希望团队能在上线首个 EAS endpoint 之前掌握这些实战指南。

第一篇先摸清路子。本文代码示例较少，重点在于建立整体认知；深度实操内容集中在第 2 至第 5 篇。这样，当你看到“DLC job”或“EAS endpoint”时，就能清楚它们属于哪个部分。
...
- **ECS、 OSS、 NAS、 CPFS** 才是算力与数据真正的落脚点——PAI 只负责帮你调度它们。
