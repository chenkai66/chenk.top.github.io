...
---
只要你的团队在阿里云上训练模型或提供服务，迟早会用到 PAI 控制台——这是一个平台级整合层（umbrella），真正干活的是底下的核心产品：笔记本（DSW）、分布式训练服务（DLC）、模型推理服务（EAS）、可视化编排界面（Designer）和快速部署能力。在支撑一个 AI 营销平台运行了十八个月的真实 LLM 负载后，我写下了这篇系列文章，希望团队能在上线首个 EAS endpoint 之前掌握这些实战指南。

第一篇先摸清路子，本文代码示例较少，重点在于建立整体认知；深度实操内容集中在第 2 至第 5 篇，这样当你看到“DLC job”或“EAS endpoint”时，就能清楚它们属于哪个部分。
...
官方文档里写，**Platform for AI (PAI)** 是“阿里云覆盖全生命周期的 AI 开发平台：数据标注、模型开发、训练和部署”。控制台 `pai.console.aliyun.com` 是个入口，但 PAI 本身是一系列相关产品，它们共用一套账号模型、基于 OSS 的存储层，以及同一个 Python SDK。

我最常用的心智模型是：**PAI 是店面；DSW、DLC、EAS、Designer、Model Gallery** 是店里的操作台；**ECS、OSS、NAS、CPFS** 才是算力与数据真正的落脚点——PAI 只负责帮你调度它们。
...
建议自底向上阅读：最底层是基础设施层（CPU、 GPU、 RDMA 网络等硬件资源），其上运行 ACK （阿里云 Kubernetes 服务）。再往上，*灵骏*（Lingjun）提供超高密度的 AI 算力，*通用算力*则提供日常的 ECS-backed GPU 资源池。平台与工具层是你每天使用的地方： PyTorch / Megatron / DeepSpeed，加上 PAI 的优化工具（TorchAcc, BladeLLM, EasyCkpt, AIMaster），以及可见的产品（DSW, DLC, EAS, Designer, FeatureStore, iTAG）。应用层展示了 PAI 如何融入阿里的 MaaS 世界（ModelScope, Bailian/DashScope, Model Studio）。业务层则是为行业用例准备的营销幻灯片。

选择 PAI 而非裸 ECS 的主要原因在于它预装了 CUDA/PyTorch 镜像，自动挂载 OSS Bucket，提供监控仪表盘，并支持按秒计费。

## 你实际会碰到的五个子产品

经过一年半的生产环境工作后，我只为下面这些付过钱，直接摘自官方的“核心组件”表：
...
此外还有 iTAG （数据标注）、 PAI-Lingjun （面向超大规模集群）、 PAI-Blade/BladeLLM （用于推理优化）和 FeatureStore，但除非你需要开展千卡以上规模的预训练或构建推荐系统，初期可暂不关注。

各产品的定位清晰对应机器学习生命周期的不同阶段：
...
Designer 和 Model Gallery 在功能上相互正交：二者均位于应用层，但底层均依赖 DLC 或 EAS 执行实际任务。

## PAI 跟 ECS 和 OSS 的关系

这一点容易让习惯直接操作云虚拟机（ECS）的用户产生误解。三个关键事实：
...
Designer 界面操作类似搭积木，但导出的是平台可重放的扁平化 JSON 文档。第一次我得调试卡住的 pipeline 时，我从画布导出了 JSON （`...` 菜单 → *Export workflow*），它看起来就不那么魔法了。结构大致如下：
...
这里有三点很重要。第一，每个节点都有明确的 `compute.engine` —— SQL 形状的组件用 `MaxCompute`， Python / PyAlink 用 `DLC`，流式处理用 `Flink`。 Designer 不会自动选择成本最低的执行方式；它基于组件类型挑选，任何自定义 Python 节点默认跑在一个 DLC pod 上，不管上游 MaxCompute 步骤花了 30 秒还是 30 分钟，这都算你的钱。第二， JSON 是 *幂等* 且 *可 diff* 的——我把我们的存在 git 里，跟它依赖的 SQL 放一起，代码 review 变更的方式跟我 review Terraform plan 一样。第三，同样的 JSON 可以通过 SDK 提交（`pai.designer.submit_workflow(json_path)`），这就是我怎么在 CI 里跑 pipeline 而不用开控制台。

隐藏的升级路径：一旦你有了 JSON，“我想把这个变成代码”就不再是重写。你可以从 Python DAG 生成它，版本化， lint 它，甚至通过 mock 输入单元测试单个节点。画布只是该 JSON 文件的可视化界面，并非独立工具。
