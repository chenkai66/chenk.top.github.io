...
---
前四篇介绍了底层原语 DSW、DLC、EAS 以及如何用 Python 编排；这篇则聚焦于两个 GUI 产品：**PAI-Designer**（拖拽式表格 pipeline 构建）和 **Model Gallery**（零代码部署与微调开源模型）。这两个工具将原语封装成开箱即用的交付能力，专为不写代码的用户设计。虽然资深工程师不常首选这些工具，但在某些关键场景下，它们却是唯一正解。
...
Designer 基于工作流实现建模与调试，用户通过拖拽组件即可构建 AI 开发流程。它内置 140 多个算法组件，支持导出 JSON 和接入 DataWorks 调度，节点还可以嵌入自定义 SQL、Python 或 PyAlink 脚本。
...
对于表格负载，我会用 Designer 替代 DLC + SQL；其他情况下，我则在 DLC 中运行自定义训练模型。

## Model Gallery —— 零代码 MaaS 捷径

Model Gallery 将 DLC + EAS 封装为零代码界面，非 MLOps 用户只需点击约六次即可完成开源模型的微调与部署。官方文档称其“封装了 Platform for AI (PAI)-DLC 和 PAI-EAS，提供高效部署与训练开源大语言模型的零代码方案”。
