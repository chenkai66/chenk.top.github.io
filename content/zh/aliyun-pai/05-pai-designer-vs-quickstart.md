...
---
前四篇介绍了底层原语 DSW、DLC、EAS 以及如何用 Python 编排；这篇则聚焦于两个 GUI 产品：**PAI-Designer**（拖拽式表格 pipeline 构建）和 **Model Gallery**（零代码部署与微调开源模型）。这两个工具将原语封装成开箱即用的交付能力，专为不写代码的用户设计——虽然资深工程师不常首选这些工具，但在某些关键场景下，它们是唯一正解。
...
对于表格负载，我会用 Designer 替代 DLC + SQL；其他情况下，则在 DLC 中运行自定义训练模型。
