...
---
前四篇聊了底层原语——DSW、DLC、EAS，以及如何用 Python 编排；这篇聚焦两个 GUI 产品：**PAI-Designer**（拖拽式表格 pipeline 构建）和 **Model Gallery**（零代码部署与微调开源模型），它们将原语封装成开箱即用的交付能力，专为不写代码的用户设计——资深工程师虽不常首选，但在两类关键场景下，恰是唯一正解。
...
Designer 基于工作流实现建模与调试：用户拖拽组件即可构建 AI 开发流程——内置 140+ 算法组件，支持导出 JSON、接入 DataWorks 调度，节点可嵌入自定义 SQL / Python / PyAlink 脚本。
...
它的强项：
- **MaxCompute 规模的表格机器学习**：与 MaxCompute 深度集成——当训练数据是 MaxCompute 上 2 亿行分区表时，source / split / encode / train 等内置组件直接在 MaxCompute 内执行，免去网络传输至 Python pod 的开销；仅需支付 MaxCompute 计算费用，无闲置 GPU 实例成本。
- **交接给不写代码的分析师。** 推荐、流失、风控团队常有懂业务不懂 Python 的专家。Designer 画布直观易懂，业务人员能自主修改并长期维护。
...
它的弱项：
- 所有 LLM 相关任务：Designer 擅长特征工程与经典 ML，不支持自定义 PyTorch 训练循环。
...
Model Gallery 将 DLC + EAS 封装为零代码界面，非 MLOps 用户点击约六次即可完成开源模型的微调与部署——官方文档称其‘封装了 Platform for AI (PAI)-DLC 和 PAI-EAS，提供高效部署与训练开源大语言模型的零代码方案’。
...
微调方面，文档走了一个物流信息提取的例子：喂给它一个 JSON 数据集，从下拉菜单选 LoRA 超参数，它就帮你提交一个 DLC 任务。Quick Start 特别提到了 **蒸馏模式** —— 用大老师（Qwen3-235B）标注数据，让小学生（Qwen3-0.6B）跟着学。这一模式值得关注——据我所知，这是目前性价比最高的微调方案。
