...
---
钱主要是 EAS 烧掉的。DSW 开发一个月几百块，DLC 训练是脉冲式消费，但 EAS 是 24/7 计费的。只要有人调你的接口，它就烧钱。自动配置里的 `minimum replica count` 这一行，是整个平台杠杆最高的旋钮。这篇文章就是我希望在上第一个生产端点前一天能知道的东西。
...
官方"EAS 概述"把它定义为：“将训练好的模型部署为在线推理服务或 AI Web 应用，支持异构资源、自动伸缩、一键压测、灰度发布和实时监控”。重点就两个：

- 它是**容器化服务层** —— 模型在 OSS 里，代码在容器镜像里。EAS 启动时拉镜像、挂载 OSS、运行启动命令，然后监听端口。
- 它是**按副本数自动伸缩** —— 不是 Serverless 函数模型（有个重要例外，见下文）。副本是真实的 GPU Pod，启动需要 30-120 秒。得按这个规划。
...
> **实战建议：** 从第一天起就把 `/mnt/data/` 写进代码路径。别让模型路径硬编码成 `/workspace/models/`。这样从本地开发切换到 EAS，改一行配置就行，不用重构代码。

## 三种推理模式

文档列了三种。选错了要么浪费钱要么浪费 latency。
...
- **异步** —— 任何耗时 5 秒以上的任务：图像生成、视频生成、PDF 批量 OCR。内置队列会根据积压情况伸缩副本，这才是这类工作负载的正确心智模型。
- **批量** —— 任何可以等几分钟的任务：夜间 Embedding、语音转录。用抢占式实例，账单砍半。

## 真正的 Quick Start

官方 Quick Start 是用 vLLM 部署 Qwen3-0.6B。控制台流程如下：
...
如果返回了一句话，说明你的端点活了，你可以像个巫师一样回到同事面前。

## 自动伸缩的正确姿势

这部分文档没讲透。默认自动伸缩行为（按请求率伸缩，min 副本 = 1）要么导致冷启动延迟投诉，要么账单爆炸。
...
- **`min_replicas`** —— 生产环境永远别设成零。7B vLLM 容器的冷启动要 60-120 秒；用户 5 秒就放弃了。我默认设 2（一个做 HA，一个做冗余）。异步服务可以设 0，靠队列顶着。
- **`max_replicas`** —— 预算刹车。计算公式：`(p99_qps_per_replica) * 2`。如果不知道单副本 QPS，跑一键压测。文档在“服务压测”部分讲了这点。
- **伸缩指标** —— 默认是 `qps`。LLM 服务要换成 `concurrent_requests`（或者 vLLM 的 `running` 指标）。QPS 会误导，因为长生成不会显示为更多请求。

> **实战建议：** 我在 PAI 上见过最严重的浪费，是一个 `max_replicas=50` 的自动伸缩器，配了 `min_replicas=10`，结果服务 Off-peak 只有 0.5 QPS。5 台 idle A10，24/7，跑了两个月。放假前一定要看周六晚上的 Dashboard。
...
文档有一整节讲一键压测器。用它。它会自动 ramp QPS，画出副本扩容曲线，告诉你单副本饱和点。这个数字就是你构建自动伸缩器的基础。不带压测上线是“模型在下午 3 点峰值挂了”工单的最常见原因。

## 180 天大坑

文档角落里藏着一句话：“如果 EAS 服务连续 180 天处于非 Running 状态，系统会自动删除服务。”设个日历提醒。我曾经丢过一个服务配置，因为负责的团队解散了，没人付账单。恢复花了一个下午重新二分查找哪个 `vllm` 版本对应哪个权重。
...
EAS 在健康检查返回 200 之前不会把真实流量路由给副本，所以要把健康检查改成预热完成后才返回。这会给*可见*冷启动增加 5-15 秒，但消除了前 1-3 个真实请求的 5-15 秒延迟惩罚。
...
**6. 异步推理模式（绕过问题）。** 对于能容忍几秒队列时间的工作负载（图像生成、长文本生成），用异步模式。队列根据积压而不是 QPS 伸缩副本，所以 surge 期间 90 秒的冷启动只意味着用户看到 90 秒队列延迟，而不是 5 秒超时。钱一样花，用户体验完全不同。
