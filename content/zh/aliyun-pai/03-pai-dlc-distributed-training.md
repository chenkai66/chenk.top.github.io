...
---
DSW 笔记本适合单人单卡场景；一旦需要八卡、跨两节点训练，或训练时长超过你愿为一个浏览器标签页持续守候的八小时，就该切到 **DLC**。DLC 是 PAI 面向托管 Kubernetes 集群的作业提交入口——你声明需求（镜像、命令、资源、数据挂载），它自动调度 Pod、运行至完成、持久化日志，并返回结果。文档称其为 *Deep Learning Containers*，日常交流中则统一简称为“DLC 任务”。
...
第一点（多样化算力）和第三点（容错能力），正是 DLC 相较于自行租用 GPU ECS 的核心优势。
...
其中两个阶段——**调度器放置 Pod** 和 **挂载 OSS/NAS**——是绝大多数“任务卡在 PENDING”工单的根源。卡在调度阶段，多因资源组配额耗尽；卡在挂载阶段，则常因 OSS/NAS 的 RAM 角色权限配置错误。跟 DSW 一样，排查手段就是起一个带同样 OSS 挂载的微型 DSW，确认 `oss ls` 能通。
...
对大多数团队，通用型按量付费已足够；但当训练规模超 8 卡、且依赖节点间 RDMA 加速时，灵骏才更具成本效益。 — 文档称灵骏支持 RDMA 配置，所谓“加速节点间通信”，实指 NCCL AllReduce 性能可达标准以太网的 5–10 倍。抢占式实例能省成本，前提是任务能干净地打检查点，得益于 EasyCKPT，大多数任务都能做到。
