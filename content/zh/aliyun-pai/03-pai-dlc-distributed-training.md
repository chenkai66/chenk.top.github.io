...
---
DSW 笔记本适合单人单卡场景。一旦你需要八卡跨两节点，或者训练时长超过你能忍受挂着浏览器标签的八小时，就得切到 **DLC**。DLC 是 PAI 面向托管 Kubernetes 集群的作业提交入口：你描述需求（镜像、命令、资源、数据挂载），DLC 负责调度 Pod、运行至完成、持久化日志，最后告诉你结果。文档里管这叫 *Deep Learning Containers*，咱们口头就直接说 "DLC 任务"。
...
官方 DLC 概览列了四点，我特意挑出来说，是因为真有用：

- **多样化算力** — 灵骏 AI 计算服务、ECS、ECI、神龙裸金属、灵骏裸金属。支持混合调度。
- **多种分布式任务类型** — 内置支持 Megatron、DeepSpeed、PyTorch DDP、TensorFlow PS/Worker、Slurm、Ray、MPI、XGBoost。不用自己搭集群。
- **容错能力** — AIMaster（看门狗）、EasyCKPT（异步检查点）、SanityCheck（跑前节点健康检查）、节点自愈。
- **训练加速** — 内置框架支持数据并行、流水线并行、算子拆分、自动并行策略探索、拓扑感知调度、通信优化。

第一点和第三点是 DLC 相比你自己租 GPU ECS 最有价值的地方。
...
其中两个阶段 — **调度器放置 Pod** 和 **挂载 OSS / NAS** — 是绝大多数 "我的任务卡在 PENDING" 工单的发源地。卡在调度意味着资源组满了；卡在挂载意味着存储 RAM 角色配错了。跟 DSW 一样，排查手段就是起一个带同样 OSS 挂载的微型 DSW，确认 `oss ls` 能通。

## 选资源池

你可以提交到三个池子里的一个。文档主要在讲配额和账单；实际决策要看你的任务耐受度。
...
对大多数团队来说，通用型按量付费就够了。一旦训练规模超过 8 卡且需要节点间 RDMA，灵骏才划算 — 文档提到灵骏可配置 RDMA 并提供 "加速节点间通信"（这是委婉的说法，实际意味着 NCCL AllReduce 会比标准以太网快 5-10 倍）。抢占式实例能省成本，前提是任务能干净地打检查点，得益于 EasyCKPT，大多数任务都能做到。
...
- **`fault_tolerance=True`** 会启动一个 AIMaster sidecar 监控每个 worker。如果 worker Pod 挂了，AIMaster 会标记它、请求替换，存活的 worker 会等待替换完成而不是让整个任务崩溃。这是运行超过几小时的任务 *最重要的开关*。
...
控制台 "Training Jobs" 视图提供日志、GPU 利用率、网络吞吐量和 AIMaster 事件。通过 SDK 你可以流式获取日志：
...
对于长任务，我把日志转发到 SLS（日志服务），并在 CloudMonitor 上设置 `gpu_util < 0.3` 持续 15 分钟的告警 — 这是数据加载卡住或分布式初始化出问题的典型信号。
...
AIMaster 作为 sidecar Pod  alongside 你的训练 Pod 运行，拥有独立的 ServiceAccount，具备标记、驱逐和重建 worker Pod 所需的 K8s 权限。它做三件事：
