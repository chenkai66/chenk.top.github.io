...
---
DSW 笔记本适合单人单卡场景。一旦你需要八卡、跨两节点的训练，或者训练时长超过你愿意一直开着浏览器标签页的八小时，就得切换到 **DLC**。DLC 是 PAI 面向托管 Kubernetes 集群的作业提交入口：你描述需求（镜像、命令、资源、数据挂载），DLC 负责调度 Pod、运行至完成、持久化日志，最后告诉你结果。文档中将其称为 *Deep Learning Containers*，日常交流中我们统一简称为“DLC 任务”。
...
第一点和第三点，正是 DLC 相比自行租用 GPU ECS 的核心优势所在。
...
其中两个阶段——**调度器放置 Pod** 和 **挂载 OSS/NAS**——是绝大多数“任务卡在 PENDING”工单的根源。卡在调度阶段，通常是资源组已无可用配额；卡在挂载阶段，则多因 OSS/NAS 的 RAM 角色权限配置错误。跟 DSW 一样，排查手段就是起一个带同样 OSS 挂载的微型 DSW，确认 `oss ls` 能通。
...
对大多数团队来说，通用型按量付费就够了。当训练规模超过 8 卡，且需依赖节点间 RDMA 加速时，选用灵骏才更具成本效益。 — 文档提到灵骏可配置 RDMA 并提供 "加速节点间通信"（这是委婉的说法，实际意味着 NCCL AllReduce 会比标准以太网快 5-10 倍）。抢占式实例能省成本，前提是任务能干净地打检查点，得益于 EasyCKPT，大多数任务都能做到。
