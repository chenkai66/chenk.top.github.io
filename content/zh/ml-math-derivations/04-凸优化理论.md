...
1947 年，George Dantzig 提出了单纯形法，用于解决线性规划问题。现代优化理论由此诞生。八十年过去了，优化已经成为机器学习的核心驱动力：无论是一行代码就能实现的线性回归，还是拥有 700 亿参数的语言模型，每一个训练完成的模型本质上都是某个优化问题的答案。

在所有优化问题中，**凸优化占据了一个独特的位置**。它的定义性质强大到让人觉得有些“作弊”——任何局部最优解一定是全局最优解。而且，针对凸优化问题，已经发展出了一套成熟算法，这些算法不仅理论清晰，还能保证收敛性。我们之所以把“凸”看作绿灯，而把“非凸”视为黄灯，根本原因就在于这一点。

本章从零开始搭建凸优化的工具箱。我会先讲几何基础：凸集、凸函数和 Jensen 不等式；然后补充微积分知识，比如次梯度，用来处理不可微的情况；最后推导具体的算法：梯度下降、Nesterov 加速、Adam、牛顿法、BFGS、约束问题中的 KKT 条件，以及用于分解结构化问题的 ADMM。

**你将学到的内容包括：**

1. **凸集与凸函数**——几何定义、一阶/二阶判别条件、Jensen 不等式  
2. **次梯度**——如何将微积分扩展到不可微目标函数（如 L1 范数）  
3. **一阶方法**——梯度下降、动量法、Nesterov 加速、Adam，以及它们背后的收敛速率  
4. **二阶方法**——牛顿法和 BFGS，理解计算代价与收敛速度之间的权衡  
5. **约束优化**——拉格朗日函数、弱对偶性与强对偶性、KKT 条件  
6. **ADMM**——把一个复杂的联合问题拆分成两个简单的交替子问题  
