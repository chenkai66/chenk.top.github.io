...
本章将从代数、几何和概率三个独立视角推导线性回归，并展示它们殊途同归于同一个公式。随后，我们将探讨当模型假设失效时会发生什么，以及 Ridge、Lasso 和稳健损失如何修复这些问题。
...
**记号技巧。** 单独处理偏置项 $b$ 很麻烦。我们将其吸收进权重向量：在每个输入末尾添加常数 1，即  
...
利用矩阵微分恒等式 $abla_w (a^\top w) = a$ 和 $abla_w (w^\top A w) = 2 A w$（$A$ 对称），可得：  
...
这就是**正规方程**——之所以称为“正规”，是因为从几何角度看，它表明残差 $y - Xw$ 与 $X$ 的每一列都正交（垂直）。稍后我们将从几何角度重新理解这一点。

**定理 1（最小二乘闭式解）。** 若 $X^\top X$ 可逆，则 $L(w)$ 的唯一最小值点为  

$$w^\* = (X^\top X)^{-1} X^\top y.$$  

**证明。** 一阶条件 $abla L = 0$ 直接导出正规方程。Hessian 矩阵为 $abla^2 L = X^\top X$。当 $X$ 列满秩时，对任意非零向量 $v$，有  
...
**下一章预告。** 我们将推广至**分类问题**——输出空间变为离散。Sigmoid 函数、交叉熵损失、决策边界的几何意义，均可从“二元输出的线性-高斯模型应如何构建？”这一问题自然导出。
