...
---
![机器学习数学推导（17）：降维与 PCA —— 图示](https://blog-pic-ck.oss-cn-beijing.aliyuncs.com/posts/zh/ml-math-derivations/17-降维与主成分分析/illustration_1.jpg)
## 这篇文章讲什么

给聚类算法喂入 10,000 维的数据，它大概率会失败。问题不是出在算法本身，而是高维空间对基于距离的学习方法非常不友好。体积几乎都集中在球壳上，最近邻和最远邻的距离比值趋近于 $1$，"近"这个概念变得毫无意义。降维就是为了解决这个问题：把数据投影到低维空间，同时保留真正重要的结构。
...
当所有点之间的距离差不多时，$k$-NN、核密度估计和聚类算法就很难发挥作用了。这就是为什么把高维数据映射到一个低维表示空间，往往是整个流程中最关键的一步。
...
重复上述过程，并要求新方向与已选方向正交，就可以得到完整的主成分集合。前 $k$ 个主成分是 $\mathbf{S}$ 的特征向量 $\mathbf{w}_1, \dots, \mathbf{w}_k$，按照特征值 $\lambda_1 \ge \lambda_2 \ge \dots \ge \lambda_k$ 排序。把这些特征向量拼成投影矩阵：
$$
...
**碎石图（scree plot）** 直接展示了特征值的变化趋势。在一个由秩为 $5$ 的潜信号加各向同性噪声生成的数据中，特征值在 $k = 5$ 处出现明显的“肘点”：之上是信号，之下是噪声。真实数据中的肘点通常没这么清晰，但这张图仍然是分析的第一步。
...
$$
这张图更适合做决策：取使 $R(k)$ 达到保留率阈值的最小 $k$。以 UCI 手写数字数据集为例，原始特征维度 $D = 64$，保留 $90\%$ 方差只需要 $k = 21$，保留 $95\%$ 需要 $k = 29$，即使保留 $99\%$ 也只需 $k = 41$。超过三分之二的特征对表示这些数字图像来说是冗余的。
...
![Reconstruction quality vs k](https://blog-pic-ck.oss-cn-beijing.aliyuncs.com/posts/zh/ml-math-derivations/17-降维�%与主成分分析/fig4_reconstruction_vs_k.png)
...
线性 PCA 只能旋转和投影，无法弯曲。如果数据分布在曲面上——比如同心圆、瑞士卷或球面——那么任何线性投影都无法保留其原始结构。
...
![机器学习数学推导（十七）：降维与主成分分析 — visual](https://blog-pic-ck.oss-cn-beijing.aliyuncs.com/posts/zh/ml-math-derivations/17-降维与主成分分析/illustration_2.jpg)
...
![PCA vs LDA vs t-SNE](https://blog-pic-ck.oss-cn-beijing.aliyuncs.com/posts/zh/ml-math-derivations/17-降维�%与主成分分析/fig5_pca_lda_tsne.png)
...
*This is Part 17 of the [ML Mathematical Derivations](/zh/ml-math-derivations/) series. Next: [Part 18 -- Clustering Algorithms](/en/ml-math-derivations/18-clustering-algorithms/). Previous: [Part 16 -- Conditional Random Fields](/en/ml-math-derivations/16-conditional-random-fields).*
