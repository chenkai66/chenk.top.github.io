...
给聚类算法输入 10,000 维的数据，它大概率会失败——问题不在于算法本身，而在于**高维空间对基于距离的学习方法天然不友好**。体积几乎都集中在球壳上，最近邻和最远邻的距离比值趋近于 $1$，“近”这个概念变得毫无意义。降维的目的正是将数据投影到低维空间，同时保留其关键结构。
...
假设有一组数据 $\{\mathbf{x}_1, \dots, \mathbf{x}_N\}$，每个样本 $\mathbf{x}_i \in \mathbb{R}^d$。首先对数据进行**中心化**，也就是减去样本均值 $\bar{\mathbf{x}} = \tfrac{1}{N}\sum_i \mathbf{x}_i$。为了简化推导，我们直接假设 $\bar{\mathbf{x}} = \mathbf{0}$。接着，把所有样本按行堆叠成矩阵 $\mathbf{X} \in \mathbb{R}^{N \times d}$，这样**样本协方差矩阵**可以写成：
...
借助 [核技巧](/zh/ml-math-derivations/08-支持向量机/)，把每个点通过特征映射 $\phi: \mathbb{R}^d \to \mathcal{H}$ 映射到一个（可能是无限维的）特征空间，然后在特征空间中执行 PCA。我们不会显式构造 $\phi$，只需要计算内积：
