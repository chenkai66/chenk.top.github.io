...
### 2.4 为何是 $1/\varepsilon^2$，而非 $1/\varepsilon$？
...
| 概念 | 关键公式 | 一句话直觉 |
|------|----------|------------|
| PAC 样本复杂度（可实现，有限） | $m \geq \tfrac{1}{\varepsilon}(\ln \vert \mathcal{H}\vert  + \ln \tfrac{1}{\delta})$ | 剔除坏假设这件事，随着样本增加会指数级变容易。|
| PAC 样本复杂度（不可知，有限） | $m \geq \tfrac{2}{\varepsilon^2}(\ln \vert \mathcal{H}\vert  + \ln \tfrac{2}{\delta})$ | 估计期望值比剔除假设更难，代价多出一个 $1/\varepsilon$。|
| VC 样本复杂度 | $m = O\!\bigl(\tfrac{d + \ln(1/\delta)}{\varepsilon^2}\bigr)$ | 对无限假设类，用 $\mathrm{VCdim}$ 替代 $\ln \vert \mathcal{H}\vert $。|
