...
---
**引子：** 一个垃圾邮件过滤器，训练只需几毫秒，支持百万级特征，几乎没有什么超参数需要调整，却能在短文本任务上胜过许多复杂得多的模型。朴素贝叶斯能做到这一点，靠的是一个大胆到近乎离谱的假设——给定类别后，所有特征条件独立。它不仅不为此道歉，还坚持到底。这个假设在几乎所有真实数据集上都不成立，但分类器依然表现良好。要搞清楚**为什么**，需要深入生成模型、 MAP 估计、 Dirichlet 先验以及偏差-方差权衡的核心概念。这篇文章将带你一步步走完这条完整的路径。
