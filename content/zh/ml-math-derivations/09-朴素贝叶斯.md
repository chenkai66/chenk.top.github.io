...
---
**引子：** 一个垃圾邮件过滤器，训练只需几毫秒，支持百万级特征，几乎没有什么超参数需要调，却能在短文本任务上胜过许多复杂得多的模型。朴素贝叶斯能做到这一点，靠的是一个大胆到近乎离谱的假设——给定类别后，所有特征条件独立。它不仅不为此道歉，还坚持到底。这个假设在几乎所有真实数据集上都不成立，但分类器依然表现良好。要搞清楚**为什么**，就得深入生成模型、MAP 估计、Dirichlet 先验以及偏差-方差权衡的核心概念。这篇文章会带你一步步走完这条完整的路径。
...
- 条件独立假设的代价是什么？它在几何上损失了什么，又在统计上带来了什么？
- 如何用极大似然估计计算先验和似然，以及拉普拉斯平滑背后的 MAP / Dirichlet 思路。
- 三种常用变体：多项式、伯努利、高斯，以及如何选择适合的模型。
- 为什么朴素贝叶斯在假设被破坏时依然有效：Domingos–Pazzani 结果、误差抵消机制，以及偏差-方差权衡的作用。
...
在引入“朴素”这个概念之前，我得先搞清楚一个最优概率分类器到底是什么样子。朴素贝叶斯只是实现它的一种具体方法。
...
[< Part 8: Support Vector Machines](/zh/ml-math-derivations/08-support-vector-machines) | **Part 9: Naive Bayes** | [Part 10: Semi-Naive Bayes >](/zh/ml-math-derivations/10-semi-naive-bayes-and-bayesian-networks)
