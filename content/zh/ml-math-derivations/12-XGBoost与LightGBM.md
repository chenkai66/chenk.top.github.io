...
- **XGBoost** 主攻*数学优化*：把损失函数的二阶导数引入目标函数，对树结构本身做正则化，并将分裂点选择转化为一个闭式解公式。
...
到此为止，监督学习这条线我已经从线性模型一路走到了集成树的工程极致。但现实中很多问题根本没有标签——客户分群、主题发现、异常检测、生成建模。下一章起我转入**无监督学习与隐变量模型**，从 EM 算法和高斯混合模型开始。
