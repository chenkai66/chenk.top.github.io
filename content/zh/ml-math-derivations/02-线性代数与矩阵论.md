...
---
![机器学习数学推导（二）：线性代数与矩阵论 — visual](https://blog-pic-ck.oss-cn-beijing.aliyuncs.com/posts/zh/ml-math-derivations/02-线性代数与矩阵论/illustration_1.jpg)
## 为什么写这一章，有什么不同

如果你学过标准的线性代数课程，里面提到的大部分内容你应该都见过。**但这一章不是那种课。** 它是专门为机器学习实践者准备的线性代数：实现梯度下降、运行 PCA、训练神经网络或者读论文时真正会用到的那几个核心概念。

具体来说，我的目标有三个：

1. 帮你建立矩阵操作的**几何直觉**：旋转、拉伸、投影、压缩。
2. 讲清楚四个无处不在的矩阵分解方法——谱分解、**SVD**、QR、Cholesky——并告诉你**什么时候该用哪个**。
3. 把**矩阵求导**讲明白，让你能随手在草稿纸上推导出任何神经网络的梯度公式。

我跳过了行化简、余子式展开的行列式计算，以及抽象向量空间的证明。如果需要这些内容，文末的参考文献里有经典教材推荐。在这里，每个概念都会回归到一张图或者一行 NumPy 代码。
...
先别管那八条公理了。在机器学习里，真正有用的心智模型是这样的：

> *向量空间是一张过原点的无限延伸的「平面」。子空间是嵌套在它里面的更小的过原点的「平面」。*

$\mathbb{R}^2$ 中过原点的一条直线是一个 1 维子空间；$\mathbb{R}^3$ 中过原点的一个平面是一个 2 维子空间。关键是「过原点」这三个字——一旦平移了这个平面，加法和数乘就不再封闭了。
...
图已经说明了一切：线性无关的向量围出一个非退化的平行四边形（面积大于零，确实张成 2 维）；线性相关的向量躺在一条直线上，平行四边形塌掉了。
...
所以 $\mathbb{R}^n$ 被分成两块正交的子空间：行空间（$A$ 在这里干活）和零空间（$A$ 在这里抹掉信息）。只要模型的参数比方程多——这在现代机器学习里几乎总是成立——零空间就不平凡，许多不同的参数向量会给出完全相同的预测。
## 2. 范数与内积——几何层
...
| 范数 | 公式 | 单位球形状 | 机器学习用途 |
|------|------|------------|--------------|
| $\ell_1$ | $\sum_i |x_i|$ | 菱形 | Lasso、稀疏性 |
| $\ell_2$ | $\sqrt{\sum_i x_i^2}$ | 圆形 / 球形 | Ridge、权重衰减 |
| $\ell_\infty$ | $\max_i |x_i|$ | 方形 / 立方体 | 对抗扰动边界 |
...
| 范数 | 公式 | 衡量内容 |
|------|------|----------|
| Frobenius $\|A\|_F$ | $\sqrt{\sum_{ij} a_{ij}^2}$ | 把矩阵 $A$ 当作向量时的欧几里得长度 |
| 谱范数 $\|A\|_2$ | $\sigma_1(A)$ | 在所有单位输入上的最大拉伸 |
| 核范数 $\|A\|_*$ | $\sum_i \sigma_i(A)$ | 矩阵秩 $\text{rank}(A)$ 的凸替代 |

谱范数是 Lipschitz 分析中的正确选择，比如 GAN 中的 spectral normalization。核范数则是低秩矩阵补全问题（如 Netflix 推荐系统）中真正需要最小化的目标。
## 3. 特征值分解——「变换中方向不变的那些向量」
...
![机器学习数学推导（二）：线性代数与矩阵论 — visual](https://blog-pic-ck.oss-cn-beijing.aliyuncs.com/posts/zh/ml-math-derivations/02-线性代数与矩阵论/illustration_2.jpg)
...
| 3 | $\|A x - b\|_2^2$ | $2 A^\top (A x - b)$ | 最小二乘问题 |
