...
---
为什么一群平庸的分类器组合起来，能胜过一个超级厉害的分类器？答案很简单，也很实在：**取平均能降低方差，逐步调整权重能减少偏差，再加点随机性就能打破相关性**——否则前面的努力都会白费。这篇文章从头推导背后的数学原理：从偏差-方差分解开始，到 Bagging 和随机森林如何利用 Bootstrap，再到 AdaBoost 如何被解释为指数损失下的前向分步优化，最后是 GBDT 把这些统一成函数空间中的梯度下降。

看完之后，你应该能对任何集成方法一眼看穿：**它在降什么、为什么有效、什么时候会失效**。

![机器学习数学推导（十一）：集成学习 — visual](https://blog-pic-ck.oss-cn-beijing.aliyuncs.com/posts/zh/ml-math-derivations/11-集成学习/illustration_1.jpg)
## 你将学到

- 平均 $T$ 个不相关的模型，为什么能把方差降到原来的 $1/T$。如果这些模型相关，又会怎样？
- Bagging 和随机森林如何通过随机化让树之间几乎不相关。
- AdaBoost 不是凭空设计的，它其实是指数损失下前向分步加法建模的一个特例。
- GBDT 怎么在函数空间里完成梯度下降。
- 面对具体问题时，我该怎么选择 Bagging、Boosting 还是 Stacking。
## 预备知识
...
这一个公式就解释了集成学习存在的全部意义，值得仔细琢磨：

- **偏差不变**：$\mathbb{E}[H] = \mu$，说明平均无法消除基学习器的系统性误差。如果每棵树都欠拟合，集成也会欠拟合。
- **方差分两部分**：一部分是 $\rho\sigma^2$，这部分无论如何加模型都消不掉；另一部分是 $\sigma^2/T$，随着 $T \to \infty$ 而逐渐减小——但前提是模型之间**完全不相关**（$\rho = 0$）。
- **相关性是敌人**：即使有无穷多个模型，方差的下限仍然是 $\rho\sigma^2$。这就是为什么随机森林不仅对样本做随机化，还要对特征做随机化——后者直接降低了 $\rho$。

所以，所有集成方法的核心都在回答一个问题：**如何生成多样化的基学习器（小 $\rho$），同时不让它们太弱（大偏差）？**
...
- **Boosting**：从高偏差低方差的基学习器出发，**逐步增加容量降低偏差**。

整个集成学习的分类法，就这么两句话。

### 1.3 平庸投票者的力量

二分类场景中，假设有 $T$ 个独立分类器，每个分类器的错误率是 $\epsilon < 1/2$。多数投票出错的概率服从二项分布：

$$P_{\text{ensemble}} = \sum_{k > T/2} \binom{T}{k}\,\epsilon^k(1-\epsilon)^{T-k}.$$

比如 $T = 21$、$\epsilon = 0.30$，计算结果大约是 $0.026$。单个分类器 30% 的错误率很普通，但集成后的错误率降到 2.6%，效果非常出色。**关键在于“独立”**——再一次指向去相关性的核心问题。
## 2. Bagging 与随机森林
...
这是泛化误差的一个几乎无偏估计，作为训练的副产品直接得到。不需要额外划分验证集，也不需要跑交叉验证。
...
仅靠 Bagging 还不够：Bootstrap 子集之间高度重叠，导致每棵树总是在相同的主导特征上做分裂。预测结果高度相关，$\rho$ 偏大，§1.1 中的方差缩减效果就难以实现。
...
强制每棵树在不同节点使用不同的特征，打破了“主导特征陷阱”，降低了 $\rho$。上图清楚展示了这一效果：单棵深度较大的树将输入空间分割成矩形块，容易过拟合噪声；而一棵 $m=1$ 的随机森林通过平均 80 棵这样的树，生成了一个平滑且泛化能力更强的决策边界。
...
这两个因素相互制约，因此 $m$ 是一个需要调整的参数，而不是固定值。
...
## 3. Bagging 和 Boosting：从偏差-方差角度看

![机器学习数学推导（十一）：集成学习 — visual](https://blog-pic-ck.oss-cn-beijing.aliyuncs.com/posts/zh/ml-math-derivations/11-集成学习/illustration_2.jpg)
...
损失函数为 $\mathcal{L}(F) = \sum_i \exp(-y_i F(\mathbf{x}_i))$。我不直接联合优化所有 $T$ 项，而是固定住 $F_{t-1}$，在第 $t$ 轮求解：
...
### 5.1 Boosting 就是函数空间中的梯度下降

指数损失太敏感了。Friedman 在 2001 年的核心洞见是：只要把问题重新理解为梯度下降，前向分步的思想就可以推广到**任意可微的损失函数**。

设 $F(\mathbf{x}) = \sum_{t=0}^T h_t(\mathbf{x})$ 是加性模型，$\mathcal{L}(F) = \sum_i L(y_i, F(\mathbf{x}_i))$ 是损失函数。在第 $t$ 轮，我需要找到一个 $h_t$ 来降低 $\mathcal{L}$。当前迭代点 $F_{t-1}$ 处的负函数梯度，在训练数据上表现为一个 $N$ 维向量：
...
1. **第一层**：训练 $K$ 个基模型，比如逻辑回归、随机森林、GBDT、k-NN。为了避免数据泄露，我用**交叉验证生成袋外预测**：每个训练样本由没见过它的模型来预测。
