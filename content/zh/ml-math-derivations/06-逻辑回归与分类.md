...
差距正好是 $\hat y(1-\hat y) \approx 0.01$ 倍——MSE 把修正信号衰减了**100 倍**，于是自信错的样本几乎不被纠正。换成 $\hat y = 0.001$ 会差到 1000 倍。这就是为什么 sigmoid + MSE 训练分类器经常"卡住不动"，而 sigmoid + CE 一上来就猛拉错样本——CE 的设计本身把 sigmoid 的饱和因子约掉了。
...
**三类输出的具体计算**：取 $K=3$，logits $z = (2, 1, 0)$。先做数值稳定平移 $z - \max(z) = (0, -1, -2)$，再求指数 $(1,\ e^{-1},\ e^{-2}) \approx (1.000,\ 0.368,\ 0.135)$。归一化分母 $\approx 1.503$，于是
$$P = (0.665,\ 0.245,\ 0.090).$$
