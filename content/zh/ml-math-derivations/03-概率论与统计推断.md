...
---
![机器学习数学推导（三）：概率论与统计推断 — visual](https://blog-pic-ck.oss-cn-beijing.aliyuncs.com/posts/zh/ml-math-derivations/03-概率论与统计推断/illustration_1.jpg)
## 本文要讲什么

1912 年，Ronald Fisher 在一篇短文中提出了**最大似然估计（MLE）**，从此悄悄改变了统计学的面貌。他的想法简单得让人有点不好意思：*如果某组参数让观察到的数据显得特别合理，那这组参数很可能就是正确的*。从逻辑回归到大语言模型，几乎所有现代学习算法都源于这个思路。

但光有似然是不够的。要用好它，我需要一套描述不确定性的语言（概率空间、分布族），一些保证经验数据能反映总体特性的定理（大数定律、中心极限定理），以及将先验知识融入推断的工具（贝叶斯框架）。这篇文章把这些关键部分拼接起来，为后续内容打下扎实的统计学基础。
...
一个常见误区：独立性和条件独立性并不互相蕴含。两枚硬币本身相互独立，但一旦给定“恰好有一枚正面”的条件，它们立刻变得相关。图模型的直觉大多建立在这个区分之上。
...
---
## 4. 极限定理：为什么机器学习在大规模数据上有效

![机器学习数学推导（三）：概率论与统计推断 — visual](https://blog-pic-ck.oss-cn-beijing.aliyuncs.com/posts/zh/ml-math-derivations/03-概率论与统计推断/illustration_2.jpg)
...
![第一类错误（$\alpha$）与第二类错误（$\beta$）的权衡](https://blog-pic-ck.oss-cn-beijing.aliyuncs.com/posts/zh/ml-math-derivations/03-概率论与统计推断/fig6_hypothesis_errors.png)
