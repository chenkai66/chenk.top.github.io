...
1912 年，Ronald Fisher 在一篇短文中提出了**最大似然估计（MLE）**，从此悄悄改变了统计学的面貌。他的想法简单得让人有点不好意思：如果某组参数让观察到的数据显得特别合理，那么这组参数很可能就是正确的。从逻辑回归到大语言模型，几乎所有现代学习算法都源于这个思路。

但光有似然是不够的。要用好它，需要一套描述不确定性的语言（概率空间、分布族）、一些保证经验数据能反映总体特性的定理（大数定律、中心极限定理），以及将先验知识融入推断的工具（贝叶斯框架）。这篇文章把这些关键部分拼接起来，为后续内容打下扎实的统计学基础。
