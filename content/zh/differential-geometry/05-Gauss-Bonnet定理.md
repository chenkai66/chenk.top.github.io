...
series_total: 12
---
上一章的绝妙定理表明，高斯曲率是内蕴的——弯曲一个表面而不拉伸，$K$ 不会改变。本章要讨论的 Gauss-Bonnet 定理则在另一个方向上揭示了同样惊人的事实：在一个闭合表面上积分 $K$，结果是一个拓扑不变量。任何球面的总曲率是 $4\pi$。任何环面的总曲率是 $0$。双环面的总曲率是 $-4\pi$。这些事实与具体的几何形状无关——无论怎么弯曲、扭转或挤压表面，总曲率都不会改变。

这样说来，Gauss-Bonnet 定理听起来几乎好得令人难以置信。一个局部微分几何量（曲率，由度量的二阶导数逐点定义）积分后得到一个全局拓扑不变量（欧拉示性数，由三角剖分组合定义）。两个完全不同的数学领域——光滑度量的分析和胞腔复形的组合学——在这里交汇。这是数学中最美丽的定理之一，也是整个“指标定理”行业的概念先驱，最终在 1960 年代达到了 Atiyah-Singer 指标定理的顶峰。

这一章将分两步发展 Gauss-Bonnet 定理：局部版本（对于有边界的区域）是计算引擎；全局版本（对于闭合表面）是拓扑结论。

关于历史背景，简单说几句。局部版本——将角度过剩与曲率联系起来——在 Gauss (1827) 的工作中已初具雏形，并由 Bonnet (1848) 精确化。全局版本也是由 Bonnet 证明的，基于 Gauss 的局部结果。认识到欧拉示性数是出现在等式右边的正确拓扑量，这一洞见来自 Euler、Cauchy 和后来 Poincare 发展的组合拓扑学。这个定理的名字是为了纪念两位贡献者：Gauss 对于局部微分几何内容的贡献，Bonnet 对于全局拓扑扩展的贡献。

这个定理之所以让人感到意外，是因为它的两边差距巨大。左边（$\int K\,dA$）是分析性的——需要微积分、光滑函数和度量的二阶导数。右边（$2\pi\chi$）是组合性的——只需要一个三角剖分和计数顶点、边和面的能力。一个光滑积分等于一个整数（乘以 $2\pi$），这种刚性预示着深层结构。这种现象在数学中反复出现：椭圆算子的指标总是整数；紧流形之间映射的度数总是整数；缠绕数总是整数。所有这些都是“离散类型的连续不变量”源于拓扑约束的一般原则的具体实例。
...
经典曲面理论已经讲完——第一基本形式、第二基本形式、内蕴曲率、Gauss-Bonnet 定理。现在，我们完全转向另一个方向。第六章将介绍*光滑流形*：这些抽象空间在局部上类似于 $\mathbb{R}^n$，但不必嵌入任何背景空间中。这是现代微分几何的起点，也是从广义相对论到规范理论等一切理论的基础。
