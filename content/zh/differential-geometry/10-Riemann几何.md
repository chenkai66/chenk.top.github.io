...
这种结构就是 黎曼度量是指每个切空间上的内积的光滑选择。在光滑流形上添加这一条数据，整个经典工具箱就上线了。你可以计算曲线的长度、切向量之间的夹角、区域的体积。你得到一种沿曲线区分向量场的标准方法（Levi-Civita 联络）。你得到 **测地线** —— 局部最短路径，直线的自然推广。你得到沿曲线的平行移动、环路的全纯性，最终得到区分球面和环面的曲率张量。
