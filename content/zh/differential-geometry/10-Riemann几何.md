...
到目前为止，已经研究了具有可微结构的光滑流形：坐标图、切向量、微分形式、外微积分、积分。这些都不需要距离的概念。可以对函数求导，积分最高阶形式，判断一个分布是否可积 —— 所有这些都不需要测量曲线的长度或两个切向量之间的夹角。光滑结构纯粹是拓扑加上导数。要做经典意义上的几何 —— 测量、比较、讨论曲率、恢复“长度”和“角度”的日常含义 —— 需要额外的结构。
...
如果将正定性放宽为 *非退化*，得到 **伪黎曼度量**。最重要的是 **洛伦兹** —— 4维流形上的签名 $(-, +, +, +)$。这是广义相对论中的时空度量。大部分机制（Levi-Civita、测地线、曲率）逐字照搬；唯一的变化是符号约定和一些“长度”变为虚数（在黎曼符号约定中，类时间隔是虚数，在物理约定中是实正数）。爱因斯坦方程 $G_{\mu\nu} = 8\pi T_{\mu\nu}$ 是洛伦兹的，而不是黎曼的，但几何是一样的。
