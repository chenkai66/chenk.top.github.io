...
到了后面学多元微积分，事情变得有点神秘起来。Green 定理：平面闭曲线包围的区域上的某个偏导组合的积分等于沿边界曲线的线积分。Stokes 定理：曲面上一个旋度的通量等于沿边界曲线的环量。散度定理：体积内散度的体积分等于穿过边界曲面的通量。每一个都被单独证明，每一个都长得有点像、又有点不像基本定理。

这一章我要把它们全部缝在一起。它们其实都是同一个公式在不同维度的化身：
$$\int_M d\omega = \int_{\partial M}\omega.$$

读法：把"对内部做导数再积分"换成"在边界上直接积分"，结果一样。一维基本定理是 0-形式 $f$ 在 1 维区间上的版本（$d\omega$ 是 1-形式 $f'\,dx$，$\partial M$ 是两点 $\{a, b\}$ 带符号）；Green 定理是 1-形式在 2 维区域上的版本；经典 Stokes 是 1-形式在 2 维曲面上的版本（与 Green 的区别只是嵌入维度）；散度定理是 2-形式在 3 维体积上的版本。一个公式打通四个定理。

为什么这件事重要？两个原因。第一，简洁。一个公式比四个公式好记，更别说推广到 $n$ 维（这个统一公式无缝处理任意维度）。第二，深度。Stokes 定理是 de Rham 上同调和奇异同调之间配对的来源（积分把闭形式 $\omega$ 和闭链 $C$ 配对成一个数），是 Gauss-Bonnet 的根本（绕一圈的角度变化等于内部曲率积分），是电磁学守恒律的根本（高斯定律 = 散度定理），最后还是 Atiyah-Singer 指标定理的局部论据的源头。这一条公式是流形上微积分*唯一*的整体定理；其他所有"积分定理"都从它推出来。

要把这个公式说清楚，需要三件准备工作。第一是*定向*——没有定向就没有"带符号的积分"，左右两边都没法谈。第二是*带边界的流形*以及*诱导定向*——边界 $\partial M$ 必须从 $M$ 那里继承一个一致的定向，否则等式右边的符号是模糊的。第三是把*微分形式在 $k$ 维子流形上的积分*用坐标图和单位分解的方式严格定义。这三件事做好之后，Stokes 定理的证明就只剩一个局部计算：在一个长方体上验证一阶情况，然后用单位分解粘起来。

这是经典微分几何的局部理论的顶点，也是后续每个全局定理（Gauss-Bonnet 全局版、Poincaré 对偶、de Rham 与奇异同调对应、特征类）的入口。读完它，本科向量微积分里那些"恰好成立"的恒等式就再也不神秘了。
...
为什么 $n$-形式（最高阶形式）才能在 $n$-流形上积分？因为 $n$-形式 $\omega$ 吃 $n$ 个切向量，吐一个有向 $n$ 维体积——而流形的局部就是 $n$ 维的，每个小盒子就需要一个体积值。低阶形式（$k < n$）只吃 $k$ 个向量，对应的是 $k$-维子流形上的积分（曲线上积 1-形式、曲面上积 2-形式），它们在整个 $n$-流形上没有对应的体积。这就是为什么积分理论按形式的阶数分层——每一阶对应一个维度的子流形上的积分。
...
Stokes 定理 $\int_M d\omega = \int_{\partial M}\omega$ 的右边是个低一维的积分，左边是高一维的积分。两边维度差 1，正是因为 $d$ 把 $k$-形式提升到 $(k+1)$-形式。这种维度差 1 的对偶是数学里反复出现的模式：链复形里的 $\partial$ 算子把 $k$-单形降到 $(k-1)$-单形；上链复形里的 $d$ 算子把 $k$-余链升到 $(k+1)$-余链；积分把这两个对偶地配对起来——这就是 de Rham 与奇异同调对应的来源。
...
斯托克斯定理的全部力量在链水平上最清晰地显现：链上的 $\partial$ 和形式上的 $d$ 在积分配对下是*互相伴随*的算子。形式上：定义配对 $\langle C, \omega\rangle = \int_C \omega$，则
$$\langle \partial C, \omega\rangle = \langle C, d\omega\rangle.$$
这是斯托克斯定理在内积语言下的重写。它告诉我们：奇异同调（链 + $\partial$）和 de Rham 上同调（形式 + $d$）通过这个配对互相对偶。这就是 **de Rham 定理**：对于光滑流形，$H^k_{dR}(M)$ 和 $H_k(M;\mathbb{R})^*$ 通过积分配对是同构的。代数拓扑（链上的边界算子）和分析（形式上的外导数）在这里碰头，斯托克斯定理是把它们焊在一起的螺丝。

我喜欢这个观点的一个原因是，它把"积分"从"求面积/体积的工具"升级成了"配对运算"。一旦你接受了 $\int_C \omega$ 是 $C$ 和 $\omega$ 的双线性配对，许多原本看起来分散的事实就变成了同一件事的不同切片：Green 公式、Gauss 散度定理、电磁学里的磁通守恒、复分析里的留数定理（这是 Stokes 定理对全纯 1-形式的特例）、甚至上面写过的绕数和链接数公式。它们都是 $\langle\partial C, \omega\rangle = \langle C, d\omega\rangle$ 在不同维度、不同流形、不同形式上的回声。

### 几个具体例子的拼图

让我把上面那些散落的例子拼回一张图。把球面 $S^2$ 当成 $B^3$（实心球）的边界，把环面 $T^2$ 当成 $S^1\times S^1$。在球面上积分一个 2-形式比如曲率形式 $K\,dA$ 给我们 $4\pi$（Gauss-Bonnet）；在 $B^3$ 上积分 $d(K\,dA)$ ——但 $K\,dA$ 不能光滑延拓进 $B^3$，所以这个积分本身是个有意思的反例，提示我们"边界的形式"和"内部的形式"不能随便互换。环面则相反：$T^2$ 是闭流形（无边界），所以任何 $\int_{T^2} d\omega = \int_{\partial T^2} \omega = 0$——闭曲面上的恰当形式积分总是零，这正是 de Rham 上同调"看到"$T^2$ 上有非平凡 1-形式（$d\theta_1, d\theta_2$）的原因。
...
我把这一章合上之前先点一下名。我们走过：定向、密度、用单位分解把局部坐标里的积分粘成全局积分；带边流形和它的诱导定向；广义斯托克斯定理 $\int_M d\omega = \int_{\partial M}\omega$ 把经典向量分析里的 Green、Gauss、Stokes 三个公式统一成一句话；然后是 de Rham 上同调、链复形、$\partial$ 和 $d$ 的互伴关系；最后是周期、绕数、链接数这些拓扑不变量作为积分配对的实际例子。

下一卷，我要从微分几何过渡到 **Riemann 几何**。表面上看是把"度量"放回中心位置——之前几章都在处理"形式"和"流"这样的不需要度量的对象，下一卷开始我们会在每个切空间上加一个内积 $g_p$，把它变成 Riemann 流形。但这不只是加一个结构那么简单。一旦有了度量，能问的问题立刻多了好几个量级：两点之间的*测地线*是哪一条？流形的曲率怎么从度量算出来？两个 Riemann 流形等距 $\Leftrightarrow$ 它们的曲率张量相同吗？

第一章我会从 Riemann 度量的定义开始，引入 Levi-Civita 联络（唯一一个既保持度量、又无挠的联络），然后是协变导数、平行移动、和测地线方程。Christoffel 符号会出现，但我尽量不让它们成为主角；它们只是"度量的一阶导数在某个坐标图下的表达"。真正的主角是几何对象本身：一条把弧长走到极小的曲线，一个度量让向量沿着曲线"平行"移动的规则。

到那时候你会发现，这一卷里的所有工具——切丛、形式、流、Stokes 定理——都变成了 Riemann 几何的语法。我们还会回头看：这一卷里那个绝妙定理（Gauss 曲率只依赖第一基本形式）原来是 Riemann 几何里更一般的"曲率张量是度量的二阶导数函数"的特例。从局部到全局、从外在到内在、从微分到拓扑——这门课走过的路径，到 Riemann 几何那里会再走一遍，只是规模更大。
