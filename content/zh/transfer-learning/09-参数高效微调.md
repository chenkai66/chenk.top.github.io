...
![迁移学习（九）：参数高效微调 — visual](https://blog-pic-ck.oss-cn-beijing.aliyuncs.com/posts/en/transfer-learning/09-parameter-efficient-fine-tuning/illustration_1.png)
...
![LoRA 在 SST-2 / MNLI / CoLA 上的 rank 敏感度与参数成本 Pareto 曲线。](https://blog-pic-ck.oss-cn-beijing.aliyuncs.com/posts/zh/transfer-learning/09-参数高效微调/fig09_lora_rank.png)

![迁移学习（九）：参数高效微调 — visual](https://blog-pic-ck.oss-cn-beijing.aliyuncs.com/posts/en/transfer-learning/09-parameter-efficient-fine-tuning/illustration_2.png)
...
LoRA 的两个自由旋钮 —— 秩 $r$ 和缩放因子 $\alpha$ —— 看似无害，大多数教程也将它们当作需要网格搜索的超参数。但它们并非相互独立，且最优选择会随模型规模以可预测的方式变化。值得思考的问题是：当 $d_{\text{model}}$ 从 768（BERT-base）增长到 12288（GPT-3）时，$r$ 应该随之增长、保持不变，还是缩小？

### 有效学习率

将 LoRA 更新写作 $\Delta W = (\alpha/r)\, BA$，其中 $A \in \mathbb{R}^{r \times d_{\text{in}}}$，$B \in \mathbb{R}^{d_{\text{out}} \times r}$。前向传播计算为：
$$
h = W_0 x + (\alpha/r)\, B(Ax),
$$
损失对 $A$ 的梯度为：
$$
\nabla_A \mathcal{L} = \frac{\alpha}{r}\, B^T \nabla_h \mathcal{L} \cdot x^T.
$$
代入步长为 $\eta$ 的 vanilla SGD：
$$
A_{t+1} = A_t - \eta \cdot \frac{\alpha}{r}\, B^T \nabla_h \mathcal{L} \cdot x^T.
$$
因此，$A$ 实际感受到的有效学习率为 $(\alpha/r)\, \|B\|^2 \cdot \eta$，而非 $\eta$。同理，$B$ 感受到的有效学习率正比于 $(\alpha/r)\, \|A\|^2 \cdot \eta$。由此得出两点结论：

- 将 $r$ 翻倍会使每个参数的有效学习率减半，尽管参数数量翻倍。这正是 $\alpha/r$ 缩放的设计初衷 —— 它解耦了步长与秩的关系，使得单一 $\alpha$ 可适用于不同秩。
- $\|B\|$ 在训练过程中增长（初始为零），因此 $A$ 的有效学习率会自动提升。这也解释了为何 LoRA 能容忍比全量微调高得多的名义学习率：早期大部分更新被较小的 $\|B\|$ 所抑制。

### 低秩 ODE 极限分析

初始化 $A \sim \mathcal{N}(0, \sigma^2)$，$B = 0$。在梯度流极限（$\eta \to 0$，连续时间）下，假设损失函数局部为二次型且 Hessian 为 $H$，则 $A$ 和 $B$ 的动态满足如下耦合双线性 ODE：
$$
\dot A = c\, B^T G, \qquad \dot B = c\, G A^T,
$$
其中 $G = -\nabla_W \mathcal{L}|_{W_0}$ 是全量微调的梯度，$c = \alpha/r$。该系统在 $G$ 的 SVD 下有闭式解：若 $G = U \Sigma V^T$，则 $BA$ 收敛至 $U_{:r} \Sigma_{:r} V_{:r}^T$ —— 即全量微调更新的秩-$r$ 截断。该结论由 Hu 等人（2024）证明，并与经验观察一致：LoRA 在训练初期能很好地逼近全量更新的 top-$r$ 子空间，后期仅有轻微漂移。

### 经验缩放规律

在广泛模型规模下，能将性能恢复至全量微调 0.5 分以内的最优秩 $r^*$ 近似满足：
$$
r^* \approx c \log d_{\text{model}},
$$
其中自然语言任务的 $c \approx 1.5$。具体而言：125M 模型只需 $r=4$，1B 模型需 $r=8$，70B 模型需 $r=16$。值得注意的是，$r^*$ 的增长远慢于 $d_{\text{model}}$ 本身，因此 LoRA 的参数开销（相对比例）随模型增大而缩小 —— 这是特性，而非巧合。

### 合成实验验证

最清晰地观察秩“拐点”的方法是构造一个已知内在秩的问题。生成 $W_{\text{true}} = W_0 + UV^T$，其中 $U \in \mathbb{R}^{d \times 6}, V \in \mathbb{R}^{d \times 6}$，然后扫描 LoRA 秩。
...
在 $r=8$（略高于真实秩 6）处出现拐点，完全符合理论预期。更高秩无助于性能提升，多余容量仅用于拟合噪声；更低秩则迫使优化器投影到无法表示 $UV^T$ 的子空间，残差 MSE 即为投影误差。

这对实践者而言是个不太舒服的结论：**合适的秩取决于适配任务的内在维度，而非模型大小**。模型更大、任务相同，往往 $r$ 也相同。我们将在 D 节用真实 GLUE 数据再次验证这一点。

### $\alpha$ 的真实作用

一个常令新手困惑的细节：$\alpha$ 并非广义上的“缩放超参数”——它实质上是学习率乘子。在扫描秩时固定 $\alpha/r$ 可保持每个参数的有效学习率恒定。社区常用启发式 $\alpha = 2r$（见下文 D 节）使 $\alpha/r = 2$ 恒定；也有实现采用 $\alpha = r$ 使比值为 1。只要不在不同实验中混用约定，两者均可行。

唯一陷阱：若改变 $\alpha$ 而不调整 $\eta$，实则在静默地扫描学习率。所谓“LoRA 在高秩下不稳定”的报告，经检查往往是有效学习率数量级偏移所致。最清晰的做法是在代码中固定 $\alpha/r$，仅将 $\eta$ 视为学习率调节旋钮。
...
并行 Adapter（He et al., 2021）则直接从 $h_{\text{in}}$ 计算 Adapter，与 FFN 并行执行：
$$
...
对已知分布，信息论最优码本应将每个码字置于该分布下等概率区间的条件均值处。对标准正态 $\mathcal{N}(0, 1)$ 的 16 个区间，码字边界设为分位数：
$$
...
![LoRA rank 扫描动画：SST-2 在 r=16 附近饱和,更难的任务需要更大 r。](https://blog-pic-ck.oss-cn-beijing.aliyuncs.com/posts/zh/transfer-learning/09-参数高效微调/anim_lora_rank_sweep.gif)

A 节的合成实验在真实秩处出现了“肘部”（elbow）。但现实任务并不提供已知的内在维度，因此唯一诚实的回答是：扫一遍。本节展示了在三个复杂度不同的 GLUE 任务上进行秩扫描的结果。

### 实验设置

我们在 SST-2（情感分析，简单）、MNLI（自然语言推理，中等）和 CoLA（语言可接受性，困难）上对 RoBERTa-base 进行微调。LoRA 仅应用于 query 和 value 投影。秩扫描范围为 $\{4, 8, 16, 32, 64\}$，并设 $\alpha = 2r$ —— 这是一种常见启发式方法，保持 $\alpha/r = 2$ 恒定。训练流程是标准的；唯一与 LoRA 相关的部分是构建适配器并冻结其余参数。
...
def train_lora_glue(task: str, rank: int, epochs: int = 3,
    ...
    for n, p in model.named_parameters():
        p.requires_grad_("lora_" in n or "classifier" in n)  # 仅训练 LoRA 和分类头
...
`build_roberta_base`、`load_glue` 和 `evaluate_glue` 是常规辅助函数；代码重点在于 LoRA 特有的参数管理，而非 GLUE 数据处理细节。

### 实验结果

下表为开发集得分（SST-2 和 MNLI 用准确率，CoLA 用 Matthews 相关系数），每项结果均取 3 个随机种子的平均值。

| 任务   | $r=4$ | $r=8$ | $r=16$ | $r=32$ | $r=64$ | 全参数微调 | 可训练参数量 |
|--------|-------|-------|--------|--------|--------|------------|--------------|
...
这一模式与 A 节在真实数据上的预测一致：

- **SST-2 在 $r=4$ 即饱和**。情感任务在嵌入空间中仅有两个簇，划分边界所需容量极小。
- **MNLI 在 $r=8$ 后趋于平稳**。三分类 NLI 需要更复杂的决策面，但仍位于低维子空间中。
- **CoLA 持续提升至 $r=32$**。语法判断依赖于分散在多个方向上的句法特征，因此需要更高秩的适配。

三个任务的最优秩均由任务本身决定，而非模型结构。RoBERTa-base 是固定主干；只有目标分布发生变化，也只有目标分布移动了“肘部”位置。

### 默认配置与秩-1 的注脚

面对一个毫无先验信息的新任务，$r=8$ 是一个合理起点——它在上述三个 GLUE 任务中的两个上已处于或接近肘部，也是 LoRA 原论文的默认值。在小型验证集上扫一次（耗时几小时），即可确定最终配置。

一个令人惊喜的事实：秩为 1 的 LoRA 往往仅比秩为 4 的版本低不到 1 个点。原因与 IA³ 有效的原因相同——许多适配操作几乎完全投影到每个矩阵的一维子空间中，大部分收益可从单一方向捕获。IA³ 正是基于此思想，每层 Transformer 仅学习三个缩放向量（key、value、FFN），在多个基准上以三分之一的参数量达到与 LoRA 相当的性能。若你的存储预算极为紧张，不妨从 IA³ 开始。

这引出了本文的核心观点：**PEFT 之所以有效，是因为适配本质上是低维的**。LoRA、Adapter、Prefix-Tuning 和 IA³ 都是对同一现象的不同参数化方式，各自在延迟、可合并性和存储之间做出权衡。选择符合你部署约束的参数化方法，在实际任务上扫出合适秩，其余皆为工程细节。

### 最后的合理性检查

如果你只记住本文一点，请记住这个诊断准则：**当 LoRA 比全参数微调差超过 1 个点时，几乎从来不是因为“秩太低”**。问题通常出在以下三点之一：学习率过低（忘了 LoRA 的学习率通常需比全微调高 $10\times$ 到 $100\times$）、不同运行间 $\alpha/r$ 比例不一致，或适配了错误的层（例如只适配 attention 层，但任务实际需要修改 FFN）。在尝试提高秩之前，请先排查这三项。任务的内在维度极少需要超过 $r=16$；绝大多数性能差距都源于参数化配置的失误，而非秩不足。
