...
![源域与目标域分布以及 RKHS 中的 MMD 差距。](https://blog-pic-ck.oss-cn-beijing.aliyuncs.com/posts/zh/transfer-learning/01-基础与核心概念/fig01_mmd_embedding.png)
...
Ben-David 界告诉你：分布差异很重要；但它没告诉你如何在周二下午用一台笔记本电脑去度量它。MMD 做到了。经验估计器将抽象的 RKHS 距离转化为一个标量——你只需两个 mini-batch 就能算出来，然后盯着看。

### 从总体 MMD 到样本估计器

给定核函数 $k$，分布 $P$ 和 $Q$ 之间的平方总体 MMD 为：
$$\mathrm{MMD}^{2}(P, Q) \;=\; \mathbb{E}[k(x, x')] \;-\; 2\,\mathbb{E}[k(x, y)] \;+\; \mathbb{E}[k(y, y')],$$
其中 $x, x' \sim P$，$y, y' \sim Q$。每个期望项在样本上对应一个 U-统计量。给定 $n$ 个源域样本和 $m$ 个目标域样本，我们在代码中实际使用的有偏估计器为：
$$\widehat{\mathrm{MMD}}^{2} \;=\; \frac{1}{n^{2}} \sum_{i, j} k(x_{i}, x_{j}) \;-\; \frac{2}{nm} \sum_{i, j} k(x_{i}, y_{j}) \;+\; \frac{1}{m^{2}} \sum_{i, j} k(y_{i}, y_{j}).$$
单个高斯核要求你选定一个带宽（bandwidth），而错误的带宽会让所有分布看起来都一样。标准做法是使用 **多尺度 RBF 核混合**：
...
def mmd_between_loaders(loader_s, loader_t, feature_fn, n_batches=20):
    """在 n_batches 个 mini-batch 特征上平均 MMD。"""
    vals = []
...
`feature_fn` 是你信任的任意嵌入——像素值可用于 sanity check，ResNet 的倒数第二层用于其他任务。像素级 MMD 衡量低层偏移；嵌入级 MMD 衡量 *特征* 是否仍对齐。

### 两组具体对比

我在 ImageNet 预训练的 ResNet-18 倒数第二层特征上运行该估计器，batch size 为 256，平均 50 个 batch：

| 对比                   | 域偏移类型                          | $\widehat{\mathrm{MMD}}^{2}$ |
| ---------------------- | ----------------------------------- | ---------------------------- |
| MNIST $\to$ USPS       | 手写数字，均为灰度图，尺寸相近      | $0.04$                       |
| MNIST $\to$ SVHN       | 手写数字 vs 自然场景彩色照片        | $0.31$                       |

USPS 使用不同扫描仪、不同分辨率，但仍是均匀背景上的干净灰度数字。SVHN 是房屋门牌号的照片——彩色、杂乱、每张裁剪图含多个数字。约 $7\times$ 的 MMD 差距与肉眼观察完全一致。

### MMD 预测微调后的准确率

在训练前计算 MMD 的原因在于：它是训练成本的廉价代理指标。我对同一骨干网络（10 轮，学习率 $3 \times 10^{-4}$，全解冻）在各目标域上微调，并测量目标测试误差：

| 源域       | 目标域         | $\widehat{\mathrm{MMD}}^{2}$ | 目标测试误差 |
| ---------- | -------------- | ---------------------------- | ------------ |
| MNIST      | USPS           | $0.04$                       | $3.1\%$      |
...
在此范围内，二者呈单调且近似线性关系。这不是定理——而是一种经验规律，只要源表示合理且目标分布仍在编码器见过的支持集内就成立。若编码器 *从未见过* 目标分布，MMD 会饱和并失去预测能力；此时应更换源数据集，而非更努力地调参。

但 MMD 是对称的——对于非对称情形（如目标远小于源，或目标标签包含源从未见过的信息），Ben-David 界更重要，你需要不同的可行性检验。
...
若 $a_{\text{frozen}}$ 显著高于 $a_{\text{rand}}$，说明源表示确实有用，完整微调很可能进一步提升性能。若 $a_{\text{frozen}} \approx a_{\text{rand}}$，源表示中性——你可能受益于低层先验，但别期待奇迹。若 $a_{\text{frozen}} < a_{\text{rand}}$，源表示会误导分类器，任何微调都可能收敛到比从零开始更差的解。
...
| 源 $\to$ 目标                | $a_{\text{frozen}}$ | $a_{\text{rand}}$ | 结论                                  |
| ---------------------------- | ------------------- | ----------------- | ------------------------------------- |
