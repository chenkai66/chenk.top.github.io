...
---
一个模型从未见过“缅甸猫”这个标签，却能正确分类一张缅甸猫的图片。传统监督学习每个类别需要几百万张标注样本，而 OpenAI 在 2021 年发布的 CLIP 完全规避了这一限制：它将图像与自然语言描述共同映射到同一向量空间，此时‘分类’即等价于从用户提供的任意候选句子中选出与该图像嵌入余弦相似度最高的一句。
...
2. **对称对比损失**
图像到文本与文本到图像两个方向的对比学习同步进行，使模型习得的不仅是单向能力（如‘看图说话’），还有图像与文本间的双向语义对齐能力，这正好满足零样本分类和检索的需求。
