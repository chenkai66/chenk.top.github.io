...
---
一个模型从来没有见过“缅甸猫”这个标签，为什么还能正确分类一张缅甸猫的图片？传统监督学习每个类别需要几百万张标注样本。而 OpenAI 在 2021 年发布的 CLIP 完全绕开了这个限制：它学会把图像和自然语言描述映射到同一个向量空间，然后“分类”就变成了从你写下的任意候选句子中，挑出离这张图片最近的一句。

关键不是架构，而是监督信号。CLIP 从网上抓取了 4 亿对（图像，alt-text）数据，训练了一个对比目标：对于每张图片，它的正确描述必须比 batch 中其他图片的描述更相似。就这么一个简单的约束，加上大规模数据，就能让两种模态对齐得非常好，以至于下游任务——零样本分类、检索、生成描述——几乎都是顺手完成的。

这篇文章会推导这种对齐背后的数学原理，详细讲解 CLIP 和它的后续版本 BLIP-2，比较三种融合策略的适用场景，最后给出一份从零开始实现的 PyTorch 版本 CLIP。
## 你将学到什么
...
1. **没有分类头**  
   模型输出的是嵌入向量，而不是固定类别集合上的 logit。它不会局限于“ImageNet 的 1000 类”，因此可以灵活应用于任何能用文字描述的概念。

2. **对称对比损失**  
   图像到文本和文本到图像的训练是同时进行的。模型学到的不是单向技能（比如“看图说话”），而是双向对齐能力。这种能力正好满足零样本分类和检索的需求。
...
就这么简单。不需要在目标任务上做梯度下降，也不需要标注数据，只需要提供 $K$ 个类别名。CLIP 就是靠这种方法，在 ImageNet 上达到了 76% 的 top-1 准确率，性能与在 ImageNet 1.28M 张标注图片上完整训练的 ResNet-50 相当。
...
![迁移学习（八）：多模态迁移 — visual](https://blog-pic-ck.oss-cn-beijing.aliyuncs.com/posts/zh/transfer-learning/08-multimodal-transfer/illustration_2.jpg)
...
每个 anchor 在 batch 里能看到 $B - 1$ 个负样本。Batch 越大，负样本越多，互信息下界越紧，学习信号越强——直到显存不够用为止。
...
如果 batch 涨不上去，有两个办法：**MoCo 风格的负样本队列**（维护一个最近样本的滚动队列）和**梯度累积**（只对损失累积有效，因为 softmax 的分母必须一次看全所有负样本）。
