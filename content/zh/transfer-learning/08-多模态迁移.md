...
---
一个模型从未见过“缅甸猫”这个标签，为何仍能正确分类一张缅甸猫的图片？传统监督学习每个类别需要几百万张标注样本。而 OpenAI 在 2021 年发布的 CLIP 完全规避了这一限制：它将图像与自然语言描述共同映射至同一向量空间，此时‘分类’即等价于在用户提供的任意候选句子中，选出与该图像嵌入余弦相似度最高的一句。

它的核心突破不在于模型架构，而在于监督信号的设计。CLIP 从网上抓取了 4 亿对（图像，alt-text）数据，训练了一个对比目标：对于每张图片，它的正确描述必须比 batch 中其他图片的描述更相似。仅靠这一简单约束，配合海量数据，即可实现图像与文本模态的高度对齐，从而自然支撑零样本分类、跨模态检索、图像描述生成等下游任务。
...
1. **没有分类头**  
   模型输出的是嵌入向量，而非限定于固定类别集合的 logits。因此，它不受限于 ImageNet 的 1000 类等预定义范畴，可直接泛化至任意文本可描述的概念。

2. **对称对比损失**  
   图像到文本与文本到图像两个方向的对比学习同步进行，使模型习得的并非单向能力（如‘看图说话’），而是图像与文本间的双向语义对齐能力。这种能力正好满足零样本分类和检索的需求。
