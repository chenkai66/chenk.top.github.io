...
**2. 生成式特征合成**（[第 4 节](#生成式-zsl-合成不可见类的特征)）：一旦能合成未见类特征，GZSL 就退化为均衡数据下的监督分类。
...
回看，**CLIP**（Radford et al., 2021）本质上就是[第 3 节](#兼容性函数-统一视角)所述的深度双塔模型——只不过它在**4 亿对网络图文数据**上训练，使用对比交叉熵损失：
$$
