...
---
英语有标注数据，但世界上有 7000 多种语言。跨语言迁移让一个只用英文 IMDB 数据训练的情感分类器能分析西班牙语推特，让一个在 SQuAD 上微调的问答模型回答印地语问题，甚至让一个从未见过斯瓦希里语标注数据的模型完成可用的斯瓦希里语命名实体识别任务。

为什么能做到这些？我从双语词向量对齐讲起，这是整个领域的起点。接着介绍多语言预训练模型（比如 mBERT、XLM-R），它们让平行语料从必需品变成了可选项。最后聊聊实际操作中的关键问题：零样本、translate-train 和 translate-test 各自适合什么场景？哪些语言对容易出问题？低资源语言又该怎么处理？
![迁移学习（十一）：跨语言迁移 — visual](https://blog-pic-ck.oss-cn-beijing.aliyuncs.com/posts/zh/transfer-learning/11-cross-lingual-transfer/illustration_1.jpg)

## 你将学到什么

- 共享语义空间假设的核心思想，以及如何用 Procrustes 对齐实现它  
- mBERT 为什么能在没有平行语料的情况下跨语言迁移，子词词汇表在其中起了什么作用  
- XLM-R 的三项改进（数据、采样、规模），每一项分别带来了哪些提升  
- translate-train、translate-test 和零样本策略的权衡，如何根据场景选择合适的方法  
- 针对没有直接标注数据的语言对，如何设计 pivot 策略  
- 迁移差距的来源——文字系统、语系、资源规模——以及如何应对这些问题
## 前置知识
...
![迁移学习（十一）：跨语言迁移 — visual](https://blog-pic-ck.oss-cn-beijing.aliyuncs.com/posts/zh/transfer-learning/11-cross-lingual-transfer/illustration_2.jpg)
...
奇怪的是，这种方法居然有效。Pires 等（2019）和 Wu & Dredze（2020）分析后发现，背后有三个关键机制：
...
最让人惊讶的实验是：**去掉所有锚点 token 后，跨语言 NER 的准确率只下降了大约 5 个点**。真正起决定作用的其实是子词重叠。
...
采样的改动比看起来重要得多。如果不调整，模型看到英文的次数会比斯瓦希里语高出几个数量级，斯瓦希里语的表示就会退化。用 $\alpha = 0.7$ 后，高资源语言仍然占主导，但长尾语言至少能分到足够的子词共现，学会基础统计规律。最终，XLM-R-large 在 XNLI 上比 mBERT 平均高出约 **10 个点**——这个差距主要来自长尾语言的表现提升。
...
图中下半部分的表格总结了各种方法的权衡。实际操作中，大多数团队的做法是：**先用零样本迁移快速上线一个基线模型，覆盖所有语言；然后针对流量最大的几种语言采用翻译后训练；最后用翻译后推理兜底长尾语言。**
