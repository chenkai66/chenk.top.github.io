...
- 本系列前面的所有内容，尤其是[第 9 章（PEFT）](/zh/transfer-learning/09-参数高效微调/)、[第 10 章（持续学习）](/zh/transfer-learning/10-持续学习/)和[第 11 章（跨语言）](/zh/transfer-learning/11-跨语言迁移/))
...
3. **业务领域和预训练语料是否接近？** 比如通用网络文本和放射科报告、法律合同、工业日志的重合度很低。这种情况下，先做一轮领域自适应预训练（[第 3 章](/zh/transfer-learning/03-域适应方法/))，往往能省下后续微调的 GPU 成本。
4. **是只服务一个任务，还是同时面向多个租户或任务？** 如果多个租户共享同一个骨干模型， LoRA / Adapter （[第 9 章](/zh/transfer-learning/09-参数高效微调/))可以让你为每个租户存储几 KB 的参数，而不是几 GB。
...
**Tesla Autopilot 的 HydraNet。** 一个 ResNet 风格的骨干网络连接了 48 个任务专用头，包括车道检测、信号灯状态识别、深度估计、占据网格、交通标志识别等。这是[第 6 章](/zh/transfer-learning/06-多任务学习/)提到的多任务学习在生产环境中的大规模实践：新增一个任务头几乎不增加成本，共享特征让改进一个任务的数据往往能顺带提升其他任务的表现，推理成本被 48 个输出一次性分摊。

**GitHub Copilot （Codex）。** 先在公开代码库上对预训练的 GPT 模型进行领域自适应预训练，再针对代码补全任务微调。这是[第 3 章](/zh/transfer-learning/03-域适应方法/)提到的领域自适应预训练在行星级规模上的应用。从通用 GPT 到专精代码的 Codex，性能提升显著到足以定义一个全新的产品类别。
...
1. 决策树分析结论（[第 1 节](#到底什么时候该用迁移学习)内容）。
2. 对比显而易见替代方案的成本分析（[第 3 节](#经济账三到五个数量级的差距)内容）。
3. 一条保守假设下的 12 个月收益曲线，并明确写出负责监控闭环的人是谁（[第 6 节](#监控分布漂移)内容）。
...
微调模型训练完成后，下一步就是决定如何部署服务。Adapter 在实验阶段很方便，但在高负载场景下通常不是最优选择。以下两种模式覆盖了大多数生产部署场景：**当延迟是主要瓶颈时，合并 Adapter；当延迟和内存都是瓶颈时，使用蒸馏**。

### LoRA 合并实现零开销推理

在基础模型上附加 LoRA Adapter 进行推理，会在每个线性层引入一次额外的矩阵乘法（将 $x B A$ 加到 $x W_0$ 上）。对于一个 7B 模型搭配 rank-16 的 Adapter，这通常会增加 8–15% 的延迟，并且会阻止使用标准优化内核（如 FlashAttention、fused MLPs）。一旦你确定只部署单一变体，就应一次性合并权重，并以普通 checkpoint 的形式提供服务。
...
# 遍历所有 LoRA 层；对每一层：W_new = W_0 + (B @ A) * (alpha / r)
...
merged = model.merge_and_unload()                    # 移除 wrapper，得到纯模型
merged.save_pretrained(OUT, safe_serialization=True, max_shard_size="5GB")
```

权衡：合并后的 checkpoint 无法与其他基于同一基础模型的 Adapter 共享 GPU 显存。如果你需要为多个客户（多租户场景）同时提供多种 Adapter，应保持未合并状态，并使用支持运行时加载的推理引擎（如 vLLM 的 `lora_modules`）。但如果你只部署单一变体且 QPS 很高，合并总是更优。

### 蒸馏流水线（教师 → 学生）

如果合并后的 7B 模型仍然太慢或太贵，可以将其蒸馏到更小的学生模型中。标准做法是：**在温度缩放后的 logits 上计算 KL 散度，再加上一小部分硬标签的交叉熵损失**。软损失传递了教师模型关于类别间“暗知识”的信息，硬损失则确保学生模型锚定真实标签。

```python
import torch, torch.nn.functional as F

def distill_step(student, teacher, batch, optim, accum=4, tau=4.0, alpha=0.5):
    """单次梯度更新。在循环中调用，配合梯度累积。"""
    teacher.eval()
    with torch.no_grad():
...
一个在客服工单上微调过的 7B 教师模型，蒸馏到 700M 的学生模型后，通常能保留教师 92–96% 的 F1 分数，同时将推理成本降低约 10 倍。如果训练初期 KL 损失发散，可将温度 $\tau$ 从 4 降到 2，并在前 500 步仅用 CE 损失预热学生模型。

## 微调成本估算器

![工业落地的 4 个案例成本拆解 + transfer-vs-scratch 收支平衡曲线。](https://blog-pic-ck.oss-cn-beijing.aliyuncs.com/posts/zh/transfer-learning/12-工业应用与最佳实践/fig12_cost_breakeven.png)

大多数团队低估微调成本 2–3 倍，因为他们只计算了 GPU 小时，却忽略了工程、评估和部署开销。下面的公式给出了合理的算力估算；在报价前，请务必结合按功能拆分的成本模板一起使用。

算力下限由经典的 **每 token 每参数 6 FLOPs** 规则决定（前向 + 反向，忽略激活重计算）：

$$
\text{GPU hours} = \frac{\text{tokens} \times \text{params} \times 6}{\text{TFLOPs} \times 10^{12} \times \text{utilization} \times 3600}
$$

对于 A100/H100，建议使用 **utilization = 0.4**（峰值 TFLOPs 假设完美内核且无数据加载开销）。对于 LoRA，优化器状态部分用 `params_trainable` 替代 `params`，但前向/反向 FLOPs 仍需使用完整 `params`，因为基础权重依然参与计算。

**示例 1：LoRA 微调，7B 基础模型，10M tokens**  
$\frac{10^7 \times 7 \times 10^9 \times 6}{312 \times 10^{12} \times 0.4 \times 3600} \approx 0.94$ A100 小时。按 $1.80/小时 spot 价格，**算力成本约 $1.69**。实际情况：多数团队报告需 2–4 A100 小时；差额来自数据加载、训练中评估和重启开销。

**示例 2：全参数微调，7B，100M tokens**  
同公式得 ~94 A100 小时。使用 8×A100 ZeRO-3，墙钟时间约 12 小时，加上 4 次 checkpoint（~80 GB）。成本：**spot 约 $170，on-demand 约 $420**。

**示例 3：70B 教师蒸馏到 7B 学生，50M tokens**  
教师推理占主导：50M tokens × 70B 参数 × 2 FLOPs/token/param = $7 \times 10^{18}$ FLOPs，仅教师 logits 就需 ~16 A100 小时。学生训练再加 ~5 小时。总计：**~21 A100 小时 + 8×A100 用于学生训练步骤**。若缓存教师 logits 并跨 epoch 复用，总成本约 $80–120。

**按功能上线成本模板（报价时请使用此表）：**

| 项目                 | 小时数 | 单价 ($/hr) | 成本   |
|----------------------|--------|--------------|--------|
| 工程开发（4 周）     | 100    | 200          | 20,000 |
| 评估标注             | 30     | 50           | 1,500  |
| 算力（训练+评估）    | 12     | 1.80         | 22     |
| 服务（90 天）        | 2160   | 0.40         | 864    |
| **总计**             |        |              |**~$22,400**|

算力成本几乎总是低于总账单的 5%。优先优化工程迭代效率和服务成本。

## 生产运维

即使离线评估通过的模型，在生产中仍可能因流量路由错误、静默数据漂移或容量意外而失败。以下三个模块构成了最低限度的监控面：**灰度发布控制、输入漂移检测、实时推理健康度**。

### A/B 灰度发布配置

切勿一次性将 100% 流量切到新模型。下面的灰度模式通过对 `user_id` 做稳定哈希，确保同一用户始终看到同一变体，并基于护栏指标（而非固定时间表）逐步放量。
...
护栏指标不可妥协：每个放量步骤必须基于前一窗口的实际指标，而非日历时间。若 `p95_latency_ms` 超过 350 ms，编排系统应自动回滚，无需人工介入。

### PSI 漂移检测

Population Stability Index（PSI）用于量化特征分布相对于参考窗口的偏移程度。每天对所有生产输入特征运行 PSI；其退出码直接接入告警管道。
...
def psi(ref, new, bins=10, eps=1e-6):
    """适用于数值型（自动分箱）或类别型（标签数组）特征的 PSI 计算。"""
    if ref.dtype.kind in "fi":  # 数值型
...
def daily_check(ref_dict, new_dict):
    """返回 0=正常, 1=警告(>0.10), 2=告警(>0.25)。适合 cron 调用。"""
...
若 top-5 特征中任一 PSI > 0.25，曾成功预测我们约 80% 的线上 F1 回退。PSI 在 0.10–0.25 之间属提示级别；但如果一周内有三个特征同时越过警告阈值，应触发重训。

### 推理监控指标

延迟百分位反映系统状态，预测分布漂移反映模型状态。应在滚动窗口内同时计算二者，并作为结构化载荷上报。
...
# 上报至 CloudWatch / Datadog / OTLP
print(json.dumps(window_metrics(lat, pred, conf, REF_PROBS)))
```

比起平均置信度，更应关注 `low_conf_rate`：校准良好的模型在漂移下平均置信度变化不大，但低于 0.6 的尾部会迅速变厚。若 `low_conf_rate` 在 24 小时内翻倍，对我们而言，这比单独使用 PSI 更可靠地触发重训。

## 推理优化检查清单

在增加 GPU 之前，请先走完此清单。根据我们的经验，前四项通常能在典型 7B 微调模型上提升 3–5 倍吞吐量，且几乎不影响质量。

1. **bf16 / fp16 权重**：Ampere+ 架构免费支持。分类微调任务的质量损失：<0.1 F1。
2. **int8 权重量化（bitsandbytes / GPTQ）**：显存减半，吞吐提升 ~1.3 倍，校准后质量损失通常 <0.5 F1。
   ```python
   ...
   ```
3. **KV 缓存**：任何自回归解码都必须开启。若自定义 generate 循环，可能意外关闭。
4. **PagedAttention（vLLM / TGI）**：通过消除 KV 内存碎片，在高 batch 下提升 2–4 倍吞吐。
5. **连续批处理（Continuous batching）**：同运行时；在请求长度不一时更高效利用 GPU。
6. **ONNX 或 TensorRT 导出**：适用于 1B 以下编码器及 CPU 部署。7B 以上模型相比 vLLM 收益递减。
7. **推测解码（Speculative decoding）**：用 0.3B 草稿模型辅助 7B 主模型，匹配则接受。在指令任务上可获 1.5–2 倍延迟收益。

| 方法                          | p50 延迟 (ms) | 吞吐 (req/s) | 相对 F1 |
|-------------------------------|---------------|--------------|---------|
| fp32 基线 (HF)                | 410           | 12           | 1.000   |
| fp16                          | 230           | 21           | 0.999   |
| int8 (bnb)                    | 175           | 28           | 0.997   |
| vLLM fp16 + paged + batch     | 95            | 140          | 0.999   |
| vLLM int8 + paged + batch     | 80            | 190          | 0.996   |
| TensorRT fp16 (仅编码器)      | 22            | 410          | 0.999   |

以上数据基于 A100-40GB 上的 7B 分类器，batch ≤ 32，prompt 256 tokens。通用建议：**默认采用 vLLM + fp16 + PagedAttention + 连续批处理**；仅当 GPU 预算是主要约束时，才考虑 int8 或 TensorRT。只要在 512–1024 条代表性生产样本上做校准，量化在 Transformer 微调任务上的 F1 损失极少超过 0.5。
...
**Q5：如何避免再训练时的灾难性遗忘？** 把一部分旧分布的数据混入新训练集，或者使用 EWC 或 LoRA + replay （[第 10 章](/zh/transfer-learning/10-持续学习/))。每次训练后，一定要在之前的测试集上评估，作为保护机制。如果模型在新数据上提升 3 分，但在旧数据上下降 5 分，这就是退步，不是进步。
