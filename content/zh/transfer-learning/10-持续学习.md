...
自然的扩展方案是：保持基础网络冻结，并为每个任务附加 LoRA 适配器。基础网络永不更新，因此不存在遗忘问题；适配器为每个任务提供独立的小型参数预算，不会挤占共享资源。PackNet 是这一思想的刚性版本；而 per-task LoRA 则是弹性版本。第 9 节已介绍适配器机制；将其与 PackNet 式的分配策略结合非常直接，这也是当前大多数生产级持续学习（CL）系统的实际形态。
