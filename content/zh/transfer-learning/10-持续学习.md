...
---
今年我可以自学吉他，同时还能记得怎么骑自行车。神经网络就不行。先在 CIFAR-10 上微调一个视觉模型，接着用 SVHN 再微调，回头再测 CIFAR-10，准确率会跌到几乎和瞎猜差不多。这种现象叫**灾难性遗忘（catastrophic forgetting）**。如何让模型像人一样，在源源不断的任务流 $\mathcal{T}_1, \mathcal{T}_2, \ldots$ 中持续学习新知识，又不忘记旧技能，正是**持续学习（continual learning，CL）**要解决的核心问题——而且必须在“看不到过去数据”的前提下解决。

这篇文章先从 SGD 在过参数化网络中的几何结构出发，推导为什么遗忘是结构性的，不是工程上的 bug。然后逐一讲解四类解决方案：正则化、重放、动态架构、元学习。每种方法都结合数学推导、直观解释和工程权衡来分析。最后我会给出一份完整的 EWC 实现代码，直接能在 Permuted MNIST 上跑通。
...
van de Ven 和 Tolias（2019）将难度具体化为三种场景，理解这三种场景是掌握持续学习的关键：
...
- **Task-IL**。测试时知道任务 ID，模型可以为每个任务单独设置输出头，共享主干部分只负责特征提取。
- **Domain-IL**。标签空间固定，但输入分布发生变化（例如干净 → 旋转 → 加噪 MNIST）。模型只有一个输出头，测试时不知道样本来自哪个任务。
- **Class-IL**。每个新任务引入**新类别**，模型需要对迄今为止见过的**所有类别**进行分类，且无法得知样本属于哪个任务。这是最难的设定，也是实际部署中最常见的场景。
...
其中 $b_j$ 是任务 $j$ 的随机或未训练基线。**BWT < 0** 表示遗忘；**BWT > 0** 是少见且宝贵的"正向后向迁移"——学习新任务提升了旧任务的表现。**FWT > 0** 则说明早期任务塑造的表征对后续任务有零样本帮助。
...
如果 $\mathbf{g}_1 \cdot \mathbf{g}_2 < 0$，那么每一步都会让任务 1 的损失**增加**。在高维网络中，无关任务的梯度通常接近正交，但只要负余弦方向的比例足够大，几千步之后旧任务就会被彻底破坏。

### 损失曲面视角

任务 1 的最优解 $\theta_1^{*}$ 和任务 2 的最优解 $\theta_2^{*}$ 通常位于不同的低损区域。从 $\theta_1^{*}$ 开始对任务 2 执行 SGD，如果没有机制把参数拉回任务 1 的区域，参数就会直接离开。下图展示了这种现象：朴素基线让所有旧任务的准确率都下降了，而 EWC 和重放则通过不同方式缓解了这个问题。
...
在局部最优解处，Fisher 矩阵等于负对数似然的（半正定）Hessian 矩阵。因此，对角元 $F_i$ 表示扰动 $\theta_i$ 时损失上升的陡峭程度。$F_i$ 大，说明 $\theta_i$ 是任务的关键参数，需要保护；$F_i$ 小，说明损失在这个方向上是平坦的，可以放心调整去适应新任务。后面提到的所有正则化方法，本质上都在回答一个问题："如何挑选出需要保护的参数"。
## 正则化方法

### Elastic Weight Consolidation (EWC)

Kirkpatrick 等人在 2017 年提出了一种方法，用高斯分布近似任务 A 训练后的参数后验。高斯中心是 $\theta_A^{*}$，精度与 Fisher 对角成正比。将任务 A 的负对数似然函数在 $\theta_A^{*}$ 处展开到二阶，得到：
...
这个量完全无监督——可以在无标签数据甚至测试流上计算。在实际部署中，这是一个很大的工程优势。
...
LwF 不需要旧数据，也不需要 Fisher 矩阵，只需要旧模型快照。温度 $T$（通常为 2-4）会软化分布，让蒸馏信号承载比 argmax 更丰富的形状信息。
...
另一种思路：保留一小部分过去的样本。即使是很小的存储缓冲区，只要把旧样本混入每个 mini-batch，效果就已经是目前持续学习（CL）领域中最强的基线方法。
...
这是一个二次规划问题，每个旧任务对应一个约束——任务数量一多就难以扩展。**A-GEM**（Chaudhry 等，2019）做了简化，只保留一个参考梯度 $\mathbf{g}_{\text{ref}}$（从 $\mathcal{M}$ 中随机采一批计算平均梯度），仅当 $\mathbf{g}_{\text{new}}$ 和 $\mathbf{g}_{\text{ref}}$ 的夹角为钝角时进行投影：
...
否则直接取 $\tilde{\mathbf{g}} = \mathbf{g}_{\text{new}}$。这种方法的开销只是一次额外的前向/反向传播和一次点积运算——比 GEM 快一千倍，效果却几乎一样好。
...
权衡是不可避免的：零遗忘要么需要参数膨胀，要么会导致容量缩减。实际应用中，真正能落地的方案通常是混合方法——固定主干网络，搭配轻量级的每任务适配器（参考第 9 篇 PEFT）。
## 实现：从零开始写 EWC
