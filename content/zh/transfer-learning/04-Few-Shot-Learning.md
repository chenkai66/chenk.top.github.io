...
---
给小孩看一张穿山甲的照片，他这辈子都能认出穿山甲。给深度学习模型看一张照片，它的回答基本就是随机瞎猜。小样本学习的目标，就是填补这个差距：让分类器在每类只有 1 到 10 个标注样本的情况下也能正常工作。

关键不是死记硬背每个类别，而是学会如何从极少的样本中快速学习，然后把这种能力用到测试时完全没见过的新类别上。这篇文章讲的就是当前最主流的两类方法：**度量学习**，学一个好用的距离函数；**元学习**，学一个优秀的初始化。
## 你将学到
...
比如 "5-way 1-shot" 的任务是这样的：给你 5 个从未见过的类别，每类只提供 1 张带标签的图片；然后让你对一批新的查询图片进行分类。
...
最终报告的数字是几百到几千次评估的平均准确率，并附上 95% 的置信区间。因为单次评估的方差很大，没有置信区间的数字很难横向对比。
...
要从小样本中学习，必须依赖 **先验知识**。小样本学习的做法是：在大量 *基类（base classes）* 上训练（每个基类有很多样本），然后在不相交的 *新类（novel classes）* 上进行小样本评估。引入先验知识主要有两条路径：
