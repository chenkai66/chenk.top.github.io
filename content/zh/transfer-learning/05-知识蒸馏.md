...
---
我训练了一个 340M 参数的 BERT 模型，准确率达到 95%，但产品团队希望将其部署到手机上，而手机只能支持 10M 参数。如果从头训练一个 10M 的小模型，准确率只能达到 85%；这时，知识蒸馏几乎可以弥补大部分差距——通过让小模型学习大模型的输出分布，而不仅仅是硬标签，最终准确率可以提升到 92%。

Hinton 提出了一个关键点：教师模型那些“错误”的预测不是噪声，而是信息。例如，当教师模型对一张猫的图片预测出 0.62 的“猫”、0.14 的“老虎”、0.07 的“狗”和 0.008 的“飞机”，它实际上在告诉你：猫和老虎很像，和狗有点像，但和飞机完全不像。这种类别之间的相似性结构——即暗知识（dark knowledge）——在 one-hot 标签中是看不到的，学生模型一旦学到这些暗知识，就能以很小的参数量实现远超预期的表现。
## 你将学到什么
- 为什么软标签比硬标签包含更多信息
- 温度缩放：一个简单参数，决定教师模型透露多少暗知识
- 蒙特蒸馏的三种类型：响应、特征、关系
- 自蒸馏和互学习：无需预训练教师也能有效果
- 蒙特蒸馏结合量化和剪枝：压缩率轻松突破 10 倍
...
### 部署的现实难题
大模型在基准测试中表现优异，但在手机、汽车和云服务账单上却让人头疼。四个限制条件迫使我们转向小模型：内存不足，移动设备和物联网设备根本装不下几十亿参数；延迟高，自动驾驶需要毫秒级响应，而不是秒级；成本高，每天调用十亿次的模型，每个 FLOP 都是真金白银；能耗高，边缘设备靠电池供电，不是靠发电厂。

剪枝和量化直接修改模型本身，但会损失精度。蒸馏则训练一个小模型去模仿大模型的输出分布，而不仅仅是它的 argmax。学生继承了教师的归纳偏置，但不需要继承其参数。
