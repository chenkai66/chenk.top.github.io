...
但预训练为什么有效？什么时候该冻结层，什么时候该用 LoRA，学习率又该小到什么程度？我将从理论和工程实践两个角度，深入剖析这个目前最成功的迁移范式。
## 你将学到什么
...
但互联网上有 PB 级别的无标注文本、图像和视频。预训练就是要利用这种不对称：从丰富的无标注数据中学通用能力，再用稀缺的标注数据做专门优化。
...
如果 $\mathcal{D}_{\mathrm{task}}$ 很少，似然项噪声大，后验基本由先验主导。一个好的先验——能把概率集中在合理参数区域的先验——能显著提升后验质量。预训练 checkpoint 提供的就是这样的先验。
...
尽可能高。ImageNet 学到的边缘、纹理、物体部件，不仅对分类有用，还对检测、分割甚至医学影像有帮助。BERT 的句法语义表示在各种 NLP 任务中都很强。**预训练模型本质上是把数据压缩成保留了可迁移信息的特征。**
...
自监督的核心在于设计一种任务，这种任务的标签可以直接从数据中**自动**生成。模型通过预测输入的一部分内容来学习另一部分内容。
...
BERT 还训练了一个 NSP（Next Sentence Prediction）任务：给定句子 A 和 B，判断 B 是否真的接在 A 后面。后来 RoBERTa 证明，NSP 几乎没用——模型可以通过主题相似性蒙混过关，并不需要真正的篇章推理。ALBERT 提出了 **SOP（Sentence Order Prediction）** 来替代 NSP：负样本是将同样的两句话**调换顺序**。这种方法逼迫模型真正建模句间关系。
## 微调：为什么收敛得这么快
...
线性探测只在冻结特征上训一个线性分类器。它是对表示质量最纯粹的检验——在极小数据场景下，它经常胜过全参微调，因为根本没有足够的标签去安全地调整 1.1 亿参数。数据量增加后，全参微调反超并拉开差距。上线前花几次实验找到交叉点，往往很值得。
...
激进的微调是一条单行道：目标任务每涨 1 个点，源任务可能掉 1--2 个点。McCloskey 和 Cohen 在 1989 年把这个现象命名为**灾难性遗忘**，至今仍是序贯迁移的核心痛点。
