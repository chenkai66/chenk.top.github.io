...
主板并不是一条简单的总线，而是由**两个独立的总线区域通过一条高速链路连接在一起**的设计。CPU 直接掌控少量高性能资源，比如 PCIe 通道和 DDR 内存通道，这些属于直连域；而其他部分——包括额外的存储插槽、USB 控制器、SATA 接口以及芯片组自身的 PCIe 通道——都挂在另一个区域，由芯片组（Intel 称为 PCH，AMD 称为 FCH）管理。两个区域之间的通信依赖于一条链路：Intel 使用 **DMI 4.0 ×8**，单向带宽约 16 GB/s，而 AMD 则采用规格相近的 PCIe ×4 链路。这条链路是整块主板的**关键瓶颈**，因为所有挂载在芯片组上的设备都在争夺它的带宽。
...
- **DIMM 内存插槽**。消费级主板通常支持双通道（A1/A2 + B1/B2）。如果只插 A2 和 B2（通常是离 CPU 最远的两个插槽），可以启用双通道模式；但如果只插 A1 和 A2，则会悄悄地将带宽减半。DDR5 内存模块自带小型稳压电路，因此 DDR5 主板的内存供电相数比 DDR4 时代少了不少。
...
从这里可以得出两个重要的实用结论。首先，**第二根物理 ×16 插槽通常并非真正的 ×16**：在 B760/B660 主板上，它是通过芯片组以 ×4 模式运行，并且与 USB 和 SATA 共享带宽。虽然可以插入第二张显卡并正常使用，但性能会被限制。其次，**使用 M.2_2 插槽往往会禁用两个 SATA 接口**，因为芯片组在这几条通道上进行了多路复用。具体禁用哪些接口，主板说明书会有详细说明。
## 如何确认你实际使用的 PCIe 插槽是否为 ×16

你买了一块标称支持“PCIe 5.0 ×16”的主板，但怎么才能确定它真的在以 ×16 的规格运行呢？在 Windows 系统上，最简单的方法是使用 **GPU-Z**。查看 *Bus Interface*（总线接口）字段，它会显示当前协商的带宽和代际信息，例如 `PCIe x16 5.0 @ x16 5.0`，这就表示插槽确实运行在 PCIe 5.0 ×16 模式下。
