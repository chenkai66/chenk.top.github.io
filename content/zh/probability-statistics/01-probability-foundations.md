...
| 条件概率 | $P(A\midB) = P(A \cap B)/P(B)$ | 基于新信息更新概率 |
| 乘法法则 | $P(A \cap B) = P(A\midB)P(B)$ | 联合事件概率 |
| 全概率公式 | $P(A) = \sum P(A\midB_i)P(B_i)$ | 按情形分解计算 |
| 贝叶斯定理 | $P(B\midA) = \frac{P(A\midB)P(B)}{P(A)}$ | 反向条件化 |
