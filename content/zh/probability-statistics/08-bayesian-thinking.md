---
title: "概率与统计（8）：贝叶斯统计——先验、后验，以及频率学派为何争论不休"
...
两位统计学家走进一家酒吧。一人说：“明天下雨的概率是 30%。”另一人反驳道：“概率是长期频率；而明天只发生一次，这个说法毫无意义。”第一个人回应：“它量化了我对这一唯一事件的不确定性。”两人于是整晚争执不休。
...
贝叶斯框架对单个问题而言 arguably 更直观：你从已有信念出发，观察数据，并据此更新。频率学派框架则 arguably 更适合方法验证：你无需指定先验，即可保证在重复使用中控制错误率。

实践中，二者常得出一致结论——尤其当样本量较大时，数据主导先验的影响。
...
后验均值（0.643）相比 MLE（0.7）被拉向先验均值（0.5）。先验起正则化作用，将估计收缩至中心。随着数据增多，这种收缩减弱。
...
- 样本量小，先验影响显著。
- 参数空间维度相对于数据较高。
- 问题涉及 **当前数据**（贝叶斯：“我现在应相信什么？”）而非 **长期性质**（频率学派：“该程序长期表现如何？”）。
