---
title: "概率与统计（6）：参数估计——极大似然估计、最大后验估计与偏差-方差分解"
...
我们此前构建的所有内容——分布、期望、极限定理——都假设参数已知。高斯分布有均值 $\mu$ 和方差 $\sigma^2$；二项分布有 $n$ 次试验和成功概率 $p$。但在实际中，你并不知道 $\mu$ 或 $p$。你只能观测数据，并试图从中推断它们。

这就是**估计理论**：它架起了概率论（参数给定）与统计学（参数待推断）之间的桥梁。它也是机器学习的理论根基所在。每次你训练一个模型，本质上就是在从数据中估计参数。该估计的质量，直接决定了你的模型能否泛化，还是陷入过拟合。
...
**记号惯例：** 带帽子的 $\hat{\theta}$ 表示估计量/估计值；不带帽子的 $\theta$ 表示真实参数。
...
若 $\text{Bias}(\hat{\theta}) = 0$（即对所有 $\theta$ 都有 $E[\hat{\theta}] = \theta$），则称该估计量是**无偏的**。

**例：** 样本均值 $\bar{X} = \frac{1}{n}\sum X_i$ 对 $\mu$ 是无偏的：

$$E[\bar{X}] = \frac{1}{n}\sum E[X_i] = \mu. \quad \checkmark$$

而样本方差 $S^2 = \frac{1}{n}\sum(X_i - \bar{X})^2$ 是**有偏的**：
...
由弱大数定律（WLLN），样本均值 $\bar{X}$ 对 $\mu$ 是相合的。同样，有偏（分母为 $1/n$）和无偏（分母为 $1/(n-1)$）的样本方差对 $\sigma^2$ 也都是相合的。
...
矩估计量易于计算，但通常非最有效。它常被用作初始化起点或合理性检验。
