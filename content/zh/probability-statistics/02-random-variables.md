...
在上一篇文章中，我们构建了概率论的公理化基础，你或许会觉得我们花了大量时间讨论集合与子集——事实的确如此。事件与 σ-代数的这套机制虽属必要，却略显枯燥；它无法自然地支持均值计算、离散度度量，或对数据拟合模型。

连接抽象概率与应用统计的桥梁，正是**随机变量（Random Variable）**。一旦我们将数值赋予样本空间中的结果，整个微积分工具箱——导数、积分、级数——便随之启用。而有了微积分，我们就能用一组命名分布（named distributions）来刻画随机性，每一类分布都编码了世界生成数据时所隐含的特定假设。

本文系统梳理你在实践中最常遇到的分布，并精确揭示每一种分布的来源。
...
可测性条件（定义的第二部分）确保诸如“$X$ 不超过 3 的概率是多少？”这类问题具有明确定义的答案。对于有限或可数的样本空间，该条件自动满足。

**示例。** 同时掷两枚骰子。令 $X$ 表示其点数之和。样本空间为 $\Omega = \{(i,j) : 1 \leq i,j \leq 6\}$，共 36 个等可能结果。随机变量 $X(i,j) = i + j$ 将每个有序对映射为 2 到 12 之间的整数。

关键转变在于：我们不再追踪完整结果 $\omega$，而是处理数值 $X(\omega)$。这会损失信息（例如 $X = 7$ 并不能告诉我们是 $(1,6)$ 还是 $(3,4)$），但我们在细节上的损失，换来了强大的数学表达力。
...
Geometric 分布具有**无记忆性（memoryless property）**：$P(X > s + t \mid X > s) = P(X > t)$。即：已等待 $s$ 次仍未成功，剩余等待时间的分布与重新开始时完全相同。
...
它是 Geometric 分布的连续类比。它也是**唯一**具备无记忆性的连续分布：
