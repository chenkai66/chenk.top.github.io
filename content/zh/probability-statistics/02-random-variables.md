...
在上一篇文章中，我们构建了概率论的公理化基础，你或许会觉得我们花了大量时间讨论集合与子集——事实的确如此。虽然事件与 σ-代数的机制是必要的，但略显枯燥，无法自然地支持均值计算、离散度度量或数据拟合模型。

连接抽象概率与应用统计的桥梁是**随机变量（Random Variable）**。一旦将数值赋予样本空间中的结果，整个微积分工具箱——导数、积分、级数——便随之启用，使我们能够用一组命名分布来刻画随机性，每种分布都编码了世界生成数据时所隐含的特定假设。

本文系统梳理你在实践中最常遇到的分布，并精确揭示它们的来源。
