...
将 LoRA 与 MoE 结合是另一条路径：以多个 LoRA 作为专家，引入 router 进行动态选择。但这会牺牲 LoRA 的两大核心优势：权重可合并性与推理零开销，同时引入路由训练不稳定、专家负载不均衡等工程挑战。

[**MoSLoRA**](https://arxiv.org/abs/2406.11909)（Wu, Huang, Wei, 2024）选择了一条更克制的路线：去掉 router，用一个**可学习的混合矩阵** $W$ 把 $k$ 个低秩子空间组合起来，整体可以重写成一个干净的 $B\,W\,A$ 乘积。该设计在保持 LoRA 可合并性与推理零开销的同时，突破了单一低秩子空间的容量限制。下文将按‘动机→结构→与 MoE 的本质差异→参数与计算开销→实证现象→调参建议’的逻辑展开，重点阐明 mixer 的作用机制，及其与 LoRA、LoRA-MoE 的关键设计边界。
