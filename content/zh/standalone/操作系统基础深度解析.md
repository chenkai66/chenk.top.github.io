...
整个过程大约 100 微秒，你看到的只是最后那一帧。本文目标是将这条执行路径的每一层逐一展开，帮助你在遇到性能瓶颈、权限异常或‘程序卡在 D 状态’等问题时，快速定位故障发生的层级。
...
操作系统（Operating System，OS）这个名字其实有点误导人——它听起来像一个「系统」，实际上是一组解决多人共用一台机器问题的代码集合。如果一台机器始终只运行一个程序、仅服务一个用户，且硬件永不发生故障，那么操作系统大部分功能确实可以省略。OS 复杂的原因在于现实情况：
...
基于内核态与用户态的隔离边界，内核应包含哪些功能模块——这是单内核（monolithic）与微内核（microkernel）的根本分歧所在。
