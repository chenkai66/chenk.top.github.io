...
整个过程大概 100 微秒，你看到的只是最后那一帧。本文的目标，是将这条执行路径的**每一层**逐一展开，帮助你在遇到性能瓶颈、权限异常，或‘程序卡在 D 状态’等问题时，快速定位故障发生的层级。
...
操作系统（Operating System，OS）这个名字其实有点误导人——它听上去像一个「系统」，实际上是一组解决**多人共用一台机器**这件事的代码集合。如果一台机器始终只运行一个程序、仅服务一个用户，且硬件永不发生故障，那么操作系统绝大部分功能确实可以省略。OS 之所以复杂，是因为现实里：
...
![Monolithic vs Microkernel — what runs in kernel mode](https://blog-pic-ck.oss-cn-beijing.aliyuncs.com/posts/zh/standalone/操作系统基础深度解析/fig1_kernel_architectures.png)
...
基于内核态与用户态的这道隔离边界，内核应当包含哪些功能模块——这正是单内核（monolithic）与微内核（microkernel）的根本分歧所在。
...
记住这张图后，每当遇到内核 oops 或 Windows 的蓝屏死机（BSOD），你便能意识到：这是单内核架构所承担的稳定性代价。每次看到一个驱动崩溃却没拖垮系统（比如手机 wifi 模块自己重启），那很可能是微内核思想在某个层面被借用了。
...
![Five-state process lifecycle](https://blog-pic-ck.oss-cn-beijing.aliyuncs.com/posts/zh/standalone/操作系统基础深度解析/fig2_process_states.png)
...
![Virtual memory + paging](https://blog-pic-ck.oss-cn-beijing.aliyuncs.com/posts/zh/standalone/操作系统基础深度解析/fig3_virtual_memory.png)
...
![Inode-based file system](https://blog-pic-ck.oss-cn-beijing.aliyuncs.com/posts/zh/standalone/操作系统基础深度解析/fig4_file_system.png)
...
![The I/O stack](https://blog-pic-ck.oss-cn-beijing.aliyuncs.com/posts/zh/standalone/操作系统基础深度解析/fig5_io_subsystem.png)
...
![The system call boundary](https://blog-pic-ck.oss-cn-beijing.aliyuncs.com/posts/zh/standalone/操作系统基础深度解析/fig6_syscall_interface.png)
...
![Four schedulers, same workload](https://blog-pic-ck.oss-cn-beijing.aliyuncs.com/posts/zh/standalone/操作系统基础深度解析/fig7_schedulers.png)
