...
整个过程大约 100 微秒，你看到的只是最后那一帧。本文的目标是将这条执行路径的每一层逐一展开，帮助你在遇到性能瓶颈、权限异常或“程序卡在 D 状态”等问题时，快速定位故障发生的层级。
...
操作系统（Operating System，OS）这个名字其实有点误导人——它听起来像一个「系统」，实际上是一组解决多人共用一台机器问题的代码集合。如果一台机器始终只运行一个程序、仅服务一个用户，且硬件永不发生故障，那么操作系统的大部分功能确实可以省略。OS 复杂的原因在于现实情况：

- **CPU 只有几十个核**，但要跑成百上千个进程，需要**调度器**。
- **物理内存只有几十 GB**，但每个进程都希望独占整台机器的地址空间，需要**虚拟内存**。
...
![单体 vs 微内核 —— 什么在内核模式下运行](https://blog-pic-ck.oss-cn-beijing.aliyuncs.com/posts/zh/standalone/操作系统基础深度解析/fig1_kernel_architectures.png)
...
基于内核态与用户态的隔离边界，内核应包含哪些功能模块——这是单内核（monolithic）与微内核（microkernel）的根本分歧。
...
- **缺点：脆弱。** 任何一个驱动崩溃 = 整个内核 panic。 Linux 内核中约 70% 的代码量来自设备驱动，因此每新增一种硬件，就相应引入新的稳定性风险。
...
- **优点：隔离。** 网卡驱动崩溃 = 一个用户进程崩溃，重启它即可，内核不动。 seL4 甚至完成了形式化验证：在其安全规约约束下，内核可被数学证明不会发生崩溃。
...
记住这张图后，每当遇到内核 oops 或 Windows 的蓝屏死机（BSOD），你便能意识到这是单内核架构所承担的稳定性代价。每次看到一个驱动崩溃却没拖垮系统（比如手机 wifi 模块自己重启），那很可能是微内核思想在某个层面被借用了。
...
![五状态进程生命周期](https://blog-pic-ck.oss-cn-beijing.aliyuncs.com/posts/zh/standalone/操作系统基础深度解析/fig2_process_states.png)
...
![虚拟内存 + 分页](https://blog-pic-ck.oss-cn-beijing.aliyuncs.com/posts/zh/standalone/操作系统基础深度解析/fig3_virtual_memory.png)
...
## 文件系统：名字、元数据、数据是三件不同的事

![基于 inode 的文件系统](https://blog-pic-ck.oss-cn-beijing.aliyuncs.com/posts/zh/standalone/操作系统基础深度解析/fig4_file_system.png)
...
## I/O 子系统：一次 read() 下钻七层

![I/O 栈](https://blog-pic-ck.oss-cn-beijing.aliyuncs.com/posts/zh/standalone/操作系统基础深度解析/fig5_io_subsystem.png)
...
![系统调用边界](https://blog-pic-ck.oss-cn-beijing.aliyuncs.com/posts/zh/standalone/操作系统基础深度解析/fig6_syscall_interface.png)
...
![四个调度器，相同的工作负载](https://blog-pic-ck.oss-cn-beijing.aliyuncs.com/posts/zh/standalone/操作系统基础深度解析/fig7_schedulers.png)
