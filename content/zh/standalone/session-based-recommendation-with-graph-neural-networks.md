...
---
用户依次点击 **A、 B、 C、 B、 D**。将其输入序列模型，得到的是五个 token 压缩而成的单一隐状态；而输入 SR-GNN，则生成一张有向图——边 `B -> C` 即便用户回到 `B` 也仍然存在，节点 `B` 只出现一次（它的入边和出邻居都会贡献到它的表示上），整段点击的拓扑都被原样保留在邻接矩阵里。这就是 [SR-GNN（Wu 等, AAAI 2019）](https://arxiv.org/abs/1811.00855) 在多个会话推荐基准上稳稳压过 GRU4Rec、 NARM 等纯序列模型的根本原因。
...
- 会话池化为何需要局部+全局，以及注意力为何要锚定在最后一个点击上
- 训练目标、合理的超参数，以及在小图上 BPTT 的实际意义
- 在带回访、长度中等的会话上，图视角为何优于序列模型
...
- **上下文极短**：典型会话只有 2 到 10 个点击，模型必须从极少的信号中反推用户当下的“意图”。
- **物品会重复出现，意图也不单调**：用户会反复比较，绕回来再点同一个商品。同一物品在一段会话里出现多次是常态，“靠后的点击”未必比“靠前的点击”代表更稳定的偏好。
...
![会话图构建：点击流 A,B,C,B,D 对应的有向加权图；即使用户回到 B，所有跳转关系也都被保留](https://blog-pic-ck.oss-cn-beijing.aliyuncs.com/posts/en/standalone/session-based-recommendation-with-graph-neural-networks/fig1_session_graph.png)
...
![SR-GNN 端到端流程：会话图 -> GGNN 传播 -> 节点表示 -> 注意力池化 -> 物品池 softmax](https://blog-pic-ck.oss-cn-beijing.aliyuncs.com/posts/en/standalone/session-based-recommendation-with-graph-neural-networks/fig2_architecture.png)

跑完 $T$ 步之后，每个节点都拿到一个“看过自己邻居、看过会话里所有环路结构、也看过每条转移触发频次”的向量 $h_v$。

![门控 GNN 单步更新：入/出邻接行先聚合出消息 0，再驱动一个 GRU 风格的状态更新](https://blog-pic-ck.oss-cn-beijing.aliyuncs.com/posts/en/standalone/session-based-recommendation-with-graph-neural-networks/fig3_ggnn_update.png)
...
![会话池化：注意力权重锚定在最后一次点击上；局部意图 + 全局上下文线性融合成会话向量](https://blog-pic-ck.oss-cn-beijing.aliyuncs.com/posts/en/standalone/session-based-recommendation-with-graph-neural-networks/fig4_attention_pooling.png)
...
![基准对比：SR-GNN vs 序列推荐基线在 Yoochoose 1/64、Yoochoose 1/4、Diginetica 上的 Recall@20 与 MRR@20](https://blog-pic-ck.oss-cn-beijing.aliyuncs.com/posts/en/standalone/session-based-recommendation-with-graph-neural-networks/fig5_benchmark_perf.png)
