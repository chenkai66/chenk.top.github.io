...
普通自编码器仅能压缩与重建，而变分自编码器（VAE）则具备生成能力——它学习到的是一个**平滑且结构化的潜在空间**，你可从中**采样**，生成全新的数据。把编码器从"输出一个向量"改成"输出一个分布"，仅这一步，模型就从一个花哨的压缩器升级成了带可优化似然下界的生成模型。
...
用作压缩或去噪没问题，但它所学习的潜在空间是**确定性且缺乏结构**的，具体表现为：
