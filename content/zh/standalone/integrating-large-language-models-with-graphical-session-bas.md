...
![LLMGR 端到端架构：LLM 语义流 + GNN 结构流 + 混合编码层 + MLP 排序头](https://blog-pic-ck.oss-cn-beijing.aliyuncs.com/posts/en/standalone/integrating-large-language-models-with-graphical-session-bas/fig1_framework.png)
...
![两类 prompt：辅助任务把文本对齐到 ID，主任务学习下一跳排序](https://blog-pic-ck.oss-cn-beijing.aliyuncs.com/posts/en/standalone/integrating-large-language-models-with-graphical-session-bas/fig2_multitask_prompts.png)
...
![混合编码层：把 ID embedding 投影到 LLM 隐层维度，再与文本 token 拼接](https://blog-pic-ck.oss-cn-beijing.aliyuncs.com/posts/en/standalone/integrating-large-language-models-with-graphical-session-bas/fig3_hybrid_encoding.png)
...
![两阶段调优：第一阶段冻结 GNN，做语义落地；第二阶段解冻 GNN，学习转移结构](https://blog-pic-ck.oss-cn-beijing.aliyuncs.com/posts/en/standalone/integrating-large-language-models-with-graphical-session-bas/fig4_two_stage_tuning.png)
...
![LLMGR 的增益主要落在冷启动桶里：暖启动 ~5-6%，冷启动 ~18-21%](https://blog-pic-ck.oss-cn-beijing.aliyuncs.com/posts/en/standalone/integrating-large-language-models-with-graphical-session-bas/fig5_coldstart_perf.png)
...
## 十、成本算账：为什么离线 embedding 必赢在线推理

“别让 LLM 上线路”这个口号需要一组真实的数字撑着。这是决定 LLMGR 是论文 demo 还是真能上生产的那条线。

假设有一个 1M 物品的 catalogue、一个能在单卡 A100 上以 50 ms/item 出 1024 维 embedding 的 LLM、以及一个在同一张卡上跑 5 ms/256 会话 batch 的 GNN。

**每请求都跑 LLM。** 每个会话 5-20 个 item，按 20 算，单请求光 LLM 部分就要 20 × 50 ms = 1 秒，这还没轮到 GNN。单卡 A100 的吞吐塌到约 50 RPS，$p_{99}$ 落在 1.2-1.5 秒区间。再加一张 A100，成本翻倍、吞吐基本也只翻倍。很多“LLM 增强推荐”的 demo 不小心就跑成了这个形态。

**离线计算 item embedding + 在线只跑 GNN。** 100 万个 item 的 embedding 一次性算完，单卡 A100 大约 1{,}000{,}000 × 50 ms ≈ 14 小时墙上时间，按 GPU 租金算大概 30 美元一次。结果是一个 4 GB 的张量，可以放在显存里或者快存上。请求来时 GNN 查 20 个向量（基本零成本）、跑消息传递、出结果，整个 5-10 ms。同一张 A100 现在能扛 5{,}000-10{,}000 RPS，两个数量级的差距。

**刷新成本。** 每天新进的 item 只需要重新 embed 它自己的文本——一个真实 catalogue 一天通常进 1{,}000-10{,}000 件，对应 1-10 分钟 GPU 时间。一个夜间 cron 能不打扰线上服务地把它消化掉。文案修改只影响命中的那几行，大批量目录变更走周末任务。

**反过来什么时候会输？** 离线方案只在“会话里超过一半的 item 是从未被 embed 过的新货”时才输给在线方案。稳态运行时几乎不会发生这种情况。唯一现实的场景是大促 / 闪购上线一次扔出几千个新 SKU，而且就算这种场景， 5 分钟的预计算延迟通常也是可以接受的。

这两个数量级的差距，就是 LLMGR 这个架构能活下来的工程原因。“每请求都得跑一个 7B 的 LLM”的设计永远走不出 GPU 农场；“用 LLM 当离线特征工厂、用小 GNN 当在线排序器”的设计能在单 region 单 GPU 上跑——而真实生产中的推荐系统也只长这种模样。

## 十一、 LLMGR 不适合的几类场景

这套架构不是万能。三类场景里它会输给更简单的基线：

- **纯 ID 推荐 + 丰富行为日志。** 当你有几年点击日志、每个 item 有上百次曝光、 item 标题就是 SKU 码这种没有语义的字符串时，一个朴素的 SR-GNN 跑 ID 已经很能打。 LLM 只能 embed `"商品 8472"`，对齐训练变成一场烧参数的仪式。
- **超长会话（>50 item）。** GNN 在长会话上还没等 LLM 特征进来就先过平滑了。论文的实验集中在平均 10 个 item 以下的会话；到 100 时头-尾差距塌掉，这种场景里基于 Transformer 的排序器（BERT4Rec）通常更稳。
