...
第一次手动计算 Self-Attention 时，多数人会惊讶地发现：它**完全不依赖输入顺序**。若将 token 序列重新排列，各注意力分数也会随之同步重排——该函数严格满足置换等变性。因此，在让 Transformer 完成任何需要理解序列顺序的任务之前，必须显式注入位置信息。

这一设计选择——即‘如何注入位置信息’——催生了一个活跃的研究方向，涵盖 Sinusoidal、Learned、Relative、T5 bucket、RoPE、ALiBi、NoPE 等多种方案……本文面向工程师，以简明为原则：每种方案提供足以理解其核心思想的数学表达，并辅以充分的横向对比，支撑工程选型决策。，重点放在 LLM 时代最关键的一项指标——**长度外推**（length extrapolation），即在比训练上下文更长的输入上还能不能用。
...
这正是‘模型无法感知序列顺序’在数学上的精确刻画：注意力函数关于输入置换具有等变性。它将‘猫坐在垫子上’与‘垫子坐在猫上’视为同一组 token 向量构成的多重集的不同排列——输入向量集合相同，输出向量集合也相同，仅顺序不同。要使模型能区分这两句话，必须显式注入位置信息；否则，该模型在数学上无法完成此项任务。
...
![可学习位置编码：随机初始化经过训练后呈现平滑结构，但一旦超出训练长度，性能直接断崖式下降](https://blog-pic-ck.oss-cn-beijing.aliyuncs.com/posts/zh/standalone/浅谈位置编码/fig2_learned.png)
...
![Sinusoidal 位置编码：左图是位置 × 维度的热力图，右图是单个位置的多尺度指纹](https://blog-pic-ck.oss-cn-beijing.aliyuncs.com/posts/zh/standalone/浅谈位置编码/fig1_sinusoidal.png)
...
![RoPE 把每对 (q, k) 按其绝对位置旋转一个角度，最终的内积只依赖于相对偏移](https://blog-pic-ck.oss-cn-beijing.aliyuncs.com/posts/zh/standalone/浅谈位置编码/fig3_rope.png)
...
![ALiBi：每个 head 在注意力分数上加一项线性距离惩罚，不同 head 衰减速率不同](https://blog-pic-ck.oss-cn-beijing.aliyuncs.com/posts/zh/standalone/浅谈位置编码/fig4_alibi.png)
...
![长度外推：sinusoidal 直接崩，RoPE 平滑下降，ALiBi 几乎无代价](https://blog-pic-ck.oss-cn-beijing.aliyuncs.com/posts/zh/standalone/浅谈位置编码/fig5_compare.png)
