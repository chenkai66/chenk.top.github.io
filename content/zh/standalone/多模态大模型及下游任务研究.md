...
将图像、视频和音频输入语言模型，使其具备视觉、听觉和推理能力——这件事在 2021 年 CLIP 出现之前还只是研究奇观，如今已成为产品标配。但要将多模态大模型（Multimodal LLM, MLLM）应用于生产，难点并不在于视觉编码器是否足够强大，而在于以下四个常被低估的方面：
...
3. **成本**：一张 1024x1024 的图像会被切分成数百个 visual token，prefill 阶段非常昂贵；视频长度增加后成本更难控制。视觉 token 压缩、KV cache 和 batch 调度都会增加成本。
...
![MLLM architecture](https://blog-pic-ck.oss-cn-beijing.aliyuncs.com/posts/zh/standalone/多模态大模型及下游任务研究/fig1_mllm_architecture.png)
...
![Model family comparison](https://blog-pic-ck.oss-cn-beijing.aliyuncs.com/posts/zh/standalone/多模态大模型及下游任务研究/fig2_family_comparison.png)
...
![视觉-语言模型管线 (BLIP-2)](https://blog-pic-ck.oss-cn-beijing.aliyuncs.com/posts/zh/standalone/multimodal-llm-downstream-tasks/fig_multimodal_pipeline_zh.png)
...
![VL downstream tasks](https://blog-pic-ck.oss-cn-beijing.aliyuncs.com/posts/zh/standalone/多模态大模型及下游任务研究/fig3_vl_tasks.png)
...
![Cross-modal alignment](https://blog-pic-ck.oss-cn-beijing.aliyuncs.com/posts/zh/standalone/多模态大模型及下游任务研究/fig4_cross_modal_alignment.png)
...
![Fine-tuning strategies](https://blog-pic-ck.oss-cn-beijing.aliyuncs.com/posts/zh/standalone/多模态大模型及下游任务研究/fig5_finetuning_strategies.png)
...
![Benchmarks](https://blog-pic-ck.oss-cn-beijing.aliyuncs.com/posts/zh/standalone/多模态大模型及下游任务研究/fig6_benchmarks.png)
...
![Production deployment](https://blog-pic-ck.oss-cn-beijing.aliyuncs.com/posts/zh/standalone/多模态大模型及下游任务研究/fig7_production_deployment.png)
...
构建一个**幻觉检测器**：用一个小模型或规则系统，在生成完成后做"图里到底有没有这个东西"的二次校验。简单实现：把模型答案里提到的实体抽出来，反过来用 CLIP 跑一次"图里有 X 吗"的零样本判断。准确率不高但能 catch 一大半明显幻觉。
...
![多模态大模型迭代优化环](https://blog-pic-ck.oss-cn-beijing.aliyuncs.com/posts/zh/standalone/multimodal-llm-downstream-tasks/fig_mllm_iteration_zh.png)
