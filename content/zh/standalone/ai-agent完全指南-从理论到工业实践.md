...
这句话里有四个关键词：**目标、决定、执行、观察**。普通的一次 LLM 调用一个都没有，它只是生成文本后就结束；而 Agent 每一步都会将最新观察结果作为新上下文输入模型，所以底层同一个 LLM 才能突然学会订机票、修 flaky test、操控浏览器。
...
值得在上面这张图上停一下。规则系统是一棵硬编码的决策树：每加一个新意图，就多一根分支。LLM Agent 是一个通才内核加一组工具——**新行为通过增加工具定义来引入，无需修改控制逻辑**。这个差别在维护成本上是数量级的。
