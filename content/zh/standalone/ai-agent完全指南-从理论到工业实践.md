...
聊天机器人用于回答问题，而 Agent 则用于**把事情做完**。同样是基于大模型，前者仅输出文字，后者则会搜索、写代码、调用 API、查询数据库，并不断迭代直至任务完成。区别不在于模型本身，而在于外层的封装——一个能保留状态的循环、一组工具和一个能审视自身输出的批评者。

这篇文章详细阐述了这一思路。我们将从底层开始讲解 Agent 的四大核心能力（规划、记忆、工具、反思），阐明主流框架的区别，讨论多 Agent 协作、评估方法，以及那些"演示时光鲜、上线后炸裂"的生产细节。

## 你会学到什么

- 为什么 Agent 不止是"GPT-4 套个 `while` 循环"
- 任何严肃的 Agent 都具备的四大能力
- ReAct、 Tree of Thoughts、 Reflexion 各自的价值
- 主流框架的选择： LangChain、 LangGraph、 AutoGen、 CrewAI、 AutoGPT
- 真正可扩展的多 Agent 拓扑（以及看起来炫酷但实际不可行的）
...
- 对 prompt、 completion 和上下文窗口有清晰的理解

---

## Agent 到底是什么

一个 AI Agent 系统在给定目标后，由 LLM 自主决定下一步行动，通过工具执行，观察结果，然后重复，直到目标达成或触发停止条件。

这句话中有四个关键词：**目标、决定、执行、观察**。普通的 LLM 调用只生成文本后就结束，而 Agent 在每一步中都将最新观察结果作为新上下文输入模型，因此同一个 LLM 可以学会订机票、修复 flaky test、操控浏览器等。

### 从一次性生成到认知循环

从"请求-响应"切换到"循环"，这是 Agent 的关键所在：
...
让这个循环在生产环境中顺利运行，需要三件事：跨步骤存活的记忆、能返回结构化结果的工具层，以及防止"热情过度的 LLM 一口气下了 200 份外卖"的停止规则。本文剩余部分将围绕这三个主题展开。

![Agent 认知循环：感知、推理、行动、记忆](https://blog-pic-ck.oss-cn-beijing.aliyuncs.com/posts/zh/standalone/ai-agent完全指南-从理论到工业实践/fig1_agent_loop.png)

### 为什么这件事重要：一个具体对比

设想任务："找出 2026 年三大机器学习顶会的截止日期，并在每个截止日期前两周提醒我。"

普通 LLM 单次调用会返回训练时残留的（可能已过期的）记忆，并且无法创建日历事件。而 Agent 会搜索当前的截止日期，解析结果，计算提醒日期，调用日历 API，并返回确认 ID。尽管底层 LLM 相同，但**外层布线**才是关键差异。

![规则系统 vs LLM Agent](https://blog-pic-ck.oss-cn-beijing.aliyuncs.com/posts/zh/standalone/ai-agent完全指南-从理论到工业实践/fig2_agent_vs_rule.png)

值得在上面这张图上停一下。规则系统是一棵硬编码的决策树，每添加一个新意图就会多出一个分支。而 LLM Agent 是一个通才内核加上一组工具——**新行为通过增加工具定义来引入，无需修改控制逻辑**。这种差异在维护成本上是数量级的。

### Agent 的五个组件

任何可用的 Agent 都包含这五个组件。尽管各框架命名不同，但职责完全一致。

**1. 大脑（LLM 内核）**：解释目标、生成计划并发出工具调用。模型选择基本决定了上限和成本。

**2. 规划器**：将目标拆分成子目标。可以是隐式的（LLM 在 ReAct 中边想边规划），也可以是显式的（一个独立的规划器输出任务 DAG）。

**3. 记忆**：工作记忆存放当前轨迹；长期记忆跨会话保存事实、教训和向量。

**4. 工具**： Agent 可以调用的所有工具——搜索、代码执行、 SQL、 HTTP API、文件 I/O，甚至其他 Agent。**给 LLM 看的 schema 比工具实现本身更能影响效果。**

**5. 反思**：自我批评，能够发现错误并触发带有纠正信息的重试。

将第 3、 4 块写成最小可用版本，几行代码即可：
...
生产系统在此基础上增加了结构化日志、重试和可观测性，但核心仍然是这些内容。
...
![ReAct trace：交错的推理与行动](https://blog-pic-ck.oss-cn-beijing.aliyuncs.com/posts/zh/standalone/ai-agent完全指南-从理论到工业实践/fig5_react_flow.png)

两个工程上的提醒。第一，**解析器（parser）是导致生产事故的最常见原因**，能用 JSON mode 或 function calling 就别自己解析自由文本。第二，工具返回的大块内容**先压缩再传入模型**——一个原始 HTML 页面三步就能把上下文窗口撑爆。
...
工作记忆其实就是你每一轮发给 LLM 的 messages 列表。它是大多数 Agent 里**工程化程度最低，且在高并发或长任务场景下最先暴露问题**的部分。三件事很有用：
...
定期把分数最低的 10% 淘汰掉。**保留淘汰操作的日志**——以后某天 Agent 行为开始变怪，你会想回头看看是不是那时候删错了什么。
...
![Function Calling 流程：schema、模型、执行、结果](https://blog-pic-ck.oss-cn-beijing.aliyuncs.com/posts/zh/standalone/ai-agent完全指南-从理论到工业实践/fig3_tool_calling_pipeline.png)
...
![Agent 框架与平台的定位图](https://blog-pic-ck.oss-cn-beijing.aliyuncs.com/posts/zh/standalone/ai-agent完全指南-从理论到工业实践/fig6_platform_landscape.png)
...
![三种多 Agent 协作模式](https://blog-pic-ck.oss-cn-beijing.aliyuncs.com/posts/zh/standalone/ai-agent完全指南-从理论到工业实践/fig4_multi_agent_patterns.png)
...
![评估框架：能力雷达 + 基准覆盖矩阵](https://blog-pic-ck.oss-cn-beijing.aliyuncs.com/posts/zh/standalone/ai-agent完全指南-从理论到工业实践/fig7_evaluation_framework.png)
