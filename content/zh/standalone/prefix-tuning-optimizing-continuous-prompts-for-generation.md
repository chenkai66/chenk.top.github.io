...
![Prefix-Tuning 架构：可学习的 K/V 前缀注入到每一层冻结的注意力](https://blog-pic-ck.oss-cn-beijing.aliyuncs.com/posts/en/standalone/prefix-tuning-optimizing-continuous-prompts-for-generation/fig1_architecture.png)
...
2. 因为每一层的 K 和 V 都被加长了，每一个被生成的 token 在每一层都能“看到”前缀。整段前缀如同一条贯穿整个 Transformer 堆栈的、可微分的上下文记忆。
...
![全量微调、Prefix-Tuning 与 Prompt Tuning 的对比](https://blog-pic-ck.oss-cn-beijing.aliyuncs.com/posts/en/standalone/prefix-tuning-optimizing-continuous-prompts-for-generation/fig2_method_comparison.png)
...
![前缀长度与质量、参数成本的关系](https://blog-pic-ck.oss-cn-beijing.aliyuncs.com/posts/en/standalone/prefix-tuning-optimizing-continuous-prompts-for-generation/fig3_prefix_length_sweep.png)
...
![解码过程中前缀 K/V 在 KV cache 中的位置](https://blog-pic-ck.oss-cn-beijing.aliyuncs.com/posts/en/standalone/prefix-tuning-optimizing-continuous-prompts-for-generation/fig4_kv_cache_prepend.png)
...
![Prefix-Tuning vs 全量微调 vs Adapter 在 E2E 和 XSum 上的对比](https://blog-pic-ck.oss-cn-beijing.aliyuncs.com/posts/en/standalone/prefix-tuning-optimizing-continuous-prompts-for-generation/fig5_gpt2_application.png)
...
**初始化技巧。** 随机初始化能用，但收敛慢。用一段**真实的提示句**（比如 "summarize the following article:"）的词嵌入去初始化前缀，能在同样的任务上明显加快收敛。该技巧实现简单，但实证效果显著。
