...
把 GPT-2 微调到一个具体任务上，意味着要再多存一份 1.5B 参数的权重。切换十几个任务时，存储和上线成本就足以让一个团队望而却步，更别提"一份基模 + 多任务共享"这种工程上很想要的架构。**Prefix-Tuning**（Li & Liang, 2021）走了一条相反的路：模型权重一个不动，只学一小段连续向量——也就是论文里所说的"前缀"——在每一层注意力里被当作"已经在那里的上下文"喂进去。模型本身保持不变，只需更换前缀，即可赋予模型对应任务的适配行为。
...
全量微调要更新模型的全部参数。这件事不仅计算开销大，存储成本更高——任务一多，你会发现绝大部分存储空间实际被多个近乎相同的基模副本占据。PEFT 的核心目标可以浓缩成三条：
