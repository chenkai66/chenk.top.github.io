...
将 GPT-2 微调到具体任务上需要额外存储 1.5B 参数的权重。切换十几个任务时，存储和上线成本会让团队望而却步，更不用说实现"一份基模 + 多任务共享"这种理想的架构。**Prefix-Tuning**（Li & Liang, 2021）走了一条相反的路：模型权重一个不动，只学一小段连续向量——也就是论文里所说的"前缀"——在每一层注意力里被当作"已经在那里的上下文"喂进去。模型本身保持不变，只需更换前缀，即可赋予模型对应任务的适配行为。
...
全量微调需要更新模型的所有参数，这不仅计算开销大，存储成本也高——任务一多，大部分存储空间会被多个几乎相同的基模副本占据。PEFT 的核心目标可以概括为三点：
...
如果想给模型添加一段"看起来像上下文"的可学习参数，有两个自然的位置：
...
第二种方案表达力更强，因为它在每一层都增加了容量，而不仅仅是在最入口处。在生成任务上，这种"深入"的注入效果更好。
