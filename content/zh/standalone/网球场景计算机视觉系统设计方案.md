...
将一颗直径 6.7 cm、时速 200+ km/h 的网球，通过 8 路 4K 摄像头实时重建为毫米级三维轨迹，并同步识别球员动作——这涉及小物体检测、多视角几何、卡尔曼滤波、物理建模和姿态估计等多个技术环节。本文按工业落地顺序，逐一解析每个子问题：界定难点、论文调研选型、提供可运行代码、讨论性能预算与部署架构。

## 你将看到什么

- 传统检测器在 10-20 像素的小球上为何“塌方”及 TrackNet 系列的解决方案
- 多相机标定、 PTP 同步、 DLT 三角测量的完整数学与代码
- 结合阻力和 Magnus 力的 9 维状态卡尔曼滤波轨迹预测
...
先明确“网球场 CV 系统”的不可妥协指标，避免后续架构选型时盲目决策。
...
任何一项超时，整条流水线就会从 60 fps 降至 30 fps，体感立刻变得卡顿。

### 1.2 物理意义上的"难"

**小目标的像素预算**。在 28 m 远的对方底线、35 mm 等效焦距下，6.7 cm 的网球只占约 12 px。一个像素的中心定位误差在 3D 空间中会导致 2.3 cm 的横向偏移——这已经是底线压线判罚的临界精度。

**运动模糊**。球速 50 m/s 加上 1/250 s 快门，单帧位移 20 cm，球会被“涂”成一条 30+ 像素的拖影；如果不用 1/1000 s 以下的全局快门，检测算法再怎么优化也无法解决问题。

**同步误差的几何放大**。两台相机时间差 5 ms 时，球在画面上会错位约 25 cm，导致三角测量无法准确进行，3D 点会沿一条“反极线”漂移。因此，PTP 同步需小于 1 ms。
