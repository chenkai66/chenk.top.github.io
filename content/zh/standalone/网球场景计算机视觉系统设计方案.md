...
将一颗直径 6.7 cm、时速 200+ km/h 的网球，通过 8 路 4K 摄像头实时重建为毫米级三维轨迹，并同步识别球员动作——这涉及小物体检测、多视角几何、卡尔曼滤波、物理建模和姿态估计等多个技术环节。本文按工业落地顺序，逐一解析每个子问题：先界定难点，再进行论文调研选型，提供可运行代码，最后讨论性能预算与部署架构。

## 你将看到什么

- 为什么传统检测器在 10-20 像素的小球上会“塌方”，以及 TrackNet 系列如何解决这一问题
- 多相机标定、 PTP 同步、 DLT 三角测量的完整数学与代码
- 9 维状态卡尔曼滤波结合阻力和 Magnus 力的轨迹预测
- 网球动作识别：模板法 vs 学习法的取舍
- 端到端 < 16.7 ms / 帧、 3D 误差 < 5 cm 的工程预算如何拆分

**前置知识**：相机模型与齐次坐标、卡尔曼滤波基本概念、 PyTorch 推理基础。

![网球 CV 系统全流程](https://blog-pic-ck.oss-cn-beijing.aliyuncs.com/posts/zh/standalone/网球场景计算机视觉系统设计方案/fig1_pipeline.png)

整条链路只有 16.7 毫秒预算（60 fps），因此每个模块都必须在毫秒级完成；后续章节将逐模块拆解该延迟预算。

---

## 1. 需求与挑战：先把"难"定量化

先明确“网球场 CV 系统”的不可妥协指标，避免后续架构选型时拍脑袋。
...
任何一项超时，整条流水线就会从 60 fps 降至 30 fps，体感立刻变“卡”。

### 1.2 物理意义上的"难"

**小目标的像素预算**。在 28 m 远的对方底线、 35 mm 等效焦距下， 6.7 cm 的网球只占约 12 px。一个像素的中心定位误差，在 3D 空间中会导致 2.3 cm 的横向偏移——这已经是底线压线判罚的临界精度。

**运动模糊**。球速 50 m/s + 1/250 s 快门 = 单帧位移 20 cm，球被"涂"成一条 30+ 像素的拖影；如果不用 1/1000 s 以下的全局快门，检测算法再怎么优化也无法解决问题。

**同步误差的几何放大**。两台相机时间差 5 ms，球在画面上会错位约 25 cm，导致三角测量无法准确进行， 3D 点会沿一条“反极线”漂移。因此， PTP 同步需小于 1 ms。

**遮挡 + 短暂消失**。球过网瞬间会被网带遮挡，落地后还有一段视野盲区——任何 single-shot detector 都会出现连续若干帧丢失，必须依靠跟踪器的物理外推来弥补。
...
![小物体检测精度对比](https://blog-pic-ck.oss-cn-beijing.aliyuncs.com/posts/zh/standalone/网球场景计算机视觉系统设计方案/fig2_detection_compare.png)
...
![网球 CV 系统 — 四层架构](https://blog-pic-ck.oss-cn-beijing.aliyuncs.com/posts/zh/standalone/tennis-cv-system-design/fig_tennis_cv_arch_zh.png)
...
![边缘 + 云架构](https://blog-pic-ck.oss-cn-beijing.aliyuncs.com/posts/zh/standalone/网球场景计算机视觉系统设计方案/fig7_architecture.png)
...
![三种旋转下的 3D 轨迹与落点](https://blog-pic-ck.oss-cn-beijing.aliyuncs.com/posts/zh/standalone/网球场景计算机视觉系统设计方案/fig3_trajectory_3d.png)
...
![球场线检测流程](https://blog-pic-ck.oss-cn-beijing.aliyuncs.com/posts/zh/standalone/网球场景计算机视觉系统设计方案/fig4_court_lines.png)
...
![三种击球动作的骨架特征](https://blog-pic-ck.oss-cn-beijing.aliyuncs.com/posts/zh/standalone/网球场景计算机视觉系统设计方案/fig5_pose_skeleton.png)
...
![动作识别混淆矩阵与 F1](https://blog-pic-ck.oss-cn-beijing.aliyuncs.com/posts/zh/standalone/网球场景计算机视觉系统设计方案/fig6_action_recognition.png)
