...
会话推荐里的"兴趣结构"几乎天然是分层的：用户先点开一个大类（"跑鞋"），然后筛选品牌、尺码和价位，最后落到一个 SKU。这条交互轨迹天然构成一棵树——每次点击都将候选集大致按固定倍数缩小。问题在于，在欧氏空间中嵌入这类树状结构需要大幅增加维度，以避免叶子节点在表示空间中过度靠近，因为欧氏空间的"体积"只随半径多项式增长；而双曲空间的体积随半径**指数级**增长，正好与树的分支增长是同一个量级，几维就能把整条长尾摊开。

[**HCGR**](https://arxiv.org/abs/2107.05366)（Guo et al., 2021）正是把这层直觉做严肃了的工作：它把会话图的节点嵌入到**洛伦兹双曲流形**上，在切空间里跑带注意力的 GNN 聚合，再加一个 InfoNCE 形式的对比损失，把同一个会话的两种增强视图拉近、把其他会话推远。最终效果在 Diginetica、Yoochoose 和 Last.FM 等公开数据集上稳定超越 SR-GNN 和 GCE-GNN 等强基线，且增益最显著的数据集恰好具备双曲几何最擅长建模的特性——长尾物品多、层级深。

下面这篇笔记将重点讲清楚四件事：会话兴趣在什么意义上是"层级的"，洛伦兹模型中的距离和注意力如何计算，对比学习为何是一个便宜且有效的辅助信号，以及如何**有判断力地**阅读这类论文的消融实验，区分"几何确实有用"和"只是参数更多"。
