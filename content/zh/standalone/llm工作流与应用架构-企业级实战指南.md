...
绝大多数 LLM 教程，只讲到真正有意思的工作开始之前就戛然而止了。它们告诉你怎么调用 chat completion 接口，怎么挂一个向量库，怎么用 Streamlit 包成一个 demo——这些都没错，但解决不了真正的痛点：凌晨三点一万人涌入、每条回答都可能出幻觉时的系统性压力。
...
这种分层之所以重要，是因为它明确了工程投入的重点所在。如果一个团队把重试逻辑写在体验层、把 prompt 模板塞进模型层、把限流到处都来一份，那这个项目八成是上不了线的。把每个关注点压回它应该所在的层，模型层就能保持得很轻。

第二个推论：可靠性工作大部分都不是 LLM 工作。对工程实现而言，模型本质上就是一个 HTTP 黑盒。真正有意思的问题——*正确的工具是否被调用了？参数对不对？是不是访问了正确租户的数据？是否在预算之内？*——全都活在编排和检索层，全是确定性的 Python 或 TypeScript，全都可以跑单测。
...
- 主流闭源旗舰模型通常不支持微调。此时应选择开源模型（如 Llama、Qwen、Mistral）进行微调，而需旗舰能力的请求则通过 API 调用。第 2 节那个 branch 模式，就是让这种"分而治之"在工程上跑得起来的关键。
...
2. **小模型路由**。第 2 节的 branch 模式：每个请求先分类，只把硬骨头送给贵模型。意图分类可由轻量级 7B 模型完成，单次推理成本通常低于 0.01 美元。
3. **语义缓存**。第 4 节讲过。其价值取决于命中率，因此需在系统上线首日即完成监控埋点。
