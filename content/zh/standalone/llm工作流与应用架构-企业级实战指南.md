...
3. **语义缓存**。[第 4 节](#工作流模式：能用最简单的就用最简单的)讲过。其价值取决于命中率，因此需在系统上线首日即完成监控埋点。
...
**BYOK 与 BYOM**。客户自己 KMS 里的密钥；可选的自托管模型端点——通过你的平台路由，但明文数据从不离开客户 VPC。[第 4 节](#工作流模式：能用最简单的就用最简单的)那个 LLM Gateway 是 BYOM 在工程上跑得通的前提。
