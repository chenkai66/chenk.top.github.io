...
把一个 MLP 的隐藏单元换个顺序，函数本身不变，但参数向量却完全不同——这是「在网络空间里做学习」绕不开的第一道坎。如果表示方式不尊重这种置换对称性，下游模型就需要大量容量来记忆同一个函数的不同写法，从而影响泛化和迁移。 Kofinas 等人在 ICML 2024 的论文 *Graph Neural Networks for Learning Equivariant Representations of Neural Networks* 提出了一个简洁的解决方案：将网络视为有向图（神经元为节点，权重为边），并使用对节点置换等变的 GNN 来读取它。下面依次介绍「为什么需要等变」「神经图怎么构造」「等变到底意味着什么」「模型怎么搭建」「四类下游任务」「细节与坑」。
...
近几年出现了一类新任务，它们将一个完整的训练好的网络视为一个数据点：
...
这五件事都被同一个问题困扰： MLP 的参数空间中存在一个很大的离散对称群，其中每个元素都能使函数保持不变。对单个隐藏层和置换矩阵 $P$，
