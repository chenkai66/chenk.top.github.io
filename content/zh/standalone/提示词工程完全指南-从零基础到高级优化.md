...
同样的模型，两种问法：一种在小学数学题上的准确率为 17%，另一种为 78%。这种差别并非玄学，而是提示词工程的结果。本文将系统梳理那些真正有效的技巧、它们为何有效，以及如何在生产环境中将提示词视为一个工程问题进行优化。
...
2020 年 GPT-3 刚推出时，研究者很快发现了一个反常识的现象：同一个模型，不同的问法会导致截然不同的结果。一个写得不好的提示词只能换来胡言乱语，而一个精心设计的提示词却能解决复杂的推理任务。这并不是 bug，而是这类模型学习语言的方式所决定的。
...
这项工作的回报显著：一个优化后的提示词，既能将 API 调用成本压缩至原来的十分之一（得益于更紧凑的上下文），也能将复杂推理任务的准确率从 40% 提升至 90%。对于每月百万级请求的生产系统，这是真金白银。
