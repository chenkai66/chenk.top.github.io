...
1. **动机**：VI 和 MCMC 看似不同，其实都在解同一个 PDE。
...
两者看似不同：VI 是有限维参数优化，MCMC 是无限时间随机过程。但从 PDE 视角看，它们只是同一种概率测度演化的不同采样方式。
