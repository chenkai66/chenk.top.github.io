...
用普通神经网络拟合单摆轨迹，训练误差可以很小。但积分几十秒后，预测的摆要么停下，要么加速到逃逸速度。能量应守恒，但网络不懂"能量"是什么。问题不在数据、优化器或深度，**而在架构**。无约束的 MLP 能表示任何向量场，包括不物理的那些。向量场存在微小的系统性偏差，经长时间积分后会被不断放大，最终表现为宏观的能量漂移。

解决办法不是"多训几轮"，而是**把守恒律嵌入模型结构**。哈密顿神经网络（HNN）、拉格朗日神经网络（LNN）和辛神经网络（SympNet）就是这么做的。它们学一个**标量势函数**（能量或作用量），再通过自动微分算出动力学；或者直接用辛构件搭出离散时间映射。无论采用哪种方式，所得预测器在结构上严格满足辛性，因而长期行为在定性层面保持正确——即便模型对势函数的拟合精度仅有一两位有效数字。
