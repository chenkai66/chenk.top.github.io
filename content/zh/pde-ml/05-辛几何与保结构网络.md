...
---
![偏微分方程与机器学习（五）：辛几何与保结构网络 — visual](https://blog-pic-ck.oss-cn-beijing.aliyuncs.com/posts/zh/pde-ml/05-Symplectic-Geometry/illustration_1.jpg)

## 这篇文章讲什么

用普通神经网络拟合单摆轨迹，训练误差可以很小。但积分几十秒后，预测的摆要么停下，要么加速到逃逸速度。能量应守恒，但网络不懂"能量"是什么。问题不在数据、优化器或深度，**而在架构**。无约束的 MLP 能表示任何向量场，包括不物理的那些。向量场有一点系统偏差，长时间积分就会放大成宏观能量漂移。

解决办法不是"多训几轮"，而是**把守恒律嵌入模型结构**。哈密顿神经网络（HNN）、拉格朗日神经网络（LNN）和辛神经网络（SympNet）就是这么做的。它们学一个**标量势函数**（能量或作用量），再通过自动微分算出动力学；或者直接用辛构件搭出离散时间映射。无论哪种方式，得到的预测器从结构上就是辛系统，长期行为定性正确——即使模型只精确到一两位有效数字。

**你将学到：**

1. 为什么相空间和辛 2-形式是经典力学的"自然居所"
2. Hamilton 方程作为单一张量恒等式 $\dot{\mathbf{z}} = J\,\nabla H$
3. Liouville 定理、Poincaré 回归——"保结构"在数值上的意义
4. 为什么所有 Runge-Kutta 方法都有能量漂移，而辛积分器（Verlet, leapfrog, 辛 Euler）没有
5. HNN / LNN / SympNet 三种嵌入辛性的方法及适用场景
6. 用受控实验复现 Greydanus et al. 的单摆结果

**前置知识：** 向量微积分（链式法则、梯度），ODE 求解器（任何数值课都行），以及对神经 ODE 的初步了解（详见[第 6 部分](/zh/pde-ml/06-连续归一化流与neural-ode/)）。

---

![相空间中的哈密顿流：左图是单摆的能量等高线轨迹，右图展示一团相空间中的"液滴"在流动中变形但面积保持不变。](https://blog-pic-ck.oss-cn-beijing.aliyuncs.com/posts/zh/pde-ml/05-辛几何与保结构络络/fig1_hamiltonian_flow.png)
...
单位质量单摆的 $H = \tfrac{1}{2}p^2 + (1 - \cos q)$。Hamilton 方程给出 $\dot q = p,\ \dot p = -\sin q$。轨迹对应 $H$ 的等高线（图 1 左）：$H<2$ 时是闭合曲线（摆动），$H>2$ 时是开放曲线（旋转），$H=2$ 是分界线。轨迹始终不会离开能量面——这是神经网络外推时必须遵守的事实。
## 2. 辛 2-形式：相空间的几何
...
![偏微分方程与机器学习（五）：辛几何与保结构网络 — visual](https://blog-pic-ck.oss-cn-beijing.aliyuncs.com/posts/zh/pde-ml/05-Symplectic-Geometry/illustration_2.jpg)
...
![三栏架构对比：HNN 学哈密顿量，LNN 学拉格朗日量，SympNet 直接学辛映射。](https://blog-pic-ck.oss-cn-beijing.aliyuncs.com/posts/zh/pde-ml/05-辛几何与保结构络络/fig6_hnn_lnn_sympnet.png)
