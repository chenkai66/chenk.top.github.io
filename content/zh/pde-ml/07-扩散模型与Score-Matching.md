...
答案藏在偏微分方程里。前向加噪过程是一个**热方程**（或者更一般的 Fokker-Planck 方程），它有一个反向版本——只要我们知道每个时刻的 score（密度的对数梯度）。**Score Matching** 就是学这个 score 的标准方法。从这个角度看，DDPM、DDIM、Score-based SDE 不是几个独立的算法，而是同一个 PDE 故事的不同离散化。
