...
将 32 层 GCN 堆叠在引文网络上，准确率从 81% 骤降至 20%，节点特征全部坍缩至同一向量。这就是**过度平滑**——GNN 的‘热寂’现象。其根源在于 PDE 理论：**单层 GCN 等价于图上热方程的显式欧拉离散一步**。热方程只有一个不动点：常数。解法早在 1952 年就已提出：Alan Turing 证明，为扩散方程引入**反应项**后，原本均匀的稳态可自发演化出条纹、斑点、迷宫等非平凡空间模式。同样的思路——引入可学习的反应项——可使深层 GNN 克服过度平滑，保持表达能力。

这是《PDE+机器学习》系列的第八章，亦为终章。此前七章共同论证了一个核心观点：**几乎所有现代神经网络架构，本质上都是某类偏微分方程的离散化形式**。以反应扩散与 GNN 的结合收束本系列：这一范式展现出最直观的 PDE 结构，也为我回溯整个系列提供了清晰的终点视角。
...
*This is Part 8 — the final part — of the [PDE and Machine Learning](/zh/categories/pde-and-machine-learning/) series. Previous: [Part 7 — Diffusion Models and Score Matching](/zh/pde-ml/07-diffusion-models/). Start from the beginning: [Part 1 — Physics-Informed Neural Networks](/zh/pde-ml/01-physics-informed-neural-networks/). Thanks for reading.*
