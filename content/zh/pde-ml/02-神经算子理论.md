...
**神经算子彻底改写这个游戏**。它学的不是某一个解，而是**解算子本身**：输入是函数（初始条件、系数场、源项），输出也是函数（某时刻的解、目标区域上的物理场）。一次离线训练，之后面对每个新实例只要一次前向传播——不再划网格，不再优化，整个传统 PDE 求解流程被绕开。

这篇文章我会先讲“在函数空间之间学映射”这件事在数学上怎么说得通，再拆解两类主流架构：**DeepONet**（按 Chen–Chen 通用逼近定理拆成 branch + trunk）和 **Fourier Neural Operator (FNO)**（在频域做可学习的全局卷积）。沿途会讨论它们为什么**分辨率无关**，理论误差界有多紧，以及各自在什么场景会失灵。
