...
**方差缩减**类算法（SVRG、SAGA、Katyusha）填上了这个空隙：在只用随机采样的前提下做到 $O((n + \kappa) \log(1/\epsilon))$，严格优于全梯度下降。本文把这条收敛速率的“升级路径”讲明白。
...
[第 02 篇](../02-smoothness-strong-convexity-nesterov/)（光滑性、强凸性）、[第 03 篇](../03-gradient-descent-family/)（梯度下降与 SGD）。
