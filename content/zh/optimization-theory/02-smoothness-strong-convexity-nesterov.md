...
满足这条的可微函数称为 **$L$-光滑（$L$-smooth）**。它有一个比原始定义更实用的等价刻画，即著名的**下降引理（descent lemma）**：
...
## 2. 强凸性：保障全局极小点存在且唯一，并使目标函数值随远离最优点而二次增长
...
换言之，**函数值偏离最优值的程度，至少随迭代点到最优点距离的平方增长**。这一性质是后续收敛性证明中“利用次优函数值反推迭代点与最优点距离”的关键。
...
加速法的主要缺点是**函数值非单调下降**——部分迭代步的函数值可能上升。这在以下两种情形中尤为不利：
