...
1. 前向 + 反向走 fp16/bf16。梯度是 fp16/bf16。
2. 在调优化器之前，把梯度 **cast 到 fp32**。
3. 优化器状态（$m, v$、权重）全部在 fp32。
4. 更新完之后再把权重 cast 回 fp16/bf16，给下一步前向用。
...
这是我在跑一个新的 Transformer 模型时第一手会拿来用的几个学习率区间。它们是贝叶斯先验，不是最终答案。

| 优化器 | LR 区间 | 备注 |
|---|---|---|
| SGD + 动量 | 0.01 – 0.5 | 线性 warmup 占总步数约 5% |
| AdamW | 1e-4 – 6e-4 | 大多数 LLM 落在 3e-4 ± 1.5 倍 |
| Lion | 1e-5 – 6e-5 | 约比 AdamW 小一个数量级 |
| Sophia | 1e-4 – 4e-4 | 也比 AdamW 小一些 |
