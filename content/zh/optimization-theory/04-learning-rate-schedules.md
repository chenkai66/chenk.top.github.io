...
![三种学习率在一维二次损失上的轨迹](https://blog-pic-ck.oss-cn-beijing.aliyuncs.com/posts/zh/standalone/学习率-从入门到大模型训练的终极指南-2026/fig1_lr_regimes.png)
...
![常见学习率调度对比：cosine、step、linear、WSD](https://blog-pic-ck.oss-cn-beijing.aliyuncs.com/posts/zh/standalone/学习率-从入门到大模型训练的终极指南-2026/fig2_lr_schedules.png)
...
### 4 WSD：Warmup–Stable–Decay（现代 LLM 默认）
...
3. **理论支持。** Schaipp et al. 2025（*The surprising agreement between convex optimization theory and learning-rate scheduling*，[arXiv:2501.18965](https://arxiv.org/abs/2501.18965)）证明 cooldown 形状刚好对上凸优化里一个紧的界，cooldown 这一段在界里对应的是“去掉对数项”。

### 5 Cosine vs WSD vs Schedule-Free 速览
...
![LR range test：取稳定边界的 0.3-1×](https://blog-pic-ck.oss-cn-beijing.aliyuncs.com/posts/zh/standalone/学习率-从入门到大模型训练的终极指南-2026/fig3_lr_range_test.png)
...
![ULMFiT 风格的逐层判别式 LR](https://blog-pic-ck.oss-cn-beijing.aliyuncs.com/posts/zh/standalone/学习率-从入门到大模型训练的终极指南-2026/fig5_layerwise_lr.png)
...
### 2 无调度 AdamW（Defazio 等, 2024，[arXiv:2405.15682](https://arxiv.org/abs/2405.15682)）

它把 iterate averaging 和一个常数基础 LR 结合起来，得到“看起来像 cosine 衰减过”的轨迹，但全程并不显式地降 $\eta$。这意味着**你不需要事先承诺训练总步数 $T$**：想停就停，想延长就延长，不必重调调度。

![Schedule-free vs cosine：不必指定 T 仍能匹敌](https://blog-pic-ck.oss-cn-beijing.aliyuncs.com/posts/zh/standalone/学习率-从入门到大模型训练的终极指南-2026/fig6_schedule_free.png)
...
![典型 LLM 预训练调度（GPT-3 / LLaMA 风格）](https://blog-pic-ck.oss-cn-beijing.aliyuncs.com/posts/zh/standalone/学习率-从入门到大模型训练的终极指南-2026/fig7_llm_schedule.png)
...
### 4 Power Scheduler —— 对 batch / token 不敏感（2024）
...
### 6 Cosine ↔ WSD 的凸优化桥梁（2025）

Schaipp et al. 2025（[arXiv:2501.18965](https://arxiv.org/abs/2501.18965)）证明 WSD 的 cooldown 形状和某个紧的凸优化界完全对应，cooldown 这一段在界里负责“消去对数项”。这给“为什么 cooldown 有用”提供了一个有原则的解释。
