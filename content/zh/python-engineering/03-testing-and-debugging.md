...
你只改了一行代码，却导致三个完全无关的功能崩溃；重构一个函数后，不得不花两小时手动检查整个应用以确认一切是否依然正常；周五上线部署后，午夜就被报警电话叫醒。所有这些现象，都指向同一个病根：**没有测试**。

测试不是官僚主义流程，而是你**最快验证代码行为是否符合预期**的手段。一套优秀的测试套件能在几秒内完成运行，捕获那些手动排查需耗费数小时的缺陷。
...
真正的价值体现在对任意一段代码进行**第二次修改时**：第一次你刚写完，自然知道它能工作；但此后每次变更，如果没有测试保障，你就无法确信它仍然正确。
