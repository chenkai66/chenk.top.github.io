...
你只改了一行代码，却导致三个毫不相关的功能崩溃；你重构了一个函数，却不得不花两小时手动点击整个应用来确认一切是否仍能正常运行；你在周五部署上线，结果午夜就被告警电话叫醒。所有这些现象，都指向同一个病根：**没有测试**。

测试不是官僚主义流程。它们是你**最快确认代码行为符合预期**的方式。一套优秀的测试套件能在几秒内完成运行，并捕获那些手动排查需耗费数小时的缺陷。
...
真正的回报，出现在对任意一段代码进行**第二次修改时**。第一次，你知道它能工作——因为你刚写完它；而每一次后续变更，若未经测试，你就无法确信它依然正确。
...
过度 Mock 是常见错误。若你 Mock 了所有东西，测试验证的只是 Mock 本身，而非你的代码。**只在边界处 Mock（网络、磁盘、时钟），并在真实环境中测试核心逻辑。**
