...
你只改了一行代码，却导致三个完全无关的功能崩溃；你重构了一个函数，却不得不花两小时手动点遍整个应用，才能确认一切是否依然正常；你在周五上线部署，结果午夜就被告警电话叫醒。所有这些现象，都指向同一个病根：**没有测试**。

测试不是官僚主义流程，而是你**最快验证代码行为是否符合预期**的手段。一套优秀的测试套件能在几秒内完成运行，并捕获那些手动排查需耗费数小时的缺陷。
...
真正的价值，体现在对任意一段代码进行**第二次修改时**：第一次你刚写完，自然知道它能工作；但此后每一次变更，如果没有测试保障，你就无法确信它仍然正确。
...
过度 Mock 是一种常见误区：如果你 Mock 了所有依赖，测试实际上只验证了 Mock 的行为，而非你自己的代码逻辑。**只在边界处 Mock（网络、磁盘、时钟），并在真实环境中测试核心逻辑。**
