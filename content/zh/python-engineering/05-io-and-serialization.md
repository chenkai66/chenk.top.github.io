...
大多数程序本质上只是不同数据格式之间的“管道”，例如读取 CSV 并转换为 JSON，加载配置文件并校验后传给应用程序。每位 Python 开发者都会编写这类代码，而其中多数人都曾因编码、路径处理或序列化的细节问题踩过坑。
...
文件与数据格式构成了 I/O 层，但当你的程序需要**同时执行大量 I/O 操作**（例如下载 100 个文件、并发调用 50 个 API）时，串行执行会把大部分时间浪费在等待上。下一篇文章将深入探讨并发编程：线程（threads）、进程（processes）与异步 I/O（asyncio），并学会为不同场景选择最合适的工具。