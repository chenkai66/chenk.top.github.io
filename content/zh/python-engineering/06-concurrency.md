...
你的脚本一次只下载 100 个文件，每个下载耗时约 2 秒——其中绝大部分时间都在等待服务器响应。总耗时：200 秒。但在此期间，CPU 有 99% 的时间处于空闲状态。你为一件事（网络延迟）付费，却浪费了另一件事（计算资源）。并发编程正是为解决这一问题而生。

Python 提供三种并发模型，各自针对不同场景设计。若选错模型，你的代码要么依然缓慢，要么充斥着竞态条件（race conditions）。本文将详解每种模型的适用场景。
...
等等，GIL 并不能防止这个？没错。`counter += 1` 编译为多个字节码指令（LOAD、ADD、STORE），而 GIL 可能在这些指令之间释放。GIL 保护的是解释器内部结构，而非你的应用逻辑。
...
切勿对小任务使用 multiprocessing。序列化与进程启动开销可能导致其比串行执行更慢。
