...
Lambda 架构由 Nathan Marz 提出，融合批处理与流处理，兼顾**历史数据的准确性**和**新数据的低延迟性**。
...
Kappa 架构由 Kafka 联合创始人 Jay Kreps 提出，通过**仅使用流处理**来简化 Lambda。其核心洞见是：如果流处理器能回溯重放历史数据（例如从 Kafka 起始 offset 重读），则无需单独的批处理层。
