...
计算机科学中有个老笑话：最难的两个问题，是缓存失效（cache invalidation）、命名（naming things），以及 off-by-one 错误。这个笑话之所以流传甚广，正是因为缓存失效确实极难处理。但与此同时，缓存又是提升系统性能最有效的单一技术。一个部署得当的缓存，可将延迟降低至原来的 1/100，减少 90% 的数据库负载，并每月节省数千美元的基础设施成本。
