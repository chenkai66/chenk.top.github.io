---
title: "系统设计（4）：缓存——在哪里缓存、淘汰什么，以及缓存何时反而有害"
...
计算机科学中有个老笑话：最难的两个问题，是缓存失效（cache invalidation）、命名（naming things），以及 off-by-one 错误。这个笑话之所以成立，是因为缓存失效确实极难处理。但与此同时，缓存又是提升系统性能最有效的单一技术。一个位置得当的缓存，可将延迟降低 100 倍，削减 90% 的数据库负载，并每月节省数千美元的基础设施成本。

关键在于：**知道在哪里缓存、采用何种模式，以及——至关重要地——何时缓存反而会让系统变得更糟，而非更好。**
...
**延迟降低**：Redis 返回缓存值仅需 0.1–0.5ms；数据库查询需 5–50ms；跨服务 API 调用则需 10–100ms。缓存直接消除了最昂贵的操作。
...
客户端缓存是零成本的性能提升。请优先配置它。

### CDN 缓存

如前文所述，CDN 在全球边缘节点缓存静态资源。对于 API 响应，CDN 缓存虽可行，但需谨慎设置 `Cache-Control` 和 `Vary` 头，以防将某用户的响应错误地返回给其他用户。
...
TTL 易实现，但不提供一致性保证。数据最多可陈旧达 TTL 时长。
...
**大体积、低频访问对象（Large, rarely-accessed objects）**：缓存一份每日仅访问一次的 10MB 报告，将浪费 10MB 缓存空间——这些空间本可存储 10,000 个高频访问的 1KB 对象。
