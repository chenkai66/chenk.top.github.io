...
一位朋友曾请我协助排查一个性能问题。他们的照片分享应用在开发环境运行良好，但在生产流量下却彻底崩溃——数据库过载、 API 网关频繁超时，用户大量看到 504 错误。当我问他系统每秒处理多少请求时，他的回答是“我不知道”。当我再问预期负载是多少时，他答道“没想过这个问题”。

这次对话直指系统设计的核心：它不是白板上的方框与箭头，而是构建一种心智模型，让你在系统崩塌前就能理性推演其行为。

## 系统设计究竟是什么？

系统设计是为了满足特定需求而定义架构、组件、接口与数据流的过程。但这个教科书式的定义并未触及要害。在实践中，系统设计是在不确定性中做出明智权衡的学科。
...
初级工程师与高级工程师之间的差异，往往不在于编码能力，而在于**对大规模系统的推理能力**。初级工程师实现功能，高级工程师则追问： 10,000 用户并发触发时会怎样？下游变慢呢？磁盘写满呢？
...
**可靠性（Reliability）**：即使发生故障（硬件损坏、软件缺陷、人为误操作、流量突增），系统仍能持续正确运行。可靠性靠冗余、故障隔离与优雅降级保障。

**可维护性（Maintainability）**：系统能被团队长期理解、修改与运维。这意味着清晰的抽象、完善的监控、简化的部署和准确的文档。一个运行完美却无法在故障时被快速定位调试的系统，绝非好设计。
