...
2017 年，某大型云服务商因一条错误配置的 DNS 记录导致互联网大范围中断数小时——成千上万个网站无法访问，并非因为其服务器宕机，而是负责将域名翻译为 IP 地址的系统失效了。这次事故尖锐地提醒我们，那些习以为常的基础设施——DNS、CDN 和负载均衡器——正是整个系统赖以运转的地基。

用户发起的每个 HTTP 请求在抵达应用代码前，至少会经过其中两个系统；如果任一环节失效或性能低下，下游所有优化都将失去意义。
...
对于未缓存的查询，整条链路通常耗时 20-120ms；缓存命中则低于 1ms。
...
- **迁移期间**：提前数天将 TTL 设为低值（如 60s），切换记录后再恢复为长 TTL。  
...
第 4 层负载均衡器接收 TCP 连接，选择后端服务器，并转发原始 TCP 数据包。它不解析 HTTP 头、 URL 或 Cookie，因此性能极高——每秒可处理数百万连接，且延迟开销极小。
