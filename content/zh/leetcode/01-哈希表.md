...
title: "LeetCode（一）：LeetCode（一）—— 哈希表"
...
这是 **LeetCode 算法精讲系列** 的开篇。我们会从零开始帮你建立对哈希表的直观理解，然后通过四道经典题——**两数之和**、**字母异位词分组**、**无重复字符的最长子串**、**前 K 个高频元素**——提炼出可以反复使用的通用解题套路。
## 系列导航
...
![查找代价：哈希 $O(1)$ vs 二分 $O(\log n)$ vs 线性 $O(n)$](https://blog-pic-ck.oss-cn-beijing.aliyuncs.com/posts/zh/leetcode/01-哈希表/fig4_complexity_compare.png)
...
![两数之和单次遍历过程：nums = [2, 7, 11, 15], target = 9](https://blog-pic-ck.oss-cn-beijing.aliyuncs.com/posts/zh/leetcode/01-哈希表/fig3_two_sum_flow.png)
