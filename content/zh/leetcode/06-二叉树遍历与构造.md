...
title: "LeetCode（六）：LeetCode（六）—— 二叉树遍历与构造"
...
二叉树的题目，核心其实不在“树”本身，而在于两点：**访问节点的顺序**，以及**在处理父节点之前，从子节点那里得到了什么信息**。搞清楚这两点，前序、中序、后序、层序遍历的区别，递归和迭代的写法，从两种遍历序列重建树，甚至像求最大深度、验证 BST 这样的经典题，都可以归结为同一套方法论。这篇文章的目的，就是带你彻底掌握这套方法。
...
**二叉树**是一种每个节点最多有两个子节点的树结构，且左右子节点是**有顺序的**——左就是左，右就是右，不能随便调换。空树也被视为二叉树，这个定义看似简单，但它是所有递归操作能干净收尾的关键。
...
这两个阶段正好对应递归调用的两个部分：入栈相当于“递归向下”，出栈相当于“从递归返回”。图 4 把递归调用栈和显式栈逐帧对比，你会发现它们本质上是一回事：
...
迭代版最容易踩的坑有两个：第一，弹栈后忘记 `cur = cur.right`，会导致同一个节点反复入栈出栈，陷入死循环；第二，把内层 `while` 写成 `if`，每次只走一步，遇到左孙子就漏掉了——必须先把整条左链一次性沉到底，再开始第一次弹栈。
### 前序、后序的迭代写法各一段
...
这个方法巧妙避开了传统迭代后序需要维护“右子树是否已访问”这种繁琐的状态标记逻辑。
## BFS：层序遍历

DFS 是一路往下钻，直到不能再深才回头。BFS 则反过来——先访问离根节点距离为 0 的节点，再访问距离为 1 的节点，依次类推。这里的核心数据结构是 FIFO 队列，因为我们需要保证**先进队的节点的孩子也先被处理**。

![层序遍历：每次处理一整层](https://blog-pic-ck.oss-cn-beijing.aliyuncs.com/posts/zh/leetcode/06-二叉树遍历与构造/fig3_bfs_levelorder.png)
...
1. **DFS 就是一个通用模板——访问节点、递归左子树、递归右子树——四种遍历方式的区别仅仅在于"访问"的时机不同。** 前序是在处理子节点之前先定调；后序是等子节点都处理完后再收尾；中序则是卡在左右子树之间；层序则把 LIFO 的递归栈换成 FIFO 队列，按照距离根节点的层数一层层处理。
