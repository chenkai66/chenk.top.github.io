...
title: "LeetCode（十）：LeetCode（十）—— 栈与队列"
...
栈和队列看起来平平无奇，尤其在图算法或动态规划面前显得毫不起眼。但它们却是面试题里的常客，原因很简单：大多数算法问题归根结底都在考察**访问顺序**。栈是后进先出（LIFO），队列是先进先出（FIFO）。再加上单调栈、双端队列、优先队列这些变体，很多经典问题——括号匹配、下一个更大元素、滑动窗口最值、前 K 大、BFS 等等——都能用它们高效解决。

这篇文章会带你从头到尾梳理一遍相关内容。我们会先讲清楚基础数据结构的原理，然后通过六道典型的 LeetCode 题目一步步演示解法，最后附上一张复杂度对比表和一个针对常见错误的 Q&A 环节。
...
![每日温度：下标的单调递减栈](https://blog-pic-ck.oss-cn-beijing.aliyuncs.com/posts/zh/leetcode/10-栈与队列/fig3_monotonic_stack.png)
