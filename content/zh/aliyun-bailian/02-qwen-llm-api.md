...
---
这个系列里，真正能落地的干货都在这篇。别的模型也挺有意思，但我在线上产品里调用的，几乎全是 Qwen。官方文档太厚太全，这篇文档就是帮你挑出一条最实用的路。
...
Qwen 家族很大。很多团队默认全用 `qwen-max`，结果钱花多了；有的默认全用 `qwen-turbo`，结果质量差了。正确答案是“看菜吃饭”：
...
- **`qwen-turbo`** — 分类、意图识别、短文本摘要，或者任何单次用户请求里调用超过 10 次的场景。这是最便宜的 sane Qwen，做信息提取 surprisingly good。
- **`qwen-plus`** — 日常聊天、RAG 合成、多步推理的主力模型。性价比的拐点。
...
> **提示：** 一个常见错误是用 `qwen-max` 做 embedding 式的分类。别这么做。用 `qwen-turbo` 配上紧凑的 system prompt，成本能砍掉 10 倍，而在只需要标签的任务上质量毫无损失。

## 实际上 wire 上传的是什么

不管你是用 OpenAI 兼容层还是 DashScope 原生接口，chat-completion 请求的核心都一样：一个模型 ID，一个 messages 数组，还有一个参数块。
...
- **`temperature`** — 0.0-2.0。提取/分类我用 0.0，默认聊天用 0.2-0.4，只有创意写作才用 0.7+。官方文档默认大概是 0.7，对大多数 agent 用途来说太高了。
- **`top_p`** — 除非你清楚为什么要改，否则留默认。同时 tweaking `temperature` 和 `top_p` 只会让你更困惑。
- **`max_tokens`** (compat) / **`parameters.max_tokens`** (native) — 这是 *输出* token 上限，不是总数。必须设。不然一旦 runaway 能让你破产。
- **`stream`** — 开关 SSE streaming。见下文。
- **`response_format={"type": "json_object"}`** — JSON mode。强烈建议优于"please return JSON"这种 prompt。
- **`tools`** / **`tool_choice`** — 函数调用。

## 函数调用：往返过程

Qwen 的函数调用协议就是 OpenAI 的 tool-calls 协议。两次 LLM 调用，中间夹着你的代码：
...
一个完整的 worked example — 一个能查天气的 tiny agent：
...
- **`messages.append(msg)` 是必须的**，在第一次响应和 tool result 之间。模型需要在历史里看到它自己的 tool_call 消息，否则第二次调用会报 400，说是"orphan tool result"。
...
- 对于 *结构化* JSON（Pydantic schema），改用函数调用模式。它更严格，而且失败模式更容易 debug。

## enable_thinking 和 streaming 陷阱

Qwen3 系列模型支持 **`enable_thinking=True`** — 它让模型在最终答案之前先生成推理链。质量会提升，尤其是重推理的任务。**但你必须用 streaming。** 不开 stream 直接报 400。
...
我把 `reasoning` 转发给我的日志系统，绝不给用户看。三个原因：(1) 如果客户看到会泄露 chain-of-thought IP，(2) 会 confuse 非技术读者，(3) 它会让你的可见响应长度翻倍。
...
从 OpenAI 生态过来的团队最大的 surprise：**`tiktoken` 关于 Qwen token 数的说法是假的**。Qwen tokenizer 跟 `cl100k_base` 或 `o200k_base` 不兼容 BPE。如果你用 `tiktoken.encoding_for_model("gpt-4o")` 来 sizing 你的 context budget，中文会偏 20-40%，英文会偏 5-10%。我就因为这事熬过一个周五晚上，一个 RAG pipeline 按 tiktoken 算"definitely under the 32k context"，实际按 Qwen 算是 41k。
...
CJK 膨胀问题是真实的，值得算进成本里。在 Qwen 上，典型的中文字符每个字跑约 1.5 个 token。英文每个字符约 0.25 个 token（每 4 个字符 1 个 token）。所以 1000 字符的中文 RAG context 花你 1500 token；同样的英文花你 250。当你 sizing context windows 时，**按 token 计划，别按字符**，并用本地 tokenizer。我见过一次"use 100k chars of context"的计划正好变成"we need a 150k context model"。
