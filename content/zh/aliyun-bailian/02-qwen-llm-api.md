...
---
这个系列中，真正能落地的干货都集中在这篇。其他模型虽有趣，但我在线上产品中实际调用的几乎全是 Qwen。官方文档内容详尽但篇幅庞大，本文旨在为你提炼一条最实用的落地路径。
...
- **`qwen-turbo`** — 分类、意图识别、短文本摘要，或者任何单次用户请求里调用超过 10 次的场景。这是成本最低的 Qwen 变体，在分类、意图识别等信息抽取任务中表现优异。
