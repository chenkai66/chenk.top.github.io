...
---
在所有百炼模型中，Qwen-Omni 是当前帮我清掉最多技术债的一个：过去处理‘帮我看看这段 2 分钟宣传视频讲了什么’这类需求，得自己实现帧提取、逐帧生成 caption、再拼接——整套流程动辄三周；现在一个 HTTP 请求就搞定。但文档对关键细节语焉不详——尤其是‘必须启用流式传输’这一硬性要求，已让多个团队白白耗费半天排查；务必警惕。
...
原因很直接：Qwen-Omni 要处理数 MB 的视频输入、生成长文本输出，传输协议本就是流式原生设计——若强行等完整响应再返回，客户端会卡住几十秒，全程零进度反馈。
