...
如果下游代码需要完整的响应字符串，只需在客户端聚合流式返回的 delta 即可。
...
对于纯转录任务，百炼还提供了一个更便宜的专用 **Paraformer** ASR 模型。转录使用 Paraformer，需要理解（情感、总结、“电话里提没提价格”）时再用 Qwen-Omni。
...
30 秒视频片段 → 60 词描述，端到端在生产环境大概 **6-8 秒 wall clock**。其端到端延迟优于人工处理，但显著高于纯文本 LLM 推理。若业务场景要求亚秒级响应，建议在视频摄入阶段完成预处理与描述缓存，而非实时调用 Qwen-Omni。
