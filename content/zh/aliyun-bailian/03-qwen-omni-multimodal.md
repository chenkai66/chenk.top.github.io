...
如果下游代码需要完整的响应字符串，只需在客户端聚合流式返回的 delta。
...
Qwen-Omni 的实际成本敏感点并非单个 vision token 的单价，而是视频输入所触发的 vision token 生成总量。我们从第一性原理算一下：
...
2. **Vision encoding (server-side)**：30 秒 720p 片段大概 1-4s。这是用户感觉“什么都没发生”直到 Token 开始流式输出的阶段，大致与帧数成正比。
