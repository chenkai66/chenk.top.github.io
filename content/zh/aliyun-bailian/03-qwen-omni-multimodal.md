...
---
在所有百炼模型里，Qwen-Omni 是目前帮我解决最多技术债的百炼模型。过去处理‘帮我看看这段 2 分钟宣传视频讲了什么’这类需求，需要自行实现帧提取、逐帧生成 caption 再拼接，整个流程至少耗时 3 周。现在 Qwen-Omni 一个 HTTP 请求就搞定。但文档对一些关键细节交代不清，尤其是‘必须启用流式传输’这一强制要求，已导致多个团队额外花费半天时间排查。请务必注意这一点。
...
原因很直观：Qwen-Omni 需处理数 MB 的视频输入，并生成较长的文本输出。其传输协议原生采用流式设计；若强制等待全部响应完成再返回，客户端将阻塞数十秒且无任何进度反馈。

如果你的下游代码需要完整的响应字符串，只需在客户端聚合流式返回的 delta 即可：
...
- **描述任务的视频** — 上传前预 resize 到 720p，静态内容（比如人物说话）fps 降到 4，动作内容（体育、快切）降到 8。超过 8 fps 通常会引入大量冗余帧，造成 vision token 的不必要消耗。
- **长视频** — 切片。模型有 context 限制。超过 ~3 分钟的内容，切成 90 秒片段，分别总结，再用 `qwen-plus` 对摘要再做摘要。该策略与长文档 RAG 的分块-摘要-聚合思路一致。
...
Qwen-Omni 的音频输入看起来简单——传个 `input_audio` 部分带上 `data` 和 `format`——但编码要求很严，而且报错不友好。若参数配置错误，API 将直接返回 400 错误（message: "unsupported audio format"），且错误信息中不提供具体原因或修复建议。

生产环境真正能用的参数：

- **采样率**：16 kHz 是最佳平衡点。模型接受 8 / 16 / 22.05 / 24 / 44.1 / 48 kHz，但除了 16 kHz 以外都会在服务端重采样，增加延迟。对于 VoIP 录音（原生 8 kHz），我用 `librosa.resample` 在客户端上采样到 16 kHz——音质无损，且可避免服务端重采样带来的额外延迟。
...
Qwen-Omni 在内部处理视频时，默认按其内置策略对视频采样帧，并将每帧编码为 vision token block。vision token 消耗量与帧数呈正比，因此帧率（fps）是控制视频处理成本最关键的调节项。

官方文档只说了视频 URL 部分有个 `fps` 参数，但没告诉你不同类型的内容该设多少。在生产环境跑了大概 2000 次视频调用后，我总结了几条规则：

- **口播内容（采访、产品演示、讲座）**：1 fps 足够。关键信号在音频加上稀疏的视觉线索（PPT 切换、手势）。更高 fps 会把 Token 浪费在看起来一模一样的帧上。我在同一段 2 分钟采访片段上测过 1 / 4 / 8 / 16 fps，描述质量在 1 fps 就趋于平稳了。部分场景下，0.5 fps 仍能生成语义合理、关键信息完整的摘要。
...
预提取能让你精确控制模型看到哪些帧。当模型默认帧采样策略遗漏关键帧（例如人物表情突变、PPT 页面切换等）时，预提取方案可确保模型稳定接收目标帧。

## 视频 Token 成本算账

Qwen-Omni 的实际成本敏感点，并非单个 vision token 的单价，而是视频输入所触发的 vision token 生成总量。咱们从第一性原理算一下：
...
30 秒视频片段 → 60 词描述，端到端在生产环境大概 **6-8 秒 wall clock**。其端到端延迟优于人工处理，但显著高于纯文本 LLM 推理。若业务场景要求亚秒级响应，建议在视频摄入阶段即完成预处理与描述缓存，而非实时调用 Qwen-Omni。
