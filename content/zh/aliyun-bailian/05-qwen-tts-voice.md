...
---
我负责的中文产品最终选用了 Qwen-TTS-Flash，并非因为价格更低——市面上确实有更便宜的 TTS API。根本原因在于，它能在同一 SDK 中同时支持**大陆方言**（如粤语、四川话、吴语）和英语，且语音自然度远超早期合成音（例如 2019 年常见的机械式播报音）。将 Qwen-TTS-Flash 应用于营销视频配音流水线约半年后，我总结了一些早期容易踩的坑。
...
模型卡片显示 Qwen-TTS-Flash 开放了 40 多种声音，我最常用的是以下几种：
...
中文产品 narration 我默认用 `Cherry`（温暖， 30 多岁女性）做营销内容，`Ethan`（稳重， 40 多岁男性）做教程或解说内容。粤语广告 spot 选 `Sunny` 最稳。声音标识符（voice ID）是稳定的，但新声音会定期上线；因此，在生产代码中硬编码指定声音前，务必先从模型卡片获取最新版权威列表（canonical list）进行核对。

## 只用原生 API

Qwen-TTS 不通过 OpenAI 兼容层，必须使用 DashScope 原生 SDK 调用。
...
- 默认输出是 *URL*，不是字节流。与 Wanxiang 一致，生成音频 URL 的有效期为 24 小时（实践中建议立即下载并转存至自有 OSS Bucket）。
- `format` 默认是 `mp3`。也支持 WAV；如果是 downstream 拼接工作，我偏好 WAV，因为每个 chunk 没有 MP3 header 开销。

## 流式 TTS —— 延迟敏感场景

做语音机器人（实时对话 UI）需要使用流式传输。delta 是音频字节，可以直接写入播放器或文件。
...
流式的首字节时间（Time-to-first-byte）在上海区域通常低于 400ms，快到让用户感觉是实时响应。非流式模式下，合成一段 30 秒语音的端到端耗时约为 4–6 秒，适用于批量配音等离线场景，但无法满足实时对话对低延迟的要求。
...
> **提示：** 做方言工作时，上线前务必与母语者进行 A/B 测试。Qwen-TTS 的粤语合成效果整体良好，但个别音节的声调仍有偏差；由于粤语是声调语言，单字声调错误可能导致语义完全不同。

## SSML —— 哪些管用，哪些不管用

文档列出了 SSML 支持，但没有说明哪些标签真正有效。经验之谈：
...
- `<say-as interpret-as="digits">` — 电话号码、代码和日期等结构化文本的发音处理效果良好。
...
为何需要拆分？主要基于两点考量：（1）短文本请求的单次调用延迟显著更低，且支持并行合成，整体效率更高；（2）若某一句合成失败，仅需重试该句，无需重新生成整段配音。我们做广告 spot 就用这个——`<break>` 标签处理句间停顿，并行合成意味着 60 秒的 clip 大概 4 秒就能 ready。

## 成本

按输出音频秒数计费，流式和非流式计费相同。60 秒广告 spot 只需几块钱——相比真人配音演员的按小时计费模式，成本显著更低，且生成速度极快，营销团队可在一下午内完成数十个版本的快速迭代。

## 声音克隆 vs 预设声音 vs SSML 控制：选对旋钮

百炼提供了三层声音控制，选择哪一层比想象中更重要。成本、实现复杂度与控制精度通常与功能的“炫酷”程度呈反比关系。

**预设声音。** 目录里的 40+ 声音。除非有特定理由，否则就用这些。它们覆盖了所有常见年龄段和注册的中文男/女声，以及粤语、四川话、吴语、英语（美/英），还有一些用于叙述的“角色”声音。每秒成本是标准费率，延迟是三层中最低的，质量达到录音棚级别。我 90% 的生产流量都是预设声音。
...
`<say-as>` 让 "DSL" 按字母读（缩略语正确读法）。`<break>` 在听众需要吸收信息的地方加 deliberate pause。`<prosody rate="95%">` 稍微放慢 URL 推荐部分，让听众来得及记下来。纯文本输入无法实现这些效果。

**声音克隆（音色迁移）。** 百炼暴露了一个单独的 clone API，上传 10-30 秒参考音频，返回一个 voice id 用于后续调用。该功能真实可用，但需注意以下两点限制：

- **质量波动。** 清晰、单人、录音棚参考音频的克隆效果极佳。电话音质、嘈杂或多人的参考音频克隆出来会诡异得吓人。生产使用前务必人工筛查克隆效果。
- **法律/伦理风险。** 未经同意克隆真人声音在许多司法辖区越来越违法，并且明确被百炼 ToS 禁止。只克隆你拥有或获得书面许可的声音。平台可能会主动撤销涉嫌模仿公众人物的 voice ID。
...
对于“用户输入文本 → 播放音频”的用户体验，端到端感知延迟大约在首个声音的 250-500ms 之间。这是“感觉即时”的阈值——低于 500ms 被视为即时响应，超过 1 秒则明显卡顿。

对于 **avatar 口型同步**，你需要音频 chunk 和每个 phoneme 的时间戳。基础 API 里 Qwen-TTS 不返回每个 phoneme 的时间戳。有两个解决方法：
...
原生 TTS 输出几乎不能直接用于生产环境。任何要交付给听众的流水线，这两步后处理都是必不可少的：
...
这行 ffmpeg 命令可以将所有 TTS 输出拉到同一感知响度。每个输出都要运行，无一例外。 CPU 开销仅为微秒级。
...
隐藏成本是后处理工程时间：花一天时间搭建响度/修剪/拼接流水线，然后摊销到未来几年发布的每个 clip 上。跳过后处理，无论模型多好，音频听起来都会很业余。
