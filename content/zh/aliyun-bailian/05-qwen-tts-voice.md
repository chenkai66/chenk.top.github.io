...
---
我负责的中文产品最终都选用了 Qwen-TTS-Flash，并非因为价格更低——市面上确实存在更便宜的 TTS API。根本原因在于，它是目前唯一能在同一 SDK 中同时支持**大陆方言**（如粤语、四川话、吴语）和英语的 TTS 服务，且语音自然度远超早期合成音（例如 2019 年常见的机械式播报音）。在将 Qwen-TTS-Flash 应用于营销视频配音流水线约半年后，我总结出一些早期容易踩的坑。
...
Qwen-TTS 不走 OpenAI 兼容层。必须使用 DashScope 原生 SDK 调用：
...
按输出音频秒数计费。流式和非流式计费一样。60 秒广告 spot 也就几块钱——相比真人配音演员的按小时计费模式，其成本显著更低，且生成速度极快——营销团队可在一下午内完成数十个版本的快速迭代。

## 声音克隆 vs 预设声音 vs SSML 控制：选对旋钮

百炼给了三层声音控制，选哪个比想象中更重要。成本、实现复杂度与控制精度，通常与功能的‘炫酷’程度呈反比关系：
...
- **法律/伦理风险。** 未经同意克隆真人声音在很多司法辖区越来越违法，且明确被百炼 ToS 禁止。只克隆你拥有或获得书面许可的声音。平台可能主动撤销涉嫌模仿公众人物的 voice ID。
...
对于“用户输入文本 → 播放音频”的 UX，端到端感知延迟落在首个声音的 250-500ms 左右。这是“感觉即时”的阈值——低于 500ms 低于 500ms 被视为即时响应，超过 1 秒则明显卡顿。
