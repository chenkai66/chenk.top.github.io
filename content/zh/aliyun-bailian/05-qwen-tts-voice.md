...
---
我负责的中文产品最终选用了 Qwen-TTS-Flash，并非因为价格更低——市面上确实有更便宜的 TTS API。根本原因在于，它能在同一 SDK 中同时支持**大陆方言**（如粤语、四川话、吴语）和英语，且语音自然度远超早期合成音（例如 2019 年常见的机械式播报音）。在将 Qwen-TTS-Flash 应用于营销视频配音流水线大约半年后，我总结了一些早期容易踩的坑。
...
模型卡片显示 Qwen-TTS-Flash 开放了 40 多种声音，我最常用的是：
...
做语音机器人（实时对话 UI）需要使用 streaming。delta 是音频字节，可以直接写入播放器或文件：
...
> **Tip:** 做方言工作，上线前**务必**跟母语者做 A/B 测试。Qwen-TTS 的粤语合成效果整体良好，但个别音节的声调仍有偏差；由于粤语是声调语言，单字声调错误可能导致语义完全不同。
