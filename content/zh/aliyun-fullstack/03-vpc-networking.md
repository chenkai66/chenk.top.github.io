...
---
我在云上排查过的每一次故障，追根溯源最后都指向了网络。要么是 CIDR 规划没做好，半年后 IP 不够用了；要么是路由缺失，流量在层级间静默丢弃；要么是安全组配置极端，要么对 `0.0.0.0/0` 开放了 22 端口（哈喽，黑客朋友），要么锁得太死导致健康检查失败，负载均衡不停剔除健康实例。网络层是所有部署的前提，必须最先规划和落地；但一旦需要调整，补救成本很高——修改 VPC 的 CIDR 段会强制重建其下的所有资源。
...
|---|---|---|
| VPC | VPC | 最高级别的隔离网络 |
| VSwitch | 子网 | 绑定到一个可用区的子网 |
| 路由表 | 路由表 | 用于流量导向的路由规则 |
| 安全组 | 安全组 | 有状态的实例级防火墙 |
| 网络 ACL | 网络 ACL | 无状态的子网级防火墙 |
| EIP | 弹性 IP | 静态公网 IP 地址 |
| NAT 网关 | NAT 网关 | 私有子网的互联网访问 |
