...
series_order: 2
description: "了解ECS所需的一切：实例系列（g8、c8、r8、GPU）、定价模型、cloud-init自动化、安全组、密钥对。从零部署生产就绪的应用服务器。"
...
在控制台“创建 ECS 实例”时，实际上是在一次性配置这六个组件。控制台在创建流程中将这些组件整合呈现以简化操作，但每个组件都有独立的生命周期。可以将磁盘从一台实例卸下并挂到另一台，ENI 可以在实例间迁移，安全组则跨实例共享。真正理解这种解耦设计，是区分‘普通使用者’与‘专业运维者’的关键。
