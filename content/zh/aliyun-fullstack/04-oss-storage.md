...
对象存储服务（Object Storage Service）就是阿里云版的 AWS S3。你把文件——称为“对象”——存进叫“桶”（Bucket）的容器里。每个对象有个唯一的 key（也就是路径）、数据本身和元数据。这就是全部的数据模型。没有目录，没有文件层级，没有 POSIX 语义。当你在 OSS 里看到 `images/2026/05/avatar.png` 时，斜杠只是 key 字符串的一部分，不是目录结构。控制台为了方便把它们渲染成文件夹，但存储层其实是扁平的。

这种简单性正是重点。因为 OSS 不需要维护文件系统树，它可以透明地把对象分发到成千上万个存储节点上。你永远不用考虑容量规划、磁盘 IOPS 或 RAID 配置。你 PUT 一个对象，OSS 自己决定存哪儿、怎么跨可用区复制以保证耐久性、以及你 GET 时怎么取回来。标准存储的耐久性保证是 99.9999999999%（12 个 9）。这意味着“如果你存一百亿个对象，设计上也最多只丢一个”。
...
**块存储**（挂载到 ECS 的云盘）给你的是一个原始块设备，操作系统用 ext4 或 xfs 格式化。它快、延迟低、支持随机 I/O——完美适合数据库、系统启动卷以及任何需要 POSIX 文件系统语义的场景。但它一次只能挂载到一个实例，而且不管你用不用，都得为预配容量付费。

**文件存储**（NAS）提供一个共享文件系统，多台 ECS 实例可以通过 NFS v3/v4 或 SMB 同时挂载。适合那些需要共享 `/data` 目录的遗留应用、CMS 系统或开发环境。但它每 GB 成本高，性能取决于你购买的容量层级。

**对象存储**（OSS）适合其他所有场景——而“其他所有”通常占你数据的 90%。静态资源、用户上传、备份、日志、数据湖文件、ML 训练数据集、视频、音频、文档。如果你通过 HTTP 访问它，且不需要修改文件中间的字节，OSS 就是正确答案。
...
OSS 有五种存储类型，选对能省 80% 账单，选错能 inflate 10 倍。心理模型：存储越便宜， retrieval 越贵越慢。
