...
series_order: 8
description: "在阿里云上无服务器化：Function Compute触发器、运行时、冷启动、定价。EventBridge事件路由。API Gateway集成。构建事件驱动的图像处理流水线。"
disableNunjucks: true
translationKey: "aliyun-fullstack-8"
---
第一次看到函数计算的账单——1 万次请求仅需 0.03 元时，我彻底重构了原有架构。之前为了一个每小时大概 200 次请求的 API，我常年跑着一台 2 核 ECS，每月 490 元；同样的工作量搬到函数计算后，每月只需不到 5 元。成本差距如此悬殊，那个周末我将所有无需常驻进程的业务全部迁移到了函数计算。

Serverless 并非没有服务器，而是将服务器运维完全交由平台托管。你只需写个函数并定义好触发条件，剩下的 provisioning、扩缩容、打补丁和下线都由平台全包。代码按运行时间收费，无流量则不收费。如果五分钟内涌入百万级请求，平台可以瞬时扩容至百万实例，无需调整任何容量配置。
...
Serverless 并非银弹，它有明确的适用边界。提前厘清这一点，可以避免数月后被迫回迁至 ECS。
...
| **突发流量 API 后端** | 缩容到零能省钱 | 营销活动 API，季节性电商 |
...
比较价格之前，先看看三种计算原语：FC、SAE 和 ECS 在颗粒度/生命周期这条轴上的位置。我见过的多数架构失误，都是把错误的工作负载放在了错误的原语上。
...
在这个配置下，交叉点大概在每月 2-3 亿次请求。低于这个数，函数计算完胜；高于这个数，专用 ECS 实例更便宜，但需要自己处理扩缩容、打补丁和高可用。
