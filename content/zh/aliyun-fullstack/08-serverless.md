...
---
第一次看到函数计算的账单——1 万次请求仅需 0.03 元时，我彻底重构了原有架构。之前为了一个每小时大概 200 次请求的 API，我常年跑着一台 2 核 ECS，每月 490 元；同样的工作量搬到函数计算后，每月只需不到 5 元，而不是每天 5 块。成本差距如此悬殊，那个周末我将所有无需常驻进程的业务全部迁移到了函数计算。

Serverless 并非没有服务器，而是将服务器运维完全交由平台托管。你只需写个函数并定义好触发条件，剩下的 provisioning、扩缩容、打补丁和下线都由平台全包。代码按运行时间收费，无流量则不收费。如果五分钟内涌入百万级请求，平台可以瞬时扩容至百万实例，你甚至无需调整任何容量配置。
