...
---
第一次看到函数计算账单只要 0.03 元处理 1 万次请求时，我彻底重新思考了我的架构。之前为了一个每小时大概 200 次请求的 API，我常年跑着一台 2 核 ECS，每月 490 元。同样的工作量搬到函数计算，每月不到 5 元。不是每天 5 块，是每月总共 5 块。这账算得太悬殊了，那个周末我把所有不需要常驻进程的业务全从 ECS 迁到了函数。

Serverless 不是说没有服务器，而是让你不再操心服务器。你写个函数，定义好触发条件，剩下的 provisioning、扩缩容、打补丁、下线，平台全包。代码跑多少毫秒收多少钱。没流量不收钱。五分钟进来一百万请求，平台瞬间弹出一百万个实例。你连容量滑块都摸不到。
...
Serverless 不是万能药。它有特定的舒适区，提前搞清楚这点，能避免三个月后痛苦地迁回 ECS。
