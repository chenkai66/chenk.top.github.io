...
---
单卡跑模型挺好玩，但要想让它稳稳当当扛住每秒 1000 个请求，那才是从实验到产品的分水岭。PAI 这两头都能管。

PAI（Platform for AI）是阿里云的托管式机器学习平台。说它是一个产品其实不太准确，更像是五个产品穿了件风衣，共用一个控制台。Notebook 用来探索，分布式训练用来扩规模，模型服务用来生产，可视化流水线给喜欢拖拽的人，模型库用来一键部署开源模型。跑了十八个月真实 LLM 负载后，我觉得单个组件从优秀（EAS）到够用（Designer）不等，但一旦你摸清了它们怎么联动，整体效果确实大于部分之和。

这篇文章做个广度优先的巡礼。如果你想要深度优先的处理——比如实例选型策略、DLC 抢占式实例生存指南、EAS 冷启动缓解——那边有个专门的 [PAI 系列](/zh/aliyun-pai/01-platform-overview/)，五篇文章深挖每个子产品。这里我们只讲 enough 让你明白 PAI 是什么、什么时候该用哪个组件，以及如何端到端训练部署模型。

![PAI ML Platform](https://blog-pic-ck.oss-cn-beijing.aliyuncs.com/posts/zh/aliyun-fullstack/11-pai-ml-platform/cover.png)

## PAI platform overview

PAI 全称 Platform for AI。名字起得通用，因为产品覆盖面广——从交互式实验到生产服务，整个 ML 生命周期都包了。其他云上最接近的对标是 AWS SageMaker、Azure Machine Learning 和 GCP Vertex AI。但这只是近似比较。SageMaker 把 Notebook、训练和端点打包成一个相对单体的体验。PAI 更模块化：每个子产品有自己的资源模型、定价和 SDK 表面，你可以单独用任何一个。
...
我觉得最顺手的心理模型是：代码沿着 DSW、DLC、EAS 从左到右成熟，而 Designer 和 QuickStart 则是跳过部分旅程的捷径。
...
PAI 从不拥有你的数据。数据集、checkpoint 和模型 artifact 都存在 OSS 或 NAS 里。PAI 代表你编排 GPU 计算——DSW Notebook 启动时，某处会 boot 一台真实的 GPU ECS 实例；EAS 端点扩容时，真实的 GPU pod 会起来。之所以用 PAI 而不是裸 ECS，是因为它预 baked 了 CUDA/PyTorch 镜像，帮你挂载存储，提供指标 dashboard，而且按秒计费而不是按小时。

### PAI vs SageMaker: the meaningful differences

如果你是从 AWS 过来的，这些区别可能会让你踩坑或者让你惊喜：
...
最大的实际区别在于：PAI 直接暴露底层的 ECS 实例类型。选 DSW 实例时，你选的是 `ecs.gn7i-c8g1.2xlarge`（1x A10, 24 GB）。提交 DLC 任务时，你指定具体的 GPU SKU。这种透明度让成本估算变得直白——你用 ECS 的价格计算器也能算 PAI。

## PAI-DSW: interactive notebooks

大多数 ML 工作在 PAI 上都是从 DSW（Data Science Workshop）开始的。它就是跑在 GPU ECS 实例上的 JupyterLab 和浏览器版 VSCode，由 PAI 帮你管理。卖点很简单：跳过 CUDA/cuDNN/PyTorch 的安装舞步，大概 90 秒就能拿到一个能用的 GPU 盒子。
...
别用 DSW 做多卡训练（那是 DLC 的事）、超过 8 小时的无人值守任务（空闲 shutdown 会杀掉它们）或生产推理（那是 EAS 的事）。
...
存储：这是关键部分。新 PAI 用户最容易犯的头号错误：训练几小时，实例一 restart 全丢了。DSW 实例有个系统盘，restart 时会重置。你想保留的一切必须放到：
...
单卡跑不动的时候，就该找 DLC (Deep Learning Container) 了。它是一个托管的批处理系统：你丢给它一个容器镜像、一条命令、资源规格和数据挂载点。DLC 负责把任务调度到 GPU 集群上，搞定节点间网络（有 RDMA 就用 RDMA），跑你的代码，流式输出日志，完事后自动清理。

![PAI distributed training pipeline](https://blog-pic-ck.oss-cn-beijing.aliyuncs.com/posts/zh/aliyun-fullstack/11-pai-ml-platform/11_training_pipeline.png)

### When to move from DSW to DLC

什么时候该从 DSW 切换到 DLC？满足以下任一条件就行：
- 你需要不止一张 GPU
- 任务需要无人值守运行超过 4 小时
...
DLC 支持抢占式实例（spot instances），大概能打 6 折。代价是：任务可能随时被中断，提前 5 分钟通知。解决办法：勤存 checkpoint，重启后从最新的 checkpoint 恢复。
...
EAS (Elastic Algorithm Service) 才是 PAI 真正值钱的地方。DSW 笔记本在你干活时每小时几块钱。DLC 任务是一次性投入。EAS endpoint 得 24/7 在线，得扛住流量峰值，闲时自动缩容，支持蓝绿部署，还得在你市场团队上午 10 点给百万用户发推送时别崩。
...
![GPU instance comparison for ML workloads](https://blog-pic-ck.oss-cn-beijing.aliyuncs.com/posts/zh/aliyun-fullstack/11-pai-ml-platform/11_gpu_comparison.png)
