...
**3. 每次打开文件都弹出 swap file 警告。** 上次 Vim 会话未正常退出。删除孤立的 swap 文件（见[第 10 节](#标记（mark）——-跨文件或跨项目的书签)），或在 Neovim 中运行 `:checkhealth`。
...
**第 7 天 —— 自定义配置。** *现在* 修改 `.vimrc`。加入[第 11 节](#Visual-模式-——-尤其是-Visual-Block（块选择）)的设置，再添加一两条个人映射。**至少再忍一周，不要装插件。**
