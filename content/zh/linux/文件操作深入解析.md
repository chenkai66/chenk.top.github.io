...
---
![章节概念图](https://blog-pic-ck.oss-cn-beijing.aliyuncs.com/posts/zh/linux/文件操作深入解析/illustration_1.jpg)

在 Linux 系统中，真正能大幅提升效率的，并不是记住更多的命令，而是学会如何将这些小工具巧妙地组合起来，形成一条清晰、流畅的数据处理管道。管道符 `|` 是 Unix 哲学的核心体现：每个工具专注于完成一项任务，并且做到极致（比如 `grep` 专门用来过滤，`awk` 专注于字段提取，`sort` 只负责排序），然后通过管道将它们串联成一个逻辑清晰、易于调试和维护的工作流。本文从数据流的基本模型入手——包括 `stdin`、`stdout`、`stderr` 以及它们背后的文件描述符——接着系统性地讲解常见的重定向用法（如 `>`、`>>`、`<`、`2>`、`2>&1`、`&>`），然后深入剖析文本处理工具链中的核心成员（如 `grep`、`awk`、`sed`、`cut`、`tr`、`sort`、`uniq`、`xargs`、`tee`），最后补充两个大多数入门教程忽略的高级主题：命名管道（FIFO）和进程替换。读完本文后，你应该能够用一两行简洁易懂的命令解决许多原本需要写脚本才能完成的小任务，同时也能更轻松地读懂别人写的那些复杂的单行命令。
...
乍一看，stdout 和 stderr 都默认输出到终端，交互式运行时似乎没什么区别。但实际上，内核将它们分配到了两个独立的文件描述符上。这种设计正是管道机制安全性的关键所在：
...
在 shell 中编写的每一个重定向或管道命令，本质上都是在程序启动之前对这些小整数进行“重新布线”。例如：
...
## 文本处理工具链
六个工具就能搞定绝大多数日志和文本处理任务。搞清楚每个工具的 *用途*，你就不用每次都想着写 Python 了。
