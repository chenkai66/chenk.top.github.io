...
---
![章节概念图](https://blog-pic-ck.oss-cn-beijing.aliyuncs.com/posts/zh/linux/系统服务管理/illustration_1.jpg)

在 Linux 系统中，"服务"指的是一种长时间运行的后台进程，它的任务就是随时待命：比如同步时钟、监听 SSH 连接、处理 HTTP 请求，或者在凌晨三点执行一次备份任务。这些服务通常不会由用户手动启动，而是需要一个管理者来完成一系列工作：开机时启动它们、崩溃时重启它们、收集它们的日志、理清它们之间的依赖关系，并在关机时有序地关闭所有服务。在现代 Linux 发行版中，这个管理者就是 **systemd**。

这篇文章的内容，正是我希望自己第一次接触生产环境服务器时就能拥有的"实用指南"。我们会从头开始，先探讨为什么需要一个专门的服务管理工具，然后逐步建立起 unit 和 target 的核心概念模型。接着，我们会梳理日常工作中真正用得上的 `systemctl` 命令，逐行解析一份典型的 unit 文件内容。最后，我们还会扩展到相关的周边工具和技巧：如何用 `journalctl` 查看日志、如何用 `.timer` 单元替代传统的 cron 任务，以及一套详细的排查步骤，帮助你解决"服务无法启动"的问题。
## 1. 为什么需要服务管理器
### systemd 出现之前的问题
...
systemd 的出现彻底改变了这一切。它引入了一个统一的监管者，能够区分“启动命令返回了”和“服务真的准备好了”，并尽可能并行化任务，同时记录所有事件到结构化的日志中。
...
![journalctl 过滤器：一个日志系统，多种筛选方式](https://blog-pic-ck.oss-cn-beijing.aliyuncs.com/posts/zh/linux/service-management/fig4_journalctl_filters.png)
