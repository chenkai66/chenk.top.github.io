...
缺点：改写会破坏它；质量成本是实打实的（强水印上的困惑度增加约 5-10%）；只适用于生成水印的文本。2023 年的后续研究（Sadasivan et al., *Can AI-Generated Text be Reliably Detected?*）认为攻击者用相当的 LLM 资源通过仔细改写就能移除水印，这给实际安全性设了个上限。
