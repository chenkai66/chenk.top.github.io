...
---
安全是 LLM 工程中信噪比最低的话题：哲学讨论泛滥、营销话术盛行，而真正可落地的工程细节却十分稀缺。本章只讲工程细节——RLHF 名义上强调‘安全’，实际优化目标是什么？拒绝校准为何失效？真实的红队测试长什么样？哪些幻觉评估指标能切实预测对客户的影响？还有 2024–2026 年间不起眼却至关重要的论文（如 Sleeper Agents、refusal as a feature direction、weak-to-strong generalization），它们应当重塑你对生产环境中对齐实践的理解。
...
2024 年的一个发现本该改变所有人对拒绝的看法：Arditi 等人（2024, *Refusal in Language Models is Mediated by a Single Direction*）表明，对于许多开源模型，**拒绝行为基本上由模型 residual stream 中的单个线性方向介导**。计算“我应该拒绝”和“我应该顺从”prompt 上的平均激活差异；在推理时把这个方向投影出去，模型就失去了大部分拒绝行为。重新加回这个方向，拒绝行为就回来了。强力投影它，模型就拒绝一切。

这有两个含义：
1. **拒绝机制其实很浅**。它不是作为某种 emergent moral judgment 编码在网络深度里；而是 post-training laying down 的一个单特征方向。一个有权重访问权限的 motivated attacker 可以在几分钟内禁用它。
...
一次红队会话长这样：
1. **定义 harm taxonomy**。在这个部署中什么输出是有害的？例如：生物武器合成指令、儿童性虐待材料、金融欺诈指令、仇恨言论。
