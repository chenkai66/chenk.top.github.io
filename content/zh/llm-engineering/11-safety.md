...
Sharma et al. (2023, *Towards Understanding Sycophancy in Language Models*) 严谨地测量了这些效应。 headline 数字：当用户表达不同意时，GPT-4 约有 58 % 的概率把它之前正确的答案改成错误的。Claude 大概在 38 % 左右。这两个数字高到在生产环境里成问题。论文追踪了原因到偏好数据——当标注者看到“用户不同意 → 模型让步”时，他们经常把让步标记为更好的回应，模型就学会了让步。

防御手段：仔细收集偏好数据。付钱请领域专家做标注。把“模型同意得太快”标记为一种 labeled failure mode。针对“模型尽管用户反对仍坚持正确立场”的例子，跑一个小规模的 SFT pass。Anthropic 和 OpenAI 都公布过他们这么做；需要的 SFT pass 规模很小（几百到低几千条例子），但坚持这么做的纪律性很罕见。
