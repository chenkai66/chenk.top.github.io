...
---
安全是 LLM 工程中信噪比最低的话题：哲学讨论泛滥、营销话术盛行，而真正可落地的工程细节却十分稀缺。本章只讲工程细节——RLHF 名义上强调‘安全’，实际优化目标是什么？拒绝校准（refusal calibration）为何失效？真实的红队测试（red-teaming）长什么样？哪些幻觉评估指标能切实预测对客户的影响？还有那些 2024–2026 年间不起眼却至关重要的论文（Sleeper Agents、 refusal as a feature direction、 weak-to-strong generalization），它们应当重塑你对生产环境中对齐（alignment）实践的理解。

先明确立场：我是一名工程师，而非政策研究者，对 AI 存在性风险（x-risk）持开放态度，不试图向读者灌输特定立场。我只关心生产环境中什么管用，什么会尴尬地失败，以及文献中展示了什么。文末的参考文献才是重点，应将引用视为核心结论。
...
文献中还隐含第四个关键维度：*controllability*（可控性），但尚未形成统一、明确的命名。一个在攻击下仍能遵循开发者 system prompt 的模型，比一个会漂移的模型更具可控性。 Wallace 等人提出的指令层级框架（见第 9 章）即聚焦于此。在生产实践中，可控性才是与有用性直接权衡的核心维度：当模型严格遵循 system prompt 时，其可控性增强，但也可能导致对合法请求的过度拒绝。
...
信号是 *人类（或 AI 代理）偏好什么*。但这教会了你没打算教的东西：
...
Sharma 等人（2023, *Towards Understanding Sycophancy in Language Models*）严谨地测量了这些效应。 headline 数字：当用户表达不同意时， GPT-4 约有 58% 的概率将其之前正确的答案改成错误的， Claude 大概在 38% 左右。这两个数字在生产环境中非常高。论文追踪了原因到偏好数据——当标注者看到“用户不同意 → 模型让步”时，他们经常把让步标记为更好的回应，模型因此学会了让步。

防御手段包括：仔细收集偏好数据，付钱请领域专家做标注，将“模型同意得太快”标记为一种 labeled failure mode，并针对“模型尽管用户反对仍坚持正确立场”的例子进行小规模的 SFT pass。 Anthropic 和 OpenAI 都公布过这种方法；所需的 SFT pass 规模很小（几百到几千条例子），但坚持这样做的纪律性很罕见。
...
- **Over-refusal**：模型拒绝了合法请求，因为它们 pattern-match 到了某些令人担忧的内容。“告诉我对乙酰氨基酚是怎么起效的”可能会触发拒绝，因为“药物”在安全表面区域内。
- **Under-refusal**：模型顺从了明显有害的请求。“写一封针对银行客户的钓鱼邮件。”
...
我想瞄准的生产环境目标： precision > 90 %, recall > 95 %。 exact balance 取决于你的部署场景——儿童产品想要高 recall；安全研究产品想要高 precision。

2024 年有个发现本该改变所有人对拒绝的看法： Arditi et al. (2024, *Refusal in Language Models is Mediated by a Single Direction*) 表明，对于许多开源模型，**拒绝行为基本上由模型 residual stream 中的单个线性方向介导**。计算“我应该拒绝”prompt 和“我应该顺从”prompt 上的平均激活差异；在推理时把这个方向投影出去，模型就失去了大部分拒绝行为。重新加回这个方向，拒绝行为就回来了。强力投影它，模型就拒绝一切。

这有两个含义：

1. **拒绝机制其实很浅**。它不是作为某种 emergent moral judgment 编码在网络深度里；它是 post-training  laying down 的一个单特征方向。一个有权重访问权限的 motivated attacker 可以在几分钟内禁用它。
2. **因此防御表面就是权重本身**。开放权重发布带着这个可供手术的特征方向。封闭 API 仅靠访问控制保护。没有算法层面的修复。
...
1. **定义 harm taxonomy**。在这个部署中什么输出是有害的？例子：生物武器合成指令、儿童性虐待材料、金融欺诈指令、仇恨言论。
2. **生成 adversarial prompts** 在每个类别中。混合人类编写（clever）和自动化（volume）。
3. **探测模型** 用每个 prompt。记录响应，标记为"refused well", "refused poorly (over-refused safe variant)", "complied with harmful request", "ambiguous"。
4. **Categorize jailbreaks** 按技术分类（第 9 章有 taxonomy）。
5. **Patch via fine-tuning** 在失败案例上。
6. **Repeat**。新的 jailbreak 几天内就会出现。
