...
---
预训练出来的基座模型只会续写文本。让它听懂指令、拒绝有害请求、维持人设，这些都是后训练（post-training）阶段的事。这里也是论文宣称效果和生产级模型差距最大的地方。本章讲讲各个后训练算法到底优化了什么，为什么大多数奖励模型都有隐疾，以及 2026 年真正管用的配方。
...
1. **SFT**（supervised fine-tuning）用在指令数据上。教模型响应格式和基本的指令遵循行为。
2. **偏好优化**（DPO, IPO, KTO, 或 RLHF）。教模型在两个有效回复中，人类（或代理）更喜欢哪一个。
3. **在线 RL**（RLHF, RLAIF, 或 RLVR — 可验证奖励）。对着奖励模型或程序检查器更激进地调优。对于非推理模型，这一步可选，而且越来越常被跳过。
4. **专项阶段**：工具使用 SFT、长上下文 SFT、安全红队测试、宪法 AI 过滤。

OpenAI、Anthropic 和 Google 还在跑接近 "SFT → 偏好 DPO → RLHF/RLAIF" 的流程。DeepSeek-R1 [DeepSeek-AI, 2025] 和 o1 系列模型加入了 **RLVR**（RL with verifiable rewards — 数学/代码这种能用程序检查正确性的场景）作为主导信号。这是 2024-2025 年最大的后训练变革。

师承关系很重要。第一个可信的 "RL from human feedback" 论文是 [Christiano et al., 2017]，把偏好学习用在了 Atari 和连续控制上。[Stiennon et al., 2020] 把它用在了摘要生成。[Ouyang et al., 2022] (InstructGPT) 是第一个通过 SFT + RLHF 端到端指令微调的 LLM — 现代所有后训练技术栈都源于这篇论文。2023-2025 这波浪潮（DPO, IPO, KTO, RLVR）本质上都是 InstructGPT 主题的变体。

## SFT：比大家以为的更重要

SFT 就是 "给模型看 10 万到 100 万条指令 - 回复对，训练仅 mask 回复部分的 next-token prediction loss"。这个 mask 很关键 — 你不想让模型学会预测用户的问题，只想让它学会助手的回答。
...
**数据质量，而不是数量。** LIMA [Zhou et al., 2023] 证明了 1000 条精心 curated 的例子能匹敌 5 万条 mediocre 的数据。Tulu-3 混合集 [Lambert et al., 2024] (AllenAI, 2024)  curated 到了约 20 万条。Qwen3 的 SFT 混合集接近 100 万，但做了重度质量过滤。数据质量高的话，超过 100K 后收益曲线就平得厉害。
