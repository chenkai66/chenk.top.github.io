...
---
预训练是大模型能力的源头，也是榜单成绩与实际表现差距最大的地方；大多数公开的训练记录更像是工程奇迹而非科学成果。这一章将讨论当你不是 OpenAI 时，预训练必须搞对的几个部分：数据、并行策略，以及只有在集群达到一定规模时才会暴露的故障模式——例如，一次失败的 NCCL all-reduce 可能导致为期 30 天的训练任务中断。
...
过去三年里所有可靠的 scaling study 都达成共识：在算力相同的情况下，两个 LLaMA 式架构之间的差异很小（约 5% perplexity），但不同数据配比带来的性能差异极为显著（超过 30%）；Chinchilla 论文中的 compute-optimal 缩放定律假设数据分布固定，一旦允许数据分布变化，数据就会占据主导地位。

现代预训练配比如下（FineWeb-Edu [Penedo et al., 2024]、RedPajama-V2 [Together AI, 2024]、Dolma [Soldaini et al., 2024]——所有开放配比彼此相差无几）：
...
- **质量过滤器主导。** 一个基于 FastText 训练的质量分类器用于预测文本是否类似“高质量”参考数据，在与基线相同的算力下训练，在 MMLU 上超越基线 6.6 个百分点，在 Core（一个 22 任务基准）上超越 3.5。
