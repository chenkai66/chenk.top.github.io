...
---
函数调用是大语言模型（LLM）连接外部世界的关健接口，也是 chat template、结构化输出内核与提示工程的交汇点。本章将深入剖析底层机制，探讨哪些行为具备可依赖的确定性保证，以及哪些 agent-loop 模式能在真实生产负载下稳定运行。
...
![LLM 工程（7）：函数调用和工具使用 —— 可视化](https://blog-pic-ck.oss-cn-beijing.aliyuncs.com/posts/zh/llm-engineering/07-function-calling/illustration_1.png)
...
3. **Schema 引导的结构化输出**。在 JSON 模式基础上，进一步要求输出严格匹配预定义的 JSON Schema，包括函数名和参数类型等。
...
![图1：工具调用请求/响应序列](https://blog-pic-ck.oss-cn-beijing.aliyuncs.com/posts/zh/llm-engineering/07-function-calling/fig1_tool_sequence.png)
...
![图3：并行与顺序工具执行](https://blog-pic-ck.oss-cn-beijing.aliyuncs.com/posts/zh/llm-engineering/07-function-calling/fig3_parallel_vs_sequential.png)
...
![图2：模式约束解码](https://blog-pic-ck.oss-cn-beijing.aliyuncs.com/posts/zh/llm-engineering/07-function-calling/fig2_schema_constrained_decode.png)
...
![图4：错误恢复模式](https://blog-pic-ck.oss-cn-beijing.aliyuncs.com/posts/zh/llm-engineering/07-function-calling/fig4_error_recovery.png)
...
![LLM 工程（7）：函数调用和工具使用 —— 可视化](https://blog-pic-ck.oss-cn-beijing.aliyuncs.com/posts/zh/llm-engineering/07-function-calling/illustration_2.png)


![图5：代理循环控制流](https://blog-pic-ck.oss-cn-beijing.aliyuncs.com/posts/zh/llm-engineering/07-function-calling/fig5_agent_loop.png)
