...
---
"1M 上下文"算是大模型圈最注水的指标之一了。模型能 attend 1M token 是架构能力，但能不能*利用*第 80 万位的信息回答问题，那是行为能力，难得多。本章咱们聊聊位置编码的数学原理、把上下文扩展到训练长度之外的工程 trick，以及为什么大多数长上下文宣称都过不了“大海捞针”测试。

大模型长上下文的发展史大概分三幕。第一幕（2017-2021）：模型训练长度卡在 512-2048 token，因为 attention 是 $O(n^2)$ 的，显存吃不消。第二幕（2022-2023）：高效 attention 算子（FlashAttention, [Dao 2022][dao-flashattention]）让长训练变得可行，后处理上下文扩展技术（Position Interpolation, NTK-aware scaling, YaRN）让从业者能把预训练 checkpoint 从 4K 推到 32K 甚至更多。第三幕（2024-2026）：原生长上下文训练（Llama 3.1 的 128K, Gemini 的 1-2M, Claude 的 200K）成了标配，但*能 attend 的上下文*和*有用的上下文*之间的差距依然存在——这正是本章要重点讨论的。
...
RoPE 赢了。我知道的所有靠谱的 2026 年大模型都用它做位置编码。原因有二：它在每一层都注入位置（信号更好），而且相对位置会自然地从点积中浮现，这正是 attention 真正需要的。
...
这个洞察催生了现代扩展方法：不要等比例缩放所有维度；按波长缩放。
...
**Position interpolation (PI)** ([Chen et al., 2023][chen-pi])：按比例 $s = L_{\text{new}} / L_{\text{train}}$ 缩小位置。位置 32768 在 $s=8$ 时变成有效位置 4096，模型见过这个。继续微调几百步能修复小的分布偏移。PI 有效但会损失一些短上下文质量——每个位置现在对应一个模型从未训练过的分数旋转。原始 PI 论文报告用 1000 步微调将 LLaMA 从 2K 扩展到 32K，质量下降极小；这是让后处理上下文扩展成为真正工程选项的突破。
...
实践中，你要部署的开源模型（Qwen3, LLaMA-3, Mistral）出厂时已经做好了上下文扩展。通常不需要重新扩展；你读技术报告知道他们用了哪种方法以及实际限制是什么就行。
