...
Self-attention 对排列是不变的，没有位置信号，模型无法区分“猫坐在垫子上”和“垫子坐在猫上”，因此需要注入位置信息，主要有以下三种方案：
...
RoPE 已成为主流——截至 2026 年，所有主流大模型都采用它作为位置编码，在每层内部注入位置信号以增强鲁棒性，并让相对位置信息自然浮现于 Q·Kᵀ 点积中，而这正是 attention 机制的本质需求。

第四个方案 **ALiBi** (attention with linear bias) 在 2022 年左右曾有激烈竞争，但最终失败了；本章后面会将其作为最有趣的替代路径进行讲解。第五个 **xPos** (Sun et al., 2022) 是 RoPE 的改进版，加了个依赖长度的 decay 让外推更稳；DeepSeek 和少数现代模型内部在用，但核心思想仍是 RoPE 加工程打磨。
...
几乎每个现代 attention 算子都内联处理 RoPE，概念上就是这样。
