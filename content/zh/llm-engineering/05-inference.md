...
一条常用经验法则是：TTFT 主要取决于 prefill 阶段，而 ITL 则主要受限于 decode 阶段的内存带宽。降低 TTFT 的关键是提升计算吞吐（如增加 SM 数量或启用张量并行）；降低 ITL 则依赖更高的内存带宽（HBM3 优于 GDDR）或模型量化，后者通过压缩参数来缓解带宽压力。

用算术强度来论证会更严谨。70B 模型处理 4K prompt 的 prefill：模型权重（BF16 下 140 GB）加载一次，操作 4096 个 token。算术强度 ≈ 每读取一个参数字节对应 4096 FLOP。同样，单 token 的 decode：权重加载一次，操作 1 个 token。算术强度 ≈ 每字节 1 FLOP。H100 在 BF16 下算力为 989 TFLOPS，HBM 带宽为 3.35 TB/s，平衡点大概在 ~295 FLOP/byte。Prefill 的 4096 FLOP/byte 远高于平衡点（计算密集型），而 Decode 的 1 FLOP/byte 低于平衡点两个数量级（内存密集型）。这两个阶段需要不同的硬件特性，如果 Serving 栈不将它们分开处理，性能就会大打折扣。
...
这是单个请求的量。如果有 50 个并发请求，那就是 430 GB 的 KV cache——远超你的模型权重。此时的瓶颈在于 KV cache 占用，而非模型权重本身。
...
原论文的基准测试显示， vLLM 在 LLaMA-13B 上的服务吞吐量，相较朴素实现的 Hugging Face Transformers 循环（即逐 token 手动调用 generate），最高可提升 14–24 倍。这一优势在当前仍具参考价值，但实际生产环境中的收益高度依赖具体工作负载构成。对于 batch-1 单流服务，提升接近 2 倍；对于高并发混合长度流量， 5-10 倍是常态； 14-24 倍这个数字是在对比 *真正* naive 的 HF 代码（完全不做 batching）时出现的。重点不在于具体的倍数，而是 paged attention 让 KV 内存变成了可管理的资源，而不是硬约束。
...
**AWQ** ([Lin et al., 2023][lin-awq])：观察到 1 % 的权重是 "salient" 的（大 activations 穿过它们）。应用 per-channel scaling $s$ 保护 salient 权重免受量化噪声影响——等价于量化前将输入 activations 乘以 $s$，权重除以 $s$。全精度下这在数学上是 空操作，但重塑了量化 landscape。然后均匀量化。比 GPTQ 快（不用解 Hessian，只需搜索 scaling 因子），大多数模型上质量略好，对 MoE 更友好。
