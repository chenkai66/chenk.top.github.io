...
---
分词层往往是大家最容易忽略的一环。但这恰恰是我在生产环境里 debug 最多的地方——静默的质量回退、莫名其妙的成本飙升、模型拒不执行指令（通常是因为某人把 chat template 格式搞错了）。这一章的内容，是我希望自己在发布多语言产品前就能彻底吃透的。
...
byte-level 这部分很重要：GPT-2  famously 把 256 个字节 remap 到一个可打印的 Unicode 子集以便检查，但编码字母表依然是 256 个符号。这意味着*没有任何输入会分词失败*。不存在 out-of-vocabulary 失败模式。
...
词表越大 → 每文档 token 越少 → prompt 计费时推理更便宜，但 embeddings 和 softmax 更大。大多数现代模型 定型了 在 100-200K 左右。Qwen / GPT-4o 这个范围在多语言覆盖和避免 embedding 表主导之间找到了 sweet spot。
...
对于 70B 模型，embedding 表（128K × 8192 = 1.05B params, tied）占总参数的 1.5 %——完全可以忽略。对于 1B 模型，同样的 embedding 会占参数的 100 %，这就是为什么小型开源模型（Phi-4-mini, Qwen3-0.5B）通常用较小的词表（50-64K）以保持 embedding 与非 embedding 的比例合理。
