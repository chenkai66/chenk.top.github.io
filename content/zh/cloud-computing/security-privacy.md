...
2019 年 Capital One 泄露了一亿条客户数据。攻击链很短：一个配置错误的 WAF 允许了对 EC2 元数据端点的服务端请求伪造（SSRF），元数据端点交出了 IAM 临时凭证，而那个 IAM 角色对账户里所有 S3 存储桶都拥有 `s3:*` 权限。一处配置错误、一个权限过宽的角色、一条缺失的防护规则——直接经济损失（不含法律费用）超过 8000 万美元。
...
本文从共担责任契约讲到事件响应，每一层都附有代码、架构图、失效模式和可演练的运行手册。
...
但这张责任划分图中有三个常被忽视的关键点：首先，数据和访问控制始终是你的责任——即使是 SaaS 服务，谁能访问你的 Salesforce 租户也由你决定；其次，厂商提供的服务配置也是你的责任——例如 AWS 提供了 S3，但桶策略、加密设置及公共访问限制开关都需自行配置，这些功能的默认值曾长期是“开放”的；最后，只有当你正确使用厂商的服务时，厂商的责任才生效——如果你在 EC2 上自行搭建数据库，补丁更新、备份和高可用性（HA）则全由你负责。

简单来说，厂商负责底层基础设施，而所有你可配置的部分均由你负责。
## 2. 你实际会面对的威胁
