...
---
![章节概念图](https://blog-pic-ck.oss-cn-beijing.aliyuncs.com/posts/zh/cloud-computing/security-privacy/illustration_1.jpg)
...
- 共享责任模型的核心概念，以及它在 IaaS、PaaS 和 SaaS 不同服务模式下的变化与适用范围
- 高效扩展的 IAM 体系：如何管理身份、组、角色和策略，并掌握那些在审计中经得起考验的最佳实践
...
但这些图里有三个容易被忽略的关键点。首先，**数据和访问控制始终是你的责任**——即便是 SaaS 服务，谁能访问你的 Salesforce 租户，也只有你能决定。其次，**厂商提供的服务配置也是你的责任**——比如 AWS 提供了 S3，但桶策略、加密设置以及公共访问限制开关都得你自己搞定，而这些功能的默认值曾经长期是“开放”的。最后，**只有当你正确使用厂商的服务时，厂商的责任才算生效**；如果你自己在 EC2 上搭建数据库，那么补丁更新、备份和高可用性（HA）又全都回到你身上。

简单来说就是：**厂商负责底层基础，你能配置的部分，就得配对。**
## 2. 你实际会面对的威胁
