...
违反某条要素有时确有必要（例如要素 6 在有状态系统中难以完全满足），但每一次妥协，都意味着你主动承担了一份技术债务。
## 容器：到底是什么

很多人觉得“容器就是轻量虚拟机”。这个理解在关键点上**错了**。**容器并不是一种虚拟化技术，而是一种进程隔离机制。** 容器就是一个 Linux 进程（或进程树），内核通过隔离机制向它呈现一个独立的视图，使其‘以为’自己运行在一个独立的系统中。
...
每条 Dockerfile 指令生成一层。层通过 union FS 堆叠，相同层在镜像间、宿主机间去重。**正因如此，两个共享同一基础镜像的镜像，体积差异通常仅几 MB，即使该基础镜像本身高达数 GB。**
...
满足这些条件的工作负载虽非万无一失，但后续遇到的问题将是真正有挑战性的，而非低级失误所致。
