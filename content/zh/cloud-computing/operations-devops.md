...
本文将带你构建这样一个健壮的运维系统：通过 CI/CD 在代码触达用户前守住质量关卡；借助基础设施即代码（IaC），让“生产环境”成为 Git 中的一个可追溯版本，而非无法复制的“雪花服务器”；部署能从噪声中识别真实信号的监控体系；建立真正可搜索的日志系统；并引入 SRE 的核心实践——错误预算、SLO 和无指责复盘——将混乱的救火行动转化为可持续的工程改进。


---

## 你将学到什么
- CI/CD 流水线：阶段划分、质量门禁、回滚机制，并附完整 GitHub Actions 示例
- 使用 Terraform 实现基础设施即代码：工作流、状态管理、模块化设计模式
- Prometheus + Grafana + Alertmanager 监控体系：抓取模型、PromQL 查询语言、智能告警规则
- 集中式日志架构（EFK / ELK）：日志采集器、缓冲队列、处理器与分层保留策略
- 能响应真实负载且避免频繁震荡的自动伸缩机制
- 无需重写应用即可实现的云成本优化方法
- SRE 核心实践：SLI / SLO / 错误预算、无指责复盘、GitOps

## 前置知识
- 熟悉 Linux 命令行操作
- 掌握 Git 基础及 CI/CD 基本概念
- 建议先阅读本系列第 1 至 5 篇文章
...
| Build | 编译、打包、构建镜像 | 缺乏可重现性（未锁定基础镜像或依赖版本） |
| 单元测试 | 快速反馈代码逻辑问题 | Flaky（不稳定）测试侵蚀团队信任，需果断隔离 |
| 安全扫描 | 执行 SAST、依赖 CVE 检查、镜像漏洞扫描 | 告警噪声过大，应按仓库调整严重性阈值 |
| 部署到 Staging | 新制品首次在类生产环境中运行 | Staging 与 Prod 存在配置漂移 |
| Smoke / E2E | 验证跨服务接口契约 | 测试执行过慢，导致开发者倾向于跳过 |
...
其中有三点设计决策尤为关键：  
第一，**使用 OIDC 替代长期有效的密钥**——通过 `id-token: write` 权限和 `role-to-assume` 配置，GitHub Actions 可直接扮演 AWS IAM 角色，彻底杜绝访问密钥在 GitHub 中硬编码或明文存储的风险。  
...
| 策略 | 工作原理 | 回滚速度 | 适用场景 |
|------|-----------|------------|-------------|
| **滚动更新（Rolling）** | 分批替换实例（如每次替换 N 个） | 分钟级（暂停发布并重新部署旧版本） | 无状态服务，且已配置健康检查 |
| **蓝绿部署（Blue/Green）** | 同时运行两套完整环境，通过 DNS 或负载均衡器切换流量 | 秒级（一键切回旧环境） | 有状态服务、数据库变更，或要求瞬时回滚的场景 |
| **金丝雀发布（Canary）** | 将一小部分真实流量导向新版本，逐步放大比例 | 秒级（快速摘除金丝雀实例） | 高流量服务，且能快速测量错误率等 SLO 指标 |
| **功能开关（Feature Flags）** | 代码先上线但默认关闭，再按用户群、百分比或条件动态启用 | 毫秒级（服务端开关即时生效） | 用户可见功能、A/B 测试、渐进式能力开放 |
...
日常真正高频使用的是 `plan` —— 它让你在变更落地前就看清影响范围。代码评审应基于 `plan` 的输出结果，而不仅是 HCL 源码本身。
...
- **每个服务、每个环境独享一个 state 文件**：例如 `services/web/prod/`、`services/web/staging/`、`services/api/prod/`。这能有效控制故障爆炸半径，并支持多环境并行部署。
...
## 使用 Prometheus、Grafana 和 Alertmanager 实现监控
...
- 若服务崩溃，仅表现为“不再被拉取”，不会残留无效的推送连接，系统更健壮。
...
4. **对容错型负载启用 Spot 实例 / Preemptible 实例**：CI 构建节点、批处理任务、开发环境等场景适用，成本可降 60–90%。
