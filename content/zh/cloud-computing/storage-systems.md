...
一块 SSD 能轻松跑出几十万 IOPS，寿命也能撑好几年。但问题在于，生产环境中依赖单点设备总是充满风险：

- 单块硬盘每隔几年就会坏一次 —— 在一个拥有上万块硬盘的集群中，几乎每天都要更换硬盘。
- 单台机器有容量瓶颈：无法突破机柜槽位或电源预算的限制。
- 单条网络路径是单点故障（SPOF）：一台 ToR 交换机重启，整台机器就失联了。

因此，我们需要引入数据副本机制。然而，一旦开始做副本，立刻会遇到两个更棘手的问题：如何保证多个副本之间的一致性（Consistency），以及在网络分区导致副本被分割成孤岛时，如何继续保持服务可用性（Partition Tolerance）。这正是 CAP 定理所描述的核心挑战。
