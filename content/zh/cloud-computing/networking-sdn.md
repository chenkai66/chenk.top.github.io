...
云平台说到底，就是「一张网络 + 接在网络上的若干计算」。计算靠加节点扩，存储靠加盘扩，*网络* 才是让这些节点和盘表现得像一个统一系统的那一层。网络做对了，整个技术栈显得轻盈流畅；网络出错——比如漏配一条路由、安全组的五元组不匹配、负载均衡规格不足——整个平台可能直接不可用。
...
VPC 是云厂商基于物理网络构建的软件定义网络区域，逻辑上等效于一个私有数据中心。我定义 IP 空间、划分子网、配置网关、写防火墙规则。背后靠 **VXLAN** 或类似技术实现隔离 —— 每个数据包带租户标签，两个客户用同一个 10.0.0.0/16 也不会互相干扰。
...
**公有子网与私有子网的本质区别在于路由配置。** 路由表里有 `0.0.0.0/0 -> igw-...` 是公有；默认路由是 `0.0.0.0/0 -> nat-...` 是私有；没有出网路由就是隔离。听起来简单，但一半的「Lambda 上不了网」问题都出在这。
