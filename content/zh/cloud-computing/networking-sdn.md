...
VPC 是云厂商在物理网络上切出的一块软件定义区域，看起来像私有数据中心。我定义 IP 空间、划分子网、配置网关、写防火墙规则。背后靠 **VXLAN** 或类似技术实现隔离 —— 每个数据包带租户标签，两个客户用同一个 10.0.0.0/16 也不会互相干扰。
...
**公有 vs 私有子网的区别就是路由。** 路由表里有 `0.0.0.0/0 -> igw-...` 是公有；默认路由是 `0.0.0.0/0 -> nat-...` 是私有；没有出网路由就是隔离。听起来简单，但一半的「Lambda 上不了网」问题都出在这。
...
- **每 AZ 一个 NAT GW。** 单 NAT GW 是单点故障；全地域共用更危险。
- **S3 / DynamoDB 用网关端点。** 不开端点的话，私有子网到 S3 的流量走 NAT GW，按 $0.045/GB 收费。端点免费。
- **`map_public_ip_on_launch` 只开在公有子网。** 防止私有数据库意外分配公网 IP。

### 1.3 VPC 互联：Peering、TGW、PrivateLink

| 模式 | 拓扑 | 适用 | 注意 |
|------|------|------|------|
| **VPC 对等** | 点对点 | 2-5 个 VPC、地域内简单互联 | 不传递：A↔B、B↔C ≠ A↔C |
| **Transit Gateway** | 星型 | 5+ VPC、VPN + 专线集中 | 按小时 + GB |
| **PrivateLink** | 服务暴露 | 一个 VPC 暴露服务给多个消费方 | 单向；无需对等整个网络 |
| **Cloud WAN** | 全球网格 | 多地域、多账号网格 | 最新、规模化最简单，价格也高 |

决策关键是 *拓扑*，不是带宽 —— 几种方案规格够大都能跑满几十 Gbps。
## 2. 负载均衡

负载均衡器把实例组变成服务。隐藏单点故障、分摊负载、终结 TLS。L7 模式还能按路径、Host、Header 调流量，不动应用。
...
现代架构常见组合：**L4（NLB） -> L7（Envoy/ALB） -> 服务**。L4 抗 DDoS、提供稳定 Anycast IP；L7 做智能路由和鉴权。各司其职。
...
最少连接          挑未完成请求数最少的后端（适合开销不均场景）
最短响应时间      挑延迟 EWMA 最低的后端
两选一（P2C）     随机挑 2 后端，选负载轻的（90% 近似最优，O(1) 开销）
一致性哈希        hash(客户端 IP / 路径) -> 后端（缓存亲和性）
Maglev 哈希       Google 的有界扰动一致性哈希（Cloud LB 在用）
```

无状态 Web 流量用 *两选一（P2C）* 就够了，简单又接近最优。缓存敏感场景（KV 前端）用一致性哈希或 Maglev，保证同 Key 打同一后端。
...
CDN 是把地理变成缓存。静态内容复制到用户附近的边缘 PoP，缓存未命中才回源（通常是 S3）。延迟从 200 ms 降到 30 ms 以内，源站出网带宽减少 10 倍以上。
