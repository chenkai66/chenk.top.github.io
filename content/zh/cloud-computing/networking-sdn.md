...
云平台本质上是一张网络加上若干连接在网络上的计算节点。计算通过增加节点扩展，存储通过增加磁盘扩展，而网络则是使这些节点和磁盘表现为一个统一系统的关键。网络配置正确时，整个技术栈显得轻盈流畅；但若出错——例如漏配路由、安全组五元组不匹配或负载均衡规格不足——整个平台可能直接不可用。
