---
title: "Docker 与容器（7）：安全——运行容器时不必交出全部权限"
...
Docker 的默认配置优先考虑便利性，而非安全性。开箱即用时，容器以 root 身份运行，拥有广泛的 Linux capabilities，并可写入整个文件系统。这对开发环境尚可接受，但在生产环境中极其危险。一旦发生容器逃逸（container escape）漏洞，且容器本身以 root 权限运行，攻击者将直接获得宿主机的完全控制权。让我们来修复这个问题。

## 威胁模型

在实施加固措施之前，需先明确你正在防御哪些威胁：
...
每种加固技术都针对上述一项或多项威胁。目标是实现纵深防御（defense in depth）——单一措施不足以保障安全，但多层加固能显著提高攻击难度。

## 以非 root 用户身份运行

默认情况下，Docker 容器内的进程以 root（UID 0）身份运行。该 root 与宿主机上的 root 是同一用户（除非启用了 user namespaces）。若攻击者成功逃逸容器，便等同于获得了宿主机的 root 权限。
...
最小权限原则（Principle of Least Privilege）：先移除所有 capabilities，再按需逐一添加。
...
密钥（API keys、数据库密码、TLS 证书）是容器化应用中最常见的安全失败点之一。
