---
title: "Docker 与容器（6）：调试与日志——当‘盒子’内部出问题时"
date: 2023-09-30 09:00:00
tags:
  - Docker
  - Containers
  - Debugging
...
一个正常运行的容器是隐形的；一个出问题的容器则成了黑盒。容器化的核心价值正是隔离 —— 但恰恰是这种隔离，让调试变得更困难。你无法直接通过 `ssh` 登录容器，也无法从宿主机直接浏览其文件系统。Docker 提供了一套专用工具，用于检查、诊断并理解正在运行（或已崩溃）的容器内部究竟发生了什么。
...
退出码 137 尤其值得关注 —— 它通常表示容器因超出内存限制而被 OOM（Out of Memory）killer 终止。
...
注意 PID 和 PPID 列显示的是宿主机上的 PID。在容器内部，主进程 PID 是 1，但在宿主机上它是 12345 —— 这是命名空间映射的结果。
