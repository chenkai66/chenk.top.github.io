...
2018 年 M4 预测竞赛放出了 100,000 条时间序列，覆盖六种频率，作为统一的基准测试。排行榜上名列前茅的几乎全是基于几十年统计预测经验手工调优的集成模型，但最终夺冠的是一个**纯神经网络**，完全不需要统计预处理、特征工程，也没有递归结构。这个模型就是 Oreshkin 等人提出的 **N-BEATS**，由多个结构相同的全连接块堆叠而成，并引入了两条并行的残差路径。其可解释变体将预测结果显式分解为多项式趋势分量和 Fourier 季节性分量，实现了经典统计学长期追求的、具备语义可读性的时序分解。

这一章将详细分析这种极简架构为何能同时超越 LSTMs 和 ARIMA 风格的集成模型，并教你如何实现和调优，应用到自己的数据上。
## 你将学到的内容
...
大多数时间序列的深度学习模型都会引入结构先验：卷积假设平移等变性， RNN 假设有序的隐藏状态， attention 假设两两相关性。 N-BEATS 的思路完全不同，它直接将整个输入窗口送入 MLP，让网络自己学习最有用的分解方式。关键不在于使用何种网络层，而在于信息在模块间的流动方式。

具体来说， N-BEATS 做了三个大胆的设计选择：

1. **堆叠相同结构的模块**，每个模块同时预测 *backcast*（输入窗口的重构）和 *forecast*（未来的预测）。
2. **双残差流**，每个模块先用自身的 backcast 重构并减去当前残差，再将其 forecast 累加至最终预测中。下一个模块只会看到上一个模块无法解释的部分。
...
其中 $r^{(0)} = x$。每个模块看到的残差越来越小，因此会专注于处理剩余未被解释的频率或形状。宏观模式（如整体趋势、主导季节周期）由浅层模块捕捉，微观细节或残余误差则由深层模块逐步修正。

这一机制与梯度提升（Gradient Boosting）思想相似，区别在于它通过端到端可微的神经网络实现。顺序同样重要：第一个模块的任务最简单（完整信号均未被分解），最后一个模块的任务最难（仅剩噪声和细微结构）。
## 块的内部
...
论文指出一项实用经验： M4 数据集上的最优性能，源于可解释模型与 generic 模型的集成，且二者采用不同的回看窗口长度。这个话题我会在“集成策略”一节详细展开。
## 为什么基函数输出头很重要
