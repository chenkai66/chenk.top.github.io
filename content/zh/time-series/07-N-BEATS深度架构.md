...
2018 年 M4 预测竞赛放出了 100,000 条时间序列，覆盖六种频率，作为统一的基准测试。排行榜上名列前茅的几乎全是基于几十年统计预测经验手工调优的集成模型，但最终夺冠的是一个**纯神经网络**——完全不需要统计预处理、特征工程和递归结构。这个模型就是 Oreshkin 等人提出的 **N-BEATS**，由多个结构相同的全连接块堆叠而成，并引入了两条并行的残差路径；其可解释变体将预测结果显式分解为多项式趋势分量和 Fourier 季节性分量，实现了经典统计学长期追求的具备语义可读性的时序分解。

这一章将详细分析这种极简架构为何能同时超越 LSTMs 和 ARIMA 风格的集成模型，并教你如何实现、调优并应用到自己的数据上。
## 你将学到的内容
