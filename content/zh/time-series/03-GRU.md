...
- GRU 的参数量比 LSTM 少了整整 **25%**，这在实际应用中带来了诸多好处。
- 通过观察 GRU 的**门激活值**，可以分析模型关注的信息，辅助调试。
- 提供了一份基于参数量、运行速度和预测质量对比数据的实用 GRU 和 LSTM **选择决策表**。
...
LSTM 通过三个门精细调控记忆，而 GRU 是其结构更简化的变体，同样基于加性记忆机制，仅用两个门和一个隐藏状态即可实现。这使得模型参数减少约四分之一，训练速度提升 10--15%，并且在大量时间序列任务中，预测精度与 LSTM 在统计意义上无显著差异。
