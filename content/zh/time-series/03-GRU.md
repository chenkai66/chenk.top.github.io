...
LSTM 是一个“三个阀门精细控制”的记忆系统，而 GRU 是它的**轻量化版本**。同样是加性记忆账本，但 GRU 只用两个门和单一隐藏状态实现。最终模型参数减少约四分之一，训练速度提升 10--15%，并且在大量时间序列任务中，预测精度与 LSTM 在统计意义上没有显著差异。
...
*图 2. 25% 的差距来自结构设计，不是实验结果：GRU 只有三块权重，而 LSTM 有四块。当隐藏层大小为 256 时，GRU 能节省约 7 万参数；512 时能节省约 27 万。在嵌入式推理场景中，这往往直接决定了模型能否部署成功。*

实际影响包括以下几点：

- **训练速度**：每个 epoch 能节省大约 10--15% 的时间（§4 会详细测试）。
- **显存占用**：反向传播时激活值和梯度更小，这对长序列任务尤其重要，因为长序列通常需要缩小 batch size。
- **正则化效果**：参数少意味着方差更低，数据量有限时这一点尤为关键。
## 3. 隐藏状态到底长什么样

公式只有亲眼看到运行效果才会让人信服。图 3 展示了一个 16 单元的 GRU 处理复合信号的过程，信号包含慢振荡、$t=27$ 附近的噪声爆发以及 $t=45$ 的阶跃变化。
...
这就是门控结构的实际好处：网络能自己学到一组多时间尺度的基，完全不用我手动指定。
...
在做原型开发或超参数搜索时，这 12% 的优势会迅速累积：原本需要一周的 LSTM 调参，换成 GRU 只需六天，省下一天时间可以用来分析结果。

---
## 5. 读懂门：GRU 的诊断工具

门控 RNN 有个常被忽略的特性：门的激活值是**可解释的信号**，能直接画出来观察。图 6 展示了一个 GRU 处理信号时的表现。这个信号在 $t=40$ 发生了 regime 切换，并在 $t \in [68, 72]$ 出现瞬态尖峰。图中显示了 reset 门和 update 门的平均轨迹。
...
- **生产环境检测分布漂移**：如果很多单元的 $r_t$ 突然下降，说明模型认为"过去的信息不再重要"。这是一个非常有用的协变量漂移信号。
## 6. PyTorch 参考实现
