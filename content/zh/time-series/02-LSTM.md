...
LSTM（Hochreiter & Schmidhuber, 1997）用细胞状态和隐藏状态两个状态替代普通 RNN 的单一循环状态，并引入三个可学习的门：遗忘门控制保留多少旧信息，输入门控制写入多少新信息，输出门控制暴露多少当前状态。这种近似加性的更新方式，使梯度能够稳定地回传数百个时间步。
## 2. LSTM 单元的内部结构
...
无论处理单变量还是多变量时间序列预测，以下核心代码片段均已足够。
...
- 建议将遗忘门偏置初始化为 **+1**，使网络初始状态下倾向于保留历史信息。PyTorch 默认不启用此设置。
