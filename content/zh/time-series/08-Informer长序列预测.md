...
**Informer（Zhou 等人，AAAI 2021 最佳论文）是让 Transformer 在这类场景中变得实用的关键架构。它做了三件事，每一件事单独拿出来都算得上重要贡献：

1. **ProbSparse 自注意力**只保留 $\mathcal{O}(\log L)$ 个最有价值的查询，把每层复杂度从 $\mathcal{O}(L^2)$ 降到 $\mathcal{O}(L \log L)$。
2. **自注意力蒸馏**在编码器层之间将序列长度减半，显存占用随深度呈几何级数下降。
3. **生成式解码器**一次前向传播就能预测整个 horizon，不再需要分 $H$ 步自回归。

这三项改进合在一起，在长 horizon 的 ETT、气象、电力基准测试中，比普通 Transformer 快 6-10 倍，MSE 提升 5-10%。本章将详细拆解背后的数学原理，并逐步实现代码。
## 这一篇你会学到
