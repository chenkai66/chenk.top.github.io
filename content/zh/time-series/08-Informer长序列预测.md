...
如果把 $L$ 提高到 2160，每头的注意力条目接近 5 M。这种规模足以让一张 24 GB 显存的 GPU 在常用训练 batch 下直接 OOM。
...
- **尖峰型**：少数几个键占据了大部分概率。这种查询很“挑剔”，它知道自己在找什么。
...
- 解码器的 masked self-attention 必须在 softmax **之前**屏蔽未来键。上面的实现用了 `-1e9` 这一标准技巧。
## 编码器蒸馏：金字塔式序列压缩
...
普通的 Transformer 解码器是自回归的。它先预测 $\hat{y}_1$，把结果喂回去，再预测 $\hat{y}_2$，依此类推。如果预测范围 $H = 168$，就要跑 168 次前向计算。延迟不说，错误还会累积：第 5 步错了，这个错误会直接影响第 6 步。
