...
---
![章节概念图](https://blog-pic-ck.oss-cn-beijing.aliyuncs.com/posts/zh/time-series/06-%E6%97%B6%E5%BA%8F%E5%8D%B7%E7%A7%AF%E7%BD%91%E7%BB%9CTCN/illustration_1.jpg)

2010 年代的大部分时间里，提到深度学习处理时间序列，大家默认就是 LSTM。直到 2018 年，Bai、Kolter 和 Koltun 发表了一篇论文，题目是 *An Empirical Evaluation of Generic Convolutional and Recurrent Networks for Sequence Modeling*。结论简单得让人有点意外：把一堆 1D 卷积叠起来，确保它们是因果卷积（不偷看未来数据），让卷积核的步距按指数增长（dilation），再套上残差连接，直接训练就行。结果呢？任务一个接一个，这种叫 **Temporal Convolutional Network**（TCN）的模型，要么和 LSTM/GRU 打平，要么直接胜出——而且训练速度快了好几倍，因为前向传播的每个时间步可以并行计算。

这一章我会讲清楚这个方法为什么这么有效。先推导 dilation 的感受野公式，看看它到底有什么用；再一步步拆解残差块的设计细节；最后用两个工业级案例收尾：交通流量预测和多变量传感器预测。代码基于 PyTorch 实现，你可以直接拿去用。
## 你将学到的内容
...
在 TCN 出现之前，深度学习处理时间序列的套路基本是固定的：堆两层 LSTM，想花哨点就加个 attention，然后慢慢训练。虽然能跑通，但每个环节都让人头疼：
...
- **隐状态是个黑箱**。问“模型为什么会这么预测？”通常找不到答案，因为隐状态把所有信息混在一起，根本理不清。
- **超参数组合复杂**。层数、隐藏维度、门的变体、dropout 类型、recurrent dropout 的位置，这些因素互相影响。选错了组合，可能白白浪费一天的训练时间。
