// Ballpit — adapted from React Bits (https://reactbits.dev) for vanilla/Hugo.
// Component inspired by Kevin Levron: https://x.com/soju22/status/1858925191671271801
// three.js is provided at runtime via an import map (see head.html).
import {
  Vector3 as a,
...
// Minimal Timer shim (avoids depending on three core/addon Timer export).
class TimerImpl {
  constructor() { this._p = performance.now(); this._d = 0; }
  update() { const n = performance.now(); this._d = (n - this._p) / 1000; this._p = n; }
  getDelta() { return this._d; }
  reset() { this._p = performance.now(); this._d = 0; }
  dispose() {}
}
...
function initHeroBallpit() {
  ...
  const fine = mq && mq('(hover: hover) and (pointer: fine)').matches;
  const wide = window.innerWidth >= 768;
  if (reduce || !fine || !wide) { canvas.style.display = 'none'; return; }
  try {
    createBallpit(canvas, {
      ...
      wallBounce: 0.95,
      followCursor: true,
      colors: [0x7c5cff, 0x4f7bff, 0x22d3ee, 0xa78bfa],
      ...
      size0: 1.4,
      maxVelocity: 0.15,
      materialParams: { metalness: 0.4, roughness: 0.35, clearcoat: 1, clearcoatRoughness: 0.15 }
    });
  } catch (err) {
...
if (document.readyState !== 'loading') initHeroBallpit();
else document.addEventListener('DOMContentLoaded', initHeroBallpit);

export { createBallpit };
