...
模型的核心公式为：$$Y_t = c + \sum_{i=1}^{p} \phi_i Y_{t-i} + \epsilon_t + \sum_{j=1}^{q} \theta_j \epsilon_{t-j}$$其中：
-$Y_t$是当前时间$t$的值；
-$c$是常数项；
-$\phi_i$是自回归系数；
-$\epsilon_t$是误差项（白噪声）；
...
SARIMA的公式如下：$$(1 - \phi_1 L - \dots - \phi_p L^p)(1 - \Phi_1 L^m - \dots - \Phi_P L^{Pm})(1 - L)^d(1 - L^m)^D Y_t\\ = (1 + \theta_1 L + \dots + \theta_q L^q)(1 + \Theta_1 L^m + \dots + \Theta_Q L^{mQ}) \epsilon_t$$其中：
-$L$是滞后算子；
...
VAR模型的数学表达式为：$$Y_t = A_1 Y_{t-1} + A_2 Y_{t-2} + \dots + A_p Y_{t-p} + \epsilon_t$$其中：
-$Y_t$是包含多个时间序列的向量；
-$A_i$是滞后系数矩阵；
-$\epsilon_t$是误差向量。
...
GARCH(1, 1)模型的核心公式为：$$\sigma_t^2 = \alpha_0 + \alpha_1 \epsilon_{t-1}^2 + \beta_1 \sigma_{t-1}^2$$其中：
...
单指数平滑公式为：$$S_t = \alpha Y_t + (1 - \alpha) S_{t-1}$$其中：
-$S_t$是平滑后的值；
...
双指数平滑通过引入趋势项来预测具有线性趋势的序列。公式为：$$S_t = \alpha Y_t + (1 - \alpha)(S_{t-1} + T_{t-1})$$$$T_t = \beta (S_t - S_{t-1}) + (1 - \beta) T_{t-1}$$其中：
-$T_t$是趋势项；
-$\beta$是趋势平滑系数。
...
**水平更新公式**（Level equation）：$$L_{t}=\alpha\left(\frac{Y_{t}}{S_{t-m}}\right)+(1-\alpha)\left(L_{t-1}+T_{t-1}\right)$$其中：
...
**趋势更新公式**（Trend equation）：$$T_{t}=\beta\left(L_{t}-L_{t-1}\right)+(1-\beta) T_{t-1}$$其中：

-$T_t$是当前时刻$t$的趋势估计；
-$\beta$是趋势平滑系数，$0 \leq \beta \leq 1$。

**季节性更新公式**（Seasonal equation）：$$S_{t}=\gamma\left(\frac{Y_{t}}{L_{t}}\right)+(1-\gamma) S_{t-m}$$其中：
...
对于未来$h$时刻的预测：$$\hat{Y}_{t+h}=\left(L_{t}+h T_{t}\right) S_{t+h-m}$$其中：
...
Prophet模型可以分为三部分：趋势、季节性和假日效应。模型的形式为：$$y(t) = g(t) + s(t) + h(t) + \epsilon_t$$其中：
-$g(t)$是趋势部分，用于捕捉长期变化；
-$s(t)$是季节性部分，表示周期性变化；
-$h(t)$是假日效应；
-$\epsilon_t$是误差项。
...
卡尔曼滤波器的状态更新公式为：$$x_{t|t-1} = F x_{t-1|t-1} + B u_t$$$$P_{t|t-1} = F P_{t-1|t-1} F^T + Q$$观测更新公式为：$$K_t = P_{t|t-1} H^T (H P_{t|t-1} H^T + R)^{-1}$$$$x_{t|t} = x_{t|t-1} + K_t (y_t - H x_{t|t-1})$$其中：
